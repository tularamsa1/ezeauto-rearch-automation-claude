...
def test_common_100_102_024():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Callback_Success_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR Callback Success via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 024: TC024
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching amount_db, payment_mode_db, payment_status_db, payment_state_db, acquirer_code_db, bank_name_db, mid_db, tid_db, payment_gateway_db, rr_number_db, settlement_status_db from database for "
                f"current merchant:{amount_db}, {payment_mode_db}, {payment_status_db}, {payment_state_db}, {acquirer_code_db}, {bank_name_db}, {mid_db}, {tid_db}, {payment_gateway_db}, {rr_number_db}, {settlement_status_db}")
...
def test_common_100_102_025():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Callback_After_Expired_Success_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR Callback After QR Expiry Success transaction  when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 025: TC025
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = "select * from bharatqr_txn where org_code='"+org_code+"' and id " \
                    "LIKE '"+datetime.utcnow().strftime('%y%m%d')+"%' order by created_time desc limit 1;"
            logger.debug(f"Query to fetch transaction id from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            pid = result["provider_ref_id"].iloc[0]
            sid = result["transaction_secondary_id"].iloc[0]
            ...
            payment_gateway_db = result["payment_gateway"].iloc[0]
            settlement_status_db = result["settlement_status"].iloc[0]
            logger.debug(f"Fetching amount_db, payment_mode_db, payment_status_db, payment_state_db, acquirer_code_db, mid_db, tid_db, payment_gateway_db, settlement_status_db from database for "
                f"current merchant:{amount_db}, {payment_mode_db}, {payment_status_db}, {payment_state_db}, {acquirer_code_db}, {mid_db}, {tid_db}, {payment_gateway_db}, {settlement_status_db}")
...
def test_common_100_102_032():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Two_Callback_Success_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR generation using API and performing two Callback Success transaction
    TC naming code description: 100: Payment Method, 102: BQR, 032: TC032
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            payer_name_db = result["payer_name"].iloc[0]
            created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created time new : {created_time} ")
            mid_db = result["mid"].iloc[0]
            tid_db = result["tid"].iloc[0]
            payment_gateway_db = result["payment_gateway"].iloc[0]
            rr_number_db = result["rr_number"].iloc[0]
            ...
            payer_name_new = result['payer_name'].values[0]
            created_time_new = result['created_time'].values[0]
            logger.debug(f"Fetching created time new : {created_time_new} ")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {"pmt_status": "AUTHORIZED","txn_amt": float(amount),"pmt_mode": "BHARATQR",
                                       ...
                                       "acquirer_code_2": "YES", "issuer_code_2": "YES",
                                       "txn_type_2": "CHARGE",
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {"txn_amt": amount,"pmt_mode": "BHARATQR","pmt_status": "AUTHORIZED",
                                      ...
                                      "bqr_org_code": org_code,
                                      "txn_amt_2": amount, "pmt_mode_2": "BHARATQR", "pmt_status_2": "AUTHORIZED",
                                      "pmt_state_2": "SETTLED", "acquirer_code_2": "YES", "bank_name_2": "Yes Bank",
                                      "payer_name_2": payer_name_new, "mid_2": mid, "tid_2": tid,
                                      "pmt_gateway_2": "ATOS",
                                      "rrn_2": str(rrn_new), "settle_status_2": "SETTLED",
                                      "bqr_pmt_status_2": "SUCCESS",
...
def test_common_100_102_033():
    ...
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Callback_After_Expired_Success_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR Callback After QR Expiry Success transaction when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 033: TC033
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_102_034():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Two_Callback_After_Expired_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR generation using API and performing two Callback after QR Expiry transaction when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 034: TC034
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {"pmt_status": "EXPIRED","txn_amt": float(amount),"pmt_mode": "BHARATQR",
                                       ...
                                       "acquirer_code_3": "YES", "issuer_code_3": "YES",
                                       "txn_type_3": "CHARGE",
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                expected_db_values = {"txn_amt": float(amount),"pmt_mode": "BHARATQR","pmt_status": "EXPIRED",
                                      ...
                                      "pmt_state_3": "SETTLED", "acquirer_code_3": "YES",
                                      "bank_name_3": "Yes Bank",
                                      "payer_name_3": payer_name_new_3, "mid_3": mid, "tid_3": tid,
                                      "pmt_gateway_3": "ATOS",
                                      "rrn_3": str(rrn_new_3), "settle_status_3": "SETTLED",
                                      "bqr_pmt_status_3": "SUCCESS",
                                      "bqr_pmt_state_3": "SETTLED",
                                      "bqr_txn_amt_3": float(amount),
                                      "bqr_txn_type_3": "DYNAMIC_QR", "brq_terminal_info_id_3": terminal_info_id,
                                      "bqr_bank_code_3": "YES",
                                      "bqr_merchant_config_id_3": bqr_mc_id, "bqr_txn_primary_id_3": txn_id_new_3,
                ...
                # bqr_merchant_pan_db_new_3 = result["merchant_pan"].iloc[0]
                ...
                actual_db_values = {"txn_amt": amount_db,"pmt_mode": payment_mode_db,
                                    ...
                                    "bqr_rrn_2": bqr_rrn_db_new,
                                    "bqr_org_code_2": bqr_org_code_db_new,
                                    "txn_amt_3": amount_db_new_3, "pmt_mode_3": payment_mode_db_new_3,
                                    ...
                                    "payer_name_3": payer_name_db_new_3, "mid_3": mid_db_new_3,
                                    "tid_3": tid_db_new_3,
                                    "pmt_gateway_3": payment_gateway_db_new_3, "rrn_3": rr_number_db_new_3,
                                    ...
                                    # "bqr_merchant_pan_3": bqr_merchant_pan_db_new_3,
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expected_portal_values = {
                    ...
                    "username": app_username,
                    "txn_id": txn_id,
                    "date_time_2": date_and_time_portal_3,
                    "pmt_state_2": "AUTHORIZED",
                    "pmt_type_2": "BHARATQR",
                    "txn_amt_2": f"{str(amount)}.00",
                    "username_2": app_username,
                    "txn_id_2": txn_id_new,
                    "auth_code_2": "-" if auth_code_new is None else auth_code_new,
                    "rrn_2": "-" if rr_number_db_new is None else rr_number_db_new,
                    ...
                    "rrn_3": "-" if rr_number_db_new_3 is None else rr_number_db_new_3
                }
                ...
                logger.info(f"fetched date_time_3 from portal {date_time_2}")
                transaction_id_3 = transaction_details[0]['Transaction ID']
                logger.info(f"fetched txn_id_3 from portal {transaction_id_2}")
                total_amount_3 = transaction_details[0]['Total Amount'].split()
                logger.debug(f"fetched total_amount_3 from portal {total_amount_2}")
                auth_code_portal_3 = transaction_details[0]['Auth Code']
                logger.debug(f"fetched auth_code_3 from portal {auth_code_portal_2}")
                rr_number_3 = transaction_details[0]['RR Number']
                logger.debug(f"fetched rr_number_3 from portal {rr_number_2}")
                transaction_type_3 = transaction_details[0]['Type']
                logger.info(f"fetched txn_type_3 from portal {transaction_type_2}")
                status_3 = transaction_details[0]['Status']
                logger.info(f"fetched status_3 {status_2}")
                username_3 = transaction_details[0]['Username']
                logger.info(f"fetched username_3 from portal {username_2}")
