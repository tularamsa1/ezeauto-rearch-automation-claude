...
def test_common_100_103_283():
    """
    Sub Feature Code: UI_Common_PM_RP_RP_UPI_Success_Via_CheckStatus_Razorpay_Tid_dep
    Sub Feature Description: Tid Dep - Verification of a Remote Pay successful upi txn via Razorpay using check status
    TC naming code description:100: Payment Method, 103: RemotePay, 283: TC_283
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(
                    f"Fetching txn app_settlement_status from txn history for the txn : {txn_id}, {app_settlement_status}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"result of status_db from txn table in db validation : {status_db}")
                payment_mode_db = result["payment_mode"].iloc[0]
                logger.debug(f"result of payment_mode_db from txn table in db validation : {payment_mode_db}")
                amount_db = int(result["amount"].iloc[0])
                logger.debug(f"result of amount_db from txn table in db validation : {amount_db}")
                state_db = result["state"].iloc[0]
                logger.debug(f"result of state_db from txn table in db validation : {state_db}")
                acquirer_code_db = result["acquirer_code"].iloc[0]
                logger.debug(f"result of acquirer_code_db from txn table in db validation : {acquirer_code_db}")
                bank_code_db = result["bank_code"].iloc[0]
                logger.debug(f"result of bank_code_db from txn table in db validation : {bank_code_db}")
                settlement_status_db = result["settlement_status"].iloc[0]
                logger.debug(f"result of settlement_status_db from txn table in db validation : {settlement_status_db}")

                query = f"select * from payment_intent where id='{payment_intent_id}';"
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result of payment_intent table in db validation : {result}")
                payment_intent_status = result["status"].iloc[0]
                logger.debug(
                    f"result of payment_intent_status from txn table in db validation : {payment_intent_status}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expected_portal_values = {
                    ...
                    "username": app_username,
                    "txn_id": txn_id,
                    "auth_code": "-" if str(db_auth_code) == "None" else db_auth_code,
                    "rrn": db_rrn
                }
...
@pytest.mark.chargeSlipVal
@pytest.mark.portalVal
def test_common_100_103_284():
    """
    Sub Feature Code: UI_Common_PM_RP_RP_UPI_Failed_Via_CheckStatus_Razorpay_Tid_dep
    Sub Feature Description: Tid Dep - Verification of a Remote Pay failed upi txn via Razorpay using check status
    TC naming code description:100: Payment Method,103: RemotePay,284: TC_284
    """
    try:
...
def test_common_100_103_292():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Failed_Via_UPI_Callback_Razorpay _08_Tid_dep
    ...
    try:
        ...
        try:
            ...
            txn_ref = result['txn_ref'].values[0]
            txn_ref_3 = result['txn_ref3'].values[0]
            logger.debug(f"Query result, txn_ref : {txn_ref} and txn_ref3 : {txn_ref_3}")
            logger.debug(f"preparing data to perform the hash generation")
            rrn = str(random.randint(1000000000000, 9999999999999))
            amount_api = amount * 100
