...
def test_common_100_113_003():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Callback_Success_Via_HDFC_MultiAcc_ActiveLabel
    Sub Feature Description: Multi Account - Verifying a BQR success callback via HDFC when label is active
    TC naming code description: 100: Payment method, 113: MultiAcc_StaticQR, 003: Testcase ID
    """
    try:
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received for callbackHDFC api is : {response}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching Transaction id of original txn from transaction history of MPOS app: Txn Id = {txn_id}")
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_113_004():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Callback_Success_Via_HDFC_MultiAcc_InactiveLabel
    Sub Feature Description: Multi Account - Verifying a BQR success callback via HDFC when label is Inactive
    TC naming code description: 100: Payment method, 113: MultiAcc_StaticQR, 004: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
