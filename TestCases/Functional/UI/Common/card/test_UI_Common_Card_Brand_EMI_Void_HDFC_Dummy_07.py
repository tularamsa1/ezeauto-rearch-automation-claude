...
def test_common_100_115_07_066():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Void_Txn_For_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by brand fixed void transaction for root org via HDFC
    Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 066: TC066
    """
    try:
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='ALL' and card_type= 'CREDIT' " \
                f"and eze_emi_enabled=b'0';"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_067():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Void_Txn_For_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by brand fixed void transaction for root org via HDFC
    Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 067: TC067
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_068():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Void_Txn_For_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_9_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by brand fixed void transaction for root org via HDFC
    Dummy PG using EMV VISA Credit card with pin for 9 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 068: TC068
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        testsuite_teardown.update_subvention_plan_status(org_code=org_code, brand_id=brand_id, card_type='CREDIT', status=0)
