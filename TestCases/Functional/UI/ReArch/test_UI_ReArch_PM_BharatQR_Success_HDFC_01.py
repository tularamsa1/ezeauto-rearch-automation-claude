...
def test_common_rearch_0002():
    """
    Sub Feature Code: UI_ReArch_PM_BharatQR_Success_HDFC
    Sub Feature Description:
        Launch ReArch app, enter a random amount (100-200), tap ⋯ button →
        Order Details → Proceed → Payment Method overlay → Bharat QR → QR screen.
        Wait 10s for QR to register, cancel via back button. HDFC PG simulator
        sends success callback on "Yes, Cancel" confirm. Verify Payment Successful
        screen, then validate via App (TxnHistory → TxnDetail) and API (txnlist).
    TC naming code description:
        100: Payment Method, 103: BharatQR, 001: TC001
    ...
    NL Source Steps:
      ...
      Test Steps:
        ...
        13. cancel payment (back → "Yes, Cancel" — HDFC simulator success callback)
      ...
      App Validation:
        ...
        21. validate amount
        22. validate date time is not empty

      API Validation:
        23. txnlist API — pmt_status=AUTHORIZED, txn_amt, pmt_mode=BharatQR,
            txn_type=CHARGE, org_code, rrn, date
            (TODO: verify exact pmt_mode value for BharatQR in API response)

      Charge Slip Validation:
        24. PAID BY=UPI, RRN, BASE AMOUNT, date, time
            (TODO: verify exact PAID BY label for BharatQR on first run; no AUTH CODE for UPI)
    ...
    try:
        ...
        try:
            ...
            # Step 14: Verify payment success screen
            complete_page = ReArchCompletePage(app_driver)
            logger.info("Payment Successful screen confirmed")

            # Resolve txn from DB — no order_id used; fetch latest BharatQR txn for this org
