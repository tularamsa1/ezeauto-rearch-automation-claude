...
def test_common_100_103_228():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_UPG_success_VIA_Razorpay_when_UPGRefund_&_UPGAutoRefund_Disabled
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"generated txn_ref is : {txn_ref}")
            txn_ref_3 = "order_" + ''.join(random.choice(char) for _ in range(14))
            logger.debug(f"generated txn_ref_3 is : {txn_ref_3}")
            ...
            logger.debug(f"fetching txn_id from invalid_pg_request table is: {txn_id}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"fetched payment_mode from invalid_pg_request table is : {ipr_payment_mode}")
                ipr_bank_code = result["bank_code"].iloc[0]
                logger.debug(f"fetched bank_code from invalid_pg_request table is : {ipr_bank_code}")
                ipr_org_code = result["org_code"].iloc[0]
                logger.debug(f"fetched org_code from invalid_pg_request table is : {ipr_org_code}")
                ipr_amount = result["amount"].iloc[0]
                logger.debug(f"fetched amount from invalid_pg_request table is : {ipr_amount}")
                ipr_rrn = result["rrn"].iloc[0]
                logger.debug(f"fetched rrn from invalid_pg_request table is : {ipr_rrn}")
                ipr_auth_code = result["auth_code"].iloc[0]
                logger.debug(f"fetched auth_code from invalid_pg_request table is : {ipr_auth_code}")
                ipr_mid = result["mid"].iloc[0]
                logger.debug(f"fetched mid from invalid_pg_request table is : {ipr_mid}")
                ipr_tid = result["tid"].iloc[0]
                logger.debug(f"fetched tid from invalid_pg_request table is : {ipr_tid}")
                ipr_config_id = result["config_id"].iloc[0]
                logger.debug(f"fetched config_id from invalid_pg_request table is : {ipr_config_id}")
                ipr_pg_merchant_id = result["pg_merchant_id"].iloc[0]
                logger.debug(f"fetched pg_merchant_id from invalid_pg_request table is : {ipr_pg_merchant_id}")
...
def test_common_100_103_230():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"query result, created_time : {created_time_original} ")
            txn_type_original = result['txn_type'].values[0]
            logger.debug(f"query result, txn_type : {txn_type_original} ")
            auth_code_original = result['auth_code'].values[0]
            logger.debug(f"query result, auth_code : {auth_code_original} ")
            ...
            logger.debug(f"query result, created_time : {created_time_refunded} ")
            txn_type_refunded = result['txn_type'].values[0]
            logger.debug(f"query result, txn_type : {txn_type_refunded} ")
            auth_code_refunded = result['auth_code'].values[0]
            logger.debug(f"query result, auth_code : {auth_code_refunded} ")
            external_ref = result['external_ref'].values[0]
            logger.debug(f"query result, external_ref : {external_ref} ")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                query = f"select * from txn where id='{txn_id_original}'"
                logger.debug(f"Query to fetch original txn data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"fetching rrn from txn table: {rr_number_original_2}")
                # ****************************************************************************************************
...
def test_common_100_103_231():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_UPG_REFUND_PENDING_VIA_Razorpay_when_UPGRefund_Enabled_&_UPGAutoRefund_Enabled
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"rrn api original : {rrn_api_original}")
                mid_api = response['mid']
                logger.debug(f"mid received from api for original txn is : {mid_api}")
                tid_api = response['tid']
                logger.debug(f"tid received from api for original txn is : {tid_api}")
...
def test_common_100_103_232():
    ...
    TC naming code description: 100: Payment Method, 103: RemotePay, 232: TC232
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            mismatch_amount = 651
            logger.debug(f"mismatch_amount is: {mismatch_amount}")
