...
def test_common_100_108_024():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Callback_Success_AutoRefund_Enabled_Via_KOTAK
    Sub Feature Description: Verifying a BQR success callback via KOTAK when autorefund is enabled
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 024: Testcase ID
    """
    try:
        ...
        logger.debug(f"Query result for org_employee table is : {result}")
        org_code = result['org_code'].values[0]
        logger.debug(f"Query result, org_code : {org_code}")

        testsuite_teardown.revert_payment_settings_default(org_code, bank_code='KOTAK_WL', portal_un=portal_username,portal_pw=portal_password, payment_mode='BQRV4')
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query to result of bharatqr_txn table : {result}")
                bqr_status_code_db = result["status_code"].iloc[0]
                logger.debug(f"bqr_status_code_db is : {bqr_status_code_db}")
                bqr_state_db = result["state"].iloc[0]
                logger.debug(f"bqr_state_db is : {bqr_state_db}")
                bqr_txn_amt_db = result["txn_amount"].iloc[0]
                logger.debug(f"bqr_txn_amt_db is : {bqr_txn_amt_db}")
                bqr_txn_type_db = result["txn_type"].iloc[0]
                logger.debug(f"bqr_txn_type_db is : {bqr_txn_type_db}")
                bqr_terminal_info_id_db = result["terminal_info_id"].iloc[0]
                logger.debug(f"bqr_terminal_info_id_db is : {bqr_terminal_info_id_db}")
                bqr_bank_code_db = result["bank_code"].iloc[0]
                logger.debug(f"bqr_bank_code_db is : {bqr_bank_code_db}")
                bqr_merchant_config_id_db = result["merchant_config_id"].iloc[0]
                logger.debug(f"bqr_merchant_config_id_db is : {bqr_merchant_config_id_db}")
                bqr_transaction_primary_id_db = result["transaction_primary_id"].iloc[0]
                logger.debug(f"bqr_transaction_primary_id_db is : {bqr_transaction_primary_id_db}")
                bqr_rrn_db = result["rrn"].iloc[0]
                logger.debug(f"bqr_rrn_db is : {bqr_rrn_db}")
                bqr_org_code_db = result["org_code"].iloc[0]
                logger.debug(f"bqr_org_code_db is : {bqr_org_code_db}")
...
@pytest.mark.chargeSlipVal
@pytest.mark.portalVal
def test_common_100_108_025():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Callback_Success_AutoRefund_Disabled_Via_KOTAK
    Sub Feature Description:Verifying a BQR success callback via KOTAK when autorefund is disabled
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 025: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_100_108_026():
    """
    Sub Feature Code: UI_Common_PM_UPI_StaticQR_Generation_Failure_With_Invalid_Username_Via_KOTAK
    ...
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 026: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Query to fetch data from the bharatqr_merchant_config for the {org_code} : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for bharatqr_merchant_config table : {result}")
        mid = result['mid'].values[0]
        logger.debug(f"Query result of mid from the bharatqr_merchant_config table : mid : {mid}")
        tid = result['tid'].values[0]
        logger.debug(f"Query result of tid from the bharatqr_merchant_config table : tid : {tid}")
        merchant_pan = result['merchant_pan'].values[0]
        logger.debug(f"Query result of  merchant_pan from the bharatqr_merchant_config table : merchant_pan : {merchant_pan}")
        config_id = result['id'].values[0]
        logger.debug(f"Query result of config_id from the bharatqr_merchant_config table : config_id : {config_id}")
        ...
        try:
            ...
            #generating STATIC QR with Username is not registered with this merchant
            username_generated_randomly = random.randint(0, 10 ** 10)

            api_details = DBProcessor.get_api_details('generate_BQRV4_staticqr_KOTAK', request_body={
                "username": portal_username,
                "password": portal_password,
                "qrUserMobileNo": app_username,
                "qrUserName": username_generated_randomly,
                "qrCodeType": "BHARAT",
                "qrOrgCode": org_code,
                "qrCodeFormat": "STRING",
                "mid": mid,
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response of generate_BQRV4_staticqr_KOTAK api is : {response}")
            success = response['success']
            logger.debug(f"Response of success from generate_BQRV4_staticqr_KOTAK api is: {success}")
            error_message = response['errorMessage']
            logger.debug(f"Response of error_message from generate_BQRV4_staticqr_KOTAK api is: {error_message}")
...
@pytest.mark.chargeSlipVal
@pytest.mark.portalVal
def test_common_100_108_028():
    """
    Sub Feature Code: UI_Common_BQR_StaticQR_KOTAK_2_Success_Callback_with_Same_RRN
    Sub Feature Description: Initiate BQR static QR via api and perform 2 BQR success callback with same rrn
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 028: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Query result of tid from the bharatqr_merchant_config table : tid : {tid}")
        merchant_pan = result['merchant_pan'].values[0]
        logger.debug(f"Query result merchant_pan from the bharatqr_merchant_config table : merchant_pan : {merchant_pan}")
        config_id = result['id'].values[0]
        logger.debug(f"Query result of config_id from the bharatqr_merchant_config table : config_id : {config_id}")
        terminal_info_id = result['terminal_info_id'].values[0]
        logger.debug(f"Query result of terminal_info_id from the bharatqr_merchant_config table : terminal_info_id : {terminal_info_id}")
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for upi_merchant_config table : {result}")
            upi_mc_id = result['id'].values[0]
            logger.debug(f"Query result of upi_mc_id from the upi_merchant_config table : tid : {upi_mc_id}")
            vpa = result['vpa'].values[0]
            logger.debug(f"Query result of vpa from the upi_merchant_config table : tid : {vpa}")
            ...
            logger.debug(f"Fetching API Response for call back : {response}")
            status = response['status']
            logger.debug(f"Response of status from call back api is: {status}")

            query = "select * from txn where org_code = '" + str(org_code) + "' and rr_number = '" + str(rrn) + "'order by created_time desc limit 1; "
            logger.debug(f"Query to fetch data from the txn for the {org_code} : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for txn table : {result}")
            txn_id = result["id"].iloc[0]
            logger.debug(f"Query result of txn_id from the txn table : {txn_id}")
            status_db = result["status"].iloc[0]
            logger.debug(f"Query result of status_db from the txn table : {status_db}")
            payment_mode_db = result["payment_mode"].iloc[0]
            logger.debug(f"Query result of payment_mode_db from the txn table : {payment_mode_db}")
            amount_db = int(result["amount"].iloc[0])
            logger.debug(f"Query result of amount_db from the txn table : {amount_db}")
            state_db = result["state"].iloc[0]
            logger.debug(f"Query result of state_db from the txn table : {state_db}")
            payment_gateway_db = result["payment_gateway"].iloc[0]
            logger.debug(f"Query result of payment_gateway_db from the txn table : {payment_gateway_db}")
            acquirer_code_db = result["acquirer_code"].iloc[0]
            logger.debug(f"Query result of acquirer_code_db from the txn table : {acquirer_code_db}")
            bank_code_db = result["bank_code"].iloc[0]
            logger.debug(f"Query result of bank_code_db from the txn table : {bank_code_db}")
            settlement_status_db = result["settlement_status"].iloc[0]
            logger.debug(f"Query result of settlement_status_db from the txn table : {settlement_status_db}")
            tid_db = result['tid'].values[0]
            logger.debug(f"Query result of tid_db from the txn table : {tid_db}")
            mid_db = result['mid'].values[0]
            logger.debug(f"Query result of mid_db from the txn table : {mid_db}")
            org_code_txn = result['org_code'].values[0]
            logger.debug(f"Query result of org_code_txn from the txn table : {org_code_txn}")
            txn_type = result['txn_type'].values[0]
            logger.debug(f"Query result of txn_type from the txn table : {txn_type}")
            created_time = result['created_time'].values[0]
            logger.debug(f"Query result of created_time from the txn table : {created_time}")
            txn_rrn_original = result['rr_number'].values[0]
            logger.debug(f"Query result of txn_rrn_original from the txn table : {txn_rrn_original}")
            txn_external_ref = result['external_ref'].values[0]
            logger.debug(f"Query result of txn_external_ref from the txn table : {txn_external_ref}")
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Fetching API Response for Duplicate call back is : {response}")

            query = "select * from txn where org_code = '" + str(org_code) + "' and rr_number = '" + str(rrn) + "'order by created_time desc limit 1; "
            logger.debug(f"Query to fetch data of second callback response for txn table is : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result of second callback response for txn table is : {result}")
            txn_id_2 = result["id"].iloc[0]
            logger.debug(f"Query result of txn_id_2 from the txn table is : {txn_id_2}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                bqr_status_code_db = result["status_code"].iloc[0]
                logger.debug(f"Query result of bqr_status_code_db from bharatqr_txn table is : {bqr_status_code_db}")
                bqr_state_db = result["state"].iloc[0]
                logger.debug(f"Query result of bqr_state_db from bharatqr_txn table is : {bqr_state_db}")
                bqr_txn_amt_db = result["txn_amount"].iloc[0]
                logger.debug(f"Query result of bqr_txn_amt_db from bharatqr_txn table is : {bqr_txn_amt_db}")
                bqr_txn_type_db = result["txn_type"].iloc[0]
                logger.debug(f"Query result of bqr_txn_type_db from bharatqr_txn table is : {bqr_txn_type_db}")
                bqr_terminal_info_id_db = result["terminal_info_id"].iloc[0]
                logger.debug(f"Query result of bqr_terminal_info_id_db from bharatqr_txn table is : {bqr_terminal_info_id_db}")
                bqr_bank_code_db = result["bank_code"].iloc[0]
                logger.debug(f"Query result of bqr_bank_code_db from bharatqr_txn table is : {bqr_bank_code_db}")
                bqr_merchant_config_id_db = result["merchant_config_id"].iloc[0]
                logger.debug(f"Query result of bqr_merchant_config_id_db from bharatqr_txn table is : {bqr_merchant_config_id_db}")
                bqr_transaction_primary_id_db = result["transaction_primary_id"].iloc[0]
                logger.debug(f"Query result of bqr_transaction_primary_id_db from bharatqr_txn table is : {bqr_transaction_primary_id_db}")
                bqr_rrn_db = result["rrn"].iloc[0]
                logger.debug(f"Query result of bqr_rrn_db from bharatqr_txn table is : {bqr_rrn_db}")
                bqr_org_code_db = result["org_code"].iloc[0]
                logger.debug(f"Query result of bqr_org_code_db from bharatqr_txn table is : {bqr_org_code_db}")
...
@pytest.mark.chargeSlipVal
@pytest.mark.portalVal
def test_common_100_108_030():
    """
    Sub Feature Code: UI_Common_UPI_StaticQR_KOTAK_2_Success_Callback_with_Same_RRN
    Sub Feature Description: Initiate UPI static QR via api and perform 2 BQR success callback with same rrn
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 030: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response of generate_BQRV4_staticqr_KOTAK is : {response}")
            generateqr_publish_id = response["publishId"]
            logger.debug(f"generateqr_publish_id is : {generateqr_publish_id}")
            ...
            logger.debug(f"Query result of created_time from the txn table : {created_time}")
            txn_auth_code = result['auth_code'].values[0]
            logger.debug(f"Query result of txn_auth_code from the txn table : {txn_auth_code}")
            txn_rrn_number = result['rr_number'].values[0]
            logger.debug(f"Query result of txn_rrn_number from the txn table : {txn_rrn_number}")
            txn_external_ref = result['external_ref'].values[0]
            logger.debug(f"Query result of txn_external_ref from the txn table : {txn_external_ref}")

            # 2nd callback for UPI
            api_details = DBProcessor.get_api_details('callbackKotak', request_body={
                "mid": mid,
                "ref_no": rrn,
                "txn_amount": "{:.2f}".format(amount),
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response of Duplicate call back is : {response}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Query result of upi_status_db from upi_txn table is : {upi_status_db}")
                upi_txn_type_db = result["txn_type"].iloc[0]
                logger.debug(f"Query result of upi_txn_type_db from upi_txn table is : {upi_txn_type_db}")
                upi_bank_code_db = result["bank_code"].iloc[0]
                logger.debug(f"Query result of upi_bank_code_db from upi_txn table is : {upi_bank_code_db}")
                upi_mc_id_db = result["upi_mc_id"].iloc[0]
                logger.debug(f"Query result of upi_mc_id_db from upi_txn table is : {upi_mc_id_db}")
                upi_org_code_db = result["org_code"].iloc[0]
                logger.debug(f"Query result of upi_org_code_db from upi_txn table is : {upi_org_code_db}")
                upi_txn_additional_field1_db = result["additional_field1"].iloc[0]
                logger.debug(f"Query result of upi_txn_additional_field1_db from upi_txn table is : {upi_txn_additional_field1_db}")
                upi_txn_additional_field3_db = result["additional_field3"].iloc[0]
                logger.debug(f"Query result of upi_txn_additional_field3_db from upi_txn table is : {upi_txn_additional_field3_db}")
                upi_txn_resp_code_db = result["resp_code"].iloc[0]
                logger.debug(f"Query result of upi_txn_resp_code_db from upi_txn table is : {upi_txn_resp_code_db}")
                upi_txn_txn_type_db = result["txn_type"].iloc[0]
                logger.debug(f"Query result of upi_txn_txn_type_db from upi_txn table is : {upi_txn_txn_type_db}")
