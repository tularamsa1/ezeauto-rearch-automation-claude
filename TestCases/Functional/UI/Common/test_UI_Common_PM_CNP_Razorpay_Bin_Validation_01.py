...
@pytest.mark.dbVal
def test_common_100_103_428():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Bin_Validation_Payment_card_type_is_CREDIT_In_Bin_Info_&DEBIT_Card_Payment_Option
    Sub Feature Description:Verify payment_option under cnp_txn table for success txn when payment_card_type is CREDIT in bin_info table and select the debit card payment option
    TC naming code description: 100: Payment Method, 103: RemotePay, 428: TC_428
    """
    ...
    try:
        ...
        try:
            ...
            query = f"select * from bin_info where bin='526731';"
            result = DBProcessor.getValueFromDB(query=query)
            bin_payment_card_brand = result['payment_card_brand'].values[0]
            bin_payment_card_type = result['payment_card_type'].values[0]
            ...
            cnp_payment_card_brand = result['payment_card_brand'].values[0]
            cnp_payment_card_type = result['payment_card_type'].values[0]
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "card_type": "CNP_DC",
                    "payment_card_type": bin_payment_card_type,
                    "payment_card_brand": bin_payment_card_brand
                }
                ...
                actual_db_values = {
                    ...
                    "payment_card_type": cnp_payment_card_type,
                    "payment_card_brand": cnp_payment_card_brand
                }
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.dbVal
def test_common_100_103_429():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Bin_Validation_Payment_card_type_is_DEBIT_In_Bin_Info_&CREDIT_Card_Payment_Option
    Sub Feature Description: Verify payment_option under cnp_txn table for success txn when payment_card_type is DEBIT in bin_info table and select the credit card payment option
    TC naming code description: 100: Payment Method, 103: RemotePay, 429: TC_429
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        try:
            ...
            query = f"select * from bin_info where bin='411111';"
            result = DBProcessor.getValueFromDB(query=query)
            bin_payment_card_brand = result['payment_card_brand'].values[0]
            bin_payment_card_type = result['payment_card_type'].values[0]
...
def test_common_100_103_431():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Bin_Validation_Payment_Card_Type_In_Upper_Case_In_Bin_Info_Txn_Success
    Sub Feature Description:Verify that when payment_card_type field at bin_info table is in upper case txn should get success
    TC naming code description: 100: Payment Method, 103: RemotePay, 431: TC_431
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query=query)
            if (len(result)) < 1:
                logger.debug(f"Card does not exist in Bin table")
            else:
                query = f"update bin_info set payment_card_type='DEBIT' where bin='411111';"
                logger.debug(f"Query for updating payment_card_type: {query}")
                result = DBProcessor.setValueToDB(query=query)
                logger.debug(f"Result of Update query : {result}")
                refresh_db()
