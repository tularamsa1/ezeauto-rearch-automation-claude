...
def test_common_100_102_002():
    ...
    TC naming code description: 100: Payment Method, 102: BQR, 002: TC002
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_102_003():
    ...
    TC naming code description: 100: Payment Method, 102: BQR, 003: TC003
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_102_012():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Callback_AfterExpiry_AutoRefund_Disabled_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Callback After QR code gets expired when auto refund is Disabled via HDFC
    TC naming code description: 100: Payment Method, 102: BQR, 012: TC012
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created time for {txn_id} : {created_time} ")
...
def test_common_100_102_013():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Callback_AfterExpiry_AutoRefund_Enabled_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Callback After QR code gets expired when auto refund is Enabled via HDFC
    TC naming code description: 100: Payment Method, 102: BQR, 013: TC013
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
