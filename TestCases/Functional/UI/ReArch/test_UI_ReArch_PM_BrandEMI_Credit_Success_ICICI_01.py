...
def test_common_rearch_0055():
    """
    Sub Feature Code: UI_ReArch_PM_BrandEMI_Credit_Success_ICICI
    Sub Feature Description:
        Launch ReArch app, login, refresh settings via Payment History,
        navigate to Catalog, select BRAND_UI brand and Product2 (₹8,000),
        clear amount and enter ₹9,500, proceed to EMI via ICICI Bank
        Credit Card, select 3-month tenure, verify breakup (Order Total
        ₹9,500, Interest +₹206.56, Additional Cashback -₹500,
        Net Effective Price ₹9,206.56), enter random 6-digit IMEI,
        proceed through card type selection (Visa Credit EMV), verify
        success, then validate via App, API, and Charge Slip.
    ...
    NL Source Steps:
      ...
      Test Steps:
        ...
        5.  click on catalog
        6.  click on brand_ui (BRAND_UI BRAND_UI)
        7.  click on product_2 (Brand_UI_Product2 ₹8,000)
        8.  clear amount and enter amount 9500
        9.  click on proceed button
        10. wait until bank button is visible
        11. click on ICICI Bank Credit Card
        12. click on 3 month tenure
        13. click on view breakup
        14. verify Order Total = ₹9,500, Interest = +₹206.56,
            Additional Cashback = -₹500, Net Effective Price = ₹9,206.56
        15. click on back button
        16. click on Proceed button
        17. tap and enter random 6 digit IMEI number
        18. click on proceed button
        19. click on proceed button
        20. select Visa Credit(EMV)
        21. click on Accept more payment
        22. validate app, api and chargeslip
    ...
    try:
        ...
        try:
            ...
            login_page.perform_click(
                (AppiumBy.XPATH, "//android.widget.Button[@text='Catalog']")
            )
            ...
            # Step 7: Click on Product2 (₹8,000)
            time.sleep(2)  # TODO: adjust wait for product list load
            login_page.perform_click(
                (AppiumBy.XPATH, "//android.widget.Button[@text='BRAND_UI Brand_UI_Product2 738750 \u20b98,000']")
            )
            logger.debug("Clicked Product2 (₹8,000)")

            # Step 8: Clear amount and enter ₹9,500 (Android system keyboard)
            time.sleep(2)  # TODO: adjust wait for amount screen
            home_page.clear_amount()
            home_page.enter_amount_via_keyboard("9500")
            logger.debug("Cleared amount and entered 9500")

            # Step 9: Click Proceed
            order_details_page = ReArchOrderDetailsPage(app_driver)
            order_details_page.click_proceed()
            logger.debug("Clicked Proceed")

            # Step 10: Wait until Bank button is visible
            emi_page = ReArchEMIPage(app_driver)
            emi_page.wait_for_bank_button()
            logger.debug("Bank button visible on EMI screen")

            # Step 11: Click ICICI Bank Credit Card
            emi_page.click_icici_bank_credit_card()
            logger.debug("ICICI Bank Credit Card selected")

            # Step 12: Click 3-month tenure
            emi_page.click_3m_plan_first()  # TODO: replace with exact plan locator after first run
            logger.debug("3-month EMI plan selected")

            # Step 13: Click View Breakup
            emi_page.click_view_breakup()
            logger.debug("View Breakup opened")

            # Step 14: Fetch breakup values for validation (no asserts here)
            actual_order_total = emi_page.fetch_order_total()
            actual_interest = emi_page.fetch_interest_charged()
            actual_additional_cashback = emi_page.fetch_additional_cashback()
            actual_net_effective_price = emi_page.fetch_net_effective_price()
            logger.info(f"Breakup — Order Total: {actual_order_total}, "
                        f"Interest: {actual_interest}, "
                        f"Additional Cashback: {actual_additional_cashback}, "
                        f"Net Effective Price: {actual_net_effective_price}")
            ...
            # Step 17: Enter random 6-digit IMEI number
            ...
            # Step 18: Click Proceed on Order Details
            order_details_page.click_proceed()
            logger.debug("Order Details: clicked Proceed (with IMEI)")

            # Step 19: Click Proceed (second proceed)
            order_details_page.click_proceed()
            logger.debug("Order Details: clicked Proceed (second)")

            # Step 20: Select Visa Credit (EMV)
            ...
            query = (
                ...
                f"AND payment_mode='CARD' "  # TODO: verify payment_mode for Brand EMI ICICI
            ...
            # Step 21: Click Accept More Payments
        ...
        # -- App Validation (Breakup + TxnDetail) --
        if ConfigReader.read_config("Validations", "app_validation") == "True":
            logger.info(f"Started APP validation for the test case: {testcase_id}")
            try:
                # Breakup verification (ICICI 3m with Additional Cashback)
                expected_breakup_values = {
                    "order_total": "\u20b99,500",
                    "interest_charged": "+\u20b9206.56",
                    "additional_cashback": "-\u20b9500",
                    "net_effective_price": "\u20b99,206.56",
                }
                actual_breakup_values = {
                    "order_total": actual_order_total,
                    "interest_charged": actual_interest,
                    "additional_cashback": actual_additional_cashback,
                    "net_effective_price": actual_net_effective_price,
                }
                ...
                app_txn_status = txn_detail_page.fetch_status("9,500.00")  # TODO: verify displayed amount
                app_date_time  = txn_detail_page.fetch_date_time()
                app_amount     = txn_detail_page.fetch_amount("9,500.00")  # TODO: verify displayed amount
                logger.info(
        ...
        if ConfigReader.read_config("Validations", "api_validation") == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "txn_amt":       float(amount),  # TODO: verify if 9500.0 on first run
                    "pmt_mode":      "CARD",         # TODO: verify payment_mode for ICICI EMI
                    "txn_type":      "CHARGE",
                    "acquirer_code": "HDFC",          # TODO: verify acquirer_code for ICICI EMI
                    "org_code":      org_code,
                    "rrn":           rrn,
                    "date":          date,
                    "pmt_card_brand": "VISA",
        ...
        if ConfigReader.read_config("Validations", "charge_slip_validation") == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    "RRN":          rrn,
                    "AUTH CODE":    auth_code,
                    "BASE AMOUNT:": f"Rs.{int(amount):,}.00",  # TODO: verify if 9,500 on first run
                    "date":         txn_date,
                    "time":         txn_time,
                }
