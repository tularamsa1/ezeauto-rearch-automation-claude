    ...
    try:
        ...
        if str(ConfigReader.read_config("ParallelExecution", "deviceOnly")).lower() == 'true':
            ...
            query = f"UPDATE terminal_info SET status = 'ACTIVE' WHERE org_code='{org_code}' and status = 'INACTIVE' and " \
                    f"acquirer_code='HDFC' and payment_gateway='HDFC' and device_serial = '{GlobalVariables.str_device_id}';"
            logger.debug(f"Query to fetch data from the terminal_info for the {org_code} : {query}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"From response_refund fetch amount for for refund txn : {api_amount_refund}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "order_id_2": order_id,
                    "pmt_msg_2": "REFUND SUCCESSFUL",
                    "customer_name_2": "MTIP14 MCD 13A",
                    "settle_status_2": "PENDING",
                    "auth_code_2": refund_auth_code,
                    "date_2": refund_date_and_time,
                    "device_serial_2": device_serial,
...
def test_common_100_115_130():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Query result, amount for refund txn : {refund_total_amount}")
            refund_created_time = result['created_time'].values[0]
            logger.debug(f"Query result, created_time for refund txn for refund txn : {refund_created_time}")
            refund_acquirer_code = result['acquirer_code'].values[0]
            logger.debug(f"Query result, acquirer_code for refund txn : {refund_acquirer_code}")
            refund_auth_code = result['auth_code'].values[0]
            logger.debug(f"Query result, auth_code for refund txn : {refund_auth_code}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "order_id_2": order_id,
                    "pmt_msg_2": "REFUND SUCCESSFUL",
                    "customer_name_2": "RUPAY CARD IMAGE 02",
                    "settle_status_2": "PENDING",
                    "auth_code_2": refund_auth_code,
                    "date_2": refund_date_and_time,
                    "device_serial_2": device_serial,
                    "batch_number_2": refund_batch_number,
                    "card_type_desc_2": "*0025 EMV",
                    "mid_2": mid,
                    "tid_2": tid
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "card_type_2": "RUPAY",
                    "display_pan_2": "0025",
                    "name_on_card_2": "RUPAY CARD IMAGE 02      /"
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
