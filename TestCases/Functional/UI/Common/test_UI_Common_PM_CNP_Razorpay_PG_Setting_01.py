...
@pytest.mark.apiVal
def test_common_100_103_466():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_PG_Setting_PG_Made_Inactive
    Sub Feature Description:Verify Error Message when PG made Inactive in settings
    TC naming code description: 100: Payment Method, 103: RemotePay, 466: TC_466
    """
    expected_error_message = "Cannot process the transaction as the payment gateway settings are not configured. Contact Razorpay POS Support for more information."
    try:
        ...
        try:
            ...
            query = f"update merchant_pg_config set status = 'INACTIVE' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
            logger.debug(f"Query to update status Settings to INACTIVE : {query}")
            result = DBProcessor.setValueToDB(query=query)
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Received Success Status: {response['success']}")
                logger.debug(f"Received Error message : {response['errorMessage']}")
                assert response["success"] == expected_response, "Response is not matching."
                assert response["errorMessage"] == expected_error_message, "Error message not matching"
            except Exception as e:
        ...
        try:
            query = f"update merchant_pg_config set status = 'ACTIVE' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
            logger.debug(f"Query to update status Settings to ACTIVE : {query}")
            result = DBProcessor.setValueToDB(query=query)
            logger.debug(f"query result after updating : {result}")
...
def test_common_100_103_467():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_PG_Setting_MLE_Enabled
    Sub Feature Description:Verify Txn flow happens normal if we enable the MLE for RAZORPAY
    TC naming code description: 100: Payment Method, 103: RemotePay, 467: TC_467
    """
    ...
    try:
        ...
        try:
            ...
            query = f"update merchant_pg_config set mle_enabled = 'ACTIVE' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
            logger.debug(f"Query to update MLE Settings to Active : {query}")
            result = DBProcessor.setValueToDB(query=query)
            logger.debug(f"query result after updating : {result}")
        ...
        try:
            query = f"update merchant_pg_config set mle_enabled = 'INACTIVE' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
            logger.debug(f"Query to update MLE Settings to Inactive : {query}")
            result = DBProcessor.setValueToDB(query=query)
            logger.debug(f"query result after updating : {result}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_468():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_PG_Setting_Hashing_Algorithm_SHA1_Enabled
    Sub Feature Description:Verify Txn flow happens normal if we configure the SHA1 Hashing Algorithm
    TC naming code description: 100: Payment Method, 103: RemotePay, 468: TC_468
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
