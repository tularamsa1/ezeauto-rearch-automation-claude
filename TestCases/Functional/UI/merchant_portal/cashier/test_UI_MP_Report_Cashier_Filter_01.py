...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_709_024():
    """
    Sub Feature Code: UI_MP_Report_based_on_order_number_as_Cashier
    Sub Feature Description: Verifying txn details on Report Page using Filter order number when logged in as Cashier
    TC naming code description:
    700: Merchant Portal
    709: Cashier
    024: TC024
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and external_ref='" + order_number + "' and username = '" + txn_username + "' order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_709_025():
    """
    Sub Feature Code: UI_MP_Report_based_on_calender_dates_as_Cashier
    Sub Feature Description: Verifying txn details on Report Page using Filter calender dates when logged in as Cashier
    TC naming code description:
    700: Merchant Portal
    709: Cashier
    025: TC025
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where " \
                    "(id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "') and org_code='" + org_code + "' and username = '" + txn_username + "' order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
