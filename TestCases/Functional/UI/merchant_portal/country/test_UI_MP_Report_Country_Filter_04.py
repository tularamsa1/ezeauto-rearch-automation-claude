...
def test_mp_700_701_038():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_CNF_Pre_Auth_as_country_head
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status CNF_Pre_Auth when logging in as Country Head
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_701_039():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Expired_as_country_head
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Expired as Country Head
    TC naming code description:
    700: Merchant Portal
    701: Country
    039: TC039
...
def test_mp_700_701_040():
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "'  AND (status='PENDING' OR status='FAILED' OR status='AUTHORIZED' OR status='AUTHORIZED_REFUNDED' OR status='VOIDED' OR status='REVERSED' OR status='REFUND_PENDING' OR status='EXPIRED' OR status='PRE_AUTH' OR status='REL_PRE_AUTH' OR status='CNF_PRE_AUTH') order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
