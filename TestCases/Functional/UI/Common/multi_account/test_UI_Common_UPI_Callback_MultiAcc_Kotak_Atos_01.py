...
def test_common_100_109_031():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Callback_MultiAcc_Kotak_ATOS
    Sub Feature Description: Multi Account - Performing a Authorized Txn using pure UPI Success callback for KotakAtos
    TC naming code description: 100: Payment Method 109: MultiAcc_UPI, 031: TC031
    """
    try:
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            txn_ref_id = result["txn_ref"].iloc[0]
            ...
            logger.debug(
                f"Fetching Txn_id,Txn ref id,Auth code,RRN from data base : Txn_id : {txn_id}, Txn ref id : {txn_ref_id}, Auth code : {auth_code}, RRN : {rrn}")
            api_details = DBProcessor.get_api_details('callbackUpiKotakAtos', request_body={
                "primary_id": txn_ref_id,
