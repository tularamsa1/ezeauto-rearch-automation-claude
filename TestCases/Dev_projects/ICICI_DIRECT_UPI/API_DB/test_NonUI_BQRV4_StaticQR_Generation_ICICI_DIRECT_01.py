...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_d102_108_018():
    """
    Sub Feature Code: NonUI_Common_BQRV4_UPI_Failed_StaticQR_Generation_With_Invalid_Username_ICICI_DIRECT
    Sub Feature Description: Performing failed static for BQRV4 UPI QR generation with username under different merchant via ICICI DIRECT
    TC naming code description: d102: ICICI DIRECT UPI Dev, 108: BQRv4 StaticQR, 018: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
