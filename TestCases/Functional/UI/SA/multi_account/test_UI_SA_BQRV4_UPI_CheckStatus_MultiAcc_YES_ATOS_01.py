...
def test_sa_100_110_021():
    """
    Sub Feature Code: UI_SA_PM_BQRV4_UPI_Success_Checkstatus_MultiAcc_YES_ATOS
    Sub Feature Description: Multi Account - Verification of a successful bqrv4 upi txn via YES ATOS using checkstatus
    TC naming code description: 100: Payment Method 110: MultiAcc_BQR, 021: TC021
    """
    try:
        ...
        try:
            ...
            logger.debug(
                f"Fetching Txn_id,Auth code,RRN from data base :Txn_id :{txn_id},Auth code :{auth_code},RRN : {rrn}")
...
@pytest.mark.appVal
@pytest.mark.portalVal
def test_sa_100_110_022():
    """
    Sub Feature Code: UI_SA_PM_BQRV4_UPI_Failed_Checkstatus_MultiAcc_YES_ATOS
    Sub Feature Description: Multi Account - Verification of a failed bqrv4 upi txn via YES ATOS using checkstatus
    TC naming code description: 100: Payment Method 110: MultiAcc_BQR, 022: TC022
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug("Restarting app after callback")
            app_driver.reset()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {"txn_amt": float(amount),"pmt_mode": "UPI","pmt_status": "FAILED",
                                      ...
                                      "settle_status": "FAILED","upi_pmt_status": "FAILED",
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_sa_100_110_038():
    """
    Sub Feature Code: UI_SA_PM_BQRV4_UPI_Success_Checkstatus_with_2nd_label_MultiAcc_YES_ATOS
    Sub Feature Description: Multi Account - Verification of a successful bqrv4 upi txn via YES ATOS using checkstatus with 2nd account label
    TC naming code description: 100: Payment Method 110: MultiAcc_BQR, 038: TC038
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            auth_code = "AE" + txn_id.split('E')[1]
            rrn = "RE" + txn_id.split('E')[1]
            logger.debug(f"Fetching Txn_id,Auth code,RRN from data base :Txn_id :{txn_id},Auth code :{auth_code},RRN : {rrn}")
            payment_page.click_on_back_btn()
            payment_page.click_on_transaction_cancel_yes()
            logger.debug("Pressed back button and clicked Yes on transaction cancel page")
            app_payment_status = payment_page.fetch_payment_status()
