...
def test_mp_700_705_006():
    """
    Sub Feature Code: UI_MP_Calender_txn_for_specific_location_using_dropdown_values_as_Taluk_head
    Sub Feature Description: Verifying txn details on dashboard  for specific location using dropdown values when logging in as Taluk Head
    ...
    try:
        ...
        try:
            ...
            location_hierarchy = ["Akole", "Mumbai1", "Hub1",
                                  str(node_name)]
            ...
            query = "select sum(amount) from txn where id between '" + current_start_date_1 + "' AND '" \
                    + current_end_date_1 + "' and org_code='" + org_code + "' and username NOT IN ('"+country_username+"', '" + region_username + "', '" + state_username + "', '" + district_username + "') and username = '" + txn_username + "' and status = 'AUTHORIZED';"
            logger.info(f"query to fetch total sales of current : {query}")
            result = DBProcessor.getValueFromDB(query)
            total_sales_db = ["0.00" if str(result).split()[-1] == "None" else str(result).split()[-1]]
            ...
            logger.debug(f"total txns from DB : {total_txns_db}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                expected_portal_values = {
                    "total_sales": total_sales_db[0] + "0",
                    "total_txns": total_txns_db,
                    "org_code": org_code,
                    # "username": username_db
                }
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_705_007():
    """
    Sub Feature Code: UI_MP_Calender_txn_for_specific_location_using_search_option_as_Taluk_head
    Sub Feature Description: Verifying txn details on dashboard  for specific location using search option when logging in as Taluk Head
    TC naming code description:
    700: Merchant Portal
    705: Taluk
    007: TC007
