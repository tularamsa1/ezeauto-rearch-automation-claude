...
@pytest.mark.appVal
def test_common_100_103_380():
    """
    Sub Feature Code: UI_Common_PM_CNP_First_Txn_Payment_Initiated_Pay_Btn_Clicked_Create_New_Intent_Txn_In_Progress_Message_Shown
    Sub Feature Description: When payment is inititaed and pay button clicked then cancel the old txn by creating new intent
    TC naming code description: 100: Payment Method, 103: RemotePay, 380: TC380
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "date": date,
                    "status_2": False,
                    "msg_2": "Payment in progress for same Ref# " + str(order_id) + " and amount " + str(
                        amount) + ".00. Please wait few minutes and check the status of the payment(s) again before you proceed.",
                    "err_msg_2": "Payment in progress for same Ref# " + str(order_id) + " and amount " + str(
                        amount) + ".00. Please wait few minutes and check the status of the payment(s) again before you proceed.",
                    "err_code_2": "EZETAP_4000008",
                    "real_code_2": "PAYMENT_IN_PROGRESS"
                }
                ...
                actual_api_values = {
                    ...
                    "status_2": status,
                    "msg_2": message,
                    "err_msg_2": error_message,
                    "err_code_2": error_code,
                    "real_code_2": real_code
                }
...
def test_common_100_103_381():
    """
    Sub Feature Code: UI_Common_PM_CNP_First_Payment_Success_New_Link_Not_Created_With_Same_Order_ID
    Sub Feature Description: One attempt payment success on link, new link with same orderid shouldn't be created
    TC naming code description: 100: Payment Method, 103: RemotePay, 381: TC381
    """
    ...
    try:
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received from Remotepay_Initiate api for second time:  {response}")
            ...
            logger.debug(f"From response fetch errorMessage : {message}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "err_msg": f"You have already made a similar payment for the same Ref# {order_id} and amount {amount}.00. Please check the status of the payment(s) made before you proceed."
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_382():
    """
    Sub Feature Code: UI_Common_PM_CNP_First_Payment_Failed_New_Link_Created_With_Same_Order_ID
    Sub Feature Description: One attempt payment failed on link, new link with same orderid should be created
    TC naming code description: 100: Payment Method, 103: RemotePay, 382: TC382
    """
    try:
        # -------------------------------Reset Settings to default(started)--------------------------------------------
        testcase_id = sys._getframe().f_code.co_name
        ...
        try:
            ...
            success_status = response.get('success')
            logger.debug(f"Status for second time link creation : {success_status}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            logger.info(f"Started API validation for the test case : {testcase_id}")
            try:
                logger.info("Started API validation")
                date = date_time_converter.db_datetime(posting_date)
                expected_api_values = {
                    "pmt_status": "FAILED",
                    "txn_amt": amount,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                query = f"select * from txn where id='{txn_id}' limit 1"
                logger.debug(f"Query to fetch data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                pmt_status_db = result["status"].iloc[0]
                ...
                # query = f"select * from payment_intent where id='{payment_intent_id}' limit 1;"
                query = f"select * from payment_intent where id='{payment_intent_id}';"
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                payment_intent_status = result["status"].iloc[0]
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_379():
    """
    Sub Feature Code: UI_Common_PM_CNP_First_Txn_Payment_Not_Initiated_Second_Txn_Initiate_New_Payment_First_Txn_Cancelled
    Sub Feature Description: When payment is not inititaed and initiate a new txn so that first txn should be cancelled
    TC naming code description: 100: Payment Method, 103: RemotePay, 379: TC379
    """
    try:
        # -------------------------------Reset Settings to default(started)--------------------------------------------
        testcase_id = sys._getframe().f_code.co_name
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    "pmt_intent_status": "CANCELLED"
                }
                logger.debug(f"expectedDBValues: {expected_db_values}")

                query = f"select * from payment_intent where external_ref='{order_id}' and org_code ='{org_code}';"
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                payment_intent_status = result["status"].iloc[0]
                logger.debug(f"payment_intent_status: {payment_intent_status}")
                ...
                actual_db_values = {
                    "pmt_intent_status": str(payment_intent_status)
                }
                logger.debug(f"actualDBValues : {actual_db_values}")
                Validator.validateAgainstDB(expectedDB=expected_db_values, actualDB=actual_db_values)
            except Exception as e:
