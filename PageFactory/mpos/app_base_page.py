...
class BasePage:
    ...
    def perform_clickIndex(self, locator,Index, time=15):
        WebDriverWait(self.driver, 45).until(EC.presence_of_all_elements_located(locator))[Index].click()
    ...
    def visibility_of_elements(self, locator, time=4):
        """
        This method is specifically used to handle the settings button pop up while logging in to the app.
        """
        return WebDriverWait(self.driver, time).until(EC.visibility_of_all_elements_located(locator))

    def wait_for_visibility_of_elements(self, locator, time=45):
        return WebDriverWait(self.driver, time).until(EC.visibility_of_all_elements_located(locator))

    def wait_for_invisibility_of_elements(self, locator, time=45):
