...
def test_mp_700_707_001():
    """
    Sub Feature Code: UI_MP_Dashboard_Sales_as_HUB_head
    Sub Feature Description: Verifying Total sales, Total Txns and hierarchy details for Upper hierarchy when logging in as HUB Head
    ...
    try:
        ...
        try:
            ...
            query = "select sum(amount) from txn where" \
                    ...
                    "' and '" + current_end_date_1 + "' and username not in ('" + country_username + "', '" + region_username + "', '" + state_username + "', '" + district_username + "', '" + taluk_username + "', '" + city_username + "');"
            ...
            query = "select count(*) from last_device_activity where org_code = '" + org_code + "';"
            logger.debug(f"query to fetch number of devices active for the org {org_code} from db : {query}")
            result = DBProcessor.getValueFromDB(query)
            org_devices_db = result.values[0]
            logger.debug(f"number of devices from db  : {org_devices_db}")
            ...
            org_devices_api = response['hierarchy']['hierarchy'][2]['count']
            logger.info(f"org hierarchy of number of org_cashier_api : {org_cashier_api},"
                        f" org_store_api : {org_store_api}, org_devices_api : {org_devices_api}")
