...
def test_common_100_115_04_004():
    """
    Sub Feature Code: UI_Common_Card_Online_Refund_HDFC_Dummy_EMVCTLS_VISA_CreditCard_Without_Pin_417666
    Sub Feature Description: Performing the EMVCTLS Refund Online transaction via HDFC Dummy PG using VISA Credit card having
     16 digit PAN length without pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 04: Online_Refund, 004: TC004
    """
    try:
        ...
        logger.debug(f"Response received for setting preconditions for online refund to be enable :  {response}")
        ...
        try:
            ...
            card_last_four_digit = result["card_last_four_digit"].iloc[0]
            logger.debug(f"Fetching card_last_four_digit from txn table : {card_last_four_digit} ")
            ...
            logger.debug(f"Query to fetch data from txn table, for refunded txn {rrn} : {query}")
            result = DBProcessor.getValueFromDB(query=query)
            logger.debug(f"result for query : {result}")
            txn_id_2 = result['id'].iloc[0]
            logger.debug(f"Fetching txn_id_2 from txn table for refunded txn : {txn_id_2}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                online_refund_txn_data = refund_page.capture_online_refund_txn_data(password=app_password, card_last_four_digit=card_last_four_digit,
                                                                                    customer_name=customer_name_db, txn_type=None)
                ...
                logger.debug(f"Fetching batch number from txn history for the txn : {txn_id}, {app_batch_number}")
                app_card_type_desc = txn_history_page.fetch_card_type_desc_text()
                logger.debug(f"Fetching card type desc from txn history for the txn : {txn_id}, {app_card_type_desc}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_04_005():
    """
    Sub Feature Code: UI_Common_Card_Online_Refund_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the EMV Refund online transaction via HDFC Dummy PG using VISA Credit card having
     16 digit PAN length with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 04: Online_Refund, 005: TC005
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching customer name from txn history for the txn : {txn_id}, {app_customer_name}")
                app_card_type_desc = txn_history_page.fetch_card_type_desc_text()
                logger.debug(f"Fetching card type desc from txn history for the txn : {txn_id}, {app_card_type_desc}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_04_006():
    """
    Sub Feature Code: UI_Common_Card_Online_Refund_HDFC_Dummy_EMVCTLS_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description: Performing the EMVCTLS Refund Online transaction via HDFC Dummy PG using RUPAY Debit card having
     16 digit PAN length with pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 04: Online_Refund, 006: TC006
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
