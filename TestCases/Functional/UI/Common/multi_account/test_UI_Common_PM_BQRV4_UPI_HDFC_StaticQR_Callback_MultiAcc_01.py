...
def test_common_100_113_001():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_StaticQR_Callback_Success_Via_HDFC_MultiAcc_ActiveLabel
    Sub Feature Description: Multi Account - Verifying upi success callback via HDFC when label is active
    TC naming code description: 100: Payment method, 113: MultiAcc_StaticQR, 001: Testcase ID
    """
    try:
        ...
        try:
            ...
            db_bqr_acc_label_id = result['acc_label_id'].values[0]
            logger.debug(f"fetched acc_label_id : {db_bqr_acc_label_id}")
            ...
            api_details = DBProcessor.get_api_details('staticqr_upi_callback_curl', curl_data={
                'ref_id': ref_id,
            ...
            api_details = DBProcessor.get_api_details('callBackUpiMerchantRes', request_body={
                'pgMerchantId': str(db_upi_config_merchant_id), 'meRes': str(data_buffer)})
            response = APIProcessor.send_request(api_details)
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching txn settlement_status from txn history for the txn : {db_txn_id}, {app_customer_name}")
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_113_002():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_StaticQR_Callback_Success_Via_HDFC_MultiAcc_InactiveLabel
    Sub Feature Description: Multi Account - Verifying upi success callback via HDFC when label is inactive
    TC naming code description: 100: Payment method, 113: MultiAcc_StaticQR, 002: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"query : {query}, to make setting_value : {setting_value}, inactive for org_code : {org_code}")
