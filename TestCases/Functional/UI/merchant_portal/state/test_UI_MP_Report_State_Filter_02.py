...
def test_mp_700_703_028():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Authorised_as_State_head
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Authorised when logging in as State Head
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_703_029():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Failed_as_State_head
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Failed when logging in as State Head
    TC naming code description:
    700: Merchant Portal
    703: State
    029: TC029
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_703_030():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Pending_as_State_head
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Pending when logging in as State Head
    TC naming code description:
    700: Merchant Portal
    703: State
    030: TC030
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_703_031():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Authorised_refunded_as_State_head
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Authorised_refunded when logging in as State Head
    TC naming code description:
    700: Merchant Portal
    703: State
    031: TC031
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and status='" + txn_status + "' and username='" + txn_username + "' and username NOT IN ('" + country_username + "', '" + region_username + "') order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_703_032():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_refunded_as_State_head
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status refunded when logging in as State Head
    TC naming code description:
    700: Merchant Portal
    703: State
    032: TC032
