...
def test_common_100_115_11_014():
    """
    Sub Feature Code: UI_Common_Card_Eze_EMI_Subvented_By_Ezetap_For_Brand_Ezetap_Cosubvention_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the eze EMI subvented by ezetap for brand-ezetap co-subvention success transaction for an org (not ezetap) via HDFC
    Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: Eze EMI, 014: TC014
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching wallet_balance from ezeemi_club_wallet_account table for MCEW : {MCEW_wallet_balance_before_txn}")

            query = f"select * from ezeemi_club_wallet_account where wallet_type = 'GCEW' and brand_id='{brand_id}';"
            logger.debug(f"Query to fetch data from the ezeemi_club_wallet_account table for GCEW : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
            GCEW_wallet_balance_before_txn = result['wallet_balance'].values[0]
            logger.debug(f"Fetching wallet_balance from ezeemi_club_wallet_account table for GCEW : {GCEW_wallet_balance_before_txn}")
            ...
            logger.debug(f"Fetching wallet_balance from ezeemi_club_wallet_account table for MCEW : {MCEW_wallet_balance_after_txn}")
            ...
            logger.debug(f"Fetching wallet_balance from ezeemi_club_wallet_account table for GCEW : {GCEW_wallet_balance_after_txn}")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_11_015():
    """
    Sub Feature Code: UI_Common_Card_Eze_EMI_For_Brand_Ezetap_Cosubvention_Verify_Visibility_Of_Razorpay_Emi_Discount_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the eze EMI for brand_ezetap co-subvention verify visibility of razorpay emi
    discount for an org (not ezetap) via HDFC Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: Eze EMI, 015: TC015
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_11_016():
    """
    Sub Feature Code: UI_Common_Card_Eze_EMI_Subvented_By_Ezetap_For_Aquirer_Ezetap_Cosubvention_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the eze EMI subvented by ezetap for aquirer-ezetap co-subvention success transaction for an org (not ezetap) via HDFC
    Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: Eze EMI, 016: TC016
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_11_017():
    """
    Sub Feature Code: UI_Common_Card_Eze_EMI_For_Aquirer_Ezetap_Cosubvention_Verify_Visibility_Of_Razorpay_Emi_Discount_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the eze EMI for aquirer_ezetap co-subvention verify visibility of razorpay emi
    discount for an org (not ezetap) via HDFC Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: Eze EMI, 017: TC017
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
