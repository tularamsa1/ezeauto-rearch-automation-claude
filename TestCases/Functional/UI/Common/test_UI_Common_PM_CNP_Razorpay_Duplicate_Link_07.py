...
@pytest.mark.dbVal
def test_common_100_103_409():
    """
    Sub Feature Code: UI_Common_PM_CNP_Remotepay_Razorpay_Max_Pay_Attempts_Two_Link_Opened_Two_Browsers_First_Txn_Debit_Card_Failed_Second_Txn_Credit_Card_Success
    Sub Feature Description: Try to make Failed payment using Debit card and then try to make a success transaction using Credit Card by opening same link in 2 browser when max pay attempts is two
    TC naming code description: 100: Payment Method, 103: RemotePay, 409: TC409
    """
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.dbVal
def test_common_100_103_410():
    """
    Sub Feature Code: UI_Common_PM_CNP_Remotepay_Razorpay_Max_Pay_Attempts_Two_Link_Opened_Two_Tabs_First_Txn_Debit_Card_Failed_Second_Txn_Netbanking_Success
    Sub Feature Description: Try to make Failed payment using Debit card and then try to make a success transaction using Netbanking by opening same link in 2 Tabs when max pay attempts is two
    TC naming code description: 100: Payment Method, 103: RemotePay, 410: TC410
    """
    expected_success_message = "Your payment is successfully completed! You may close the browser now."
    expected_failed_message = "Your payment attempt failed, Sorry for the inconvenience. Please contact pos-support@razorpay.com for further clarifications."
    try:
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.dbVal
def test_common_100_103_411():
    """
    Sub Feature Code: UI_Common_PM_CNP_Remotepay_Razorpay_Max_Pay_Attempts_Two_Link_Opened_Two_Tabs_First_Txn_Netbanking_Failed_Second_Txn_Credit_Card_Success
    Sub Feature Description: Try to make Failed payment using Netbanking and then try to make a success transaction using Credit Card by opening same link in 2 Tabs when max pay attempts is two
    TC naming code description: 100: Payment Method, 103: RemotePay, 411: TC411
    """
    expected_success_message = "Your payment is successfully completed! You may close the browser now."
    expected_failed_message = "Your payment attempt failed, Sorry for the inconvenience. Please contact pos-support@razorpay.com for further clarifications."
    try:
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.dbVal
def test_common_100_103_412():
    """
    Sub Feature Code: UI_Common_PM_CNP_Remotepay_Razorpay_Max_Pay_Attempts_Two_Link_Opened_Two_Tabs_First_Txn_Netbanking_Failed_Second_Txn_Debit_Card_Success
    Sub Feature Description: Try to make Failed payment using Netbanking and then try to make a success transaction using Debit Card by opening same link in 2 Tabs when max pay attempts is two
    TC naming code description: 100: Payment Method, 103: RemotePay, 412: TC412
    """
    expected_success_message = "Your payment is successfully completed! You may close the browser now."
    expected_failed_message = "Your payment attempt failed, Sorry for the inconvenience. Please contact pos-support@razorpay.com for further clarifications."
    try:
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.dbVal
def test_common_100_103_413():
    """
    Sub Feature Code: UI_Common_PM_CNP_Remotepay_Razorpay_Max_Pay_Attempts_Two_Failed_Attempts_Two_Same_Link
    Sub Feature Description: Try to make two failed attempts to the same link with Failed state when max pay attempts is two
    TC naming code description: 100: Payment Method, 103: RemotePay, 413: TC413
    """
    expected_failed_message = "Your payment attempt failed, Sorry for the inconvenience. Please contact pos-support@razorpay.com for further clarifications."
    expected_max_attempt_message = "Maximum number of attempts for this url exceeded. Please request for a new remote pay url."
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        logger.debug(f"Setup Timer resumed in testcase function : {testcase_id}")
        ...
        try:
            ...
            if not response['success']:
                ...
                logger.info(f"Your max attempt message is:  {max_attempt_message}")
                logger.info(f"Your expected Message is:  {expected_max_attempt_message}")
                assert max_attempt_message == expected_max_attempt_message, "Max attempt Message is not matching."
                ...
                logger.debug(f"Fetching result from txn table :{result}")
                settle_status_2 = result['settlement_status'].values[0]
                logger.debug(f"Fetching settlement_status from the txn table for failed txn: {settle_status_2}")
                pmt_status_2 = result['status'].values[0]
                logger.debug(f"Fetching status from the txn table for failed txn: {pmt_status_2}")
                payment_mode_2 = result['payment_mode'].values[0]
                logger.debug(f"Fetching payment_mode from the txn table for failed txn: {payment_mode_2}")
                pmt_state_2 = result['state'].values[0]
                logger.debug(f"Fetching state from the txn table for failed txn: {pmt_state_2}")
                amount_txn_2 = result['amount'].values[0]
                logger.debug(f"Fetching amount from the txn table for failed txn: {amount_txn_2}")
                order_id_txn_2 = result['external_ref'].values[0]
                logger.debug(f"Fetching order_id from the txn table for failed txn: {order_id_txn_2}")
                payment_gateway_2 = result['payment_gateway'].values[0]
                logger.debug(f"Fetching payment_gateway from the txn table for failed txn: {payment_gateway_2}")
                ...
                logger.debug(f"Fetching result for cnp_txn table : {result} ")
                cnp_txn_payment_option_2 = result['payment_option'].values[0]
                logger.debug(f"Fetching payment_option from cnp_txn table for failed txn: {cnp_txn_payment_option_2}")
                cnp_txn_payment_flow_2 = result['payment_flow'].values[0]
                logger.debug(f"Fetching payment_flow from cnp_txn table for failed txn: {cnp_txn_payment_flow_2}")
                cnp_txn_payment_status_2 = result['payment_status'].values[0]
                logger.debug(f"Fetching payment_status from cnp_txn table for failed txn: {cnp_txn_payment_status_2}")
                cnp_txn_type_2 = result['txn_type'].values[0]
                logger.debug(f"Fetching txn_type from cnp_txn table for failed txn: {cnp_txn_type_2}")
                cnp_txn_payment_mode_2 = result['payment_mode'].values[0]
                logger.debug(f"Fetching payment_mode from cnp_txn table for failed txn: {cnp_txn_payment_mode_2}")
                cnp_txn_payment_state_2 = result['state'].values[0]
                logger.debug(f"Fetching state from cnp_txn table for failed txn: {cnp_txn_payment_state_2}")
                cnp_txn_payment_card_type_2 = result['payment_card_type'].values[0]
                logger.debug(f"Fetching payment_card_type from cnp_txn table for failed txn: {cnp_txn_payment_card_type_2}")
                cnp_txn_payment_gateway_2 = result['payment_gateway'].values[0]
                logger.debug(f"Fetching payment_gateway from cnp_txn table for failed txn: {cnp_txn_payment_gateway_2}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "cnp_pmt_option_2": "CNP_CC",
                    "cnp_pmt_flow_2": "REMOTEPAY",
                    "cnp_pmt_status_2": "PAYMENT_FAILED",
                    "cnp_pmt_state_2": "FAILED",
                    "cnp_pmt_card_type_2": "CREDIT",
                    "cnp_txn_type_2": "REMOTE_PAY",
                    "cnp_pmt_mode_2": "CNP",
                    "cnp_pmt_gateway_2": "RAZORPAY"
