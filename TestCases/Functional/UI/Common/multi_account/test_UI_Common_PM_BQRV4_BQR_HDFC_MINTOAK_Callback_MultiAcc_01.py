...
def test_common_100_110_047():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Success_Via_BQR_Success_Callback_MultiAcc_HDFC_MINTOAK
    Sub Feature Description: Multi Account - Performing a Authorized Txn using pure BQRV4_BQR Success callback for HDFC_MINTOAK
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_110_048():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Failed_Via_BQR_Failed_Callback_MultiAcc_HDFC_MINTOAK
    Sub Feature Description: Multi Account - Performing a Failed Txn using BQRV4_BQR Failed callback for HDFC_MINTOAK
    ...
    try:
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Response after running querry: {result}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"fetched auth_code from txn table is : {auth_code}")
            rrn = result['rr_number'].values[0]
            logger.debug(f"fetched rrn from txn table is : {rrn}")
