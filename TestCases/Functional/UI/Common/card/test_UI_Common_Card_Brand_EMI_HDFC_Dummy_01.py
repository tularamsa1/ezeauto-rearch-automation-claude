...
def test_common_100_115_07_001():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand fixed success transaction for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 001: TC001
    """
    try:
        ...
        try:
            ...
            cal_monthly_emi_amt = (amount - subvention_value) * monthly_interest_rate * ((1 + monthly_interest_rate) ** emi_plan_in_months) / ((1 + monthly_interest_rate) ** emi_plan_in_months - 1)
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                txn_history_page.scroll_to_given_input_text("LOAN AMOUNT")
                ...
                txn_history_page.scroll_to_given_input_text("NET EFFECTIVE PRICE:")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "emi_term": (f"{emi_plan_in_months} month"),
                    "emi_type": "BRAND_EMI",
                    "emi_payback_fixed": api_subvention_fixed,
                    "emi_payback_amount": api_subvention_fixed,
                ...
                actual_db_values = {
                    ...
                    "emi_payback_fixed": emi_payback_fixed,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_003():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_By_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand fixed success transaction for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 003: TC003
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Fetching emi id from the emi table : {emi_id}")
        interest_rate = result['interest_rate'].values[0]
        logger.debug(f"Fetching interest_rate from the emi table : {interest_rate}")
        term = result['term'].values[0]
        logger.debug(f"Fetching term from the emi table : {term}")
        ...
        try:
            ...
            query = f"select * from brand_sku_detail_mapping where id='{emi_id}';"
            logger.debug(f"Query to fetch data from the brand_sku_detail_mapping table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for brand_sku_detail_mapping table : {result}")
            entity_type_db = result['entity_type'].values[0]
            logger.debug(f"Fetching entity_type value from the txn_emi table : {entity_type_db}")

            query = f"select * from txn_emi_meta where id='{emi_id}';"
            logger.debug(f"Query to fetch data from the txn_emi_meta table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for txn_emi_meta table : {result}")
            txn_emi_subvention_rate = result['subvention_rate'].values[0]
            logger.debug(f"Fetching subvention_rate value from the txn_emi_meta table : {txn_emi_subvention_rate}")

            query = f"select * from brand_sku_detail_mapping where id='{emi_id}';"
            logger.debug(f"Query to fetch data from the brand_sku_detail_mapping table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for brand_sku_detail_mapping table : {result}")
            brand_sku_entity_type = result['entity_type'].values[0]
            logger.debug(f"Fetching entity type value from the brand_sku_detail_mapping table : {brand_sku_entity_type}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "subvention_tenure": (f"{emi_plan_in_months} month"),
                    "subvention_discount_type": "Additional",
                    "entity_type": "SUBVENTION_PLAN",
                    "brand_sku_entity_type": "SUBVENTION_PLAN",
                }
                logger.debug(f"expected_db_values: {expected_db_values}")
                ...
                actual_db_values = {
                    ...
                    "brand_sku_entity_type": brand_sku_entity_type,
                }
                logger.debug(f"actual_db_values: {actual_db_values}")
                Validator.validateAgainstDB(expectedDB=expected_db_values, actualDB=actual_db_values)
            except Exception as e:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_005():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_By_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_9_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand fixed success transaction for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 9 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 005: TC005
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_007():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand percentage success transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 007: TC007
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_009():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand percentage success transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 009: TC009
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
