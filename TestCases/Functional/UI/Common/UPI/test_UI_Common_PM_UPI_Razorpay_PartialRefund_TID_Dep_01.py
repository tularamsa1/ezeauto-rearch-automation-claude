...
def test_common_100_101_285():
    ...
    TC naming code description: 100: Payment Method, 101: UPI, 285: TC285
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values_1 = {
                    ...
                    'RRN': str(refund_rrn),
                    'date': refund_txn_date,
                    'time': refund_txn_time,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_286():
    """
    Sub Feature Code: Tid Dep -  UI_Common_PM_UPI_Two_Times_Successful_Partial_Refund_Via_API_Razorpay
    Sub Feature Description: Tid Dep - Verification of two times partial refund for upi txn via api for Tid Dep Razorpay
    TC naming code description: 100: Payment Method, 286: UPI, 112: TC286
    """
    ...
    try:
        ...
        try:
            ...
            device_serial = result['device_serial'].values[0]
            logger.info(f"fetched device_serial is : {device_serial}")
            amount = random.randint(3200, 3300)
            partial_refunded_amount = random.randint(1500, 1600)
            full_refund_amount = amount - partial_refunded_amount
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.debug(f"initiating upi qr for the amount of {amount}")
            api_details = DBProcessor.get_api_details('TidDepUpiQRGenerate',
                                                      request_body={"username": app_username, "password": app_password,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                query = f"select * from txn where id='{partial_refund_txn_id_1}'"
                logger.debug(f"Query to fetch data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                ...
                query = f"select * from upi_txn where txn_id='{partial_refund_txn_id_1}'"
                logger.debug(f"Query to fetch data from upi_txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                ...
                query = f"select * from txn where id='{partial_refund_txn_id_2}'"
                logger.debug(f"Query to fetch data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                ...
                query = f"select * from upi_txn where txn_id='{partial_refund_txn_id_2}'"
                logger.debug(f"Query to fetch data from upi_txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values_2 = {
                    ...
                    'BASE AMOUNT:': "Rs." + "{:,.2f}".format(full_refund_amount),
                }
                charge_slip_val_result_1 = receipt_validator.perform_charge_slip_validations(
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_287():
    """
    Sub Feature Code: Tid Dep -  UI_Common_PM_Pure_UPI_PartialRefund_In_Decimal_via_api_Razorpay
    Sub Feature Description: Tid Dep -  Verification of  a pure UPI txn when Partial refund is performed via api for Razorpay
    TC naming code description: 100: Payment Method, 101: UPI, 287: TC287
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_292():
    """
    Sub Feature Code: UI_Common_PM_UPI_Partial_Refund_Via_API_Razorpay
    Sub Feature Description: Tid Dep - Verification of  a pure UPI txn when Partial refund is performed via api for Razorpay
    TC naming code description: 100: Payment Method, 101: UPI, 292: TC292
    """
