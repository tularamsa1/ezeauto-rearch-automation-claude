...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_023():
    """
    Sub Feature Code: UI_RP_UPI_Collect_Refund_Via_Api_KOTAK_OLIVE
    Sub Feature Description: Verification of a successful refund for Remote Pay UPI Collect using api for KOTAK_OLIVE
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RP_UPI_COLLECT, 023->TC023
    """
    try:
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_024():
    """
    Sub Feature Code: UI_RP_UPI_Collect_Refund_Failed_Via_Api_KOTAK_OLIVE
    Sub Feature Description: Verification of a failed refund for Remote Pay UPI Collect using api for KOTAK_OLIVE
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RP_UPI_COLLECT, 024->TC024
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            amount = random.randint(701, 1000)
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.info(f"You order id is: {order_id}")
            api_details = DBProcessor.get_api_details('Remotepay_Initiate', request_body={
                "amount": amount, "externalRefNumber": order_id, "username": app_username, "password": app_password})
            ...
            logger.debug(f"performing refund failed through api using amount range 0 to 700")
            refund_amount = random.randint(0, 699)
            api_details = DBProcessor.get_api_details('paymentRefund', request_body={
