...
def test_common_100_115_05_047():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Refund_Via_API_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_9_Months_Tenure
    Sub Feature Description: Performing the normal EMI refund via API transaction for fallback to root org via HDFC Dummy
    PG using EMV VISA Debit card with pin for 9 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 047: TC047
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_048():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Refund_Via_API_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_12_Months_Tenure
    Sub Feature Description: Performing the normal EMI refund via API transaction for fallback to root org via HDFC Dummy
    PG using EMV VISA Debit card with pin for 12 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 048: TC048
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
