...
@pytest.mark.apiVal
def test_common_rearch_0060():
    """
    Sub Feature Code: UI_ReArch_PM_P2P_Cash_Cancel_Device_HDFC
    Sub Feature Description:
        Launch ReArch app, apply P2P + Cash preconditions, verify P2P connected
        notification, hit p2p_start API with CASH payment mode, verify Confirm
        Payment button is visible, cancel payment from device (back → Yes, Cancel),
        then call p2p_status API and validate P2P_DEVICE_CANCELED response.
    ...
    try:
        ...
        # Fetch app_key for p2p_start / p2p_status APIs
        ...
        try:
            ...
            logger.info("Confirm Payment button verified on device after P2P push")

            # Step 6: Click back button on cash confirm screen
            app_driver.find_element(
                AppiumBy.XPATH,
                '//android.view.View[@resource-id="page"]/android.view.View[1]'
                '/android.view.View/android.widget.Button'
            ).click()
            logger.debug("Clicked back button on cash confirm screen")

            # Step 7: Click 'Yes, Cancel' on the confirmation dialog
            time.sleep(1)
            app_driver.find_element(
                AppiumBy.XPATH,
                '//android.widget.Button[@text="Yes, Cancel"]'
            ).click()
            logger.debug("Clicked 'Yes, Cancel' on cancel confirmation dialog")
        ...
        if ConfigReader.read_config("Validations", "api_validation") == "True":
            ...
            try:
                expected_api_values = {
                    ...
                    "status_success":    True,
                    "status_mssg_code":  "P2P_DEVICE_CANCELED",
                    "status_mssg":       "PushToPay Notification has been Canceled on Receiving device.",
                    "status_real_code":  "P2P_DEVICE_CANCELED",
                }
