...
def test_common_100_116_001():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_PLANET_Opt_In_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt in MC Chip KWD sale transaction via HDFC PLANET PG using Credit card
    with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_DCC, 001: TC001
    """
    try:
        ...
        try:
            ...
            selected_payment_message = payment_page.fetch_selected_currency_type_message()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "offered_currency_selection": 'I have been offered a choice of currencies and agree to pay in (KWD)'
                }
                ...
                actual_app_values = {
                    ...
                    "offered_currency_selection": selected_payment_message
                }
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "RRN": str(rrn),
                    "TRANSACTION AMOUNT:": "USD " + str(dcc_converted_amount_db) + "0",
                    "AUTH CODE": auth_code,
                    "date": txn_date,
                    "time": txn_time,
                    "payment_option": "SALE",
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_002():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Opt_In_VISA_ctls_GBP_CreditCard_With_Online_Pin_417666
    Sub Feature Description: Performing the opt in MC Chip KWD sale transaction via HDFC PLANET PG using Credit card
    with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_DCC, 002: TC002
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "currency_conversion_fee_rate": "0.011 USD/INR",
                    "currency_converted_rate": "USD {:,.2f}".format(converted_amount),
                    "offered_currency_selection": 'I have been offered a choice of currencies and agree to pay in (GPB)'
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_003():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Opt_In_VISA_ctls_JPY_CreditCard_With_Online_Pin_405170
    Sub Feature Description: Performing the opt in VISA ctls JPY sale transaction via HDFC PLANET PG using Credit card
    with online pin  (bin:405170)
    TC naming code description: 100: Payment Method, 116: CARD_DCC, 003: TC003
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select bank_code from bin_info where bin='405170'"
        logger.debug(f"Query to fetch bank_code from the bin_info table : {query}")
        result = DBProcessor.getValueFromDB(query=query)
        issuer_code = result["bank_code"].values[0]
        logger.debug(f"Fetching bank_code from the bin_info table : bank_code : {issuer_code}")
        ...
        try:
            ...
            logger.info("Selected payment mode is Card")
            card_page = CardPage(driver=app_driver)
            logger.debug(f"selecting the card type as : VISA ctls JPY")
            card_page.select_cardtype(text="VISA ctls JPY")
            logger.debug(f"selected the card type as : VISA ctls JPY")
            payment_page.select_currency_type_foreign()
            selected_payment_message = payment_page.fetch_selected_currency_type_message()
            converted_currency_rate = payment_page.fetch_converted_rate_currency_screen()
            payment_page.click_agree_and_pay()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "currency_conversion_fee_rate": "0.011 USD/INR",
                    "currency_converted_rate": "USD {:,.2f}".format(converted_amount),
                    "offered_currency_selection": 'I have been offered a choice of currencies and agree to pay in (JPY)'
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "dcc_payment_card_brand": "VISA",
                    "dcc_markup_fee": "04.75",
                    "dcc_payment_card_bin": "405170",
                    "dcc_card_last_four_digit": "1116",
                    "dcc_mid": mid,
                    "dcc_tid": tid
                }
                logger.debug(f"expected_db_values: {expected_db_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_004():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Opt_In_MC_ctls_USD_CreditCard_With_Online_Pin_556763
    Sub Feature Description: Performing the opt in MC ctls USD sale transaction via HDFC PLANET PG using Credit card
    with online pin  (bin:556763)
    TC naming code description: 100: Payment Method, 116: CARD_DCC, 004: TC004
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            amount = random.randint(5001, 5800)
            converted_amount = amount * 0.0107033
            order_id = datetime.now().strftime('%m%d%H%M%S')
            home_page.enter_amount_and_order_number_and_device_serial_for_card(amt=amount, order_number=order_id, device_serial=device_serial)
            logger.debug(f"Entered amount is : {amount}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "currency_conversion_fee_rate": "0.011 USD/INR",
                    "currency_converted_rate": "USD {:,.2f}".format(converted_amount),
                    "offered_currency_selection": 'I have been offered a choice of currencies and agree to pay in (USD)'
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
