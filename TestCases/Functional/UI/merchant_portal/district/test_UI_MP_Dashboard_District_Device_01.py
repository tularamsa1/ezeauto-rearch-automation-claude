...
@pytest.mark.dbVal
@pytest.mark.apiVal
def test_mp_700_704_018():
    """
    Sub Feature Code: UI_MP_Device_Usage_as_District_head
    Sub Feature Description: Verifying device usage details on dashboard for payment done thru device when logging in as District Head
    ...
    try:
        ...
        try:
            ...
            query = "select sum(amount) as SUM, org_code from txn where id between '" + current_date_2[2:] + "' AND '" + \
                    ...
                    " '" + org_code + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "')  and payment_mode = 'CARD' and status = 'AUTHORIZED';"
            ...
            query = "select device_serial, count(*) from txn where id between '" + current_date_2[2:] + "' AND '" + \
                    current_date_actual[2:] + "' and org_code =" \
                    " '" + org_code + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "') and payment_mode = 'CARD' and status = 'AUTHORIZED' group by device_serial;"
...
@pytest.mark.dbVal
@pytest.mark.apiVal
def test_mp_700_704_019():
    """
    Sub Feature Code: UI_MP_Devices_as_District_head
    Sub Feature Description: Verifying devices details on dashboard for payment done thru device when logging in as District Head
    TC naming code description:
    700: Merchant Portal
    704: District
    019: TC019
