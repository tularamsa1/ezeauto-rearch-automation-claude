...
def test_common_100_115_054():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMVCTLS_VISA_DebitCard_With_Pin_476173
    Sub Feature Description: Performing the EMVCTLS Refund transaction via HDFC Dummy PG using VISA Debit card having
    16 digit PAN length with pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 054: TC054
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching posting_date from txn table : {posting_date_2}")
            # ------------------------------------------------------------------------------------------------
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_055():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMVCTLS_MASTER_DebitCard_With_Pin_222360
    Sub Feature Description: Performing the EMVCTLS Refund transaction via HDFC Dummy PG using MASTER Debit card having
    16 digit PAN length with pin (bin: 222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 055: TC055
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"API DETAILS for Offline_Refund api txn : {api_details}")
            response = APIProcessor.send_request(api_details=api_details)
            logger.debug(f"Response received for Offline_Refund api is : {response}")
            txn_id_2 = response["txnId"]
            logger.debug(f"Fetching transaction id for refund from response : {txn_id_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_056():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMVCTLS_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description: Performing the EMVCTLS Refund transaction via HDFC Dummy PG using RUPAY Debit card having
    16 digit PAN length with pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 056: TC056
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = f"select * from txn where id='{txn_id_2}'"
            logger.debug(f"Query to fetch data from txn table, for refunded txn {txn_id} : {query}")
            result = DBProcessor.getValueFromDB(query=query)
            logger.debug(f"result for query : {result}")
            auth_code_2 = result['auth_code'].values[0]
            logger.debug(f"Fetching auth code from txn table : {auth_code_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_057():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMVCTLS_VISA_DebitCard_Without_Pin_476173
    Sub Feature Description: Performing the EMVCTLS Refund transaction via HDFC Dummy PG using VISA Debit card having
    16 digit PAN length without pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 057: TC057
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"API DETAILS for Offline_Refund api : {api_details}")
            response = APIProcessor.send_request(api_details=api_details)
            logger.debug(f"Response received for Offline_Refundt api is : {response}")
            txn_id_2 = response["txnId"]
            logger.debug(f"Fetching transaction id for refund from response : {txn_id_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_058():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMVCTLS_MASTER_DebitCard_Without_Pin_222360
    Sub Feature Description: Performing the EMVCTLS Refund transaction via HDFC Dummy PG using MASTER Debit card having
    16 digit PAN length without pin (bin: 222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 058: TC058
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                app_driver.reset()
                logger.debug(f"reseting driver for username {app_username}")
                login_page.perform_login(username=app_username, password=app_password)
                home_page.wait_for_home_page_load()
                home_page.click_on_history()
                txn_history_page = TransHistoryPage(driver=app_driver)
