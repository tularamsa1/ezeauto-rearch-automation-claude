...
def test_common_100_101_216():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Via_Pure_UPI_Callback_Razorpay
    Sub Feature Description: Verification of a successful pure upi txn via Razorpay using callback
    TC naming code description: 100: Payment Method, 101: UPI, 216: TC216
    """
    try:
        ...
        logger.debug(f"Query to fetch org_code from org_employee table: {query}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_211():
    """
    Sub Feature Code: UI_Common_PM_UPI_Failed_Via_Pure_UPI_Callback_Razorpay
    Sub Feature Description: Verification of a failed UPI txn via Razorpay using Callback
    TC naming code description: 100: Payment Method, 101: UPI, 211: TC211
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_217():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Callback_After_QR_Expiry_Razorpay
    Sub Feature Description: Performing a  upi success callback via Razorpay after expiry the qr
    TC naming code description: 100: Payment Method, 101: UPI, 217: TC217
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(
                    f"Fetching rrn from txn history for the txn : {txn_id_2}, {new_app_rrn}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_218():
    """
    Sub Feature Code: UI_Common_PM_UPI_Failed_Callback_After_QR_Expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Performing a  upi failed callback via Razorpay after expiry the qr when auto refund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 218: TC0218
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                status_db = result["status"].values[0]
                payment_mode_db = result["payment_mode"].values[0]
                amount_db = int(result["amount"].values[0])  # actual=345.0000, expected should be in the same format
                state_db = result["state"].values[0]
                payment_gateway_db = result["payment_gateway"].values[0]
                acquirer_code_db = result["acquirer_code"].values[0]
                bank_code_db = result["bank_code"].values[0]
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_221():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Callback_After_QR_Expiry_AutoRefund_Enabled_Razorpay
    Sub Feature Description: Performing a  upi success callback via Razorpay after qr expiry when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 221: TC221
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
