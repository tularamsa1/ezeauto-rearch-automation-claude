...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_008():
    """
    Sub Feature Code: UI_RP_UPI_Success_Via_upi_Callback_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay successful upi txn via KOTAK OLIVE using callback
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RemotePay, 008->TC008
    """
    try:
        ...
        try:
            ...
            if not response['success']:
                ...
                ui_page = TestSuiteSetup.initialize_ui_browser()
                paymentLinkUrl = response['paymentLink']
                ui_page.goto(paymentLinkUrl)
                logger.info("Opening the link in the browser")
                rp_upi_txn = RemotePayTxnPage(ui_page)
            ...
            api_details = DBProcessor.get_api_details('upi_confirm_kotakolive', request_body={
                "transactionid": f"KMBMABCD00{payment_intent_id}",
                "aggregatorcode": "",
                "merchantcode": mid,
                "status": "SUCCESS",
                "statusCode": "00",
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_009():
    """
    Sub Feature Code: UI_RP_UPI_Failed_Via_upi_Callback_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay failed upi txn via KOTAK OLIVE using callback
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RemotePay, 009->TC009
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_010():
    """
    Sub Feature Code: UI_RP_UPI_Expired_Via_upi_Callback_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay expired upi txn via KOTAK OLIVE using callback
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RemotePay, 010->TC010
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_011():
    """
    Sub Feature Code: UI_RP_UPI_Rejected_Via_upi_Callback_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay rejected upi txn via KOTAK OLIVE using callback
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RemotePay, 011->TC011
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
