...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_029():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_CTLS_WITH_PIN_VISA_DEBIT_476173
    Sub Feature Description:  Performing the EMV void transaction via FDC PG using CTLS With Pin VISA Debit card
    having 16 digit PAN length with pin (bin: 476173)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 029: TC029
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_031():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_CTLS_WITH_PIN_MASTER_DEBIT_222360
    Sub Feature Description:  Performing the EMV void transaction via FDC PG using CTLS With Pin MASTER Debit card
    having 16 digit PAN length with pin(bin: 222360)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 031: TC031
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
