...
def test_common_100_115_05_116():
    """
        Sub Feature Code: UI_Common_Card_Normal_EMI_Instant_Subvention_Inactive_Payback_By_Merchant_Percentage_Success_Txn
        _For_An_Org_HDFC_HDFC_EMV_WITH_PIN_VISA_CREDIT_417666_3_Months_Tenure
        Sub Feature Description: Performing Normal EMI inactive instant subvention payback by merchant percentage success
        transaction for an org via HDFC HDFC PG using EMV VISA credit with pin card for 3 months tenure (bin:417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 0116: TC0116
    """
    try:
        ...
        query = f"select * from subvention_plan where org_code='{org_code}' and card_type= 'CREDIT' and bank='HDFC' order by created_time desc limit 1;"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_117():
    """
        Sub Feature Code: UI_Common_Card_Normal_EMI_Instant_Subvention_Inactive_Payback_By_Merchant_Percentage_Success
        _Txn_For_An_Org_HDFC_HDFC_EMV_VISA_DEBIT_476173_3_Months_Tenure
        Sub Feature Description: Performing Normal EMI inactive instant subvention payback by merchant percentage success
        transaction for an org via HDFC HDFC PG using EMV VISA Debit card for 3 months tenure (bin:476173)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 0117: TC0117
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
