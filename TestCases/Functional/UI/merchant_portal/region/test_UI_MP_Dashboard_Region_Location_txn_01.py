...
def test_mp_700_702_006():
    """
    Sub Feature Code: UI_MP_Calender_txn_for_specific_location_using_dropdown_values_as_Regional_Head
    Sub Feature Description: Verifying txn details on dashboard  for specific location using dropdown values when logging in as Regional Head
    ...
    try:
        ...
        try:
            ...
            location_hierarchy = ["West", "Maharastra", "Mumbai_City_District", "Akole", "Mumbai1", "Hub1",
                                  str(node_name)]
            ...
            query = "select sum(amount) from txn where id between '" + current_start_date_1 + "' AND '" \
                    + current_end_date_1 + "' and org_code='" + org_code + "' and username NOT IN ('"+country_username+"') and username = '" + txn_username + "' and status = 'AUTHORIZED';"
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_702_007():
    """
    Sub Feature Code: UI_MP_Calender_txn_for_specific_location_using_search_option_as_Regional_Head
    Sub Feature Description: Verifying txn details on dashboard  for specific location using search option when logging in as Regional Head
    TC naming code description:
    700: Merchant Portal
    702: Region
    007: TC007
