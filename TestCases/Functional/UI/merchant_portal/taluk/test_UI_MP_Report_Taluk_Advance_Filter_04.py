...
def test_mp_700_705_056():
    """
    Sub Feature Code: UI_MP_Report_based_on_Advance_filter_Payment_mode_Card_as_Taluk_head
    Sub Feature Description: Verifying txn details on Report Page using Advance Filter for payment mode Card when logging in as Taluk Head
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and payment_mode='" + pmt_mode_search_value + "' and username NOT IN ('"+country_username+"', '" + region_username + "', '" + state_username + "', '" + district_username + "') order by created_time asc limit 1;"
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_705_057():
    """
    Sub Feature Code: UI_MP_Report_based_on_Advance_filter_Payment_mode_UPI_as_Taluk_head
    Sub Feature Description: Verifying txn details on Report Page using Advance Filter for payment mode UPI when logging in as Taluk Head
    TC naming code description:
    700: Merchant Portal
    705: Taluk
    057: TC057
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_705_058():
    """
    Sub Feature Code: UI_MP_Report_based_on_Advance_filter_Payment_mode_BharatQR_as_Taluk_head
    Sub Feature Description: Verifying txn details on Report Page using Advance Filter for payment mode BharatQR when logging in as Taluk Head
    TC naming code description:
    700: Merchant Portal
    705: Taluk
    058: TC058
    ...
    try:
        ...
        try:
            ...
            order_id = datetime.now().strftime('%m%d%H%M%S')
            api_details = DBProcessor.get_api_details('bqrGenerate', request_body={"amount": amount,
                                                                                   "password": txn_password,
                                                                                   "qrCodeType": "BHARAT",
                                                                                   "username": txn_username,
                                                                                   "qrCodeFormat": "string",
            ...
            logger.debug(f"API details for bqrGenerate : {api_details} ")
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received after BHARATQR transaction is : {response}")
            txn_id = response['txnId']
            logger.debug(f"txn_id for BQR transaction is : {txn_id}")
            ...
            logger.debug(f"API details for stopPayment : {api_details} ")
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received after BHARATQR transaction after hitting stopPayment is : {response}")

            pmt_mode_search_value = "BHARATQR"
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_705_059():
    """
    Sub Feature Code: UI_MP_Report_based_on_Advance_filter_Payment_mode_CNP_as_Taluk_head
    Sub Feature Description: Verifying txn details on Report Page using Advance Filter for payment mode CNP when logging in as Taluk Head
    TC naming code description:
    700: Merchant Portal
    705: Taluk
    059: TC059
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Response received for txn via api : {response}")

            query = "select * from txn where org_code = '" + str(org_code) + "' AND external_ref = '" + \
                    str(order_id) + "';"
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_705_060():
    """
    Sub Feature Code: UI_MP_Report_based_on_Advance_filter_Payment_mode_cheque_as_Taluk_head
    Sub Feature Description: Verifying txn details on Report Page using Advance Filter for payment mode Cheque when logging in as Taluk Head
    TC naming code description:
    700: Merchant Portal
    705: Taluk
    060: TC060
