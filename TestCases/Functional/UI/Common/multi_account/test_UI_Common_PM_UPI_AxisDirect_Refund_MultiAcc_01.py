...
def test_common_100_109_029():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"response received for the upiqrGenerate_multiAcc api : {response}")

            api_details = DBProcessor.get_api_details('stopPayment', request_body={
                "username": app_username, "password": app_password, "txnId": str(txn_id_original)
            })
            ...
            logger.debug(f"Fetching txnId from the paymentRefund API_OUTPUT, txnId : {refund_txn_id}")
            logger.debug(f"response received for the paymentRefund api : {response}")

            query = f"select * from txn where id='{refund_txn_id}' and orig_txn_id ='{txn_id_original}'"
