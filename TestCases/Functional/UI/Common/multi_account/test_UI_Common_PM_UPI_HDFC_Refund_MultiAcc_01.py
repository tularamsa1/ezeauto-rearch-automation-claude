...
def test_common_100_109_005():
    """
    Sub Feature Code: UI_Common_PM_Pure_UPI_full_Refund_via_API_MultiAcc_HDFC
    Sub Feature Description: Multi Account - Verification of a UPI full refund using api for HDFC
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 005: TC005
    """
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Query to fetch txn data of refunded txn from txn table : {query}")
            result = DBProcessor.getValueFromDB(query)
            ...
            logger.debug(f"fetched refund_label_ids from txn table is : {refund_label_ids}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                expected_portal_values = {
                    "pmt_status_2": "Refunded", "pmt_mode": "UPI",
                    "txn_amt": str(amount),
                ...
                actual_portal_values = {
                    "pmt_status_2": portal_status, "pmt_mode": portal_txn_type,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_109_006():
    """
    Sub Feature Code: UI_Common_PM_Pure_UPI_partial_Refund_via_API_MultiAcc_HDFC
    Sub Feature Description: Multi Account - Verification of a UPI partial refund using api for HDFC
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 006: TC006
    """
    ...
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('paymentRefund', request_body={
                "username": app_username, "password": app_password,
                "amount": refund_amount, "originalTransactionId": str(txn_id_original)
            })
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received for paymentRefund api is : {response}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_chargeslip_values_original = {
                    'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id),
                    'RRN': str(rrn), 'time': txn_time, 'date': txn_date, 'AUTH CODE': auth_code,
                    'BASE AMOUNT:': "Rs." + str(amount) + ".00"}
                expected_chargeslip_values_refunded = {
                    'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id),
                    'RRN': str(refund_rrn), 'BASE AMOUNT:': "Rs." + str(refund_amount) + ".00",
                    'date': txn_date_2, 'time': txn_time_2, 'AUTH CODE': refund_auth_code}
