...
def test_common_100_115_06_036():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Refund_Via_API_Txn_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_9_Months_Tenure
    Sub Feature Description: Performing the instant EMI refund via API transaction for fallback to root org via HDFC Dummy PG
    using EMV VISA Credit card with pin for 9 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 036: TC036
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching created_time from the txn table for refunded txn : {created_time_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_037():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Refund_Via_API_Txn_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_12_Months_Tenure
    Sub Feature Description: Performing the instant EMI refund via API transaction for fallback to root org via HDFC Dummy PG
    using EMV VISA Credit card with pin for 12 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 037: TC037
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_046():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Refund_Via_API_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the instant EMI refund via API transaction for fallback to root org via HDFC Dummy PG
    using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 046: TC046
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_047():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Refund_Via_API_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    Sub Feature Description: Performing the instant EMI refund via API transaction for fallback to root org via HDFC Dummy PG
    using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 047: TC047
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_048():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Refund_Via_API_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_9_Months_Tenure
    Sub Feature Description: Performing the instant EMI refund via API transaction for fallback to root org via HDFC Dummy PG
    using EMV VISA Debit card with pin for 9 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 048: TC048
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching txn settlement_status from txn history for the original txn : {txn_id}, {app_settlement_status}")
                app_auth_code = txn_history_page.fetch_auth_code_text()
                logger.debug(f"Fetching AUTH CODE from txn history for the original  txn : {txn_id}, {app_auth_code}")
                app_date_and_time = txn_history_page.fetch_date_time_text()
                logger.debug(f"Fetching date_time from txn history for the original txn : {txn_id}, {app_date_and_time}")
                app_device_serial = txn_history_page.fetch_device_serial_text()
                logger.debug(f"Fetching device serial number from txn history for the original txn : {txn_id}, {app_device_serial}")
