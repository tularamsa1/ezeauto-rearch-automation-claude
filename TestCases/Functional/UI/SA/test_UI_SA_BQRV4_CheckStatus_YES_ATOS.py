    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            try:
                ...
                # --------------------------------------------------------------------------------------------
                expectedAppValues = {"Payment Status": "AUTHORIZED", "Payment mode": "UPI",
                ...
                app_payment_status = transactions_history_page.fetch_txn_status_text().split(":")[1]
                logger.debug(
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                ...
                actualAPIValues = {"Payment Status": status_api, "Amount": amount_api, "Payment Mode": payment_mode_api,
                                   "Acquirer Code": accuirer_code_api}
                # ---------------------------------------------------------------------------------------------
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                # --------------------------------------------------------------------------------------------
                expectedDBValues = {"Payment Status": "AUTHORIZED", "Payment mode": "UPI",
                                    "Payment amount": "{:.2f}".format(amount), "Acquirer Code": "YES",
                                    "State": "SETTLED", "State Bharatqr": "SETTLED", "Amount Bharatqr": amount,
                                    "Status Bharatqr": "SUCCESS"}
                #
                query = "select status,amount,payment_mode,acquirer_code,state from txn where id='" + txn_id + "'"
                logger.debug(
                    f"DB query to fetch status, amount,acquirer_code, payment mode and state from DB : {query}")
                result = DBProcessor.getValueFromDB(query)
                ...
                actualDBValues = {"Payment Status": status_db, "Payment mode": payment_mode_db,
                                  "Payment amount": amount_db, "State": state_db, "Acquirer Code": accuirer_code_db,
                                  "State Bharatqr": state_bharatqr_db, "Amount Bharatqr": amount_bharatqr_db,
                                  "Status Bharatqr": status_bharatqr_db}
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            try:
                ...
                # --------------------------------------------------------------------------------------------
                expectedPortalValues = {"Payment Status": "Settled", "Payment mode": "UPI",
...
def test_sa_100_102_073():
    ...
    :Sub Feature code: UI_Common_BQRV4_CheckStatus_Expired_YES_ATOS_073
    :TC naming code description: 100->Payment Method, 102->BQR, 073-> TC73
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {"txn_amt": amount, "pmt_mode": "BHARATQR", "pmt_status": "EXPIRED",
                                      ...
                                      "pmt_state_new": "SETTLED", "acquirer_code_new": "YES",
                                      "bank_name_new": "Yes Bank",
