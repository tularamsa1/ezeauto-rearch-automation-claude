...
def test_common_100_115_248():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_CTLS_WITH_PIN_VISA_DEBIT_476173
    Sub Feature Description:  Performing the CTLS void transaction via MASHREQ PG using CTLS With Pin VISA Debit card
    having 16 digit PAN length with pin (bin: 476173)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 248: TC248
    """
    try:
        ...
        query = f"select * from terminal_info where org_code='{org_code}' and status='ACTIVE' and acquirer_code=" \
                f"'MASHREQ' and payment_gateway='MASHREQ'"
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"query result : {result}")
        ...
        logger.debug(f"fetched mid : {mid}, tid : {tid}, device_serial : {device_serial}, terminal_info_id : {terminal_info_id}")
        ...
        try:
            ...
            card_page.select_cardtype("CTLS_WITH_PIN_VISA_DEBIT_476173")
            ...
            logger.debug(f"txn_created_time:{txn_created_time}, txn_id:{txn_id}, rrn:{rrn}, auth_code:{auth_code}, "
                         ...
                         f"card_last_four_digit_db:{card_last_four_digit_db}, merchant_name:{merchant_name}, currency_code:{currency_code_db}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                currency_code_api = response["currencyCode"]
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_249():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_CTLS_WITH_PIN_MASTER_DEBIT_222360
    Sub Feature Description:  Performing the CTLS void transaction via MASHREQ PG using CTLS With Pin MASTER Debit card
    having 16 digit PAN length with pin (bin: 222360)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 249: TC249
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            card_page.select_cardtype("CTLS_WITH_PIN_MASTER_DEBIT_222360")
            payment_page.click_on_proceed_homepage()
            home_page.click_on_history()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_250():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_EMV_VISA_CREDIT_417666
    Sub Feature Description:  Performing the EMV void transaction via MASHREQ PG using VISA Credit card
    having 16 digit PAN length with pin (bin: 417666)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 250: TC250
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        # Set the below variables depending on the log capturing need of the test case.
        Configuration.configureLogCaptureVariables(apiLog=True, portalLog=True, middlewareLog=True, q2_log=True, khata_log=True)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_251():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_EMV_MASTER_CREDIT_541333
    Sub Feature Description:  Performing the EMV void transaction via MASHREQ PG using MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 251: TC251
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_252():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the EMV void transaction via MASHREQ PG using VISA Credit card having
    16 digit PAN length with pin (bin:417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 252: TC252
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            txn_posting_date = result['posting_date'].values[0]
            logger.debug(f"Fetching posting_date from the txn table : {txn_posting_date}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                api_currency_code = response.get('currencyCode')
                logger.debug(f"From response fetch currency_code : {api_currency_code}")
