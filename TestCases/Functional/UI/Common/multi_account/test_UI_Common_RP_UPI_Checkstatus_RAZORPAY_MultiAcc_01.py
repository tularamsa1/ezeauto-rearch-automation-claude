...
def test_common_100_111_033():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Success_Via_CheckStatus_RAZORPAY_MultiAcc
    Sub Feature Description: MultiAccount - Verification of a Remote Pay successful upi txn via RAZORPAY using check status
    TC naming code description: 100: Payment Method, 111: MultiAcc_RemotePay, 033: TC_033
    """
    try:
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_111_034():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Failed_Via_Common_CheckStatus_MultiAcc_Razorpay
    Sub Feature Description: Multi Account - Performing a failed RP_UPI txn via Razorpay using check status
    TC naming code description:100: Payment Method, 111: MultiAcc_RemotePay, 034: TC_034
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_111_048():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Success_Via_Common_CheckStatus_with_2nd_label_MultiAcc_Razorpay
    Sub Feature Description: Multi Account - Performing a successful RP_UPI txn via Razorpay using check status second account (ex:acc2 label)
    TC naming code description:100: Payment Method, 111: MultiAcc_RemotePay, 048: TC_048
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
