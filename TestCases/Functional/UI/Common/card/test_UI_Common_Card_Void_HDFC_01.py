...
def test_common_100_115_010():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_CTLS_VISA_DEBIT_476173
    Sub Feature Description:  Performing the EMV void transaction via HDFC PG using CTLS VISA Debit card having 16 digit
     PAN length without pin (bin: 476173)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 010: TC010
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_011():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_CTLS_WITH_PIN_VISA_DEBIT_476173
    Sub Feature Description:  Performing the EMV void transaction via HDFC PG using CTLS With Pin VISA Debit card
    having 16 digit PAN length with pin (bin: 476173)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 011: TC011
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_012():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_CTLS_MASTER_DEBIT_222360
    Sub Feature Description:  Performing the EMV void transaction via HDFC PG using CTLS MASTER Debit card
    having 16 digit PAN length without pin (bin: 222360)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 012: TC012
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_013():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_CTLS_WITH_PIN_MASTER_DEBIT_222360
    Sub Feature Description:  Performing the EMV void transaction via HDFC PG using CTLS With Pin MASTER Debit card
    having 16 digit PAN length with pin (bin: 222360)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 013: TC013
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_014():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_CTLS_RUPAY_DEBIT_608326
    Sub Feature Description:  Performing the EMV void transaction via HDFC PG using CTLS RUPAY Debit card
    ...
    try:
        ...
        try:
            ...
            query = f"select * from txn where id ='{str(txn_id)}';"
            logger.debug(f"Query to fetch txn data from the txn table for the {org_code} : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"query result : {result}")
            txn_created_time = result['created_time'].values[0]
            txn_id = result['id'].values[0]
