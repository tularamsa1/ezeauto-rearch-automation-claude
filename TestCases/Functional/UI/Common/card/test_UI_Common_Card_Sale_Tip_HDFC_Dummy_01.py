...
def test_common_100_115_01_001():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Success_Txn_HDFC_Dummy_EMVCTLS_VISA_DebitCard_Without_pin_476173
    Sub Feature Description: Performing the EMVCTLS sale tip success transaction via HDFC Dummy PG using VISA Debit card
    having 16 digit PAN length without pin (bin:476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 001: TC001
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                txn_history_page.scroll_to_given_input_text("Tip Details")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                api_tip_amt = response.get('amountAdditional')
                logger.debug(f"From response fetch tip_amt : {api_tip_amt}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "TIP AMOUNT:": "Rs." + "{:,.2f}".format(tip_amount),
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_115_01_002():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Failed_Txn_HDFC_Dummy_EMVCTLS_VISA_DebitCard_Without_pin_476173
    Sub Feature Description: Performing the EMVCTLS sale tip failed transaction via HDFC Dummy PG using VISA Debit card
    having 16 digit PAN length without pin (bin:476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 002: TC002
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_01_003():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Success_Txn_HDFC_Dummy_EMVCTLS_MASTER_DebitCard_Without_pin_222360
    Sub Feature Description: Performing the EMVCTLS sale tip success transaction via HDFC Dummy PG using MASTER Debit card
    having 16 digit PAN length without pin (bin:222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 003: TC003
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Query result, card_txn_type : {card_txn_type}")
            org_code_txn = result['org_code'].values[0]
            logger.debug(f"Query result, org_code_txn : {org_code}")
            posting_date = result['posting_date'].values[0]
            logger.debug(f"Query result, posting_date : {posting_date}")
            # ------------------------------------------------------------------------------------------------
            GlobalVariables.EXCEL_TC_Execution = "Pass"
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_115_01_004():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Failed_Txn_HDFC_Dummy_EMVCTLS_MASTER_DebitCard_Without_pin_222360
    Sub Feature Description: Performing the EMVCTLS sale tip failed transaction via HDFC Dummy PG using MASTER Debit card
    having 16 digit PAN length without pin (bin:222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 004: TC004
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Entered device_serial is : {device_serial}")
            payment_page = PaymentPage(driver=app_driver)
            payment_page.is_payment_page_displayed_card_with_tip(amount, order_id, tip_amount, device_serial)
            payment_page.click_on_Card_paymentMode()
            logger.info(f"Selected payment mode is Card")
            card_page = CardPage(driver=app_driver)
            logger.debug(f"Selecting the card type as : CTLS_MASTER_DEBIT_222360")
            card_page.select_cardtype("CTLS_MASTER_DEBIT_222360")
            logger.debug(f"Selected the card type as : CTLS_MASTER_DEBIT_222360")
            payment_page.click_on_proceed_to_home_page_for_failed_txn()
            payment_page.click_on_back_btn_in_enter_amt_window()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_01_005():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Success_Txn_HDFC_Dummy_EMVCTLS_RUPAY_DebitCard_Without_pin_608326
    Sub Feature Description: Performing the EMVCTLS sale tip success transaction via HDFC Dummy PG using RUPAY Debit card
    having 16 digit PAN length without pin (bin:608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 005: TC005
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching txn_id from txn history for the txn : {txn_id}, {app_txn_id}")
                app_date_time = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching app_date_time from txn history for the txn : {txn_id}, {app_date_time}")
                app_settle_status = txn_history_page.fetch_settlement_status_text()
                logger.info(f"Fetching settlement_status from txn history for the txn : {txn_id}, {app_settle_status}")
                app_rrn = txn_history_page.fetch_RRN_text()
                logger.info(f"Fetching rrn from txn history for the txn : {txn_id}, {app_rrn}")
