...
def test_common_100_115_07_032():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Merchant_Percentage_Void_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by merchant percentage void transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 032: TC032
    """
    try:
        ...
        query = f"select * from emi where org_code='{org_code}' and status = 'ACTIVE' and " \
                ...
                f"and tid_type='SUBVENTION' order by interest_rate desc limit 1"
        ...
        query = f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' and subventing_entity='MERCHANT' and subvention_type='PAYBACK' and subvention_value_type='PERCENTAGE' and tenure='{emi_plan_in_months} month'"
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "total_payback": emi_payback_amount,
                ...
                api_orig_amount = response['amountOriginal']
                logger.debug(f"Value of amountOriginal obtained from txnlist api : {api_orig_amount}")
                ...
                api_total_payback = response['emiDetails']['totalPayBack']
                logger.debug(f"Value of totalCashBack obtained from txnlist api : {api_total_payback}")
                ...
                actual_api_values = {
                    ...
                    "total_payback": api_total_payback,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    ...
                    "amt_original": "{:,.2f}".format(original_amount_db),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_034():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Merchant_Percentage_Void_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenture
        Sub Feature Description: Performing the brand EMI payback by merchant percentage void transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 034: TC034
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_036():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Merchant_Percentage_Void_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_9_Months_Tenture
        Sub Feature Description: Performing the brand EMI payback by merchant percentage void transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 9 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 036: TC036
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_038():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Merchant_Fixed_Void_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenture
        Sub Feature Description: Performing the brand EMI cashback by merchant fixed void transaction for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 038: TC038
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' and subventing_entity='MERCHANT' and subvention_type='CASHBACK' and subvention_value_type='FIXED' and tenure='{emi_plan_in_months} month'"
        logger.debug(f"Query to update subvention_plan_details with status as ACTIVE : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"Query to fetch result from subvention_plan_details for status as ACTIVE : {result}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                api_amount = response['amount']
                logger.debug(f"Value of amount obtained from txnlist api : {api_amount}")
                api_amount_original = response['amountOriginal']
                logger.debug(f"Value of amountOriginal obtained from txnlist api : {api_amount_original}")
                api_payment_mode = response['paymentMode']
                logger.debug(f"Value of payment mode obtained from txnlist api : {api_payment_mode}")
                api_payment_status = response['status']
                logger.debug(f"Value of status obtained from txnlist api : {api_payment_status}")
                ...
                actual_api_values = {
                    "txn_amt": api_amount,
                    "amount_original": float(api_amount_original),
                    "pmt_mode": api_payment_mode,
                    "pmt_status": api_payment_status,
                    "pmt_state": api_payment_state,
                    "mid": api_mid,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                query = f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and subventing_entity='MERCHANT' and subvention_value_type= 'FIXED' and subvention_type='CASHBACK' and tenure='{emi_plan_in_months} month' ;"
                logger.debug(f"Query to fetch data from the subvention_plan_details table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result for subvention_plan_details table : {result}")
                subvention_entity = result['subventing_entity'].values[0]
                ...
                actual_db_values = {
                    ...
                    "emi_txn_amt": emi_txn_amount,
                    "emi_original_amt": emi_original_amount,
                    "emi_payback_fixed": emi_cashback_fixed,
                    "emi_payback_amount": emi_cashback_amount,
                    "emi_total_amount": emi_total_amount,
                    "emi_cashback_type": emi_cashback_type,
                    "emi_interest_rate": emi_interest_rate,
                    "brand_name": brand_name,
                    ...
                    "subvention_tenure": subvention_tenure,
                    "subvention_discount_type": subvention_discount_type,
                    "amt_original": "{:,.2f}".format(original_amt_db),
                    "merchant_code": merchant_code_db,
                    "auth_code": auth_code_db,
                    "rrn": rrn_db,
                    "emi_amt": emi_amount_monthly,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Total Cashback Amt": "Rs. " + "{:,.2f}".format(emi_cashback_fixed),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_040():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Merchant_Fixed_Void_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenture
        Sub Feature Description: Performing the brand EMI cashback by merchant fixed void transaction for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 040: TC040
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
