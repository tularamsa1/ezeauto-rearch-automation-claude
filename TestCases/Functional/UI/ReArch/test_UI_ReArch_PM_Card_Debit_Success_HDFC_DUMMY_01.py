...
def test_common_rearch_0003():
    ...
    Sub Feature Description:
        Launch ReArch app, enter a random 3-digit amount (200-300), select Card,
        proceed through Order Details overlay, select Visa Debit (EMV) on the
        DUMMY card type selection screen, verify Payment Successful screen, then
        validate via App (TxnHistory → TxnDetail), API, and Charge Slip.
    TC naming code description:
        100: Payment Method, 102: Card, 001: TC001
    ...
    NL Source Steps:
      ...
      Test Steps:
        ...
        6.  enter amount <random 200-300>
        7.  click card
        8.  wait for order details screen
        9.  click proceed on order details
        10. wait for card type screen
        11. select visa debit emv
        12. verify payment success screen
        13. click accept more payments

      App Validation:
        14. navigate to transaction history → first transaction → txn detail
        15. validate payment id is not empty
        16. validate txn status
        17. validate payment mode
        18. validate date time is not empty

      API Validation:
        pmt_status=AUTHORIZED, txn_amt, pmt_mode=CARD, txn_type=CHARGE,
        acquirer_code=HDFC, issuer_code=DUMMY, org_code, rrn, date

      Charge Slip Validation:
        PAID BY=CARD, RRN=rrn, AUTH CODE=auth_code, BASE AMOUNT=amount, date, time
    ...
    try:
        ...
        try:
            ...
            # Steps 10-11: Card type selection → Visa Debit (EMV)
