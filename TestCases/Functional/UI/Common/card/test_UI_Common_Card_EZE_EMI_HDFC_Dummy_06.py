...
def test_common_100_115_11_029():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Subvented_by_Ezetap_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Instant Discount Subvented by Ezetap percentage success transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11:EZEEMI, 029: TC029
    """
    try:
        ...
        logger.debug(
            f"Response received when emi, emi for client, offering_emi cashback, brand_emi, businessDiscountEnabled, boInstantDiscountEnabled and bocashback is enabled in preconditions settings : {response}")
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='HDFC' and eze_emi_enabled=b'1' order by created_time desc limit 1;"
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='INSTANT' and name LIKE '{brand_name}_HDFC_CREDIT%'"
        ...
        query = f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' and subventing_entity='EZETAP' and subvention_type='INSTANT' and subvention_value_type='PERCENTAGE' and tenure='{emi_plan_in_months} month'"
        ...
        try:
            ...
            logger.debug(f"Selected payment mode is EMI plus")
            ...
            logger.debug(f"Entered the mobile number")
            imei_no = str(random.randint(1, 500))
            logger.debug(f"Randomly generated imei number is, {imei_no}")
            payment_page.click_and_enter_imei_number_for_eze_emi(imei=imei_no)
            ...
            emi_instant_discount = result['emi_instant_discount'].values[0]
            logger.debug(f"Fetching emi payback amount from txn_emi table : {emi_instant_discount}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "loan_amt": "{:,.2f}".format(amount - cal_additional_payback - total_discount_value),
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "subvention_amount": emi_instant_discount,
                ...
                api_bo_cashback_discount = response.get('emiDetails')['boInstantDiscount']
                logger.debug(f"From response fetch total boInstantDiscount : {api_bo_cashback_discount}")
                ...
                actual_api_values = {
                    ...
                    "instant_discount": api_bo_cashback_discount
                }
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "txn_amount_bo": float(final_amount + cal_additional_payback),
                    "payment_mode_bo": "CARD",
                    "bo_txn_type": "EZE_EMI",
                    "orig_amt": float(final_amount + cal_additional_payback + total_discount_value)
                }
                ...
                actual_db_values = {
                    ...
                    "emi_payback_amount": emi_instant_discount,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_11_030():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Subvented_by_Ezetap_Fixed_Success_Txn_For_An_Org_HDFC_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Instant Discount subvented by ezetap fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Credit card with pin for  months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZEEMI, 030: TC030
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and subventing_entity ='EZETAP' and subvention_value_type= 'FIXED' and subvention_type='INSTANT' and tenure='{emi_plan_in_months} month' ;"
        logger.debug(f"Query to fetch data from the subvention_plan_details table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for subvention_plan_details table : {result}")
        subvention_entity = result['subventing_entity'].values[0]
        ...
        query = f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' and subventing_entity='EZETAP' and subvention_type='INSTANT' and subvention_value_type='FIXED' and tenure='{emi_plan_in_months} month'"
        logger.debug(f"Query to update subvention_plan_details with status as ACTIVE : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"Query to fetch result from subvention_plan_details for status as ACTIVE : {result}")
        ...
        try:
            ...
            final_amount = amount - total_discount_value
            logger.debug(f"final amount after deducting discount amount: {final_amount}")
            cal_additional_payback = subvention_value
            logger.debug(f"cal_additional_payback is : {cal_additional_payback}")
            final_amount = final_amount - cal_additional_payback
            logger.debug(f"final amount after deducting discount amount: {final_amount}")
            ...
            emi_original_amount = result['original_amount'].values[0]
            logger.debug(f"Fetching original_amount from txn_emi table : {emi_original_amount}")
            emi_instant_discount_fixed = result['emi_instant_discount_fixed'].values[0]
            logger.debug(f"Fetching emi cashback fixed amount from txn_emi table : {emi_instant_discount_fixed}")
            emi_cashback_amount = result['emi_instant_discount'].values[0]
            logger.debug(f"Fetching emi cashback amount from txn_emi table : {emi_cashback_amount}")
            emi_cashback_type = result['emi_cashback_type'].values[0]
            logger.debug(f"Fetching emi cashback type from txn_emi table : {emi_cashback_type}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "entity": "EZETAP",
                    "subvention_type": "INSTANT",
                    "subvention_amount": emi_instant_discount_fixed,
                    "subvention_fixed": emi_instant_discount_fixed,
                    "emi_interest_rate": interest_rate,
                    "total_amount_with_int": total_emi,
                    "product_serial": str(imei_no),
                    "scheme_name": subvention_scheme_name,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "credit_type_bo": "INSTANT",
                    "org_code_bo": org_code,
                    "txn_amount_bo": final_amount + cal_additional_payback,
                    "payment_mode_bo": "CARD",
                    "bo_txn_type": "EZE_EMI",
                    "orig_amt": float(amount)
                }
                ...
                actual_db_values = {
                    ...
                    "emi_txn_amt": emi_txn_amount,
                    "emi_original_amt": emi_original_amount,
                    "emi_cashback_fixed": emi_instant_discount_fixed,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_11_031():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Subvented_by_Ezetap_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    ...
    via HDFC Dummy PG for AU issuer using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZEEMI, 031: TC031
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        response = APIProcessor.send_request(api_details=api_details)
        logger.debug(
            f"Response received when emi, emi for client, offering_emi cashback, brand_emi, businessDiscountEnabled and bocashback is enabled in preconditions settings : {response}")
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='CASHBACK' and name LIKE '{brand_name}_HDFC_CREDIT%'"
        logger.debug(f"Query to fetch data from the rule table : {query}")
        result = DBProcessor.getValueFromDB(query=query, db_name='rule_engine')
        logger.debug(f"Fetching result from rule table :{result}")
        rule_id = result['id'].values[0]
        ...
        try:
            ...
            logger.debug(f"Selected payment mode is EMI plus")
            payment_page.click_and_enter_search_products_or_brands(prod=brand_sku_name)
            logger.debug(f"Entered the product")
            customer_no = '9988776655'
            payment_page.click_and_enter_customer_number(customer_no)
            logger.debug(f"Entered the mobile number")
            imei_no = str(random.randint(1, 500))
            ...
            monthly_interest_rate = interest_rate / (12 * 100)
            logger.debug(f"monthly_interest_rate is : {monthly_interest_rate}")
            cal_monthly_emi_amt = (final_amount + total_discount_value) * monthly_interest_rate * (
                    (1 + monthly_interest_rate) ** emi_plan_in_months) / (
                                          (1 + monthly_interest_rate) ** emi_plan_in_months - 1)
            logger.debug(f"cal_monthly_emi_amt is : {cal_monthly_emi_amt}")
            monthly_emi = round(cal_monthly_emi_amt, 2)
            ...
            logger.debug(f"Fetching emi cashback fixed amount from txn_emi table : {emi_instant_discount_fixed}")
            emi_instant_discount = result['emi_instant_discount'].values[0]
            logger.debug(f"Fetching emi cashback amount from txn_emi table : {emi_instant_discount}")
            emi_cashback_type = result['emi_cashback_type'].values[0]
            logger.debug(f"Fetching emi cashback type from txn_emi table : {emi_cashback_type}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                date_and_time = date_time_converter.to_app_format(posting_date_db=posting_date)
                expected_app_values = {
                    "txn_amt": "{:,.2f}".format(amount - cal_additional_payback),
                    "pmt_mode": "CARD",
                    "txn_id": txn_id,
                    "pmt_status": "AUTHORIZED",
                    "rr_number": rrn,
                    ...
                    "interest_amt": "{:,.2f}".format(total_interest),
                    "net_eff_price": "{:,.2f}".format(net_amt + cal_additional_payback),
                    "brand": brand_name,
                    "product": brand_sku_name,
                    "imei_no": str(imei_no),
                    "scheme": subvention_scheme_name,
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "interest_amt": total_interest,
                    "total_emi_amt": total_emi,
                    "net_cost": float(net_amt + cal_additional_payback),
                    "emi_type": "EZE_EMI",
                    "manufacturer": brand_name,
                    "product_name": brand_sku_name,
                    "sku_code": brand_sku_code,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "total_emi_amt": total_emi,
                    "scheme_code": scheme_code,
                    "emi_txn_amt": float(amount - cal_additional_payback),
                    "emi_original_amt": float(amount),
                    "emi_status": "PENDING",
                    "emi_term": f"{emi_plan_in_months} month",
                    "emi_type": "EZE_EMI",
                ...
                actual_db_values = {
                    ...
                    "emi_original_amt": emi_original_amount,
                    "emi_cashback_fixed": emi_instant_discount_fixed,
                    "emi_cashback_amount": emi_instant_discount,
                    "emi_total_amount": emi_total_amount,
                    "emi_cashback_type": emi_cashback_type,
                    "emi_interest_rate": emi_interest_rate,
                    "brand_name": brand_name,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "EMI Amt": "Rs." + "{:.2f}".format(monthly_emi),
                    "Total Amt with Interest": "Rs." + "{:.2f}".format(total_emi),
                    "BASE AMOUNT:": "Rs." + "{:,.2f}".format(amount - cal_additional_payback),
                    "Manufacturer Name": brand_name,
                    "Product Name": brand_sku_name,
                    "Product Desc": brand_sku_code,
                    "Sr. No.": str(imei_no),
                    "unnamed_section_text": f"{customer_name_db} I agree to pay as per the card issuer agreement.",
                    "Net cost": "Rs." + "{:,.2f}".format(total_emi - (emi_instant_discount + total_discount_value) + cal_additional_payback)
                }
                logger.debug(f"expected_charge_slip_values: {expected_charge_slip_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_11_032():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Subvented_by_Ezetap_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Cashback Discount subvented by ezetap percentage success transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZEEMI, 032: TC032
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            amount = random.randint(4001, 6000)
            order_id = datetime.now().strftime('%m%d%H%M%S')
            home_page.enter_amount_and_order_number_and_device_serial_for_card(amt=amount, order_number=order_id,
                                                                               device_serial=device_serial)
            logger.debug(f"Entered amount, order_id and device_serial is : {amount}, {order_id}, {device_serial}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "loan_amt": "{:,.2f}".format(amount - cal_additional_payback),
                    "interest_amt": "{:,.2f}".format(total_interest),
                    "net_eff_price": "{:,.2f}".format(total_emi - discount_bo),
                    "brand": brand_name,
                    "product": brand_sku_name,
                    "imei_no": str(imei_no),
                    "scheme": subvention_scheme_name
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "interest_amt": total_interest,
                    "total_emi_amt": total_emi,
                    "net_cost": total_emi - discount_bo,
                    "emi_type": "EZE_EMI",
                    "manufacturer": brand_name,
                    "product_name": brand_sku_name,
                    "sku_code": brand_sku_code,
                ...
                api_original_amount = response.get('amountOriginal')
                logger.debug(f"From response fetch amountOriginal : {api_original_amount}")
                api_bo_cashback_discount = response.get('emiDetails')['boAdditionalCashback']
                logger.debug(f"From response fetch total boInstantDiscount : {api_bo_cashback_discount}")
                api_subvention_rate = response['emiDetails']['subventionDetails'][0]['subventionRate']
                logger.debug(f"Value of subventionAmount obtained from txnlist api : {api_subvention_rate}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_11_033():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Subvented_by_Ezetap_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Instant Discount instant by brand percentage success transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZEEMI, 033: TC033
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        # Below function to update bank_code, bank for the bin: 417666
        testsuite_teardown.update_bin_info(bin_number='428090', bank_code='HDFC', bank='HDFC')
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'DEBIT' and bank='HDFC' and eze_emi_enabled=b'1' order by created_time desc limit 1;"
        logger.debug(f"Query to fetch data from the subvention_plan table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for subvention_plan table : {result}")
        subvention_plan_id = result['id'].values[0]
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='INSTANT' and name LIKE '{brand_name}_HDFC_DEBIT%'"
        logger.debug(f"Query to fetch data from the rule table : {query}")
        result = DBProcessor.getValueFromDB(query=query, db_name='rule_engine')
        logger.debug(f"Fetching result from rule table :{result}")
        rule_id = result['id'].values[0]
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "scheme_name": subvention_scheme_name,
                    "user_agreement": f"I agree to pay as per the card issuer agreement. I understand that purchase(s) would be converted into EMI at the sole discretion of the Bank by charging an interest of {interest_rate}% on the monthly reducing balance.",
                    "orig_amt": float(amount - cal_additional_payback - total_discount_value),
                    "instant_discount": total_discount_value
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
