...
def get_value_from_db(query: str, db_name: str = 'ezetap_demo'):
    ...
    df_query_result = ""
    try:
        dict_db_credentials = {'username': 'ezedemo', 'password': 'abc123'}  # get_db_credentials_from_excel()
        print(
        ...
        print(f"DB name in environment is {actual_db_name}")
        ...
        print(f"Unable to connect to run query in db due to error {str(e)}")
    tunnel.close()
