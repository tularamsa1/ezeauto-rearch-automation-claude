...
def test_common_100_102_274():
    """
    Sub Feature code: TID_Dep_UI_Common_PM_BQRV4_UPI_Callback_Success_AXIS_ATOS
    Sub Feature Description: TID Dep - Verification of upi success txn using BQRV4_UPI callback via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 274: TC274
    """
    try:
        ...
        logger.debug(f"Fetc301hing mid, tid,terminal_info_id,bqr_mc_id,bqr_m_pan  from database for current merchant:"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_275():
    """
    Sub Feature code: TID_Dep_UI_Common_BQRV4_UPI_Callback_After_expired_Success_AXIS_ATOS
    Sub Feature Description: TID Dep - Verification of  a BQRV4_UPI Callback After Expiry via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 275: TC275
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            amount = random.randint(301, 400)
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.debug("Generating QR using TidDep BQR QR generate API")
            api_details = DBProcessor.get_api_details('TidDepBqrGenerate',
                                                      request_body={"username": app_username, "password": app_password,
                                                                    "amount": str(amount), "orderNumber": str(order_id),
                                                                    "deviceSerial": str(device_serial)})
            ...
            auth_code = "AE" + txn_id.split('E')[1]
            rrn = "RE" + txn_id.split('E')[1]
            logger.debug(f"Fetching Txn_id,Auth code,RRN, primary id and secondary id from data base : Txn_id : {txn_id}, Auth code : {auth_code}, RRN : {rrn}, Primary id : {pid}, Secondary id : {sid}")
            sleep(60)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_276():
    """
    Sub Feature code: TID_Dep_UI_Common_BQRV4_UPI_two_Callback_Success_AXIS_ATOS
    Sub Feature Description: TID Dep - Verification of  BQRV4_UPI  two callback via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 276: TC276
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_277():
    """
    Sub Feature code: TID_Dep_UI_Common_BQRV4_UPI_two_Callback_After_expired_AXIS_ATOS
    Sub Feature Description: TID Dep - Verification of  BQRV4_UPI two callback after expiry via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 277: TC277
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            txn_id_new = result["id"].iloc[0]
            logger.debug(f"new txn id  from database : {txn_id_new}")
            auth_code_new = result['auth_code'].values[0]
            logger.debug(f"auth code new from database : {auth_code_new}")
            created_time_new = result['created_time'].values[0]
            customer_name_new = result['customer_name'].values[0]
            payer_name_new = result['payer_name'].values[0]
            ...
            logger.debug(f"auth code new 3 from database : {auth_code_new}")
            ...
            device_serial_db_new_3 = result["device_serial"].iloc[0]
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {"txn_amt": amount_db,
                                    ...
                                    "device_serial_3": device_serial_db_new_3,
                                    }
                logger.debug(f"actual_db_values : {actual_db_values}")
                # ---------------------------------------------------------------------------------------------
                Validator.validateAgainstDB(expectedDB=expected_db_values, actualDB=actual_db_values)
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expected_portal_values = {
                    ...
                    "username_2": app_username,
                    "txn_id_2": txn_id_new,
                    "auth_code_2":  "-" if auth_code_new is None else str(auth_code_new),
                    ...
                    "txn_id_3": txn_id_new_3,
                    "auth_code_3": "-" if auth_code_new_3 is None else str(auth_code_new_3),
                    "rrn_3": "-" if rrn_new_3 is None else str(rrn_new_3)
                }
                logger.debug(f"expected_portal_values : {expected_portal_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_278():
    """
    Sub Feature code: TID_Dep_UI_Common_BQRV4_UPI_amt_missmatch_callback_AXIS_ATOS
    Sub Feature Description: TID Dep - Verification of a BQRV4_UPI callback for amount mismatch via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 278: TC278
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Fetching external_ref : {external_ref}")
            amount_db = int(result["amount"].iloc[0])
            payment_mode_db = result["payment_mode"].iloc[0]
            payment_status_db = result["status"].iloc[0]
            payment_state_db = result["state"].iloc[0]
