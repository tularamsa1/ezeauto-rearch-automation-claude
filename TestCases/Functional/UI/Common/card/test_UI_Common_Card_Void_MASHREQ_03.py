...
def test_common_100_115_253():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_EMV_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description: Performing the EMV Void transaction via MASHREQ PG using MASTER Credit card having
    16 digit PAN length with pin (bin:541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 253: TC253
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"From response fetch currencyCode : {api_currency_code}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_254():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_CTLS_VISA_CREDIT_417666
    Sub Feature Description:  Performing the EMV void transaction via MASHREQ PG using CTLS VISA Credit card
    having 16 digit PAN length without pin (bin: 417666)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 254: TC254
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_255():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_CTLS_MASTER_CREDIT_541333
    Sub Feature Description:  Performing the EMV void transaction via MASHREQ PG using CTLS MASTER Credit card
    having 16 digit PAN length without pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 255: TC255
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_256():
    """
    Sub Feature Code: UI_Common_Card_Void_MASHREQ_CTLS_WITH_PIN_MASTER_CREDIT_541333
    Sub Feature Description:  Performing the EMV void transaction via MASHREQ PG using CTLS with pin MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 256: TC256
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
