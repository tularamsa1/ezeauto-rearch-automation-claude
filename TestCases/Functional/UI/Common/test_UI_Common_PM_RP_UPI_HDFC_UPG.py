...
@pytest.mark.appVal
def test_common_100_103_049():
    """
    Sub Feature Code: UI_Common_PM_RP_upi_Callback_Amount_Mismatch_HDFC_upg_refund_and_auto_refund_disabled.
    ...
    try:
        ...
        try:
            ...
            if response['success'] == False:
                ...
                logger.info("Performing amount mismatch using callback")
            ...
            for line in iter(lambda: ssh_stdout.readline(), ''):
                data_buffer += line
            logger.debug(f"Response from curl request is : {data_buffer}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_050():
    """
    Sub Feature Code: UI_Common_PM_RP_upi_Callback_Amount_Mismatch_HDFC_upg_refund_and_auto_refund_enabled.
    ...
    try:
        ...
        try:
            ...
            if response['success'] == False:
                ...
                logger.info("Launching UPI")
                remote_pay_upi_txn.clickOnRemotePayLaunchUPI()
                logger.info("Waiting for amount mismatch")
            ...
            rrn = random.randint(1111110, 9999999)
            logger.debug(f"generated random rrn number is : {rrn}")
            txn_ref = '211115084892E01' + str(rrn)
            ...
            amount2 = random.randint(10, 50)
            logger.debug(f"replacing the Txn_id with {payment_intent_id}, amount with {amount2}.00, vpa with "
                f"{vpa} and rrn with {rrn} in the curl_data ")
            ...
            api_details = DBProcessor.get_api_details('callBackUpiMerchantRes',
                                                      request_body={'pgMerchantId': str(pg_merchant_id),
                                                                    'meRes': str(data_buffer)})
            response = APIProcessor.send_request(api_details)
            logger.info(f"Response from curl request api is : {response}")
