...
@pytest.mark.apiVal
def test_common_100_103_332():
    """
     Sub Feature Code: UI_Common_PM_RP_Transaction_Currency_Dollar
     Sub Feature Description: Perform a Remote Pay transaction with different Currency than INR(Say Dollar)
     TC naming code description: 100: Payment Method, 103: RemotePay, 332: TC332
    """
    ...
    try:
        ...
        query = f"update org set currency_code='USD' where org_code='{org_code}';"
        logger.debug(f"Query to update org table: {query}")
        ...
        Configuration.configureLogCaptureVariables(apiLog=True, portalLog=False, cnpwareLog=True, middlewareLog=True)
        ...
        try:
            query = f"update org set currency_code='INR' where org_code='{org_code}';"
            logger.debug(f"Query to update org table: {query}")
            result = DBProcessor.setValueToDB(query=query)
            logger.debug(f"query result : {result}")
            refresh_db()
...
def test_common_100_103_340():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Enable_Time_Based_Restriction_with_RemotePay_Option_Restricted_Success_Transaction_via_AXIS_DIRECT
    Sub Feature Description: Successful Remote Pay transaction when "Time Based Restriction" is enabled and Remote Payment Mode is not selected via AXIS_DIRECT
    TC naming code description: 100: Payment Method, 103: RemotePay, 340: TC340
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_341():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Disable_Time_Based_Restriction_Via_Success_Transaction_via_AXIS_DIRECT
    Sub Feature Description: Successful Remote Pay transaction when "Time Based Restriction" is disabled via AXIS_DIRECT
    TC naming code description: 100: Payment Method, 103: RemotePay, 341: TC341
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"API details  : {api_details} ")
        response = APIProcessor.send_request(api_details=api_details)
        logger.debug(f"Response received for setting preconditions for Time Based Txn Restriction to be disabled:  {response}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_344():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Enable_Time_Based_Restriction_Before_Start_Time_Via_Axis_Direct
    Sub Feature Description: Verify remote Pay success transactions via axis direct, before the start time of "Time Based Restriction"
    TC naming code description: 100: Payment Method, 103: RemotePay, 344: TC344
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_345():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Enable_Time_Based_Restriction_After_End_Time_Via_Axis_Direct
    Sub Feature Description: Perform a successful Remote Pay transaction via axis direct after the end time of "Time Based Restriction"
    TC naming code description: 100: Payment Method, 103: RemotePay, 345: TC345
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        indian_timezone = pytz.timezone('Asia/Kolkata')
        time_based_restriction_start_time = datetime.now(indian_timezone).strftime("%H%M")
        time_based_restriction_end_time = (datetime.now(indian_timezone) + timedelta(minutes=1)).strftime("%H%M")
        logger.debug(f"time_based_restriction_start_time (IST) : {time_based_restriction_start_time}")
        logger.debug(f"time_based_restriction_end_time (IST) :  {time_based_restriction_end_time}")
