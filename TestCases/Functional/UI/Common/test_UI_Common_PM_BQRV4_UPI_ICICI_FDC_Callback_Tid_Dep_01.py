...
def test_common_100_102_151():
    """
    Sub Feature Code: Tid_Dep - UI_Common_PM_BQRV4_UPI_Success_Via_BQRV4_UPI_Callback_ICICI_FDC
    Sub Feature Description: Tid Dep - Verification of a successful BQRV4 upi txn via ICICI_FDC using success callback
    TC naming code description: 100: Payment Method, 102: BQRV4, 151: TC151
    """
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('bharatqr_confirm_FDC',
                                                      request_body={"payload": data_buffer['payload'],
                                                                    "sequenceNo": data_buffer['sequenceNo']})
            response = APIProcessor.send_request(api_details)
            ...
            logger.debug(f"Fetching customer_name, payer_name, auth_code, created_time, status_db,payment_mode_db, amount_db, state_db, payment_gateway_db, acquirer_code_db, bank_code_db, settlement_status_db, mid_db, tid_db, device_serial_db,order_id_db, rr_number_db from database for "
                f"current merchant:{customer_name},{payer_name},{auth_code},{created_time},{status_db},{payment_mode_db}, {amount_db}, {state_db}, {payment_gateway_db}, {acquirer_code_db}, {bank_code_db}, {settlement_status_db}, {mid_db}, {tid_db}, {device_serial_db},{order_id_db},{rr_number_db}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_values = {'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id), 'RRN': str(authid),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_161():
    """
    Sub Feature Code: Tid_Dep - UI_Common_PM_BQRV4_UPI_Success_Callback_After_QR_Expiry_ICICI_FDC
    Sub Feature Description: Tid_Dep - Performing successful BQRV4 UPI callback after qr expiry via ICICI_FDC Tid Dep
    TC naming code description: 100: Payment Method, 102: BQRV4, 161: TC161
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            order_id_db = result['external_ref'].values[0]
            rr_number_db = result['rr_number'].values[0]
            logger.debug(f"Fetching status_db, payment_mode_db, amount_db, state_db, payment_gateway_db, acquirer_code_db, bank_code_db, settlement_status_db, mid_db, tid_db, device_serial_db,order_id_db, rr_number_db from database for "
                f"current merchant:{status_db},{payment_mode_db}, {amount_db}, {state_db}, {payment_gateway_db}, {acquirer_code_db}, {bank_code_db}, {settlement_status_db}, {mid_db}, {tid_db}, {device_serial_db},{order_id_db},{rr_number_db}")
            ...
            logger.debug(f"preparing the request payload data to trigger the /api/2.0/bharatqr/confirm/FDC")
            data_buffer = json.loads(data_buffer)
            api_details = DBProcessor.get_api_details('bharatqr_confirm_FDC',
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_162():
    """
    Sub Feature Code: Tid_Dep - UI_Common_PM_BQRV4_1_UPI_1_BQR_Success_Callback_After_QR_Expiry_ICICI_FDC
    Sub Feature Description: Tid Dep - Performing successful BQRV4 1 UPI and 1 BQR callback after qr expiry via ICICI_FDC
    TC naming code description: 100: Payment Method, 102: BQRV4, 162: TC162
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"replacing the Txn_id with {txn_id}, amount with {amount}.00, vpa with {vpa} and rrn with {rrn_3} in the curl_data")
            api_details = DBProcessor.get_api_details('fdc_icici_bqrv4_success_curl', curl_data={
                'mid': mid, 'tid': tid,
                'trans_amount': str(amount) + "00",
                'baseAmt': str(amount) + "00",
            ...
            query = "select * from txn where org_code = '" + str(org_code) + "' AND external_ref = '" + str(
                order_id) + "' and orig_txn_id='" + str(txn_id) + "' and payment_mode='BHARATQR';"
            logger.debug(f"Query to fetch txn data from the txn table : {query}")
            result = DBProcessor.getValueFromDB(query)
            txn_id_2 = result['id'].values[0]
            logger.debug(f"Fetching txn_id from the txn table : txn_id_2 : {txn_id_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_164():
    """
    Sub Feature Code: Tid_Dep - UI_Common_PM_BQRV4_UPI_2_Success_Callback_Before_QR_Expiry_ICICI_FDC
    Sub Feature Description: Tid Dep - Performing 2 successful BQRV4 UPI callback before qr expiry via ICICI_FDC
    TC naming code description: 100: Payment Method, 102: BQRV4, 164: TC164
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "order_id_2": order_id,
                    "pmt_msg_2": "PAYMENT SUCCESSFUL",
                    "date_2": date_and_time_3,
                    "auth_code_2": authid_3,
                    "rrn_2": str(authid_3),
                    "customer_name_2": customer_name_3,
                    "payer_name_2": payer_name_3,
                }
                logger.debug(f"expectedAppValues: {expected_app_values}")
                ...
                actual_app_values = {
                    ...
                    "order_id": app_order_id_2,
                    "pmt_msg": app_payment_msg_2,
                    "auth_code": app_auth_code_2,
                    "date": app_date_and_time_2,
                    "pmt_mode_2": payment_mode_3,
                    "pmt_status_2": payment_status_3.split(':')[1],
                    "txn_amt_2": str(app_amount_3).split(' ')[1],
                    "settle_status_2": app_settlement_status_3,
                    "txn_id_2": txn_id_3,
                    "order_id_2": app_order_id_3,
                    "pmt_msg_2": app_payment_msg_3,
                    "date_2": date_and_time_3,
                    "auth_code_2": auth_code_3,
                    "rrn_2": str(app_rrn_3),
                    "customer_name_2": app_customer_name_3,
                    "payer_name_2": app_payer_name_3,
                }
                logger.debug(f"actual_app_values: {actual_app_values}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "pmt_status": "AUTHORIZED",
                    "txn_amt": float(amount), "pmt_mode": "UPI",
                    "pmt_state": "SETTLED", "rrn": str(authid_2),
                    "settle_status": "SETTLED",
                    "acquirer_code": "ICICI",
                    "issuer_code": "ICICI",
                    "order_id": order_id,
                    ...
                    "pmt_status_2": "AUTHORIZED",
                    "txn_amt_2": float(amount), "pmt_mode_2": "UPI",
                    "pmt_state_2": "SETTLED", "rrn_2": str(authid_3),
                    "settle_status_2": "SETTLED",
                    "acquirer_code_2": "ICICI",
                    "issuer_code_2": "ICICI",
                    "order_id_2": order_id,
                ...
                actual_api_values = {
                    ...
                    "txn_amt": float(amount_api_2), "pmt_mode": payment_mode_api_2,
                    "pmt_state": state_api_2, "rrn": str(rrn_api_2),
                    "settle_status": settlement_status_api_2,
                    "acquirer_code": acquirer_code_api_2,
                    "issuer_code": issuer_code_api_2,
                    "order_id": order_id_api_2,
                    "txn_type": txn_type_api_2, "mid": mid_api_2, "tid": tid_api_2,
                    "org_code": orgCode_api_2,
                    "auth_code": auth_code_api_2,
                    "date": date_time_converter.from_api_to_datetime_format(date_api_2),
                    "device_serial": str(device_serial_api_2),
                    "pmt_status_2": status_api_3,
                    "txn_amt_2": float(amount_api_3), "pmt_mode_2": payment_mode_api_3,
                    "pmt_state_2": state_api_3, "rrn_2": str(rrn_api_3),
                    "settle_status_2": settlement_status_api_3,
                    "acquirer_code_2": acquirer_code_api_3,
                    "issuer_code_2": issuer_code_api_3,
                    "order_id_2": order_id_api_3,
                    "txn_type_2": txn_type_api_3, "mid_2": mid_api_3, "tid_2": tid_api_3,
                    "org_code_2": orgCode_api_3,
                    "auth_code_2": auth_code_api_3,
                    "date_2": date_time_converter.from_api_to_datetime_format(date_api_3),
                    "device_serial_2": str(device_serial_api_3),
                }
                logger.debug(f"actual_api_values: {actual_api_values}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "bqr_txn_type": "DYNAMIC_QR", "bqr_terminal_info_id": terminal_info_id,
                    "bqr_bank_code": "ICICI",
                    "bqr_merchant_config_id": bqr_merchant_config_id, "bqr_txn_primary_id": txn_id_2,
                    "bqr_org_code": org_code,
                    "device_serial": str(device_serial),
                    "upi_txn_status": "AUTHORIZED",
                    "upi_txn_type": "PAY_BQR",
                ...
                actual_db_values = {
                    ...
                    # "bqr_org_code": bqr_org_code_db,
                    # "device_serial": str(device_serial_db),
                    "pmt_status": status_db_2,
                    "pmt_state": state_db_2,
                    "pmt_mode": payment_mode_db_2,
                    "rrn": str(rr_number_db_2),
                    "auth_code": str(auth_code_2),
                    "txn_amt": float(amount_db_2),
                    "settle_status": settlement_status_db_2,
                    "order_id": order_id_db_2,
                    "acquirer_code": acquirer_code_db_2,
                    "bank_code": bank_code_db_2,
                    "pmt_gateway": payment_gateway_db_2,
                    "mid": mid_db_2,
                    "tid": tid_db_2,
                    "bqr_pmt_status": bqr_status_db_2, "bqr_pmt_state": bqr_state_db_2,
                    "bqr_txn_amt": float(bqr_amount_db_2),
                    "bqr_txn_type": bqr_txn_type_db_2, "bqr_terminal_info_id": bqr_terminal_info_id_db_2,
                    ...
                    "device_serial": str(device_serial_db_2),
                    "upi_txn_status": upi_status_db_2,
                    "upi_txn_type": upi_txn_type_db_2,
                    "upi_bank_code": upi_bank_code_db_2,
                    "upi_mc_id": upi_mc_id_db_2,
                    "pmt_status_2": status_db_3,
                    "pmt_state_2": state_db_3,
                    "pmt_mode_2": payment_mode_db_3,
                    "rrn_2": str(rr_number_db_3),
                    "auth_code_2": str(auth_code_3),
                    "txn_amt_2": float(amount_db_3),
                    "settle_status_2": settlement_status_db_3,
                    "order_id_2": order_id_db_3,
                    "acquirer_code_2": acquirer_code_db_3,
                    "bank_code_2": bank_code_db_3,
                    "pmt_gateway_2": payment_gateway_db_3,
                    "mid_2": mid_db_3,
                    "tid_2": tid_db_3,
                    # "bqr_pmt_status_3": bqr_status_db_3,
                    # "bqr_pmt_state_3": bqr_state_db_3,
                    ...
                    # "bqr_txn_primary_id_3": bqr_txn_primary_id_db_3,
                    # "bqr_org_code_3": bqr_org_code_db_3,
                    "device_serial_2": str(device_serial_db_3),
                    "upi_txn_status_2": upi_status_db_3,
                    "upi_txn_type_2": upi_txn_type_db_3,
                    "upi_bank_code_2": upi_bank_code_db_3,
                    "upi_mc_id_2": upi_mc_id_db_3,
                }
                logger.debug(f"actual_db_values : {actual_db_values}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                date_and_time_portal_1 = date_time_converter.to_portal_format(txn_created_time_3)
                expected_portal_values = {
                    "date_time": date_and_time_portal,
                    "pmt_state": "AUTHORIZED",
                    "pmt_type": "UPI",
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                txn_date_2, txn_time_2 = date_time_converter.to_chargeslip_format(txn_created_time_2)
                expected_values_1 = {'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id),
                                     'RRN': str(authid_2),
                                     'BASE AMOUNT:': "Rs." + str(amount) + ".00",
                                     'date': txn_date_2, 'time': txn_time_2, 'AUTH CODE': authid_2}
                logger.debug(f"expected_values : {expected_values_1}")
