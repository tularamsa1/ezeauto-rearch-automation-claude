...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_132():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_Failed_Via_AXIS_DIRECT
    Sub Feature Description: Verification of a Remote Pay failed UPI collect txn via AXIS DIRECT using callback
    TC naming code description:
    100: Payment Method
    103: RemotePay
    132: TC_132
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_133():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_success_callback_after_expiry_AXIS_DIRECT
    Sub Feature Description: Performing a UPI Collect success callback via Axis Direct after expiry
    TC naming code description:
    100: Payment Method
    103: RemotePay
    133: TC_133
    ...
    try:
        ...
        try:
            ...
            mid = result['mid'].values[0]
            logger.debug(f"generated random mid number is : {mid}")
            tid = result['tid'].values[0]
            logger.debug(f"generated random tid number is : {tid}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            logger.info(f"Started API validation for the test case : {testcase_id}")
            try:
                date_and_time = date_time_converter.db_datetime(original_created_time)  # Replace date with dateand time
                expected_api_values = {
                    "pmt_status": "FAILED",
                    "txn_amt": amount,
                    "pmt_mode": "UPI",
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
                ...
                try:
                    ...
                    date_and_time_portal_2 = date_time_converter.to_portal_format(new_txn_date_time)
                    expected_portal_values = {
                        "date_time": date_and_time_portal,
                        "pmt_state": "FAILED",
                        "pmt_type": "UPI",
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_134():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_failed_callback_after_expiry_AXIS_DIRECT_AutoRefund_Disabled
    Sub Feature Description: Performing a UPI collect failed callback via Axis Direct after expiry when auto refund is disabled
    100: Payment Method
    103: RemotePay
    134: TC134
    """
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                transaction_details = get_transaction_details_for_portal(
                    app_un=app_username, app_pw=app_password, order_id=order_id)
                date_time = transaction_details[0]['Date & Time']
                transaction_id = transaction_details[0]['Transaction ID']
                total_amount = transaction_details[0]['Total Amount'].split()
                transaction_type = transaction_details[0]['Type']
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_135():
    """
    Sub Feature Code: UI_Common_PM_RP_Two_UPI_Collect_Success_Callback_After_Expiry_AutoRefundDisabled_AXIS_DIRECT
    Sub Feature Description: Verification of a two upi collect success callback after expiry via Axis Direct when auto refund is disabled.
    100: Payment Method
    103: RemotePay
    135: TC135
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                date_and_time_portal_2 = date_time_converter.to_portal_format(new_txn_modified_time_app_1)
                ...
                actual_portal_values = {
                    ...
                    "date_time_3": callback_date_time,
                    "pmt_state_3": str(callback_status),
                    "pmt_type_3": callback_transaction_type,
                    "txn_amt_3": callback_total_amount[1],
                    "username_3": str(callback_username),
                    "rrn_3": str(callback_rrn),
                    "txn_id_3": callback_transaction_id
                }
                logger.debug(f"actual_portal_values : {actual_portal_values} for the testcase_id : {testcase_id}")
                # ---------------------------------------------------------------------------------------------
                Validator.validateAgainstPortal(expectedPortal=expected_portal_values,
