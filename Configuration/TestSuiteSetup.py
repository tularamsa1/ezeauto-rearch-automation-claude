...
def prepareDevicesAndDB():
    ...
    if str(ConfigReader.read_config("standalone_features", "setup_for_NonUI")).lower() == "true":
        ...
        if str(ConfigReader.read_config("standalone_features", "setup_for_card")).lower() == "true":
...
def initialize_app_driver_for_sample_app(request, no_reset="false"):
    """
    This method is used for initializing the sample app driver for the app operations
    """
    # test_case_id = request.node.name
    function_start_time = datetime.datetime.now()
    test_case_id = request
