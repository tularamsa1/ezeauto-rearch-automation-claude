...
def test_sa_100_101_068():
    """
    Sub Feature Code: UI_SA_PM_UPI_Success_Via_SA_CheckStatus_APB
    Sub Feature Description: Verification of a successful upi txn via APB using SA check status
    TC naming code description: 100: Payment Method, 101: UPI, 068: TC068
    """
    try:
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_sa_100_101_069():
    """
    Sub Feature Code: UI_SA_PM_UPI_Failed_Via_SA_CheckStatus_APB
    Sub Feature Description: Verification of a failed UPI txn via APB using SA check status
    TC naming code description: 100: Payment Method, 101: UPI, 069: TC69
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
