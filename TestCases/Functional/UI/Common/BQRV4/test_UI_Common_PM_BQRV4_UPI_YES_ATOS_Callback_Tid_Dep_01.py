...
def test_common_100_102_285():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_UPI_Success_Callback_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_UPI Callback Success via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 285: TC285
    """
    try:
        ...
        try:
            ...
            pid += sid
            logger.debug(f"Value of both pid and sid obtained from bharatqr_txn table : {pid}")
            txn_id = result["id"].iloc[0]
            logger.debug(f"Value of txn_id obtained from bharatqr_txn table : {txn_id}")
            auth_code = "AE" + txn_id.split('E')[1]
            logger.debug(f"Value of auth_code is : {auth_code}")
            rrn = "RE" + txn_id.split('E')[1]
            logger.debug(f"Value of rrn is : {rrn}")

            api_details = DBProcessor.get_api_details('callbackUpiYES',request_body={
                "primary_id": pid,
                "secondary_id": sid,
            ...
            logger.debug(f"Value of amount_db obtained from txn table : {amount_db}")
            ...
            logger.debug(f"Value of payment_state obtained from txn table : {payment_state_db}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Value of status_desc obtained from bharatqr_txn table : {bqr_status_db}")
...
@pytest.mark.appVal
@pytest.mark.portalVal
@pytest.mark.chargeslip
def test_common_100_102_286():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_UPI_Callback_After_Expiry_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_UPI Callback After QR Expiry Success transaction when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 286: TC286
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Value of rr_number obtained from txn table based on org_code : {rrn_db}")
            auth_code_db = result['auth_code'].values[0]
            logger.debug(f"Value of auth_code obtained from txn table based on org_code : {auth_code_db}")
            ...
            logger.debug(f"Value of device_serial obtained from txn table based on org_code : {device_serial_db_new}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Response after filtering data of another txn is : {response_2}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Value of status obtained from upi_txn table : {upi_status_db_new}")
                upi_txn_type_db_new = result["txn_type"].iloc[0]
                logger.debug(f"Value of txn_type obtained from upi_txn table : {upi_txn_type_db_new}")
                upi_mc_id_db_new = result["upi_mc_id"].iloc[0]
                logger.debug(f"Value of upi_mc_id obtained from upi_txn table : {upi_mc_id_db_new}")
                upi_bank_code_db_new = result["bank_code"].iloc[0]
                logger.debug(f"Value of bank_code obtained from upi_txn table : {upi_bank_code_db_new}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                receipt_validator.perform_charge_slip_validations(txn_id_new,{"username": app_username, "password": app_password},
                                                                  expected_values)
            except Exception as e:
                Configuration.perform_charge_slip_val_exception(testcase_id, e)
            logger.info(f"Completed ChargeSlip validation for the test case : {testcase_id}")
...
@pytest.mark.portalVal
@pytest.mark.chargeslip
def test_common_100_102_287():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_UPI_Two_Callback_Success_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_UPI generation using API and performing two Callback Success transaction when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 287: TC287
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Value of rrn is : {rrn_new}")
            ...
            order_id_new = result['external_ref'].values[0]
            logger.debug(f"Value of order_id obtained from txn table for second callback : {order_id_new}")
            auth_code_new = result['auth_code'].values[0]
            ...
            device_serial_db_new = result['device_serial'].values[0]
            logger.debug(f"Value of device_serial obtained from txn table for second callback: {device_serial_db_new}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Value of rrNumber obtained from first txnlist api is : {rrn_api}")
                settlement_status_api = response_1["settlementStatus"]
                ...
                logger.debug(f"Value of authCode obtained from first txnlist api is : {auth_code_api}")
                device_serial_api = response_1["deviceSerial"]
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                upi_status_db = result["status"].iloc[0]
                logger.debug(f"Value of status obtained from upi_txn table based on txn_id : {upi_status_db}")
                upi_txn_type_db = result["txn_type"].iloc[0]
                logger.debug(f"Value of txn_type obtained from upi_txn table based on txn_id : {upi_txn_type_db}")
                upi_mc_id_db = result["upi_mc_id"].iloc[0]
                logger.debug(f"Value of upi_mc_id obtained from upi_txn table based on txn_id : {upi_mc_id_db}")
                upi_bank_code_db = result["bank_code"].iloc[0]
                logger.debug(f"Value of bank_code obtained from upi_txn table based on txn_id : {upi_bank_code_db}")
...
@pytest.mark.portalVal
@pytest.mark.chargeslip
def test_common_100_102_288():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_Two_Callback_After_Expiry_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_UPI generation using API and performing two Callback transaction when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 288: TC288
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Value of rrn is : {rrn_new_3}")
            ...
            query = "select * from txn where org_code='" + org_code + "' and id LIKE '" + datetime.utcnow().strftime('%y%m%d') + "%' order by created_time desc limit 1;"
            logger.debug(f"Query to fetch data from txn table after second callabck : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result of txn table after second callback : {result}")
            txn_id_new_3 = result["id"].iloc[0]
            logger.debug(f"Value of txn_id obtained from txn table after second callback : {txn_id_new_3}")
            auth_code_new_3 = result['auth_code'].values[0]
            logger.debug(f"Value of auth_code obtained from txn table after second callback : {auth_code_new_3}")
            created_time_new_3 = result['created_time'].values[0]
            logger.debug(f"Value of created_time obtained from txn table after second callback : {created_time_new_3}")
            customer_name_new_3 = result['customer_name'].values[0]
            logger.debug(f"Value of customer_name obtained from txn table after second callback : {customer_name_new_3}")
            payer_name_new_3 = result['payer_name'].values[0]
            logger.debug(f"Value of payer_name obtained from txn table after second callback : {payer_name_new_3}")
            amount_db_new_3 = float(result["amount"].iloc[0])
            logger.debug(f"Value of amount obtained from txn table after second callback : {amount_db_new_3}")
            payment_mode_db_new_3 = result["payment_mode"].iloc[0]
            logger.debug(f"Value of payment_mode obtained from txn table after second callback : {payment_mode_db_new_3}")
            payment_status_db_new_3 = result["status"].iloc[0]
            logger.debug(f"Value of status obtained from txn table after second callback : {payment_status_db_new_3}")
            payment_state_db_new_3 = result["state"].iloc[0]
            logger.debug(f"Value of state obtained from txn table after second callback : {payment_state_db_new_3}")
            acquirer_code_db_new_3 = result["acquirer_code"].iloc[0]
            logger.debug(f"Value of acquirer_code obtained from txn table after second callback : {acquirer_code_db_new_3}")
            bank_name_db_new_3 = result["bank_name"].iloc[0]
            logger.debug(f"Value of bank_name obtained from txn table after second callback : {bank_name_db_new_3}")
            payer_name_db_new_3 = result["payer_name"].iloc[0]
            logger.debug(f"Value of payer_name obtained from txn table after second callback : {payer_name_db_new_3}")
            mid_db_new_3 = result["mid"].iloc[0]
            logger.debug(f"Value of mid obtained from txn table after second callback : {mid_db_new_3}")
            tid_db_new_3 = result["tid"].iloc[0]
            logger.debug(f"Value of tid obtained from txn table after second callback : {tid_db_new_3}")
            payment_gateway_db_new_3 = result["payment_gateway"].iloc[0]
            logger.debug(f"Value of payment_gateway obtained from txn table after second callback : {payment_gateway_db_new_3}")
            rr_number_db_new_3 = result["rr_number"].iloc[0]
            logger.debug(f"Value of rr_number obtained from txn table after second callback : {rr_number_db_new_3}")
            settlement_status_db_new_3 = result["settlement_status"].iloc[0]
            logger.debug(f"Value of settlement_status obtained from txn table after second callback : {settlement_status_db_new_3}")
            device_serial_db_new_3 = result['device_serial'].values[0]
            logger.debug(f"Value of device_serial obtained from txn table after second callback : {device_serial_db_new_3}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching txn payer name from txn history for the txn : {txn_id_new_3}, {app_payer_name_new_3}")
                ...
                logger.info(f"Fetching txn_id from txn history for the txn : {txn_id_new_3}, {app_rrn_new_3}")  # behavior is diff on both emulator and device (Number/NUMBER)
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                actual_api_values = {
                    ...
                    "device_serial": str(device_serial_api),
                    "device_serial_2": str(device_serial_api_new),
                    "device_serial_3": str(device_serial_api_new_3)
                }
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Query to fetch data from upi_txn table based on third txn_id : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result of upi_txn table based on third txn_id : {result}")
                upi_status_db_new_3 = result["status"].iloc[0]
                logger.debug(f"Value of status obtained from upi_txn table based on third txn_id : {upi_status_db_new_3}")
                upi_txn_type_db_new_3 = result["txn_type"].iloc[0]
                logger.debug(f"Value of txn_type obtained from upi_txn table based on third txn_id : {upi_txn_type_db_new_3}")
                upi_mc_id_db_new_3 = result["upi_mc_id"].iloc[0]
                logger.debug(f"Value of upi_mc_id obtained from upi_txn table based on third txn_id : {upi_mc_id_db_new_3}")
                upi_bank_code_db_new_3 = result["bank_code"].iloc[0]
                logger.debug(f"Value of bank_code obtained from upi_txn table based on third txn_id : {upi_bank_code_db_new_3}")
                ...
                actual_db_values = {
                    ...
                    "upi_pmt_status_3": upi_status_db_new_3,
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                date_and_time_portal = date_time_converter.to_portal_format(created_time)
                date_and_time_portal_new = date_time_converter.to_portal_format(created_time_new)
                date_and_time_portal_new_3 = date_time_converter.to_portal_format(created_time_new_3)
                expected_portal_values = {
                    "date_time": date_and_time_portal,
                    "pmt_state": "EXPIRED",
                    "pmt_type": "BHARATQR",
                    ...
                    "txn_id_3": txn_id_new_3,
                    "auth_code_3": "-" if auth_code_new_3 is None else auth_code_new_3,
                    "rrn_3": "-" if rrn_new_3 is None else rrn_new_3,
                }
                logger.debug(f"expected_portal_values : {expected_portal_values}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values_1 = {
                    'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id), 'RRN': str(rrn_new),
                    'BASE AMOUNT:': "Rs." + str(amount) + ".00", 'date': txn_date_1,
                    'time': txn_time_1, 'AUTH CODE': str(auth_code_new)
                }

                expected_charge_slip_values_2 = {
                    'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id), 'RRN': str(rrn_new_3),
...
@pytest.mark.appVal
@pytest.mark.portalVal
def test_common_100_102_289():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_UPI_Amount_Mismatch_Callback_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_UPI amount mismatch txn when Auto refund is disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 289: TC289
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Fetching pgMerchantId from the upi_merchant_config table : pg_merchant_id : {pg_merchant_id}")
        ...
        try:
            ...
            rrn = "RE" + txn_id.split('E')[1]
            logger.debug(f"Value of rrn is : {rrn}")
            customer_vpa = str(order_id) + "@upi"
            logger.debug(f"Value of customer_vpa is : {customer_vpa}")
            ...
            logger.debug(f"Value of payment_mode obtained invalid_pg_request table : {ipr_payment_mode}")
            ipr_bank_code = result["bank_code"].iloc[0]
            logger.debug(f"Value of bank_code obtained invalid_pg_request table : {ipr_bank_code}")
            ipr_org_code = result["org_code"].iloc[0]
            logger.debug(f"Value of org_code obtained invalid_pg_request table : {ipr_org_code}")
            ipr_amount = result["amount"].iloc[0]
            logger.debug(f"Value of amount obtained invalid_pg_request table : {ipr_amount}")
            ipr_rrn = result["rrn"].iloc[0]
            logger.debug(f"Value of rrn obtained invalid_pg_request table : {ipr_rrn}")
            ipr_mid = result["mid"].iloc[0]
            logger.debug(f"Value of mid obtained invalid_pg_request table : {ipr_mid}")
            ipr_tid = result["tid"].iloc[0]
            logger.debug(f"Value of tid obtained invalid_pg_request table : {ipr_tid}")
            ipr_config_id = result["config_id"].iloc[0]
            logger.debug(f"Value of config_id obtained invalid_pg_request table : {ipr_config_id}")
            ipr_vpa = result["vpa"].iloc[0]
            logger.debug(f"Value of vpa obtained invalid_pg_request table : {ipr_vpa}")
            ipr_pg_merchant_id = result["pg_merchant_id"].iloc[0]
            logger.debug(f"Value of pg_merchant_id obtained invalid_pg_request table : {ipr_pg_merchant_id}")
            ...
            logger.debug(f"Value of created_time obtained from txn table : {posting_date_upg}")
            rrn_upg = result['rr_number'].iloc[0]
            logger.debug(f"Value of rr_number obtained from txn table : {rrn_upg}")
            auth_code_upg = result['auth_code'].iloc[0]
            logger.debug(f"Value of auth_code obtained from txn table : {auth_code_upg}")
            order_id_upg = result['external_ref'].values[0]
            logger.debug(f"Value of external_ref obtained from txn table : {order_id_upg}")
            status_db_new = result["status"].iloc[0]
            logger.debug(f"Value of status obtained from txn table : {status_db_new}")
            payment_mode_db_new = result["payment_mode"].iloc[0]
            ...
            logger.debug(f"Value of state obtained from txn table : {state_db_new}")
            payment_gateway_db_new = result["payment_gateway"].iloc[0]
            ...
            logger.debug(f"Value of bank_code obtained from txn table : {bank_code_db_new}")
            settlement_status_db_new = result["settlement_status"].iloc[0]
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Response after filtering data of upg txn_id is : {response}")
                status_api_new = response_2["status"]
                logger.debug(f"Value of status obtained from upg txnlist api is : {status_api_new}")
                amount_api_new = float(response_2["amount"])
                logger.debug(f"Value of amount obtained from upg txnlist api is : {amount_api_new}")
                payment_mode_api_new = response_2["paymentMode"]
                logger.debug(f"Value of paymentMode obtained from upg txnlist api is : {payment_mode_api_new}")
                state_api_new = response_2["states"][0]
                logger.debug(f"Value of states obtained from upg txnlist api is : {state_api_new}")
                rrn_api_new = response_2["rrNumber"]
                logger.debug(f"Value of rrNumber obtained from upg txnlist api is : {rrn_api_new}")
                settlement_status_api_new = response_2["settlementStatus"]
                logger.debug(f"Value of settlementStatus obtained from upg txnlist api is : {settlement_status_api_new}")
                issuer_code_api_new = response_2["issuerCode"]
                logger.debug(f"Value of issuerCode obtained from upg txnlist api is : {issuer_code_api_new}")
                acquirer_code_api_new = response_2["acquirerCode"]
                logger.debug(f"Value of acquirerCode obtained from upg txnlist api is : {acquirer_code_api_new}")
                org_code_api_new = response_2["orgCode"]
                logger.debug(f"Value of orgCode obtained from upg txnlist api is : {org_code_api_new}")
                mid_api_new = response_2["mid"]
                logger.debug(f"Value of mid obtained from upg txnlist api is : {mid_api_new}")
                tid_api_new = response_2["tid"]
                logger.debug(f"Value of tid obtained from upg txnlist api is : {tid_api_new}")
                txn_type_api_new = response_2["txnType"]
                logger.debug(f"Value of txnType obtained from upg txnlist api is : {txn_type_api_new}")
                date_api_new = response_2["createdTime"]
                logger.debug(f"Value of createdTime obtained from upg txnlist api is : {date_api_new}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "pmt_mode_2": "UPI",
                    "txn_amt_2": amount,
                    "upi_txn_status_2": "UPG_AUTHORIZED",
                    "settle_status_2": "SETTLED",
                    "acquirer_code_2": "YES",
                    "bank_code_2": "YES",
                    "payment_gateway_2": "ATOS",
                    "upi_txn_type_2": "UNKNOWN",
                    "upi_bank_code_2": "YES",
                    "upi_mc_id_2": upi_mc_id,
                    "mid_2": mid,
                    "tid_2": tid,
                ...
                query = "select * from upi_txn where txn_id='" + txn_id_upg + "'"
                logger.debug(f"Query to fetch data from upi_txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result of upi_txn table : {result}")
                upi_status_db = result["status"].iloc[0]
                ...
                actual_db_values = {
                    ...
                    "pmt_mode_2": payment_mode_db_new,
                    "txn_amt_2": amount_db_new,
                    "upi_txn_status_2": upi_status_db,
                    "settle_status_2": settlement_status_db_new,
                    "acquirer_code_2": acquirer_code_db_new,
                    "bank_code_2": bank_code_db_new,
                    "payment_gateway_2": payment_gateway_db_new,
                    "upi_txn_type_2": upi_txn_type_db,
                    "upi_bank_code_2": upi_bank_code_db,
                    "upi_mc_id_2": upi_mc_id_db,
                    "mid_2": mid_db_new,
                    "tid_2": tid_db_new,
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                date_and_time_portal = date_time_converter.to_portal_format(posting_date)
                date_and_time_portal_new = date_time_converter.to_portal_format(posting_date_upg)
                expected_portal_values = {
                    "date_time": date_and_time_portal,
                    "pmt_state": "PENDING",
                    "pmt_type": "BHARATQR",
