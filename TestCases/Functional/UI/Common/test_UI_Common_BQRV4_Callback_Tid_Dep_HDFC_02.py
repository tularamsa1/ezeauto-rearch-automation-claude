...
def test_common_100_102_115():
    """
    Sub Feature Code: UI_Common_BQRV4_success_two_callback_before_qr_expiry_Auto_Refund_Disabled_TID_Dep_HDFC
    Sub Feature Description: Performing a pure upi success two callback via tid dep HDFC
     when auto refund is disabled.
    TC naming code description:100-> Payment Method, 102-> BQRV4, 113-> TC113
    """
    try:
...
def test_common_100_102_116():
    """
    Sub Feature Code: UI_Common_BQRV4_success_two_callback_before_qr_expiry_Auto_Refund_Enabled_TID_Dep_HDFC
    Sub Feature Description: Performing a pure upi success two callback via tid dep HDFC
     when auto refund is enabled.
    TC naming code description:100-> Payment Method, 102-> BQRV4, 116-> TC116
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_102_117():
    """
    Sub Feature Code: UI_Common_BQRV4_two_callback_after_qr_expiry_Auto_Refund_Disabled_TID_Dep_HDFC
    Sub Feature Description: Performing a pure upi two callback via tid dep HDFC after expiry the qr
     when auto refund is disabled.
    TC naming code description:100-> Payment Method, 102-> BQRV4, 117-> TC117
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_102_118():
    """
    Sub Feature Code: UI_Common_BQRV4_two_callback_after_qr_expiry_Auto_Refund_Enabled_TID_Dep_HDFC
    Sub Feature Description: Performing two callback via tid dep HDFC after expiry the qr when auto refund enabled
     when auto refund is enabled.
...
def test_common_100_102_125():
    """
    Sub Feature Code: UI_Common_BQRV4_UPI_amount_mismatch_Via_Pure_UPI_Success_Callback_HDFC_TID_Dep
    Sub Feature Description: Performing a amount mismatch using pure upi success callback via Tid Dep HDFC
    TC naming code description:100-> Payment Method, 102->BQRV4 UPI, 125-> TC125
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            upi_mc_id = result['id'].values[0]
            logger.debug(f"captured upi_mc_id from upi_merchant_config table : {upi_mc_id}")
            mid = result['mid'].values[0]
            logger.debug(f"captured mid from upi_merchant_config table : {mid}")
            tid = result['tid'].values[0]
            logger.debug(f"captured tid from upi_merchant_config table : {tid}")
