...
def test_common_100_103_024():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"replacing the Txn_id with {original_txn_id}, amount with {amount}.00, vpa with {vpa} and rrn with {callback_2_rrn} in the curl_data")
            api_details = DBProcessor.get_api_details('upi_success_curl',
                                                      curl_data={'ref_id': txn_ref, 'Txn_id': payment_intent_id,
                                                                 'amount': str(amount) + ".00",
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "customer_name_1": orig_txn_customer_name,
                    "payer_name_1": orig_txn_payer_name,
                    "order_id_1": order_id,
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"{org_code} is waiting for the txn to timeout:  {org_setting_value}")
            except Exception as e:
            ...
            try:
                ...
                logger.info(f"waiting for Ezetap timeout: {setting_value}")
...
def test_common_100_103_026():
    """
    Sub Feature Code: UI_Common_PM_UPI_Collect_Success_One_Callback_AutoRefundDisabled_HDFC
    Sub Feature Description: Verification of a successful upi callback via HDFC when auto refund is disabled.
    ...
    try:
        ...
        try:
            ...
            if GlobalVariables.time_calc.execution.is_started and (not GlobalVariables.time_calc.execution.is_paused):
                GlobalVariables.time_calc.execution.pause()
                print(colored("Execution Timer paused in except block (bcz not paused in try block) of testcase function".center(
                        shutil.get_terminal_size().columns, "="), 'cyan'))
            GlobalVariables.time_calc.execution.resume()
            print(colored("Execution Timer resumed in except block of testcase function".center(shutil.get_terminal_size().columns, "="), 'cyan'))
    ...
    try:
        ...
        try:
            ...
            if org_setting_value:
                ...
                logger.info(f"Value for TxnTimeoutDuration Ezetap org is: {org_setting_value} min.")
                time.sleep(3 + (setting_value * 60))
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_028():  # Make sure to add the test case name as same as the sub feature code.
    """
    Sub Feature Code: UI_Common_PM_2_UPI_Collect_failed_callback_after_expiry_HDFC_AutoRefund_Disabled
    Sub Feature Description: Performing two  UPI Collect failed callback via HDFC after expiry the  when autorefund is disabled
    100: Payment Method
    103: RemotePay
    028: TC028
    """
