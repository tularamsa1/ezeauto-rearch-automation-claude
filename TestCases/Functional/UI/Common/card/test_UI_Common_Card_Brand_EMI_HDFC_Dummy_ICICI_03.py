...
def test_common_100_115_07_135():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Block_Success_Txn_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    ...
    via HDFC Dummy PG for ICICI issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 135: TC135
    """
    try:
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'DEBIT' and bank='ICICI' and eze_emi_enabled=b'0';"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_136():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Band_Percentage_Unblock_Success_Txn_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenture
    Sub Feature Description: Performing the brand EMI payback by brand percentage unblock success transaction for
    an org via HDFC Dummy PG for ICICI issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 136: TC136
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_137():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Block_Failed_Txn_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    ...
    via HDFC Dummy PG for ICICI issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 137: TC137
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.dbVal
def test_common_100_115_07_143():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Failed_Failed_txn_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI failed_failed txn for an org (not ezetap) via HDFC Dummy PG
    for ICICI issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 143: TC143
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_115_07_088():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Brand_Percentage_With_No_Cost_EMI_Success_Txn_For_An_Org_HDFC_Dummy_PG_ICICI_Issuer_EMV_VISA_CREDIT_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand percentage with no cost emi success transaction
    for an org via HDFC Dummy PG ICICI isssuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 088: TC088
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
