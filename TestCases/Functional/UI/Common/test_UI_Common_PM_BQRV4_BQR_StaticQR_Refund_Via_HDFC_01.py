...
def test_common_100_108_012():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Full_Refund_Via_HDFC
    Sub Feature Description: Verifying BQRV4 static QR BQR full refund using api for HDFC
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 012: Testcase ID
    """
    try:
        ...
        try:
            ...
            # Calling unified refund API for BQR refund
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for the BQR refund : {response}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                date = date_time_converter.db_datetime(created_time_orig_txn)
                refund_date = date_time_converter.db_datetime(created_time_second_txn)
                expected_api_values = {
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "bqr_merchant_config_id_2": db_bqr_config_id,
                    "bqr_txn_primary_id_2": second_txn_id,
                ...
                query = "select * from bharatqr_txn where id='" + second_txn_id + "'"
                logger.debug(f"Query to fetch data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_values = {
                    'PAID BY:': 'BHARATQR', 'BASE AMOUNT:': "Rs." + str(amount) + ".00",
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_108_013():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Partial_Refund_Via_HDFC
    Sub Feature Description: Verifying BQRV4 static QR BQR partial refund via HDFC
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 013: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "issuer_code": "HDFC",
                    "txn_type": "CHARGE",
                    "mid": db_bqr_config_mid, "tid": db_bqr_config_tid,
                    "org_code": org_code,
                    "acquirer_code_2": "HDFC",
                    "txn_type_2": "REFUND",
                    "mid_2": db_bqr_config_mid, "tid_2": db_bqr_config_tid,
                    "org_code_2": org_code,
                    "auth_code": auth_code,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "bqr_txn_amt": float(amount),
                    "bqr_txn_type": "STATIC_QR",
                    "brq_terminal_info_id": db_bqr_config_terminal_info_id,
                    "bqr_bank_code": "HDFC",
                    "bqr_merchant_config_id": db_bqr_config_id,
                    "bqr_txn_primary_id": orig_txn_id,
                    "bqr_merchant_pan": db_bqr_config_merchant_pan,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                txn_date, txn_time = date_time_converter.to_chargeslip_format(created_time_second_txn)
                expected_values = {
                    'PAID BY:': 'BHARATQR', 'BASE AMOUNT:': "Rs." + str(refund_amount) + ".00",
                    'date': txn_date, 'time': txn_time,
                }
                receipt_validator.perform_charge_slip_validations(second_txn_id,
                                                                  {"username": app_username,
...
@pytest.mark.dbVal
@pytest.mark.appVal
def test_common_100_108_014():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Refund_Posted_Via_HDFC
    Sub Feature Description: Verifying BQRV4 static QR BQR refund posted via HDFC
    TC naming code description: 100: Payment method, 108: BQRV4 Static QR, 014: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching status from txn history for the txn : {second_txn_id}, {payment_status_new_2}")
                app_date_and_time_new_2 = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching date from txn history for the txn : {second_txn_id}, {app_date_and_time_new_2}")
                ...
                logger.info(f"Fetching txn amount from txn history for the txn : {second_txn_id}, {app_amount_new_2}")
...
@pytest.mark.dbVal
@pytest.mark.appVal
def test_common_100_108_015():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Refund_Failed_Via_HDFC
    Sub Feature Description: Verifying BQRV4 static QR BQR refund failed via HDFC
    TC naming code description: 100: Payment method, 108: BQRV4 static QR, 015: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result : {result}")
            orig_txn_id = result["id"].values[0]
            auth_code = result['auth_code'].values[0]
            created_time_orig_txn = result["created_time"].values[0]
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "pmt_state": "SETTLED", "rrn": str(rrn_num),
                    "settle_status": "SETTLED",
                    "acquirer_code": "HDFC",
                    "issuer_code": "HDFC",
                    "txn_type": "CHARGE",
...
def test_common_100_108_016():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Refund_With_Decimal_Via_HDFC
    Sub Feature Description: Verifying BQRV4 static QR BQR refund with decimal value using api for HDFC
    TC naming code description: 100: Payment method, 108: BQRV4 static QR, 016: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        # -----------------------------PreConditions(Setup to be done for the test case)--------------------------
        logger.info(f"Starting Precondition setup for the test case : {testcase_id}")
        # Adding the function call for playWright browser launch and initialize
        ...
        try:
            ...
            auth_code = result['auth_code'].values[0]
            created_time_orig_txn = result["created_time"].values[0]
            txn_amt = result["amount"].values[0]
            rrn_num_db = result["rr_number"].values[0]
            order_id = result["external_ref"].values[0]
            logger.debug(
                f"preparing the request payload data to trigger the unified refund for UPI")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expected_portal_values = {
                    ...
                    "pmt_type": "BHARATQR",
                    "txn_amt":  (str(txn_amt).split('.')[0]) + ".00",
                    "username": app_username,
                    "txn_id": orig_txn_id,
                    "auth_code": auth_code,
                    "rrn": rrn_num_db,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                txn_date, txn_time = date_time_converter.to_chargeslip_format(created_time_second_txn)
                expected_values = {
                    'PAID BY:': 'BHARATQR', 'BASE AMOUNT:': "Rs." + str(refund_amount) + "0",
                    'date': txn_date, 'time': txn_time,
                }
                receipt_validator.perform_charge_slip_validations(second_txn_id,
                                                                  {"username": app_username,
