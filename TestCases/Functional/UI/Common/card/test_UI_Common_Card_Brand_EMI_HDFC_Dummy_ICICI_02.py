...
@pytest.mark.appVal
def test_common_100_115_07_129():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Txn_EMI_Options_Not_Available_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    ...
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 129: TC129
    """
    try:
        ...
        logger.debug(f"updated emi settings for {org_code} as inactive for debit card")
        testsuite_teardown.update_emi_status_for_root_org(root_org_code=root_org_code, card_type='DEBIT', status='INACTIVE', issuer_code=issuer_code, emi_type='BRAND')
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_130():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Txn_ICICI_Debit_Issuer_Not_Enabled_In_Org_Settings_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    ...
    Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 130: TC130
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        api_details["RequestBody"]["settings"]["brandEmiEnabled"] = "true"
        api_details["RequestBody"]["settings"]["boCashbackEnabled"] = "true"
        api_details["RequestBody"]["settings"]["enabledEmiIssuerBanks"] = "{\"HDFC\":\"DEBIT\",\"AXIS\":\"DEBIT\",\"KOTAK\":\"DEBIT\"}"
        logger.debug(f"API details  : {api_details} ")
        response = APIProcessor.send_request(api_details=api_details)
        logger.debug(f"Response received when emi, emi for client, offering_emi cashback, brand_emi and bocashback is enabled in preconditions settings : {response}")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_131():
    """
    Sub Feature Code: UI_Common_Card_Brand_Txn_EMI_Not_Enabled_Bin_Info_Level_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    ...
    Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 131: TC131
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
