...
@pytest.mark.appVal
def test_UI_SA_CheckStatus_BQR_01(method_setup,appium_driver,ui_driver):
    ...
    # ==========================Portal validation===============================
        ...
        try:
           ...
           # query = "select status from txn where rr_number='" +rr_num+ "'"
            ...
            #order_num_db  = result["external_ref"].iloc[0]

            print("Result DB:", result)
            expectedDBValues = ""+status+":"+status_db+",BHARATQR:"+payment_mode_db+","+str(amount)+":"+str(amount_db)+""
