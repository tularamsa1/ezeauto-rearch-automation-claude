...
def test_mp_700_706_020():
    """
    Sub Feature Code: UI_MP_Report_for_Daily_txn_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using Daily quick Filter when logged in as City Head
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"validate_auth_token after hitting mp_login API is : {validate_auth_token}")
            ...
            # payment_mode in db is type in api and portal
            ...
            trans_history_page.click_on_daily()
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "', '" + taluk_username + "', '" + district_username + "') order by created_time asc limit 1;"
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_706_021():
    """
    Sub Feature Code: UI_MP_Report_for_Monthly_txn_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using Monthly quick Filter when logged in as City Head
    TC naming code description:
    700: Merchant Portal
    706: City
    021: TC021
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"last_30_days for TxnReport API : {current_date}")
            current_date = current_date.strftime('%Y-%m-%d')
            logger.debug(f"current_date in Y/M/D format : {current_date}")
            ...
            trans_history_page.click_on_reports()
            trans_history_page.click_on_transactions()
            trans_history_page.click_on_monthly()
            txn_details = trans_history_page.get_transaction_details_first_last_page()
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_706_022():
    """
    Sub Feature Code: UI_MP_Report_for_Weekly_txn_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using weekly quick Filter when logged in as City Head
    TC naming code description:
    700: Merchant Portal
    706: City
    022: TC022
    ...
    try:
        ...
        try:
            ...
            current_date = datetime.now()
            logger.debug(f"current_date for TxnReport API : {current_date}")
            last_7_days = current_date - timedelta(days=6)
            logger.debug(f"last_7_days for TxnReport API : {last_7_days}")
            current_date = current_date.strftime('%Y-%m-%d')
            logger.debug(f"current_date in Y/M/D format : {current_date}")
            last_7_days = last_7_days.strftime('%Y-%m-%d')
            logger.debug(f"last_7_days in Y/M/D format : {last_7_days}")

            api_details = DBProcessor.get_api_details('mp_txn_report', request_body={
                "startDateAndTime": last_7_days + " 00:00",
                "endDateAndTime": current_date + " 23:59",
                "maxRecordsPerPage": 100,
                "pageNumber": 0,
                "usersList": [],
            ...
            trans_history_page.click_on_reports()
            trans_history_page.click_on_transactions()
            trans_history_page.click_on_weekly()
            txn_details = trans_history_page.get_transaction_details_first_last_page()

            current_date = datetime.now()
            logger.debug(f"current_date for UTC converison: {current_date}")
            #  here we are finding the end date for last 7 days
            last_7_days = current_date - timedelta(days=6)
            logger.debug(f"last_7_days for UTC converison: {last_7_days}")
            # now we are taking the previous day of the last 7 days for UTC conversion
            last_7_days_prev_day = last_7_days - timedelta(days=1)
            logger.debug(f"last_7_days_prev_day for UTC converison: {last_7_days_prev_day}")
            last_7_days_prev_day = last_7_days_prev_day.strftime('%y%m%d')
            logger.debug(f"last_7_days_prev_day in Y/M/D format for UTC conversion: {last_7_days_prev_day}")
            current_date = current_date.strftime('%y%m%d')
            logger.debug(f"current_date in Y/M/D format for UTC conversion: {current_date}")
            start_date_time_utc = last_7_days_prev_day + "1830"
            logger.debug(f"start_date_time_utc : {start_date_time_utc}")
            end_date_time_utc = current_date + "1829"
            logger.debug(f"end_date_time_utc : {end_date_time_utc}")
