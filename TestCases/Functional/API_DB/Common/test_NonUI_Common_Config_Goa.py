...
# From below use only the markers that are applic   able for the test case and remove the rest.
@pytest.mark.apiVal
@pytest.mark.dbVal
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_300_301_001(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "OCCUPIER_NAME": "RAHIMA S. SHAIKH (BUS. ACT.)", "ORDER_NO": "24094041*05*19I/186/CL/4/1*0*0", "HOUSE_NO":"19I/186/CL/4/1"}
                logger.debug(f"expectedAPIValues: {expectedAPIValues}")
                actualAPIValues = {"success": success, "OCCUPIER_NAME": occupierName, "ORDER_NO": orderNo,"HOUSE_NO":houseNo}
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_tax_outstandingV3'));"
        ...
        Configuration.executeFinallyBlock("test_comm_300_301_001")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_300_301_002(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "MSG": "Record not found", "ERROR_CODE": "7"}
                logger.debug(f"expectedAPIValues: {expectedAPIValues}")

                actualAPIValues = {"success": success, "MSG": message, "ERROR_CODE": errorCode}
                logger.debug(f"actualAPIValues: {actualAPIValues}")
        #          ---------------------------------------------------------------------------------------------
                Validator.validationAgainstAPI(expectedAPI= expectedAPIValues, actualAPI=actualAPIValues)
            except Exception as e:
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedDBValues = {"url": "http://139.162.27.215:80/castlemock/mock/rest/project/vylhDe/application/bkBGyV/"}
                logger.debug(f"expectedDBValues: {expectedDBValues}")

                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_trade_outstanding') and active = 1);"
                logger.debug(f"Query to fetch data from external_api_adapter table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result URL: {result}")
                url_db = result["url"].iloc[0]
        ...
        Configuration.executeFinallyBlock("test_common_300_301_002")
        if GlobalVariables.EXCEL_TC_Execution == "Fail" or GlobalVariables.str_api_val_result == "Fail" or GlobalVariables.str_db_val_result == 'Fail':
            query = "select * from ca_usergroup_org_map where org_code='GOACONFIGMERCHANT' and is_active;"
            logger.debug(f"Query to fetch data from ca_usergroup_org_map table : {query}")
            result = DBProcessor.getValueFromDB(query)
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_300_301_003(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "LEASE_NO": "15/95", "ORDER_NO": "03102054*06*15/95*369*0", "LESSEE_NAME":"JERONIMO D`SOUZA"}
                logger.debug(f"expectedAPIValues: {expectedAPIValues}")

                actualAPIValues = {"success": success, "LEASE_NO": leaseNo, "ORDER_NO": orderNo, "LESSEE_NAME":lesseName}
                logger.debug(f"actualAPIValues: {actualAPIValues}")
        #          ---------------------------------------------------------------------------------------------
                Validator.validationAgainstAPI(expectedAPI= expectedAPIValues, actualAPI=actualAPIValues)
            except Exception as e:
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_rental_outstanding') and active = 1);"
                logger.debug(f"Query to fetch data from external_api_adapter table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result URL: {result}")
                url_db = result["url"].iloc[0]
        ...
        Configuration.executeFinallyBlock("test_common_300_301_003")
        if GlobalVariables.EXCEL_TC_Execution == "Fail" or GlobalVariables.str_api_val_result == "Fail" or GlobalVariables.str_db_val_result == 'Fail':
            query = "select * from ca_usergroup_org_map where org_code='GOACONFIGMERCHANT' and is_active;"
            logger.debug(f"Query to fetch data from ca_usergroup_org_map table : {query}")
            result = DBProcessor.getValueFromDB(query)
