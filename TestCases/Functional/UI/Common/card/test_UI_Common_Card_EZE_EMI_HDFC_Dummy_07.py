...
def test_common_100_115_11_034():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Subvented_by_Ezetap_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    ...
    via HDFC Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZEEMI, 034: TC034
    """
    try:
...
def test_common_100_115_11_035():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Subvented_by_Ezetap_Percentage_with_Inactive_rule_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Cashback Discount subvented by ezetap percentage success transaction with inactive rule for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZEEMI, 035: TC035
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"update rule set status = 'INACTIVE' where org_code='{org_code}' and name LIKE '{brand_name}_HDFC_CREDIT%'"
        logger.debug(f"Query to update rule with status as INACTIVE : {query}")
        result = DBProcessor.setValueToDB(query=query, db_name='rule_engine')
        logger.debug(f"Query to fetch result from rule for status as INACTIVE : {result}")
...
def test_common_100_115_11_036():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Subvented_by_Ezetap_Percentage_Success_Txn_with_different_issuer_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Cashback Discount payback by brand percentage success transaction with differ issuer card for an org (not ezetap)
    via HDFC Dummy PG with different issuer using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZEEMI, 036: TC036
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        # Below function to update bank_code, bank for the bin: 417666
        testsuite_teardown.update_bin_info(bin_number='417666', bank_code='ICICI', bank='ICICI')
        # changing the bin to axis bank as we don't have bo test cases of axis bank
...
def test_common_100_115_11_037():
    ...
    try:
        ...
        scheme_code = result['scheme_code'].values[0]
        logger.debug(f"Fetching scheme_code from the emi table : {scheme_code}")
        emi_min_amt = result['min_amount'].values[0]
        logger.debug(f"Fetching emi_min_amount from the emi table : {emi_min_amt}")
        emi_id = result['id'].values[0]
        logger.debug(f"Fetching emi_id from the emi table : {emi_id}")
        brand_id = result['brand'].values[0]
        logger.debug(f"Fetching brand from the emi table : {brand_id}")
        ...
        brand_sku_code = result['sku_code'].values[0]
        logger.debug(f"Fetching sku_code value from the brand_sku_details table : {brand_sku_code}")
        prod_min_amt = result['min_amount'].values[0]
        logger.debug(f"Fetching prod_min_amt value from the brand_sku_details table : {prod_min_amt}")
        ...
        query = f"update emi set min_amount ='{total_discount_value - 10}'  where id='{emi_id}'"
        logger.debug(f"Query to update emi table : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"Query to fetch result from emi table : {result}")

        query = f"update brand_sku_details set min_amount ='{total_discount_value - 10}'  where brand_id='{str(brand_id)}' and sku_name='{brand_sku_name}'"
...
def test_common_100_115_11_038():
    ...
    try:
        ...
        rule_id = result['id'].values[0]
        logger.debug(f"Fetching id from the rule table : {rule_id}")
        bo_end_date = result['end_date'].values[0]

        today = datetime.now() - timedelta(days=1)

        today = today.strftime('%Y-%m-%d %H:%M:%S')

        query = f"update rule set end_date='{today}' where org_code='{org_code}' and credit_type='CASHBACK' and name LIKE '{brand_name}_HDFC_CREDIT%'"
