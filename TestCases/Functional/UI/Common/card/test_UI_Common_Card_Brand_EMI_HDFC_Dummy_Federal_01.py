...
def test_common_100_115_07_094():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Federal_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by Brand fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG for Federal issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 094: TC094
    """
    try:
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='FEDERAL' and eze_emi_enabled=b'0';"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_095():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by Brand fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG for Federal issuer using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 095: TC095
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_096():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666_For_9_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by Brand fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG for Federal issuer using EMV VISA Credit card with pin for 9 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 096: TC096
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_097():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Txn_With_CardType_UNKNOWN_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenture
    Sub Feature Description: Performing the brand EMI payback by brand fixed transaction with card type UNKNOWN for
    an org via HDFC Dummy PG for federal issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 097: TC097
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"update bin_info set payment_card_type='UNKNOWN' where bin = '417666';"
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_098():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Txn_With_CardType_PREPAID_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenture
    Sub Feature Description: Performing the brand EMI payback by brand fixed transaction with card type PREPAID for an
    org via HDFC Dummy PG for federal issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 098: TC098
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"update bin_info set payment_card_type='PREPAID' where bin = '417666';"
        logger.debug(f"query to update bin_info table : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"fetching query result : {result}")
        refresh_db()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_099():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Invalid_Amount_Lesser_Than_EMI_Range_Txn_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666
    ...
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 099: TC099
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Fetching result from emi table :{result}")
        min_amount = result['min_amount'].values[0]
        logger.debug(f"fetching min_amount from emi table : {min_amount}")
        brand_id = result['brand'].values[0]
        logger.debug(f"Fetching brand_id from the emi table : {brand_id}")
