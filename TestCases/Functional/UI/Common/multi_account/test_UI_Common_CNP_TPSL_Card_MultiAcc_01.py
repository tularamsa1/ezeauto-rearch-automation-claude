...
def test_common_100_112_003():
    """
    Sub Feature Code: UI_Common_PM_CNP_NetBanking_Successful_TPSL_MultiAcc
    ...
    try:
        ...
        try:
            ...
            query = f"select id from label where org_code = '{org_code}' and name =" \
                    f"(select account_label from cnp_txn where txn_id = '{txn_id}')"
            logger.debug(f"Query to fetch Txn_id from the DB : {query}")
            result = DBProcessor.getValueFromDB(query)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_112_004():
    """
    Sub Feature Code: UI_Common_PM_CNP_NetBanking_Failed_TPSL_MultiAcc
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_112_005():
    """
    Sub Feature Code: UI_Common_PM_CNP_Credit_NetBanking_Success_TPSL_with_2nd_label_MultiAcc
