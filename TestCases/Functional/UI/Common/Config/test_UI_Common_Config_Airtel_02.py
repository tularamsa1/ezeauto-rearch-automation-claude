...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal ##  Chargeslip validation is failing ticket is raised and validation is not covered
def test_common_100_300_302_026():
    """
    Sub Feature Code: UI_Common_Config_Airtel_Verify_Success_Cash_Txn_Preapid_LOB
    Sub Feature Description: Verify success cash txn Prepaid LOB
    TC naming code description: 100: Payment Method, 300: Config, 302: Airtel, 026: TC026
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.click_on_start_btn()
            logger.info(f"Clicked on start button")
            home_page.click_on_prepaid_mobile_LOB()
            logger.info(f"Clicked on pos sale LOB")
            home_page.click_on_prepaid_recharge()
            logger.info(f"Clicked on prepaid recharge")
            amount = random.randint(100, 1000)
            logger.info(f"Randomly generated amount is ", amount)
            mobile_no = '9642595990'  #for prepaid txn, should use airtel number only
            home_page.enter_prepaid_recharge_details(ph_number=mobile_no,amount=amount)
            logger.info("Entered the prepaid recharge details")
