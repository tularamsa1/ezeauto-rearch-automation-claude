...
def test_common_rearch_0057():
    """
    Sub Feature Code: UI_ReArch_PM_P2P_Card_Success_HDFC
    Sub Feature Description:
        ...
        payment mode, complete successful payment on device (HDFC simulator
        auto-authorizes), navigate to Payment History and validate via App,
        API, and Charge Slip.
    ...
    NL Source Steps:
      ...
      2. Apply precondition settings
      3. Scroll down and check P2P is connected (notification bar)
      4. Hit p2p_start API with CARD payment mode
      5. On device: UPI screen pops up, complete successful payment
      6. After payment, device redirects to collect payment screen
      7. Navigate to Payment History, do app validation
      8. Do API and charge slip validation
    """
    try:
        ...
        try:
            ...
            # Step 5: Wait for UPI QR screen on device after P2P push
            # After p2p_start success, device receives push notification and
            # displays the QR "Scan & Pay" screen.
            time.sleep(3)

            # Wait for QR screen to appear
            qr_page = ReArchQRPage(app_driver)
            qr_page.wait_for_qr_screen(timeout=60)
            logger.info("QR Scan & Pay screen displayed after P2P push")

            # HDFC simulator auto-authorizes the UPI payment when QR is shown.
            # Wait for simulator to process, then cancel QR navigation
            # (payment is already AUTHORIZED in backend by this point).
            time.sleep(6)
            qr_page.cancel_payment()
            logger.debug("QR screen cancelled")

            # After QR cancel, failure screen appears on UI
            # (but the backend transaction is actually AUTHORIZED)
            complete_page = ReArchCompletePage(app_driver)
            complete_page.wait_for_failure_screen()
            logger.info("Payment Failed screen confirmed (expected after QR cancel — txn is AUTHORIZED in backend)")
            ...
            logger.debug(
                ...
                f"posting_date={posting_date}, rrn={rrn}"
            )

            # Step 6: After payment, device redirects to collect payment screen
        ...
        if ConfigReader.read_config("Validations", "app_validation") == "True":
            ...
            try:
                ...
                # Step 7: Navigate to Payment History from collect payment screen
                home_page.wait_for_initial_home_screen()
