...
def test_common_rearch_0009():
    """
    Sub Feature Code: UI_ReArch_PM_Card_Debit_Success_HDFC_DUMMY
    Sub Feature Description:
        Launch ReArch app, enter a random amount between 300 and 500, select Card,
        proceed through the Order Details overlay, select Visa Debit with PIN
        (Contactless) on the DUMMY card type selection screen, verify Payment
        Successful screen, then navigate to transaction history from the collect
        payment screen, click the transaction and verify amount and Payment
        Authorized status.
    ...
    NL Source Steps:
      ...
      Test Steps:
        ...
        3.  enter any amount between 300 to 500
        4.  select card
        5.  click on proceed on the next screen (order details overlay)
        6.  select Visa Debit with PIN (Contactless)
      ...
      API Validation:
        ...
        card_txn_type=CTLS  # TODO: verify on first run
    ...
    try:
        ...
        try:
            ...
            # Step 3: Wait for amount screen and enter amount (300–500)
            ...
            # Step 6: Card type selection → Visa Debit with PIN (Contactless)
            card_type_page = ReArchCardTypePage(app_driver)
            card_type_page.wait_for_card_type_screen()
            card_type_page.click_visa_debit_with_pin_contactless()
            logger.debug("Selected Visa Debit with PIN (Contactless)")
