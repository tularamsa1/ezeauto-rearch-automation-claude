...
def test_mp_700_706_023():
    """
    Sub Feature Code: UI_MP_Report_based_on_username_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using Filter username when logged in as City Head
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and username='" + txn_username + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "', '" + taluk_username + "', '" + district_username + "') order by created_time asc limit 1;"
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_706_024():
    """
    Sub Feature Code: UI_MP_Report_based_on_order_number_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using Filter order number when logged in as City Head
    TC naming code description:
    700: Merchant Portal
    706: City
    024: TC024
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and external_ref='" + order_number + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "', '" + taluk_username + "', '" + district_username + "') order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_706_025():
    """
    Sub Feature Code: UI_MP_Report_based_on_calender_dates_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using Filter calender dates when logged in as City Head
    TC naming code description:
    700: Merchant Portal
    706: City
    025: TC025
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where " \
                    "(id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "') and org_code='" + org_code + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "', '" + taluk_username + "', '" + district_username + "') order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_706_026():
    """
    Sub Feature Code: UI_MP_Report_based_on_Location_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using Filter All Location when logged in as City Head
    TC naming code description:
    700: Merchant Portal
    703: State
    026: TC026
    ...
    try:
        ...
        txn_cred = ResourceAssigner.get_org_users_credentials(testcase_id, 'STORE21')
        logger.debug(f"Fetched txn credentials from the ezeauto db : {txn_cred}")
        txn_username = txn_cred['Username']
        txn_password = txn_cred['Password']
        txn_org_code = txn_cred['Merchant_Code']
        ...
        try:
            ...
            query = "select node_name from org_structure where " \
                    "org_code = '" + org_code + "' and org_employee_username = '" + txn_username + "';"
            logger.debug(f"Query to fetch node_name from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            location = result['node_name'].values[0]
            logger.debug(f"location from db : {location}")

            # make sure that txn_username and the node_name's org_employee_username are the same so that on searching we get some txn data
            query = "select parent_node_id from org_structure where org_code = '" + org_code + "' and org_employee_username ='"+ txn_username +"'  and node_name = '" + location + "';"
            ...
            trans_history_page.click_on_reports()
            trans_history_page.click_on_transactions()
            trans_history_page.search_based_on_location(location)
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_706_027():
    """
    Sub Feature Code: UI_MP_Report_based_on_User_as_City_head
    Sub Feature Description: Verifying txn details on Report Page using Filter User when logged in as City Head
    TC naming code description:
    700: Merchant Portal
    706: City
    027: TC027
