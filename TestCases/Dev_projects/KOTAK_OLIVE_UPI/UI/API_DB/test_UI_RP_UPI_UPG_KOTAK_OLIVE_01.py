...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_016():
    """
    Sub Feature Code: UI_RP_UPI_Callback_Amount_Mismatch_KOTAK_OLIVE
    Sub Feature Description: Verification a Remote Pay UPI callback for upg txn using amount mismatch via KOTAK_OLIVE
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RemotePay, 016->TC016
    """
    try:
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_017():
    """
    Sub Feature Code: UI_RP_UPI_UPG_AUTHORIZED_via_KOTAK_OLIVE
    Sub Feature Description: Verification of a upg txn using upi success callback for KOTAK_OLIVE
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RemotePay, 017->TC017
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = f"select * from invalid_pg_request where pg_txn_ref ='{transactionid}';"
            logger.debug(f"query : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result : {result}")
            ipr_payment_mode = result["payment_mode"].iloc[0]
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "ipr_config_id": upi_mc_id,
                    "ipr_pg_merchant_id": pg_merchant_id,
                    "ipr_err_msg": f"UPI Txn Id Tampered {transactionid}",
                }
                logger.debug(f"expected_db_values: {expected_db_values}")
