...
def test_common_100_115_142():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_Dummy_EMV_MASTER_CreditCard_Without_Pin_541333
    Sub Feature Description:  Performing the EMV Void transaction via HDFC Dummy PG using MASTER Credit card having
     16 digit PAN length without pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 142: TC142
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_143():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_Dummy_EMV_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description:  Performing the CTLS Void transaction via HDFC Dummy PG using MASTER Credit card having
     16 digit PAN length with pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 143: TC143
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_144():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_Dummy_EMV_RUPAY_DebitCard_Without_Pin_608326
    Sub Feature Description:  Performing the EVM Void transaction via HDFC Dummy PG using Rupay Debit card having
     16 digit PAN length without pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 144: TC144
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "pmt_card_bin": "608326",
                    "name_on_card": "MRUPAY CARD IMAGE 02      /",
                    "display_pan": "0025"
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_145():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_Dummy_EMV_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description:  Performing the EMV Void transaction via HDFC Dummy PG using Rupay Debit card having
     16 digit PAN length with pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 145: TC145
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
