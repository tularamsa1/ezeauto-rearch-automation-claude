...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_044():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_Dummy_EMVCTLS_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description: Performing the EMVCTLS sale transaction via HDFC Dummy PG using MASTER Credit card having
    16 digit PAN length with pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 044: TC044
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_115_135():
    """
    Sub Feature Code: UI_Common_Card_Sale_Failed_Txn_HDFC_Dummy_EMVCTLS_VISA_DebitCard_With_Pin_476173
    Sub Feature Description: Performing the EMVCTLS sale failed transaction via HDFC Dummy PG using VISA Debit card
    having 16 digit PAN length with pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 135: TC135
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_115_136():
    """
    Sub Feature Code: UI_Common_Card_Sale_Failed_Txn_HDFC_Dummy_EMVCTLS_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description:  Performing the EMVCTLS sale failed transaction via HDFC Dummy PG using MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 136: TC136
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
