...
def test_sa_100_110_009():
    """
    Sub Feature Code: UI_SA_PM_BQRV4_UPI_Success_Via_SA_CheckStatus_MultiAcc_HDFC
    Sub Feature Description: Multi Account - Verification of a successful BQRV4 upi txn via HDFC using SA check status
    TC naming code description: 100: Payment Method, 110: MultiAcc_BQRV4, 008: TC009
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_sa_100_110_013():
    """
    Sub Feature Code: UI_SA_PM_BQRV4_UPI_Success_Via_SA_CheckStatus_with_2nd_label_MultiAcc_HDFC
    Sub Feature Description: Multi Account - Performing a successful BQRV4_UPI txn via HDFC using SA check status via
    second account (ex:acc2 label)
    TC naming code description: 100: Payment Method, 110: MultiAcc_BQRV4, 013: TC013
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
