...
def test_mp_700_705_062():
    """
    Sub Feature Code: UI_MP_Performance_Matrix_top10_performer_for_below_20_nodes_as_Taluk_head
    Sub Feature Description: Verifying Performance Matrix for TOP 10 performers when Taluk head has logged in for all lower level hierarchies having  less than 20 nodes when logging in as Taluk Head
    ...
    try:
        ...
        try:
            ...
            # del names_and_volumes["'India'"]
            # top_performer_db = sum(names_and_volumes.values())
            # top_performer_db_1 = "name :" + 'West' + " , volume :" + str(top_performer_db)
            # logger.debug(f"top performer name and volume from db : {top_performer_db_1}")

            names_and_volumes = {}
            for name, values in zip(node_names, total_amount_all_performer):
                names_and_volumes[str(name).strip('[]')] = float(values)
            categories = ["'India'", "'West'", "'Maharastra'", "'Mumbai_City_District'", "'Akole'"]
            for category in categories:
                del names_and_volumes[category]
            top_performer_db = sum(names_and_volumes.values())
            top_performer_db_1 = "name :" + 'Mumbai1' + " , volume :" + str(top_performer_db)
            logger.debug(f"top performer name and volume from db : {top_performer_db_1}")
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_705_063():
    """
    Sub Feature Code: UI_MP_Performance_Matrix_top10_and_bottom10_performer_for_more_than_20_nodes_as_Taluk_head
    Sub Feature Description: Verifying Performance Matrix for TOP 10  and BOTTOM 10 performers when Taluk head has logged in for all lower level hierarchies having only more than 20 nodes when logging in as Taluk Head
    TC naming code description:
    700: Merchant Portal
    705: Taluk
    063: TC063
