...
def test_common_100_115_03_009():
    """
        Sub Feature Code: UI_Common_Card_Preauth_Txn_Confirm_Success_Txn_HDFC_Dummy_MASTER_CreditCard_541333
        Sub Feature Description: Performing the preauth txn confirm success transaction via HDFC Dummy PG using MASTER Credit card (bin : 541333)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 03:PreAuth , 009: TC009
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    'BATCH NO': batch_number, 'CARD TYPE': 'MasterCard',
                    'payment_option': 'SALE', 'TID': tid,
                    'time': txn_time, 'AUTH CODE': str(auth_code)
                }
                chargeslip_val_result = receipt_validator.perform_charge_slip_validations(original_txn_id,{"username": app_username,
                                                                                                           "password": app_password},
                                                                                            expected_charge_slip_values)
                ...
                chargeslip_val_result_2 = receipt_validator.perform_charge_slip_validations(txn_id, {"username": app_username,
                                                                                             "password": app_password},
                                                                                            expected_charge_slip_values_2)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_03_010():
    """
        Sub Feature Code: UI_Common_Card_Preauth_Txn_Confirm_Void_Txn_HDFC_Dummy_MASTER_CreditCard_541333
        Sub Feature Description: Performing the preauth txn confirm void transaction via HDFC Dummy PG using MASTER Credit card (bin : 541333)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 03:PreAuth , 010: TC010
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.click_on_pre_auth()
            logger.debug(f"Clicked on pre_auth")
            amount = random.randint(200, 500) # Amount range is not yet confirmed
            home_page.enter_amt_order_no_and_device_serial_for_pre_auth(amount, order_id, device_serial)
            logger.debug(f"Enter the amount, order_id and device serial for pre_auth txn : {amount}, {order_id}, {device_serial}")
            card_page = CardPage(app_driver)
            logger.debug(f"Selecting the card type as : CTLS_MASTER_CREDIT_541333")
            ...
            logger.debug(f"Fetching original txn_id from the txn table : {original_txn_id}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching auth_code value from the txn table : {auth_code}")
            rrn = result['rr_number'].values[0]
            logger.debug(f"Fetching rr_number value from the txn table : {rrn}")
            txn_created_time = result['created_time'].values[0]
            ...
            logger.debug(f"Query to fetch data from the txn table after confirm pre_auth : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Fetching result for txn table after confirm pre_auth : {result}")
            txn_id = result['id'].values[0]
            logger.debug(f"Fetching result for txn_id table after confirm pre_auth : {result}")

            time.sleep(10)
            logger.debug(f"Waiting for 10 secs after selecting card type as CTLS_MASTER_CREDIT_541333")
            app_driver.back()
            txn_history_page.click_on_transaction_by_txn_id(txn_id)
            ...
            query = f"select * from txn where id='{txn_id}';"
            logger.debug(f"Query to fetch data from the txn table for void txn : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Fetching result for txn table for void txn : {result}")
            txn_created_time_2 = result['created_time'].values[0]
            logger.debug(f"Fetching created_time value from for void txn : {txn_created_time_2}")
            rrn_2 = result['rr_number'].values[0]
            logger.debug(f"Fetching rrn value from the txn table for void txn : {rrn_2}")
            auth_code_2 = result['auth_code'].values[0]
            logger.debug(f"Fetching auth_code value from the txn table for void txn : {auth_code_2}")
            batch_number = result['batch_number'].values[0]
            logger.debug(f"Fetching batch_number value from the txn table for void txn : {batch_number}")
            amount_db = result['amount'].values[0]
            logger.debug(f"Fetching amount value from the txn table for void txn : {amount_db}")
            amt_cash_back_db = result['amount_cash_back'].values[0]
            logger.debug(f"Fetching amount_cash_back value from the txn table for void txn : {amt_cash_back_db}")
            payment_mode_db = result['payment_mode'].values[0]
            logger.debug(f"Fetching payment_mode value from the txn table for void txn : {payment_mode_db}")
            payment_status_db = result['status'].values[0]
            logger.debug(f"Fetching status value from the txn table for void txn : {payment_status_db}")
            payment_state_db = result['state'].values[0]
            logger.debug(f"Fetching state value from the txn table for void txn : {payment_state_db}")
            acquirer_code_db = result['acquirer_code'].values[0]
            logger.debug(f"Fetching acquirer_code value from the txn table for void txn : {acquirer_code_db}")
            mid_db = result['mid'].values[0]
            logger.debug(f"Fetching mid value from the txn table for void txn : {mid_db}")
            tid_db = result['tid'].values[0]
            logger.debug(f"Fetching tid value from the txn table for void txn : {tid_db}")
            payment_gateway_db = result['payment_gateway'].values[0]
            logger.debug(f"Fetching payment_gateway value from the txn table for void txn : {payment_gateway_db}")
            settlement_status_db = result['settlement_status'].values[0]
            logger.debug(f"Fetching settlement_status value from the txn table for void txn : {settlement_status_db}")
            device_serial_db = result['device_serial'].values[0]
            logger.debug(f"Fetching device_serial value from the txn table for void txn : {device_serial_db}")
            merchant_name_db = result['merchant_name'].values[0]
            logger.debug(f"Fetching merchant_name value from the txn table for void txn : {merchant_name_db}")
            payment_card_brand_db = result["payment_card_brand"].iloc[0]
            logger.debug(f"Fetching payment_card_brand value from the txn table for void txn : {payment_card_brand_db}")
            payment_card_type_db = result["payment_card_type"].iloc[0]
            logger.debug(f"Fetching payment_card_type value from the txn table for void txn : {payment_card_type_db}")
            payment_card_bin_db = result["payment_card_bin"].iloc[0]
            logger.debug(f"Fetching payment_card_bin value from the txn table for void txn : {payment_card_bin_db}")
            terminal_info_id_db = result["terminal_info_id"].iloc[0]
            logger.debug(f"Fetching terminal_info_id value from the txn table for void txn : {terminal_info_id_db}")
            txn_type_db = result["txn_type"].iloc[0]
            logger.debug(f"Fetching txn_type value from the txn table for void txn : {txn_type_db}")
            card_txn_type_db = result["card_txn_type"].iloc[0]
            logger.debug(f"Fetching card_txn_type value from the txn table for void txn : {card_txn_type_db}")
            card_last_four_digit_db = result["card_last_four_digit"].iloc[0]
            logger.debug(f"Fetching card_last_four_digit value from the txn table for void txn : {card_last_four_digit_db}")
            org_code_db = result["org_code"].iloc[0]
            logger.debug(f"Fetching org_code value from the txn table for void txn : {org_code_db}")
            order_id_db = result["external_ref"].iloc[0]
            logger.debug(f"Fetching external_ref value from the txn table for void txn : {order_id_db}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            logger.info(f"Started APP validation for the test case : {testcase_id}")
            try:
                date_and_time = date_time_converter.to_app_format(txn_created_time_2)
                expected_app_values = {
                    "txn_amt": "{:,.2f}".format(pre_auth_amt),
                    "pmt_mode": "CARD",
                    "txn_id": txn_id,
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            logger.info(f"Started API validation for the test case : {testcase_id}")
            try:
                expected_date_and_time_api = date_time_converter.db_datetime(txn_created_time_2)
                expected_api_values = {
                    "pmt_status": "VOIDED",
                    "txn_amt": float(pre_auth_amt),
                    "pmt_mode": "CARD",
                    "pmt_state": "VOIDED",
                    "rrn": str(rrn_2),
                    "settle_status": "SETTLED",
                ...
                for elements in response_in_list:
                    if elements["txnId"] == txn_id:
                        status_api = elements["status"]
                        logger.debug(f"Value of status obtained from txnlist api for confirm pre_auth txn : {status_api}")
                        amount_api = float(elements["amount"])
                        logger.debug(f"Value of amount obtained from txnlist api for confirm pre_auth txn : {amount_api}")
                        payment_mode_api = elements["paymentMode"]
                        logger.debug(f"Value of paymentMode obtained from txnlist api for confirm pre_auth txn : {payment_mode_api}")
                        state_api = elements["states"][0]
                        logger.debug(f"Value of states obtained from txnlist api for confirm pre_auth txn : {state_api}")
                        rrn_api = elements["rrNumber"]
                        logger.debug(f"Value of rrNumber obtained from txnlist api for confirm pre_auth txn : {rrn_api}")
                        settlement_status_api = elements["settlementStatus"]
                        logger.debug(f"Value of settlementStatus obtained from txnlist api for confirm pre_auth txn : {settlement_status_api}")
                        acquirer_code_api = elements["acquirerCode"]
                        logger.debug(f"Value of acquirerCode obtained from txnlist api for confirm pre_auth txn : {acquirer_code_api}")
                        org_code_api = elements["orgCode"]
                        logger.debug(f"Value of orgCode obtained from txnlist api for confirm pre_auth txn : {org_code_api}")
                        mid_api = elements["mid"]
                        logger.debug(f"Value of mid obtained from txnlist api for confirm pre_auth txn : {mid_api}")
                        tid_api = elements["tid"]
                        logger.debug(f"Value of tid obtained from txnlist api for confirm pre_auth txn : {tid_api}")
                        txn_type_api = elements["txnType"]
                        logger.debug(f"Value of txnType obtained from txnlist api for confirm pre_auth txn : {txn_type_api}")
                        auth_code_api = elements["authCode"]
                        logger.debug(f"Value of authCode obtained from txnlist api for confirm pre_auth txn : {auth_code_api}")
                        date_and_time_api = elements["createdTime"]
                        logger.debug(f"Value of createdTime obtained from txnlist api for confirm pre_auth txn : {date_and_time_api}")
                        device_serial_api = elements["deviceSerial"]
                        logger.debug(f"Value of deviceSerial obtained from txnlist api for confirm pre_auth txn : {device_serial_api}")
                        username_api = elements["username"]
                        logger.debug(f"Value of username obtained from txnlist api for confirm pre_auth txn : {username_api}")
                        txn_id_api = elements["txnId"]
                        logger.debug(f"Value of txnId obtained from txnlist api for confirm pre_auth txn : {txn_id_api}")
                        payment_card_brand_api = elements["paymentCardBrand"]
                        logger.debug(f"Value of paymentCardBrand obtained from txnlist api for confirm pre_auth txn : {payment_card_brand_api}")
                        payment_card_type_api = elements["paymentCardType"]
                        logger.debug(f"Value of paymentCardType obtained from txnlist api for confirm pre_auth txn : {payment_card_type_api}")
                        card_txn_type_api = elements["cardTxnTypeDesc"]
                        logger.debug(f"Value of cardTxnTypeDesc obtained from txnlist api for confirm pre_auth txn : {card_txn_type_api}")
                        batch_number_api = elements["batchNumber"]
                        logger.debug(f"Value of batchNumber obtained from txnlist api for confirm pre_auth txn : {batch_number_api}")
                        card_last_four_digit_api = elements["cardLastFourDigit"]
                        logger.debug(f"Value of cardLastFourDigit obtained from txnlist api for confirm pre_auth txn : {card_last_four_digit_api}")
                        external_ref_number_api = elements["externalRefNumber"]
                        logger.debug(f"Value of externalRefNumber obtained from txnlist api for confirm pre_auth txn : {external_ref_number_api}")
                        merchant_name_api = elements["merchantName"]
                        logger.debug(f"Value of merchantName obtained from txnlist api for confirm pre_auth txn : {merchant_name_api}")
                        payment_card_bin_api = elements["paymentCardBin"]
                        logger.debug(f"Value of paymentCardBin obtained from txnlist api for confirm pre_auth txn : {payment_card_bin_api}")
                        display_pan_api = elements["displayPAN"]
                        logger.debug(f"Value of displayPAN obtained from txnlist api for confirm pre_auth txn : {display_pan_api}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                date_and_time_portal_2 = date_time_converter.to_portal_format(txn_created_time)
                expected_portal_values = {
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                txn_date, txn_time = date_time_converter.to_chargeslip_format(txn_created_time)
                txn_date_2, txn_time_2 = date_time_converter.to_chargeslip_format(txn_created_time_2)
                print("txn_date_2, txn_time_2", txn_date_2, txn_time_2)
                print("txn_date, txn_time",   txn_date, txn_time)
                expected_charge_slip_values = {
                    'merchant_ref_no': 'Ref # ' + str(order_id), 'RRN': str(rrn),
                    'BASE AMOUNT:': "Rs." + str(amount) + ".00", 'date': txn_date,
                    'BATCH NO': batch_number, 'CARD TYPE': 'MasterCard',
                ...
                expected_charge_slip_values_2 = {
                    ...
                    'BASE AMOUNT:': "Rs." + str(pre_auth_amt) + ".00", 'date': txn_date_2,
                    'BATCH NO': batch_number, 'CARD TYPE': 'MasterCard',
                    'payment_option': 'VOID SALE', 'TID': tid,
                    'time': txn_time_2, 'AUTH CODE': str(auth_code_2)
                }
...
def test_common_100_115_03_014():
    ...
    try:
        ...
        logger.debug(f"API details  : {api_details} ")
        response = APIProcessor.send_request(api_details)
        logger.debug(f"Response obtained when preauth and refund is enabled in precondition settings : {response}")
        ...
        try:
            ...
            logger.debug(f"Fetching auth_code value from the txn table : {original_auth_code}")
            original_rrn = result['rr_number'].values[0]
            logger.debug(f"Fetching rr_number value from the txn table : {original_rrn}")
            original_txn_created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created_time value from the txn table : {original_txn_created_time}")
            ...
            query = f"select * from txn where orig_txn_id='{original_txn_id}';"
            logger.debug(f"Query to fetch data from the txn table after confirm pre_auth based on original_txn_id : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Fetching result for txn table after confirm pre_auth based on original_txn_id : {result}")
            txn_id = result['id'].values[0]
            logger.debug(f"Fetching result for txn_id table after confirm pre_auth based on original_txn_id : {result}")
            ...
            logger.debug(f"Fetching refund txn_id value after performing settlement status : {refund_txn_id}")
            ...
            txn_created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created_time value from the txn table after confirm pre_auth txn : {txn_created_time}")
            rrn = result['rr_number'].values[0]
            logger.debug(f"Fetching rrn value from the txn table after confirm pre_auth txn : {rrn}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching auth_code value from the txn table after confirm pre_auth txn : {auth_code}")
            batch_number = result['batch_number'].values[0]
            logger.debug(f"Fetching batch_number value from the txn table after confirm pre_auth txn : {batch_number}")
            amount_db = result['amount'].values[0]
            logger.debug(f"Fetching amount value from the txn table after confirm pre_auth txn : {amount_db}")
            payment_mode_db = result['payment_mode'].values[0]
            logger.debug(f"Fetching payment_mode value from the txn table after confirm pre_auth txn : {payment_mode_db}")
            payment_status_db = result['status'].values[0]
            logger.debug(f"Fetching status value from the txn table after confirm pre_auth txn : {payment_status_db}")
            payment_state_db = result['state'].values[0]
            logger.debug(f"Fetching state value from the txn table after confirm pre_auth txn : {payment_state_db}")
            acquirer_code_db = result['acquirer_code'].values[0]
            logger.debug(f"Fetching acquirer_code value from the txn table after confirm pre_auth txn : {acquirer_code_db}")
            mid_db = result['mid'].values[0]
            logger.debug(f"Fetching mid value from the txn table after confirm pre_auth txn : {mid_db}")
            tid_db = result['tid'].values[0]
            logger.debug(f"Fetching tid value from the txn table after confirm pre_auth txn : {tid_db}")
            payment_gateway_db = result['payment_gateway'].values[0]
            logger.debug(f"Fetching payment_gateway value from the txn table after confirm pre_auth txn : {payment_gateway_db}")
            settlement_status_db = result['settlement_status'].values[0]
            logger.debug(f"Fetching settlement_status value from the txn table after confirm pre_auth txn : {settlement_status_db}")
            device_serial_db = result['device_serial'].values[0]
            logger.debug(f"Fetching device_serial value from the txn table after confirm pre_auth txn : {device_serial_db}")
            merchant_name = result['merchant_name'].values[0]
            logger.debug(f"Fetching merchant_name value from the txn table after confirm pre_auth txn : {merchant_name}")
            merchant_code_db = result["merchant_code"].iloc[0]
            logger.debug(f"Fetching merchant_code value from the txn table after confirm pre_auth txn : {merchant_code_db}")
            payment_card_brand_db = result["payment_card_brand"].iloc[0]
            logger.debug(f"Fetching payment_card_brand value from the txn table after confirm pre_auth txn : {payment_card_brand_db}")
            payment_card_type_db = result["payment_card_type"].iloc[0]
            logger.debug(f"Fetching payment_card_type value from the txn table after confirm pre_auth txn : {payment_card_type_db}")
            payment_card_bin_db = result["payment_card_bin"].iloc[0]
            logger.debug(f"Fetching payment_card_bin value from the txn table after confirm pre_auth txn : {payment_card_bin_db}")
            terminal_info_id_db = result["terminal_info_id"].iloc[0]
            logger.debug(f"Fetching terminal_info_id value from the txn table after confirm pre_auth txn : {terminal_info_id_db}")
            txn_type_db = result["txn_type"].iloc[0]
            logger.debug(f"Fetching txn_type value from the txn table after confirm pre_auth txn : {txn_type_db}")
            card_txn_type_db = result["card_txn_type"].iloc[0]
            logger.debug(f"Fetching card_txn_type value from the txn table after confirm pre_auth txn : {card_txn_type_db}")
            card_last_four_digit_db = result["card_last_four_digit"].iloc[0]
            logger.debug(f"Fetching card_last_four_digit value from the txn table after confirm pre_auth txn : {card_last_four_digit_db}")
            org_code_db = result["org_code"].iloc[0]
            logger.debug(f"Fetching org_code value from the txn table after confirm pre_auth txn : {org_code_db}")
            order_id_db = result["external_ref"].iloc[0]
            logger.debug(f"Fetching external_ref value from the txn table after confirm pre_auth txn : {order_id_db}")
            ...
            logger.debug(f"Fetching amount value from the txn table for refund txn : {amount_db_2}")
            payment_mode_db_2 = result["payment_mode"].iloc[0]
            logger.debug(f"Fetching payment_mode value from the txn table for refund txn : {payment_mode_db_2}")
            payment_status_db_2 = result["status"].iloc[0]
            logger.debug(f"Fetching status value from the txn table for refund txn : {payment_status_db_2}")
            payment_state_db_2 = result["state"].iloc[0]
            logger.debug(f"Fetching state value from the txn table for refund txn : {payment_state_db_2}")
            acquirer_code_db_2 = result["acquirer_code"].iloc[0]
            logger.debug(f"Fetching acquirer_code value from the txn table for refund txn : {acquirer_code_db_2}")
            mid_db_2 = result["mid"].iloc[0]
            logger.debug(f"Fetching mid value from the txn table for refund txn : {mid_db_2}")
            tid_db_2 = result["tid"].iloc[0]
            logger.debug(f"Fetching tid value from the txn table for refund txn : {tid_db_2}")
            payment_gateway_db_2 = result["payment_gateway"].iloc[0]
            logger.debug(f"Fetching payment_gateway value from the txn table for refund txn : {payment_gateway_db_2}")
            refund_rrn = result["rr_number"].iloc[0]
            logger.debug(f"Fetching rr_number value from the txn table for refund txn : {refund_rrn}")
            merchant_code_db_2 = result["merchant_code"].iloc[0]
            logger.debug(f"Fetching merchant_code value from the txn table for refund txn : {merchant_code_db_2}")
            payment_card_brand_db_2 = result["payment_card_brand"].iloc[0]
            logger.debug(f"Fetching payment_card_brand value from the txn table for refund txn : {payment_card_brand_db_2}")
            settlement_status_db_2 = result["settlement_status"].iloc[0]
            logger.debug(f"Fetching settlement_status value from the txn table for refund txn : {settlement_status_db_2}")
            payment_card_type_db_2 = result["payment_card_type"].iloc[0]
            logger.debug(f"Fetching payment_card_type value from the txn table for refund txn : {payment_card_type_db_2}")
            batch_number_db_2 = result["batch_number"].iloc[0]
            logger.debug(f"Fetching batch_number value from the txn table for refund txn : {batch_number_db_2}")
            order_id_db_2 = result["external_ref"].iloc[0]
            logger.debug(f"Fetching external_ref value from the txn table for refund txn : {order_id_db_2}")
            org_code_db_2 = result["org_code"].iloc[0]
            logger.debug(f"Fetching org_code value from the txn table for refund txn : {org_code_db_2}")
            payment_card_bin_db_2 = result["payment_card_bin"].iloc[0]
            logger.debug(f"Fetching payment_card_bin value from the txn table for refund txn : {payment_card_bin_db_2}")
            terminal_info_id_db_2 = result["terminal_info_id"].iloc[0]
            logger.debug(f"Fetching terminal_info_id value from the txn table for refund txn : {terminal_info_id_db_2}")
            txn_type_db_2 = result["txn_type"].iloc[0]
            logger.debug(f"Fetching txn_type value from the txn table for refund txn : {txn_type_db_2}")
            card_txn_type_db_2 = result["card_txn_type"].iloc[0]
            logger.debug(f"Fetching card_txn_type value from the txn table for refund txn : {card_txn_type_db_2}")
            card_last_four_digit_db_2 = result["card_last_four_digit"].iloc[0]
            logger.debug(f"Fetching card_last_four_digit value from the txn table for refund txn : {card_last_four_digit_db_2}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                payment_status = txn_history_page.fetch_txn_status_text()
                logger.info(f"Fetching payment status value from txn history for the confirm preauth txn : {txn_id}, {payment_status}")
                payment_mode = txn_history_page.fetch_txn_type_text()
                logger.info(f"Fetching payment mode value from txn history for the confirm preauth txn : {txn_id}, {payment_mode}")
                app_txn_id = txn_history_page.fetch_txn_id_text()
                logger.info(f"Fetching txn_id value from txn history for the confirm preauth txn : {txn_id}, {app_txn_id}")
                app_amount = txn_history_page.fetch_txn_amount_text()
                logger.info(f"Fetching amount value from txn history for the confirm preauth txn : {txn_id}, {app_amount}")
                app_settlement_status = txn_history_page.fetch_settlement_status_text()
                logger.info(f"Fetching settlement status value from txn history for the confirm preauth txn : {txn_id}, {app_settlement_status}")
                app_payment_msg = txn_history_page.fetch_txn_payment_message_text()
                logger.info(f"Fetching payment msg value from txn history for the confirm preauth txn : {txn_id}, {app_payment_msg}")
                app_date_and_time = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching date and time value from txn history for the confirm preauth txn : {txn_id}, {app_date_and_time}")
                app_rrn = txn_history_page.fetch_RRN_text()
                logger.info(f"Fetching rrn value from txn history for the confirm preauth txn : {txn_id}, {app_rrn}")
                app_auth_code = txn_history_page.fetch_auth_code_text()
                logger.info(f"Fetching auth_cde value from txn history for the confirm preauth txn : {txn_id}, {app_auth_code}")
                app_batch_no = txn_history_page.fetch_batch_number_text()
                logger.info(f"Fetching batch number value from txn history for the confirm preauth txn : {txn_id}, {app_batch_no}")
                app_device_serial = txn_history_page.fetch_device_serial_text()
                logger.info(f"Fetching device serial from txn history for the confirm preauth txn : {txn_id}, {app_device_serial}")
                app_order_id = txn_history_page.fetch_order_id_text()
                logger.info(f"Fetching txn order id value from txn history for the confirm preauth txn : {txn_id}, {app_order_id}")
                app_mid = txn_history_page.fetch_mid_text()
                logger.info(f"Fetching mid value from txn history for the confirm preauth txn : {txn_id}, {app_mid}")
                app_tid = txn_history_page.fetch_tid_text()
                logger.info(f"Fetching tid value from txn history for the confirm preauth txn : {txn_id}, {app_tid}")
                app_card_type_desc = txn_history_page.fetch_card_type_desc_text()
                logger.info(f"Fetching card type value from txn history for the confirm preauth txn : {txn_id}, {app_card_type_desc}")

                txn_history_page.click_back_Btn_transaction_details()
                txn_history_page.click_on_transaction_by_txn_id(txn_id=refund_txn_id)
                app_amount_2 = txn_history_page.fetch_txn_amount_text()
                logger.debug(f"Fetching txn amount from txn history for the refund txn : {refund_txn_id}, {app_amount_2}")
                payment_mode_2 = txn_history_page.fetch_txn_type_text()
                logger.debug(f"Fetching payment mode from txn history for the refund txn : {refund_txn_id}, {payment_mode_2}")
                app_txn_id_2 = txn_history_page.fetch_txn_id_text()
                logger.debug(f"Fetching txn_id from txn history for the refund txn : {refund_txn_id}, {app_txn_id_2}")
                payment_status_2 = txn_history_page.fetch_txn_status_text()
                logger.debug(f"Fetching status from txn history for the refund txn : {refund_txn_id}, {payment_status_2}")
                app_rrn_2 = txn_history_page.fetch_RRN_text()
                logger.debug(f"Fetching rrn_number from txn history for the refund txn : {refund_txn_id}, {app_rrn_2}")
                app_order_id_2 = txn_history_page.fetch_order_id_text()
                logger.debug(f"Fetching txn order_id from txn history for the refund txn : {refund_txn_id}, {app_order_id_2}")
                app_payment_msg_2 = txn_history_page.fetch_txn_payment_message_text()
                logger.debug(f"Fetching txn payment msg from txn history for the refund txn : {refund_txn_id}, {app_payment_msg_2}")
                app_settlement_status_2 = txn_history_page.fetch_settlement_status_text()
                logger.debug(f"Fetching txn settlement_status from txn history for the refund txn : {refund_txn_id}, {app_settlement_status_2}")
                app_auth_code_2 = txn_history_page.fetch_auth_code_text()
                logger.debug(f"Fetching AUTH CODE from txn history for the refund txn : {refund_txn_id}, {app_auth_code_2}")
                app_date_and_time_2 = txn_history_page.fetch_date_time_text()
                logger.debug(f"Fetching date_time from txn history for the refund txn : {refund_txn_id}, {app_date_and_time_2}")
                app_device_serial_2 = txn_history_page.fetch_device_serial_text()
                logger.debug(f"Fetching device serial number from txn history for the refund txn : {refund_txn_id}, {app_device_serial_2}")
                app_mid_2 = txn_history_page.fetch_mid_text()
                logger.debug(f"Fetching mid from txn history for the refund txn : {refund_txn_id}, {app_mid_2}")
                app_tid_2 = txn_history_page.fetch_tid_text()
                logger.debug(f"Fetching tid from txn history for the refund txn : {refund_txn_id}, {app_tid_2}")
                app_batch_number_2 = txn_history_page.fetch_batch_number_text()
                logger.debug(f"Fetching batch number for the refund txn : {refund_txn_id}, {app_batch_number_2}")
                app_card_type_desc_2 = txn_history_page.fetch_card_type_desc_text()
                logger.debug(f"Fetching card type desc for the refund txn : {refund_txn_id}, {app_card_type_desc_2}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                expected_date_and_time_api = date_time_converter.db_datetime(txn_created_time)
                expected_date_and_time_api_2 = date_time_converter.db_datetime(refund_created_time)
                expected_api_values = {
                    "pmt_status": "AUTHORIZED_REFUNDED",
                    "txn_amt": float(amount),
                    "pmt_mode": "CARD",
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                date_and_time_portal_2 = date_time_converter.to_portal_format(created_date_db=txn_created_time)
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                txn_date_2, txn_time_2 = date_time_converter.to_chargeslip_format(txn_created_time)
                expected_charge_slip_values = {
