...
# def calculate_mse_between_two_images(img1, img2):
#    """This function taken two numpy BGR image arrays and calculates the mean squared error"""
#    h, w, _ = img1.shape
...
def get_maximum_error_threshold(entity="COMPANY"):
    """This function returns the maximum Mean Squared Error (MSE). The param `entity` should be either `COMPANY` or `BANK`"""
    config_info = config_utils.load_configuration()
    if entity == "COMPANY":
        return config_info["COMPANY_LOGO_MAX_MSE"]
    elif entity == "BANK":
        return config_info["BANK_LOGO_MAX_MSE"]
    else:
        raise Exception(
            f"The entity '{entity}' is not recognized. Therefore Cannot get the 'MAX MSE THRESHOLD' key is not found in configuration yaml file")


COMPANY_LOGO_MAX_MSE = get_maximum_error_threshold(entity="COMPANY")
...
def crop_the_screenshot(img: np.ndarray):
    ...
    '''
    avg = np.mean(img,
                  axis=2)  # img (3 dim) is converted to 2 dim (with height and width) so that filtering with 255 is easy
    threshold = 125

    # LEFT RIGHT WHITE SPACE CUTTING
    # any or all (all preferred if an)  # variabl name ends with non_255 bcz initially it was avg != 255
    locatons_in_y_axis_with_non_255 = np.where(
        np.any(avg < threshold, axis=0))  # axis 0 will check for only y axis section
...
def add_padding_to_image(img, padding_width=10):
    ...
    padding_left_n_right = np.full(
        (top_bottom_padding_added_img.shape[0], padding_width, top_bottom_padding_added_img.shape[-1]), 255)
    right_left_top_bottom_padding_added_img = np.hstack(
        [padding_left_n_right, top_bottom_padding_added_img, padding_left_n_right])
...
def create_reference_logo_image_from_receipt_image(
    ...
    cropped_img = img[found_top_left_loc[-1]:found_bottom_right_loc[-1],
                  found_top_left_loc[0]:found_bottom_right_loc[0]]
    ...
    bank_or_company_referece_logo_coordinates_json = os.path.join(ideal_reference_logo_save_dir_path,
                                                                  f'{bank_or_company}.json')
...
def validate_chargeslip_image_logos(img_path, bank, company, visualize=False):
    ...
    if bank is not None:
        ...
        cropped_from_screenshot_to_compare_with_bank_reference_logo = img[ \
                                                                      bank_reference_logo_coordinates[
                                                                          'found_top_left_loc'][-1] \
                                                                      :bank_reference_logo_coordinates[
                                                                          'found_bottom_right_loc'][-1], \
                                                                      bank_reference_logo_coordinates[
                                                                          'found_top_left_loc'][0] \
                                                                      :bank_reference_logo_coordinates[
                                                                          'found_bottom_right_loc'][0]]
    ...
    calc_relative_top = company_reference_logo_coordinates["found_top_left_loc"][-1] - \
                        company_reference_logo_coordinates['reference_receipt_height']
    calc_top = current_image_height + calc_relative_top

    calc_relative_bottom = company_reference_logo_coordinates["found_bottom_right_loc"][-1] - \
                           company_reference_logo_coordinates['reference_receipt_height']
    ...
    # previous logic - pixel to pixel matching for company logo validation
    ...
    # current logic - mean squared error based validation
    # if the MSC of cropped logo from incoming screenshot is not going
    # beyond the COMPANY_LOGO_MAX_MSE (in configuration.yaml file)
    # then, we are passing the comparison else we will
    mse_for_found_company_logo = calculate_mse_between_two_images(company_reference_logo,
                                                                  cropped_from_screenshot_to_compare_with_company_reference_logo)
    print(f"{mse_for_found_company_logo=}\n{COMPANY_LOGO_MAX_MSE=}")
    if mse_for_found_company_logo < COMPANY_LOGO_MAX_MSE:
        COMPANY_LOGO_MATCH = True
    else:
        COMPANY_LOGO_MATCH = False
...
def validate_chargeslip_image_logos_from_url(url, bank, company='ezetap', visualize=False):
    ...
    COMPANY_LOGO_MATCH, BANK_LOGO_MATCH = validate_chargeslip_image_logos(cropped_img_path, bank, company,
                                                                          visualize=visualize)
    ...
    # if the above condition fails
        ...
        if isinstance(BANK_LOGO_MATCH, bool) and isinstance(COMPANY_LOGO_MATCH, bool):
            print(
                f"Partial Match! {'Company logo' if COMPANY_LOGO_MATCH is not True else 'Bank logo' if BANK_LOGO_MATCH is not True else 'Something'} does not match")
        # checking if anyone of them is none
            ...
            raise Exception(
                "Unknown Error: [it seems both of them are None] CHECK screenshot_processor.py line no. 469")
