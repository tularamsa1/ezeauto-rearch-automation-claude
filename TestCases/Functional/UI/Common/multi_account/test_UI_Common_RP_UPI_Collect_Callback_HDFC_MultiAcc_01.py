...
def test_common_100_111_003():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Success_Via_Pure_UPI_Collect_Callback_HDFC_MultiAcc
    Sub Feature Description: MultiAccount - Verification of a Remote Pay successful pure upi txn via HDFC using callback
    Sub Feature Code: UI_Common_PM_UPI_Collect_success_callback_before_expiry_AutoRefund_Enabled_HDFC_MultiAcc //Discuss with team
    Sub Feature Description: MultiAccount - Performing a  UPI Collect success callback via HDFC before expiry when auto refund is enabled
    ...
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('upi_success_curl', curl_data={
                'ref_id': txn_ref, 'Txn_id': payment_intent_id,
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_111_004():
    """
    Sub Feature Code: UI_Common_PM_RP_upi_collect_Failed_Via_upi_collect_Callback_HDFC_MultiAcc
    Sub Feature Description: MultiAccount - Verification of a Remote Pay failed pure upi collect txn via HDFC using callback
    TC naming code description:
    100: Payment Method
    111: MultiAcc_RemotePay
    004: TC_004
