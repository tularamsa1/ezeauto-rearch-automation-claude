...
def test_common_100_115_08_007():
    """
    Sub Feature Code: UI_Common_Card_Online_EMI_Void_Txn_For_An_Org_With_MID1_TID1_Via_HDFC_Dummy_CreditCard_For_3_Months
    Sub Feature Description:  Performing the online EMI void transaction for an org via HDFC Dummy PG
     using Credit card with MID 1 and TID 1 for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 08: ONLINE_EMI, 007: TC007
    """
    try:
        ...
        query = f"select * from terminal_info where emi_id=(select id from emi where org_code='{org_code}' " \
                f"and issuer_code='HDFC' and term='3 month' and card_type='CREDIT' and emi_type='NORMAL')"
        ...
        try:
            ...
            emi_external_tid = result['emi_external_tid'].values[0]
            logger.debug(f"Fetching emi_external_tid from txn_emi table : {emi_external_tid}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                # Below data are fetching for normal txn from txn history page
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    ...
                    "emi_external_tid": emi_external_tid,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Txn Amt": "Rs." + str(amount) + ".00",
                    "Loan Amt": "Rs." + str(amount) + ".00",
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_08_008():
    """
    Sub Feature Code: UI_Common_Card_Online_EMI_Void_Txn_For_An_Org_With_MID2_TID2_Via_HDFC_Dummy_CreditCard_For_6_Months
    Sub Feature Description:  Performing the online EMI void transaction for an org via HDFC Dummy PG
     using Credit card with MID 2 and TID 2 for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 08: ONLINE_EMI, 008: TC008
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                # Below data are fetching for emi txn form txn history page
                app_payment_by = txn_history_page.fetch_payment_by_text()
                logger.debug(f"Fetching payment by from txn history for the txn : {txn_id}, {app_payment_by}")
                app_card_type = txn_history_page.fetch_card_type_text()
                logger.debug(f"Fetching card type from txn history for the txn : {txn_id}, {app_card_type}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_08_009():
    """
    Sub Feature Code: UI_Common_Card_Online_EMI_Void_Txn_For_An_Org_With_MID3_TID3_Via_HDFC_Dummy_CreditCard_For_9_Months
    Sub Feature Description:  Performing the online EMI void transaction for an org via HDFC Dummy PG
     using Credit card with MID 3 and TID 3 for 9 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 08: ONLINE_EMI, 009: TC009
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_08_010():
    """
    Sub Feature Code: UI_Common_Card_Online_EMI_Void_Txn_For_Root_Org_With_MID1_TID1_Via_HDFC_Dummy_CreditCard_For_3_Months
    Sub Feature Description:  Performing the online EMI void transaction for root org via HDFC Dummy PG
     using Credit card with MID 1 and TID 1 for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 08: ONLINE_EMI, 010: TC010
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Query to fetch data from the terminal_info for the {org_code} where emi_id for {root_org_code} : {query}")
        result = DBProcessor.getValueFromDB(query=query)
        logger.debug(f"Fetching result for terminal_info table : {result}")
        mid = result["mid"].values[0]
        logger.debug(f"Fetching mid from the terminal_info table : mid : {mid}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_08_011():
    """
    Sub Feature Code: UI_Common_Card_Online_EMI_Void_Txn_For_Root_Org_With_MID2_TID2_Via_HDFC_Dummy_CreditCard_For_6_Months
    Sub Feature Description:  Performing the online EMI void transaction for root org via HDFC Dummy PG
     using Credit card with MID 2 and TID 2 for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 08: ONLINE_EMI, 011: TC011
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
