...
def test_common_100_115_231():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_EMV_VISA_DebitCard_Without_Pin_476173
    Sub Feature Description: Performing the EMV Void transaction via FDC PG using VISA Debit card having 16 digit PAN length without pin (bin: 476173)
...
def test_common_100_115_232():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_EMV_MASTER_DebitCard_Without_Pin_222360
    Sub Feature Description: Performing the EMV Void transaction via FDC PG using MASTER Debit card having 16 digit PAN length without pin (bin: 222360)
...
def test_common_100_115_233():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_EMV_RUPAY_DebitCard_Without_Pin_608326
    Sub Feature Description: Performing the EMV Void transaction via FDC PG using RUPAY Debit card having 16 digit PAN length without pin (bin: 608326)
...
def test_common_100_115_234():
    """
    Sub Feature Code: UI_Common_Card_Void_HDFC_EMV_VISA_DebitCard_With_Pin_428090
    Sub Feature Description: Performing the EMV Void transaction via HDFC PG using VISA Debit card having 16 digit PAN length with pin (bin : 428090)
    ...
    try:
        ...
        if str(ConfigReader.read_config("ParallelExecution", "deviceOnly")).lower() == 'true':
            ...
            query = f"UPDATE terminal_info SET status = 'ACTIVE' WHERE org_code='{org_code}' and status = 'INACTIVE' and " \
                    f"acquirer_code='ICICI' and payment_gateway='FDC' and device_serial = '{GlobalVariables.str_device_id}';"
            logger.debug(f"Query to fetch data from the terminal_info for the {org_code} : {query}")
            ...
            query = (f"select * from terminal_info where org_code= '{org_code}' and status = 'ACTIVE' and"
                     f" acquirer_code='ICICI' and payment_gateway='FDC'")
            logger.debug(f"Query to fetch data from the terminal_info table: {query}")
            result = DBProcessor.getValueFromDB(query=query)
            logger.debug(f"Query result for terminal_info table : {result}")
            mid = result["mid"].iloc[0]
...
def test_common_100_115_235():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_EMV_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description: Performing the EMV Void transaction via HDFC PG using RUPAY Debit card having 16 digit PAN length with pin (bin : 608326)
