...
def test_common_100_101_200():
    ...
    Sub Feature Description: TID Dep - Verification of one success callback after expiry when auto refund enabled via
    Axis acq and Axis PG where upi config is Axis_Direct
    ...
    try:
        ...
        response = APIProcessor.send_request(api_details)
        logger.debug(f"Response received for setting UPI QR Expiry time is : {response}")
        ...
        try:
            ...
            new_device_serial_db = result['device_serial'].values[0]
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                actual_api_values = {
                    ...
                    "device_serial_2": new_device_serial_api
                }
                logger.debug(f"actual_api_values: {actual_api_values}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    ...
                    "device_serial_2": new_device_serial_db,
                }
                logger.debug(f"actual_db_values : {actual_db_values}")
...
def test_common_100_101_202():
    ...
    try:
        ...
        try:
            ...
            txn_id = response["txnId"]
            logger.debug(f"Fetching Txn_id from the API_OUTPUT, Txn_id : {txn_id}")
            logger.info(f"Waiting for 60 seconds for QR expiry ...")
            time.sleep(60)
