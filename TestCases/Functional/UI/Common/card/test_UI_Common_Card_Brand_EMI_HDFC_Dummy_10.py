...
def test_common_100_115_07_113():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Instant Discount payback by brand percentage success transaction
    for an org (not ezetap) via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 113: TC113
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_114():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Block_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by brand percentage block success transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 114: TC114
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            emi_cashback_fixed = result['emi_cashback_fixed'].values[0]
            logger.debug(f"Fetching emi cashback fixed amount from txn_emi table : {emi_cashback_fixed}")
            debit_emi_validation_details_id = result['debit_emi_validation_details_id'].values[0]
            logger.debug(f"Fetching original_amount from txn_emi table : {debit_emi_validation_details_id}")

            query = f"select * from debit_emi_txn_request where id='{debit_emi_validation_details_id}'"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_115():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Band_Percentage_Unblock_Failed_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenture
    Sub Feature Description: Performing the brand EMI payback by brand percentage unblock failed transaction for
    an org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 115: TC115
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                     ...
                     "subvention_tenure": f"{emi_plan_in_months} month",
                     "subvention_discount_type": None if subvention_discount_type is None else "Additional",
                    "txn_amt_posting_status": "UNBLOCK_FAILED",
                    "eligibility_status": "PROCESSED"
                }
                logger.debug(f"expected_db_values: {expected_db_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_116():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Band_Percentage_Unblock_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenture
    Sub Feature Description: Performing the brand EMI payback by brand percentage unblock success transaction for
    an org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 116: TC116
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_117():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Block_Failed_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by brand percentage block failed transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 117: TC117
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            payment_page.select_emi_plan(emi_plan_in_months=emi_plan_in_months)
            logger.debug(f"Selected the emi plan in months : {emi_plan_in_months}")
            payment_page.click_on_ok_error_btn_for_brand_emi()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                app_driver.reset()
                logger.info(f"Logging in the MPOSX application after reseting driver using username and password : {app_username}, {app_password}")
                login_page.perform_login(username=app_username, password=app_password)
                home_page.wait_for_home_page_load()
                home_page.click_on_history()
                txn_history_page = TransHistoryPage(driver=app_driver)
