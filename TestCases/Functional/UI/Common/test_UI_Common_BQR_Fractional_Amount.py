...
def test_common_100_102_422():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_CheckStatus_HDFC_Fractional_Amount
    Sub Feature Description: Performing BQRV4_BQR Successful txn with fractional amount Via CheckStatus API for HDFC pg
    TC naming code description: 100: Payment Method, 102: BQR, 422: TC422
    """
    try:
        ...
        try:
            ...
            amount = round(random.uniform(401, 1000), 2)
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for check status api is : {response}")
            rrn = response['rrNumber']
            logger.debug(f"Fetching rrn from the API_OUTPUT, rrn : {rrn}")
            ...
            logger.debug(
                f"Fetching auth_code, rrn, created_time, customer name and payer name from database for " f"current merchant:{auth_code}, {rrn}, {created_time}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {"pmt_mode": "BHARAT QR", "pmt_status": "AUTHORIZED",
                                       "txn_amt": float(amount), "settle_status": "SETTLED",
                ...
                actual_app_values = {"pmt_mode": payment_mode, "pmt_status": payment_status.split(':')[1],
                                     "txn_amt": float(app_amount.split(' ')[1]), "txn_id": app_txn_id,
                                     "rrn": str(app_rrn),
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {"txn_amt": float(amount_db),
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {'PAID BY:': 'BHARATQR', 'merchant_ref_no': 'Ref # ' + str(order_id),
                                               'RRN': str(rrn),
                                               'BASE AMOUNT:': f"Rs.{str(amount)}", 'date': txn_date, 'time': txn_time,
                                               'AUTH CODE': auth_code}
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_423():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_CheckStatus_AXIS_ATOS_Fractional_Amount
    Sub Feature Description: Performing BQRV4_BQR Successful txn with fractional amount Via CheckStatus API for AXIS_ATOS pg
    TC naming code description: 100: Payment Method, 102: BQR, 423: TC423
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from bharatqr_merchant_config where org_code='{org_code}' and status = 'ACTIVE' and bank_code='AXIS'"
        result = DBProcessor.getValueFromDB(query)
        mid = result["mid"].iloc[0]
        tid = result["tid"].iloc[0]
        terminal_info_id = result["terminal_info_id"].iloc[0]
        ...
        try:
            ...
            logger.debug(f"Execution Timer started in testcase function : {testcase_id}")
            # ------------------------------------------------------------------------------------------------
            amount = round(random.uniform(301, 400), 2)
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.debug(f"initiating upi qr for the amount of {amount}")
            api_details = DBProcessor.get_api_details('bqrGenerate', request_body={
                "username": app_username, "password": app_password, "amount": str(amount), "orderNumber": str(order_id)
            ...
            settlement_status_db = result["settlement_status"].iloc[0]
            logger.debug(
                f"Fetching amount_db, payment_mode_db, payment_status_db, payment_state_db, acquirer_code_db, bank_name_db, mid_db, tid_db, payment_gateway_db, rr_number_db, settlement_status_db from database for "
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                date_and_time = date_time_converter.to_app_format(created_time)
                expected_app_values = {"pmt_mode": "UPI", "pmt_status": "AUTHORIZED", "txn_amt": float(amount),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_424():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_CheckStatus_YES_Fractional_Amount
    Sub Feature Description: Performing BQRV4_BQR Successful txn with fractional amount Via CheckStatus API for YES pg
    TC naming code description: 100: Payment Method, 102: BQR, 424: TC424
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from bharatqr_merchant_config where org_code='{org_code}' and status = 'ACTIVE' and bank_code='YES'"
        result = DBProcessor.getValueFromDB(query)
        mid = result["mid"].iloc[0]
        tid = result["tid"].iloc[0]
        terminal_info_id = result["terminal_info_id"].iloc[0]
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_425():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_CheckStatus_FDC_ICICI_Fractional_Amount
    Sub Feature Description: Performing BQRV4_BQR Successful txn with fractional amount Via CheckStatus API for FDC_ICICI pg
    TC naming code description: 100: Payment Method, 102: BQR, 152: TC152
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from bharatqr_merchant_config where org_code ='{str(org_code)}' AND status = 'ACTIVE' AND bank_code = 'FDC_ICICI'"
        ...
        try:
            ...
            amount = round(random.uniform(301, 999), 2)
            print("amount: ", amount)
            order_id = datetime.now().strftime('%m%d%H%M%S')
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {'PAID BY:': 'BHARATQR', 'merchant_ref_no': 'Ref # ' + str(order_id),
                                               'RRN': str(rrn),
                                               'BASE AMOUNT:': f"Rs.{str(amount)}", 'date': txn_date,
                                               'time': txn_time,
                                               'AUTH CODE': auth_code}
                receipt_validator.perform_charge_slip_validations(txn_id,
                                                                  {"username": app_username, "password": app_password},
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_426():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_CheckStatus_KOTAK_WL_Fractional_Amount
    Sub Feature Description: Performing BQRV4_BQR Successful txn with fractional amount Via CheckStatus API for KOTAK_WL pg
    TC naming code description: 100: Payment Method, 102: BQRV4, 246: TC246
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from bharatqr_merchant_config where org_code ='{str(org_code)}' AND status = 'ACTIVE' AND bank_code = 'KOTAK_WL'"
        logger.debug(f"Query to fetch data from the bharatqr_merchant_config for the {org_code} : {query}")
        result = DBProcessor.getValueFromDB(query)
        mid = result['mid'].values[0]
        logger.debug(f"Fetching mid from the bharatqr_merchant_config table : mid : {mid}")
        ...
        try:
            ...
            amount = round(random.uniform(401, 999), 2)
            print("amount: ", amount)
            order_id = datetime.now().strftime('%m%d%H%M%S')
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "bqr_txn_primary_id": txn_id,
                    "bqr_merchant_pan": str(rrn).replace('R', 'M', 1),
                    "bqr_rrn": rrn,
                    "bqr_org_code": org_code,
                    "bqr_bank_code": "KOTAK",
                }
