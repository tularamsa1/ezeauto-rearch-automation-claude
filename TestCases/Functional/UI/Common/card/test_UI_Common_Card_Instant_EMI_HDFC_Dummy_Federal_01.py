...
def test_common_100_115_06_051():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Success_Txn_For_An_Org_HDFC_Dummy_Fedral_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the instant EMI success transaction for an org (not ezetap) via HDFC Dummy PG
    for Fedral issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 051: TC051
    """
    try:
        ...
        try:
            ...
            query = f"select * from emi where org_code='{org_code}' and status = 'ACTIVE' and " \
                    f"issuer_code='FEDERAL' and card_type='CREDIT' and term = '{emi_plan_in_months} month' and emi_type='NORMAL'" \
                    f"and tid_type='CIB'"
