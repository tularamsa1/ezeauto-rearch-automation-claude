...
def test_common_100_115_11_025():
    """
    Sub Feature Code: UI_Common_Card_EZE_EMI_Subvented_By_Brand_Ezetap_For_Brand_Ezetap_Cosubvention_Topup_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the EZE EMI subvented by brand Ezetap for brand-Ezetap cosubvention topup success transaction for an org (not ezetap) via
    HDFC Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 025: TC025
    """
    try:
        ...
        try:
            ...
            amount = random.randint(10000, 15000)
            ...
            query = f"select * from ezeemi_club_wallet_account where org_code='{org_code}' and wallet_type='MCEW' and " \
                    f"brand_id='{brand_id}'"
            logger.debug(f"Query to fetch data from the ezeemi_club_wallet_account table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
            ...
            logger.debug(
                f"Fetching wallet_balance after the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_after_txn}")

            query = f"select * from ezeemi_club_wallet_txn where org_code='{org_code}' and wallet_type='MCEW' and " \
                    f"ref_txn_id ='{txn_id}' order by created_time desc limit 1"
            logger.debug(f"Query to fetch data from the ezeemi_club_wallet_txn table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for ezeemi_club_wallet_txn table : {result}")
            ezeemi_club_wallet_txn_amount = result['txn_amount'].values[0]
            logger.debug(f"Fetching txn amount from ezeemi_club_wallet_account table : {ezeemi_club_wallet_txn_amount}")
            ezeemi_club_wallet_txn_mode = result['txn_mode'].values[0]
            logger.debug(f"Fetching txn mode from ezeemi_club_wallet_account table : {ezeemi_club_wallet_txn_mode}")
            ezeemi_club_wallet_txn_status = result['status'].values[0]
            logger.debug(f"Fetching status from ezeemi_club_wallet_account table : {ezeemi_club_wallet_txn_status}")
            ezeemi_club_wallet_txn_emi_type = result['emi_type'].values[0]
            logger.debug(f"Fetching emi_type from ezeemi_club_wallet_account table : {ezeemi_club_wallet_txn_emi_type}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                date_time = date_time_converter.db_datetime(date_from_db=txn_created_time)
                expected_api_values = {
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "ezeemi_club_wallet_txn_amount": float(amount),
                    "ezeemi_club_wallet_txn_mode": "TOPUP",
                    "ezeemi_club_wallet_txn_status": "SUCCESS",
                    "ezeemi_club_wallet_txn_emi_type": "BRAND_EMI"
                }
                ...
                actual_db_values = {
                    ...
                    "ezeemi_club_wallet_txn_amount": ezeemi_club_wallet_txn_amount,
                    "ezeemi_club_wallet_txn_mode": ezeemi_club_wallet_txn_mode,
                    "ezeemi_club_wallet_txn_status": ezeemi_club_wallet_txn_status,
                    "ezeemi_club_wallet_txn_emi_type": ezeemi_club_wallet_txn_emi_type
                }
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_11_026():
    """
    Sub Feature Code: UI_Common_Card_EZE_EMI_Subvented_By_Brand_Ezetap_For_Brand_Ezetap_Cosubvention_Without_Topup_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the EZE EMI subvented by brand Ezetap for brand-Ezetap cosubvention topup success transaction for an org (not ezetap) via
    HDFC Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 026: TC026
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(
            f"Fetching wallet_balance before the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_before_txn}")
        mcew_wallet_balance_max_topup_limit = result['max_topup_limit'].values[0]
        logger.debug(
            f"Fetching max_topup_limit before the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_max_topup_limit}")

        query = f"update ezeemi_club_wallet_account set wallet_limit='{mcew_wallet_balance_max_topup_limit}' where " \
                f"org_code='{org_code}' and wallet_type='MCEW' and brand_id='{brand_id}';"
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_11_027():
    """
        Sub Feature Code: UI_Common_Card_Eze_EMI_For_Issuer_Ezetap_Cosubvention_Verify_Visibility_Of_Razorpay_Emi_Discount_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
        Sub Feature Description: Performing the eze EMI for issuer_ezetap co-subvention verify visibility of razorpay emi
        discount for an org (not ezetap) via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 027: TC027
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            imei_no = random.randint(1, 500)
            logger.debug(f"Randomly generated imei number is, {imei_no}")
            payment_page.click_and_enter_imei_number_for_eze_emi(imei_no)
            logger.debug(f"Entered IMEI number, {imei_no}")
            card_page = CardPage(app_driver)
            logger.debug(f"Selecting the card type as : EMV_WITH_PIN_VISA_CREDIT_417666")
            card_page.select_cardtype("EMV_WITH_PIN_VISA_CREDIT_417666")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                expected_app_values = {
                    "visibility_of_razorpay_emi_discount": "razorpay emi discount is visible"
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_11_028():
    """
        Sub Feature Code: UI_Common_Card_Eze_EMI_For_Issuer_Ezetap_Cosubvention_Verify_Visibility_Of_Razorpay_Emi_Discount_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
        Sub Feature Description: Performing the eze EMI for issuer_ezetap co-subvention verify visibility of razorpay emi
        discount for an org (not ezetap) via HDFC Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 028: TC028
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                expected_app_values = {
                    "visibility_of_razorpay_emi_discount": "razorpay emi discount is not visible"
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
