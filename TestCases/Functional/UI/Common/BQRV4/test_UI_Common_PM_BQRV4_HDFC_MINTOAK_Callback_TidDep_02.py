...
def test_common_100_102_412():
    """
    Sub Feature Code: Tid_dep_UI_Common_BQRV4_UPI_Success_Via_Pure_UPI_Callback_HDFC_MINTOAK
    Sub Feature Description: Tid dep - Verification of a BQRV4 successful pure upi txn via HDFC_MINTOAK  using callback
    TC naming code description: 100: Payment Method, 102: BQR, 412: TC412
    """
    try:
        ...
        query = f"update terminal_dependency_config set terminal_dependent_enabled=1 where org_code ='{org_code}' and " \
                f"acquirer_code='HDFC' and payment_gateway='MINTOAK'"
        ...
        try:
            ...
            logger.debug(f"Fetching customer name from db : {customer_name}")
            payer_name = result['payer_name'].values[0]
            logger.debug(f"Fetching payer name from db : {payer_name}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching auth code from db : {auth_code}")
            created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created time from db : {created_time}")
            ...
            logger.debug(f"Fetching device serial from db : {device_serial_db}")
            order_id_db = result['external_ref'].values[0]
            logger.debug(f"Fetching order id from db : {order_id_db}")
            rrn = result['rr_number'].values[0]
            logger.debug(f"Fetching rrn from db : {rrn}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_102_413():
    """
    Sub Feature Code: Tid_dep_UI_Common_BQRV4_UPI_Failed_Via_Pure_UPI_Callback_HDFC_MINTOAK
    Sub Feature Description: Tid Dep - Verification of a BQRV4 failed pure upi txn via HDFC_MINTOAK using callback
    TC naming code description: 100: Payment Method, 102: BQR, 413: TC413
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching bqr txn type from db : {bqr_txn_type_db}")
                brq_terminal_info_id_db = result["terminal_info_id"].values[0]
                logger.debug(f"Fetching bqr status from db : {brq_terminal_info_id_db}")
                bqr_bank_code_db = result["bank_code"].values[0]
                logger.debug(f"Fetching bqr bank code from db : {bqr_bank_code_db}")
                bqr_merchant_config_id_db = result["merchant_config_id"].values[0]
                logger.debug(f"Fetching bqr merchant config id from db : {bqr_merchant_config_id_db}")
