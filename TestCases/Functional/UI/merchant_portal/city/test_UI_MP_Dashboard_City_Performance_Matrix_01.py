...
def test_mp_700_706_062():
    """
    Sub Feature Code: UI_MP_Performance_Matrix_top10_performer_for_below_20_nodes_as_City_head
    Sub Feature Description: Verifying Performance Matrix for TOP 10 performers when City head has logged in for all lower level hierarchies having less than 20 nodes
    ...
    try:
        ...
        login_cred = ResourceAssigner.get_org_users_login_Credentials('CITY', txn_org_code)
        ...
        try:
            ...
            categories = ["'India'", "'West'", "'Maharastra'", "'Mumbai_City_District'", "'Akole'", "'Mumbai1'"]
            for category in categories:
                del names_and_volumes[category]
            top_performer_db = sum(names_and_volumes.values())
            top_performer_db_1 = "name :" + 'Hub1' + " , volume :" + str(top_performer_db)
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_706_063():
    """
    Sub Feature Code: UI_MP_Performance_Matrix_top10_and_bottom10_performer_for_more_than_20_nodes_as_City_head
    Sub Feature Description: Verifying Performance Matrix for TOP 10  and BOTTOM 10 performers when City head has logged in for all lower level hierarchies having only more than 20 nodes
    TC naming code description:
    700: Merchant Portal
    706: City
    063: TC063
