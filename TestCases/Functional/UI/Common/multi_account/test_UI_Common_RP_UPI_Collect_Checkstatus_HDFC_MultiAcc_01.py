...
def test_common_100_111_007():
    """
    Sub Feature Code: UI_Common_PM_RP_upi_collect_Success_Via_upi_Collect_Checkstatus_HDFC_MultiAcc
    Sub Feature Description: MultiAccount - Verification of a Remote Pay successful upi collect txn via HDFC using check status
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_111_008():
    """
    Sub Feature Code: UI_Common_PM_RP_upi_collect_failed_Via_Pure_upicollect_Checkstatus_HDFC_MultiAcc
    Sub Feature Description: MultiAccount - Verification of a Remote Pay failed upi collect txn via HDFC using check status
    TC naming code description:
    100: Payment Method
    111: MultiAcc_RemotePay
    008: TC_008
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_111_014():
    """
    Sub Feature Code: UI_Common_PM_RP_upi_collect_Success_Via_upi_Collect_Checkstatus_with_second_labelAccountHDFC_MultiAcc
    Sub Feature Description: MultiAccount - Verification of a Remote Pay successful upi collect txn via HDFC using check status second account (ex:acc2 label)
    TC naming code description:
    100: Payment Method
    111: MultiAcc_RemotePay
    014: TC_014
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                actual_portal_values = {"pmt_state": str(portal_status), "pmt_type": portal_type,
                    "txn_amt": portal_amount, "username": portal_username}
                logger.debug(f"actual_portal_values : {actual_portal_values}")
