...
def test_common_rearch_0038():
    """
    Sub Feature Code: UI_ReArch_PM_MyDiscountEMI_Credit_Success_SBI
    Sub Feature Description:
        Launch ReArch app, handle login if required, refresh settings via
        Payment History, enter amount 5959, navigate to My Discount EMI via
        overlay, select SBI Bank Credit Card, choose 3-month No Cost EMI plan,
        verify breakup amounts (Item Price, MyDiscount, Order Total, Interest,
        Total Amount), proceed through card type selection (Visa Credit EMV),
    ...
    NL Source Steps:
      ...
      Test Steps:
        ...
        7.  click on My Discount EMI
        8.  wait until Bank button is visible
        9.  click on SBI Bank Credit Card
        10. click on 3-month No Cost EMI plan radio button
        11. click on View Breakup
        12. verify Item Price = ₹5,959
        13. verify MyDiscount = -₹117.39
        14. verify Order Total = ₹5,841.61
        15. verify Interest charged by Bank = +₹117.23
        16. verify Total Amount = ₹5,958.84
        17. click on back button
        18. click on Proceed
        19. click on Proceed
        20. tap on Visa Credit (EMV)
        21. click on accept more payment
        22. do app, api and chargeslip validation
    ...
    try:
        ...
        try:
            ...
            # Step 7: Click My Discount EMI from payment methods
            ...
            # Step 10: Click 3-month No Cost EMI plan radio button
            emi_page.click_3m_no_cost_plan()
            logger.debug("3-month No Cost EMI plan selected")
            ...
            actual_mydiscount = emi_page.fetch_mydiscount()
            ...
            logger.info(
                f"Breakup — Item Price: {actual_item_price}, "
                f"MyDiscount: {actual_mydiscount}, "
                f"Order Total: {actual_order_total}, "
                f"Interest: {actual_interest}, "
                f"Total Amount: {actual_total_amount}"
            )

            # Step 17: Click back button (close breakup sheet)
            emi_page.go_back()
            logger.debug("Back from breakup sheet")

            # Step 18: Click Proceed on EMI screen
            ...
            query = (
                ...
                f"AND payment_mode='CARD' "  # TODO: verify payment_mode for My Discount EMI
        ...
        if ConfigReader.read_config("Validations", "app_validation") == "True":
            ...
            try:
                # My Discount EMI Breakup verification (fetched during execution)
                expected_breakup_values = {
                    "item_price": "\u20b95,959",
                    "mydiscount": "-\u20b9117.39",
                    "order_total": "\u20b95,841.61",
                    "interest_charged": "+\u20b9117.23",
                    "total_amount": "\u20b95,958.84",
                }
                actual_breakup_values = {
                    "item_price": actual_item_price,
                    "mydiscount": actual_mydiscount,
                ...
                app_txn_status = txn_detail_page.fetch_status(amount_formatted)
                app_date_time  = txn_detail_page.fetch_date_time()
                app_amount     = txn_detail_page.fetch_amount(amount_formatted)
                logger.info(
