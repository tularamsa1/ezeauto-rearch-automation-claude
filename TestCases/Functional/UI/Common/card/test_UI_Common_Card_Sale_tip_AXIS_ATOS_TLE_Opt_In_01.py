...
def test_common_100_116_024():
    ...
    TC naming code description: 100: Payment Method, 116: CARD_DCC, 024: TC024
    """
    try:
        ...
        try:
            ...
            logger.debug(f"max tip amount: {max_tip_amount}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                converted_tip_amount = tip_amount * 0.0107033
                converted_tip_amount_2 = "{:,.2f}".format(converted_tip_amount)
                txn_amount = 2.14 - float(converted_tip_amount_2)
                expected_charge_slip_values = {
                    "CARD TYPE": "MasterCard",
                    "merchant_ref_no": "Ref # " + str(order_id),
                    "TIP AMOUNT:": "USD " + str(converted_tip_amount_2),
                    "RRN": str(rrn),
                    "TRANSACTION AMOUNT:": "USD " + "{:,.2f}".format(txn_amount),
                    "AUTH CODE": auth_code,
                    "date": txn_date,
                    "time": txn_time,
                    ...
                    "TID": tid,
                    "Fx Rate": "1 INR = 0.52480573 USD",
                    "AMOUNT CHARGED:": "JPY {:,.2f}".format(converted_amount)
                }
...
def test_common_100_116_025():
    ...
    with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_DCC, 025: TC025
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            amount = random.randint(5000, 6000)
            converted_amount = amount * 0.52480573
            max_tip_amount = int((amount * 10) / 100)
            logger.debug(f"max tip amount: {max_tip_amount}")
            tip_amount = random.randint(1, max_tip_amount)
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    "CARD TYPE": "MasterCard",
                    "merchant_ref_no": "Ref # " + str(order_id),
                    "TIP AMOUNT:": "Rs." + str(tip_amount) + ".00",
