...
def test_common_100_101_226():
    """
    Sub Feature Code: UI_Common_PM_2_Pure_UPI_Success_Callback_After_QR_Expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Performing two  upi success callback via Razorpay after expiry the qr when autorefund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 226: TC226
    """
    try:
        ...
        try:
            ...
            logger.debug(f"api_details for razorpay_callback_generator_HMAC is: {api_details_hmac_2}")

            response = APIProcessor.send_request(api_details_hmac_2)
            logger.debug(f"Response received for razorpay_callback_generator_HMAC api is : {response}")
            razorpay_signature_2 = response['razorpay_signature']
            logger.debug(
                f"razorpay_signature received for razorpay_callback_generator_HMAC api is : {razorpay_signature_2}")
            logger.debug(f"performing upi callback for razorpay")
            api_details = DBProcessor.get_api_details('upi_confirm_razorpay',
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values_2 = {
                    ...
                    'BASE AMOUNT:': "Rs." + "{:,.2f}".format(amount),
                    "AUTH CODE": "" if auth_code_3 is None else auth_code_3,
                }
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_227():
    """
    Sub Feature Code: UI_Common_PM_2_Pure_UPI_Success_Callback_After_QR_Expiry_Razorpay_AutoRefund_Enabled
    Sub Feature Description: Performing two  upi success callback via Razorpay after expiry the qr when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 227: TC227
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(
                f"razorpay_signature received for razorpay_callback_generator_HMAC api is : {razorpay_signature_2}")
            logger.debug(f"performing 2nd upi callback for razorpay")
            api_details = DBProcessor.get_api_details('upi_confirm_razorpay',
                                                      request_body=api_details_hmac_2['RequestBody'])
            api_details['Header'] = {'x-razorpay-signature': razorpay_signature_2,
                                     'Content-Type': 'application/json'}
            logger.debug(f"api details for 2nd callback upi_confirm_razorpay : {api_details}")
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received for 2nd callback upi_confirm_razorpay api is : {response}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {"pmt_status": "EXPIRED",
                                       ...
                                       "acquirer_code": "RAZORPAY",
                                       "order_id": order_id,
                                       "issuer_code": "RAZORPAY", "rrn": str(rrn),
                                       "txn_type": txn_type,
                                       "mid": mid,
                                       "tid": tid,
                                       "org_code": org_code,
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_228():
    """
    Sub Feature Code: UI_Common_PM_UPI_Failed_Callback_After_QR_Expiry_AutoRefund_Enabled_Razorpay
    Sub Feature Description: Performing a  upi failed callback via Razorpay after expiry the qr when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 228: TC228
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_229():
    """
    Sub Feature Code: UI_Common_PM_UPI_2_Failed_Callback_After_QR_Expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Performing two  upi failed callback via Razorpay after expiry the qr when autorefund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 229: TC229
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            razorpay_signature_2 = response['razorpay_signature']
            logger.debug(
                f"razorpay_signature received for razorpay_callback_generator_HMAC  api for 2nd callback is : {razorpay_signature_2}")
            logger.debug(f"performing upi for 2nd callback for razorpay")
            api_details = DBProcessor.get_api_details('upi_confirm_razorpay',
                                                      request_body=api_details_hmac_2['RequestBody'])
            api_details['Header'] = {'x-razorpay-signature': razorpay_signature_2,
                                     'Content-Type': 'application/json'}
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_230():
    """
    Sub Feature Code: UI_Common_PM_2_Pure_UPI_failed_callback_before_qr_expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Performing two  upi failed callback via Razorpay before expiry the qr when autorefund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 230: TC230
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"preparing data to perform the hash generation for second callback")
            callback_2_rrn = str(random.randint(1000000000000, 9999999999999))
            amount_api = amount * 100
            payment_id = txn_ref.replace("qr_", "pay_")
            ...
            razorpay_signature_2 = response['razorpay_signature']
            logger.debug(
                f"razorpay_signature received for razorpay_callback_generator_HMAC api for second callback is : {razorpay_signature_2}")
            logger.debug(f"performing upi second callback for razorpay")
            api_details = DBProcessor.get_api_details('upi_confirm_razorpay',
                                                      request_body=api_details_hmac_2['RequestBody'])
            api_details['Header'] = {'x-razorpay-signature': razorpay_signature_2,
                                     'Content-Type': 'application/json'}
            logger.debug(f"api details for upi_confirm_razorpay second callback : {api_details}")
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received for upi_confirm_razorpay api second callback is : {response}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                new_txn_status_db_1 = result["status"].values[0]
                new_txn_payment_mode_db_1 = result["payment_mode"].values[0]
                new_txn_amount_db_1 = int(result["amount"].values[0])
                new_txn_state_db_1 = result["state"].values[0]
                new_txn_payment_gateway_db_1 = result["payment_gateway"].values[0]
                new_txn_acquirer_code_db_1 = result["acquirer_code"].values[0]
                new_txn_bank_code_db_1 = result["bank_code"].values[0]
