...
def test_common_100_115_262():
    """
    Sub Feature Code: UI_Common_Card_Refund_MASHREQ_EMVCTLS_VISA_DebitCard_With_Pin_476173
    Sub Feature Description: Performing the EMVCTLS Refund transaction via MASHREQ PG using VISA Debit card having
     16 digit PAN length with pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 262: TC262
    """
    try:
        ...
        try:
            ...
            logger.debug(f"selecting the card type as : CTLS_WITH_PIN_VISA_DEBIT_476173")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_263():
    """
    Sub Feature Code: UI_Common_Card_Refund_MASHREQ_EMVCTLS_MASTER_DebitCard_With_Pin_222360
    Sub Feature Description: Performing the EMVCTLS Refund transaction via MASHREQ PG using MASTER Debit card having
     16 digit PAN length with pin (bin: 222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 263: TC263
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.info("Selected payment mode is Card")
            card_page = CardPage(driver=app_driver)
            logger.debug(f"selecting the card type as : CTLS_WITH_PIN_MASTER_DEBIT_222360")
            card_page.select_cardtype(text="CTLS_WITH_PIN_MASTER_DEBIT_222360")
            logger.debug(f"selected the card type as : CTLS_WITH_PIN_MASTER_DEBIT_222360")
            payment_page.click_on_proceed_homepage()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching txn payment msg from txn history for the txn : {txn_id_2}, {app_payment_msg_2}")
                app_settlement_status_2 = txn_history_page.fetch_settlement_status_text()
                logger.debug(f"Fetching txn settlement_status from txn history for the txn : {txn_id_2}, {app_settlement_status_2}")
                app_auth_code_2 = txn_history_page.fetch_auth_code_text()
                logger.debug(f"Fetching AUTH CODE from txn history for the txn : {txn_id_2}, {app_auth_code_2}")
                app_date_and_time_2 = txn_history_page.fetch_date_time_text()
                logger.debug(f"Fetching date_time from txn history for the txn : {txn_id_2}, {app_date_and_time_2}")
                app_device_serial_2 = txn_history_page.fetch_device_serial_text()
                logger.debug(f"Fetching device serial number from txn history for the txn : {txn_id_2}, {app_device_serial_2}")
                app_mid_2 = txn_history_page.fetch_mid_text()
                logger.debug(f"Fetching mid from txn history for the txn : {txn_id_2}, {app_mid_2}")
                app_tid_2 = txn_history_page.fetch_tid_text()
                logger.debug(f"Fetching tid from txn history for the txn : {txn_id_2}, {app_tid_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_264():
    """
    Sub Feature Code: UI_Common_Card_Refund_MASHREQ_EMV_VISA_CreditCard_Without_Pin_417666
    Sub Feature Description: Performing the EMV Refund transaction via MASHREQ PG using VISA Credit card having
    16 digit PAN length without pin (bin:417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 264: TC264
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            card_page.select_cardtype(text="EMV_VISA_CREDIT_417666")
            ...
            if settle_success:
                ...
                result = DBProcessor.setValueToDB(query, "mware")
                logger.info(
                    f"Query result: Updated the state and settlement status in txn table of mware_demo: {result}")
            ...
            currency_code = result['currency_code'].values[0]
            logger.debug(f"Fetching currency_code value from the txn table : {currency_code}")
            created_time = result['created_time'].values[0]
            ...
            refund_total_amount = result['amount'].values[0]
            logger.debug(f"Query result, amount for refund txn : {refund_total_amount}")
            refund_currency_code = result['currency_code'].iloc[0]
            logger.debug(f"Fetching currency_code value from the txn table : {refund_currency_code}")
            refund_created_time = result['created_time'].values[0]
            logger.debug(f"Query result, created_time for refund txn : {refund_created_time}")
            refund_acquirer_code = result['acquirer_code'].values[0]
            logger.debug(f"Query result, acquirer_code for refund txn : {refund_acquirer_code}")
            refund_auth_code = result['auth_code'].values[0]
            logger.debug(f"Query result, auth_code for refund txn : {refund_auth_code}")
            refund_customer_name = result['customer_name'].values[0]
            logger.debug(f"Query result, customer_name for refund txn : {refund_customer_name}")
            refund_payer_name = result['payer_name'].values[0]
            logger.debug(f"Query result, payer_name for refund txn : {refund_payer_name}")
            refund_rrn = result['rr_number'].values[0]
            logger.debug(f"Query result, rrn for refund txn : {refund_rrn}")
            refund_settle_status = result['settlement_status'].values[0]
            logger.debug(f"Query result, settlement_status for refund txn : {refund_settle_status}")
            refund_pmt_status = result['status'].values[0]
            logger.debug(f"Query result, status for refund txn : {refund_pmt_status}")
            refund_issuer_code_txn = result['issuer_code'].values[0]
            logger.debug(f"Query result, issuer_code for refund txn : {refund_issuer_code_txn}")
            refund_txn_type = result['txn_type'].values[0]
            logger.debug(f"Query result, txn_type for refund txn : {refund_txn_type}")
            refund_pmt_state = result['state'].values[0]
            logger.debug(f"Query result, state for refund txn : {refund_pmt_state}")
            refund_payment_gateway = result['payment_gateway'].values[0]
            logger.debug(f"Query result, payment_gateway for refund txn : {refund_payment_gateway}")
            refund_batch_number = result['batch_number'].values[0]
            logger.debug(f"Query result, batch_number for refund txn : {refund_batch_number}")
            refund_pmt_card_brand = result['payment_card_brand'].values[0]
            logger.debug(f"Query result, payment_card_brand for refund txn : {refund_pmt_card_brand}")
            refund_pmt_card_type = result['payment_card_type'].values[0]
            logger.debug(f"Query result, payment_card_type for refund txn : {refund_pmt_card_type}")
            refund_card_last_four_digit = result['card_last_four_digit'].values[0]
            logger.debug(f"Query result, card_last_four_digit for refund txn : {refund_card_last_four_digit}")
            refund_payment_mode = result['payment_mode'].values[0]
            logger.debug(f"Query result, payment_mode for refund txn : {refund_payment_mode}")
            refund_merchant_name = result['merchant_name'].values[0]
            logger.debug(f"Query result, merchant_name for refund txn : {refund_merchant_name}")
            refund_pmt_card_bin = result['payment_card_bin'].values[0]
            logger.debug(f"Query result, payment_card_bin for refund txn : {refund_pmt_card_bin}")
            refund_terminal_info_id_txn = result['terminal_info_id'].values[0]
            logger.debug(f"Query result, terminal_info_id for refund txn : {refund_terminal_info_id_txn}")
            refund_mid_txn = result['mid'].values[0]
            logger.debug(f"Query result, mid for refund txn : {refund_mid_txn}")
            refund_tid_txn = result['tid'].values[0]
            logger.debug(f"Query result, tid for refund txn : {refund_tid_txn}")
            refund_device_serial_txn = result['device_serial'].values[0]
            logger.debug(f"Query result, device_serial for refund txn : {refund_device_serial_txn}")
            refund_order_id_txn = result['external_ref'].values[0]
            logger.debug(f"Query result, order_id for refund txn : {refund_order_id_txn}")
            refund_card_txn_type = result['card_txn_type'].values[0]
            logger.debug(f"Query result, card_txn_type for refund txn : {refund_card_txn_type}")
            refund_org_code_txn = result['org_code'].values[0]
            logger.debug(f"Query result, org_code for refund txn : {refund_org_code_txn}")
            posting_date_2 = result['posting_date'].values[0]
            logger.debug(f"Fetching posting_date from txn table for refunded txn : {posting_date_2}")
            # ------------------------------------------------------------------------------------------------
            GlobalVariables.EXCEL_TC_Execution = "Pass"
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                refund_date_and_time = date_time_converter.to_app_format(posting_date_db=posting_date_2)
                expected_app_values = {
                    "txn_amt": "{:,.2f}".format(amount),
                    "currency_code": "AED",
                    "pmt_mode": "CARD",
                ...
                actual_app_values = {
                    ...
                    "currency_code_2": app_amount_refund.split(' ')[0],
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "auth_code_2": refund_auth_code,
                    "card_last_four_digit_2": "0029",
                    "customer_name_2": "L3TEST/CARD0002",
                    "ext_ref_number_2": order_id,
                    "merchant_name_2": refund_merchant_name,
                    "payer_name_2": "L3TEST/CARD0002",
                    "pmt_card_bin_2": "417666",
                    "card_type_2": "VISA",
                    "display_pan_2": "0029",
                    "name_on_card_2": "L3TEST/CARD0002"
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
                ...
                api_currency_code = response_original.get('currencyCode')
                logger.debug(f"Fetching Currency Code from response : {api_currency_code}")
                ...
                logger.debug(f"From response_refund fetch issuer_code for refund txn : {api_issuer_code_refund}")
                ...
                logger.debug(f"From response_refund fetch device_serial for refund txn : {api_device_serial_refund}")
                ...
                api_currency_code_refund = response_refund.get('currencyCode')
                logger.debug(f"Fetching Currency Code from response for refunded txn: {api_currency_code_refund}")
                ...
                actual_api_values = {
                    ...
                    "currency_code_2": api_currency_code_refund,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    ...
                    "txn_amt_2": refund_total_amount,
                    "currency_code_2": refund_currency_code,
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching date_time from portal txn history for the order_id : {order_id}, {portal_date_time_refund}")
                portal_txn_id_refund = transaction_details[0]['Transaction ID']
                logger.info(f"Fetching txn_id from portal txn history for the order_id : {order_id}, {portal_txn_id_refund}")
                portal_total_amount_refund = transaction_details[0]['Total Amount']
                logger.info(f"Fetching total_amount from portal txn history for the order_id : {order_id}, {portal_total_amount_refund}")
                portal_auth_code_refund = transaction_details[0]['Auth Code']
                logger.info(f"Fetching auth_code from portal txn history for the order_id : {order_id}, {portal_auth_code_refund}")
                portal_rrn_refund = transaction_details[0]['RR Number']
                logger.info(f"Fetching rrn from portal txn history for the order_id : {order_id}, {portal_rrn_refund}")
                portal_txn_type_refund = transaction_details[0]['Type']
                logger.info(f"Fetching txn_type from portal txn history for the order_id : {order_id}, {portal_txn_type_refund}")
                portal_txn_status_refund = transaction_details[0]['Status']
                logger.info(f"Fetching txn_status from portal txn history for the order_id : {order_id}, {portal_txn_status_refund}")
                portal_user_refund = transaction_details[0]['Username']
                logger.info(f"Fetching username from portal txn history for the order_id : {order_id}, {portal_user_refund}")

                actual_portal_values = {
                    "pmt_type": portal_txn_type,
                    "txn_amt": portal_total_amount.split(' ')[1],
                    "currency_code": portal_total_amount.split(' ')[0],
                    ...
                    "currency_code_2": portal_total_amount_refund.split(' ')[0],
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "time": txn_time,
                    "RRN": refund_rrn,
                    "AUTH CODE": str(refund_auth_code).strip(),
                    "CARD TYPE": "VISA",
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_265():
    """
    Sub Feature Code: UI_Common_Card_Refund_MASHREQ_EMV_MASTER_CreditCard_Without_Pin_541333
    Sub Feature Description:  Performing the EMV Refund transaction via MASHREQ PG using MASTER Credit card having
     16 digit PAN length without pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 265: TC265
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            card_page.select_cardtype(text="EMV_MASTER_CREDIT_541333")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_266():
    """
    Sub Feature Code: UI_Common_Card_Refund_MASHREQ_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description:  Performing the EMV Refund transaction via MASHREQ PG using Visa Credit card having
     16 digit PAN length with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 266: TC266
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
