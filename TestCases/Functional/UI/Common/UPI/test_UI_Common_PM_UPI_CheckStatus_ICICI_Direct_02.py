...
def test_common_100_101_305():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Txn_Through_App_For_ICICI_Direct
    Sub Feature Description: Generate QR through app and perform UPI successful txn via checkstatus for ICICI Direct pg.
    TC naming code description: 100: Payment Method, 101: UPI, 305: TC305
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching txn id from txn table is : {txn_id}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_common_100_101_306():
    """
    Sub Feature Code: UI_Common_PM_UPI_Failed_Txn_Through_App_For_ICICI_Direct
    Sub Feature Description: Generate QR through app and perform UPI failed txn via checkstatus for ICICI Direct pg.
    TC naming code description: 100: Payment Method, 101: UPI, 306: TC306
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_101_309():
    """
    Sub Feature Code: UI_Common_PM_UPI_ICICI_Direct_Success_Transaction_Same_Amount_Same_Order_ID
    Sub Feature Description: Perform UPI ICICI_DIRECT Successful Transactions Via CheckStatus Api
                            with same amount and order ID within duplicate payment interval time.
    TC naming code description: 100: Payment Method, 101: UPI, 309: TC309
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.info(f"Initiating transaction with same amount: {amount} and order id: {order_id} as previous "
                        f"transaction")
            logger.debug(f"initiating upi qr for the amount of {amount}")
            api_details_2 = DBProcessor.get_api_details('upiqrGenerate', request_body={
                "username": app_username,
                "password": app_password,
                "amount": str(amount),
                "orderNumber": str(order_id)
            ...
            logger.debug(f"response received for the upi qr generate api: {response_2}")
            success_2 = response_2['success']
            logger.debug(f"Fetching success value from the upi qr generate api, txn_id : {success_2}")
            error_message = response_2['errorMessage']
            logger.debug(f"Fetching message from the upi qr generate api, txn_id : {error_message}")
            error_code = response_2['errorCode']
            logger.debug(f"Fetching real code from the upi qr generate api, txn_id : {error_code}")
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_310():
    """
    Sub Feature Code: UI_Common_PM_UPI_CheckStatus_ICICI_Direct_Fractional_Amount
    Sub Feature Description: Performing UPI Successful txn with fractional amount Via CheckStatus API for ICICI_DIRECT pg
    TC naming code description: 100: Payment Method, 101: UPI, 310: TC310
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            amount = round(random.uniform(501, 1000), 2)
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.debug(f"Generating unique order ID  : {order_id}")
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for the upi qr generate : {response}")
            txn_id = response["txnId"]
            logger.debug(f"Fetching txn_id from the upi qr generate api, txn_id : {txn_id}")
