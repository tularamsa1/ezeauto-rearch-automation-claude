...
def test_common_100_113_011():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Callback_Success_Via_KOTAK_ATOS_MultiAcc_ActiveLabel
    Sub Feature Description: Multi Account - Verifying a bqrv4 bqr success callback via Kotak Atos static qr
    when label is active
    TC naming code description: 100: Payment method, 113: MultiAcc_StaticQR, 011: Testcase ID
    """
    try:
        ...
        query = f"select * from bharatqr_merchant_config where status = 'ACTIVE' AND " \
                f"bank_code = 'KOTAK_WL' AND acc_label_id=(select id from label " \
        ...
        logger.debug(f"fetched mid from the bharatqr_merchant_config table : mid : {mid}")
        tid = result['tid'].values[0]
        logger.debug(f"fetched tid from the bharatqr_merchant_config table : tid : {tid}")
        merchant_pan = result['merchant_pan'].values[0]
        logger.debug(f"fetched merchant_pan from the bharatqr_merchant_config table : merchant_pan : {merchant_pan}")
        config_id = result['id'].values[0]
        logger.debug(f"fetched config_id from the bharatqr_merchant_config table : config_id : {config_id}")
        bqr_acc_label_id = result['acc_label_id'].values[0]
        logger.debug(f"fetched acc_label_id from the bharatqr_merchant_config table : {bqr_acc_label_id}")
        terminal_info_id = result['terminal_info_id'].values[0]
        logger.debug(f"fetched terminal_info_id from the bharatqr_merchant_config table : {terminal_info_id}")
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"fetched result for upi_merchant_config table : {result}")
            ...
            logger.debug(f"fetched acc_label_id : {upi_acc_label_id}")

            # generating BQRV4 STATIC QR
            ...
            logger.debug(f"response received for generate_BQRV4_staticqr_KOTAK api is : {response}")
            generateqr_publish_id = response["publishId"]
            logger.debug(f"fetched publishId from the api response is : {generateqr_publish_id}")

            # preparing data to perform callback
            ...
            # Performing BQRV4_BQR callback for Kotak Atos Static QR
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for callbackKotak API is  : {response}")
            status = response['status']
            logger.debug(f"status is : {status}")

            query = f"select * from txn where org_code='{str(org_code)}' and rr_number='{str(rrn)}'order by created_time" \
                    f" desc limit 1; "
            ...
            label_ids_db = str(result['label_ids'].values[0]).strip(',')
            logger.debug(f"label_ids is : {label_ids_db}")
            ...
            try:
                ...
                for elements in response_in_list:
                    if elements["txnId"] == txn_id:
                        ...
                        account_label_name_api = elements["accountLabel"]
                        logger.debug(f"account_label_name_api is : {account_label_name_api}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "bqr_bank_code": "KOTAK",
                    "acc_label_id": str(bqr_acc_label_id),
                }
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_113_012():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_StaticQR_Callback_Success_Via_KOTAK_ATOS_MultiAcc_InactiveLabel
    Sub Feature Description: Multi Account - Verifying a bqrv4 bqr success callback via Kotak Atos static qr
    when label is inactive
    TC naming code description: 100: Payment method, 113: MultiAcc_StaticQR, 012: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
