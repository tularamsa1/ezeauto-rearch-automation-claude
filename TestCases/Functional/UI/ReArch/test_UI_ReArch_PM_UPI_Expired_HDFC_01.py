...
@pytest.mark.apiVal
@pytest.mark.appVal
def test_common_rearch_0024():
    """
    Sub Feature Code: UI_ReArch_PM_UPI_Expired_HDFC
    Sub Feature Description:
        Launch ReArch app, handle login if required, enter amount 95,
        select UPI, proceed through Order Details overlay, wait for QR screen,
        cancel payment (HDFC simulator Expired scenario), verify app returns to
        enter amount screen, navigate to transaction history, then validate via
    ...
    NL Source Steps:
      ...
      Test Steps:
        ...
        7.  click proceed
        8.  wait for qr screen
        9.  cancel payment (HDFC simulator Expired scenario)
        10. click on history button on enter amount screen
        11. do app and api validation

      NOTE: No charge slip validation — expired txns have no RRN/auth code.
    ...
    try:
        ...
        try:
            ...
            # Step 3-4: Wait for amount screen and enter amount
            ...
            # Step 6-7: Wait for Order Details overlay → Proceed
            ...
            # Step 8: Wait for QR screen
            qr_page = ReArchQRPage(app_driver)
            qr_page.wait_for_qr_screen()
            logger.debug("QR screen confirmed")

            # Step 9: Cancel payment — HDFC simulator Expired scenario
            time.sleep(7)
            qr_page.cancel_payment()
            logger.info("Payment cancelled — HDFC simulator expired scenario")

            # App returns to enter amount screen after expired cancel
            home_page.wait_for_home_page_load()
            logger.debug("Returned to home/amount screen after cancel")

            # Resolve txn_id from DB
            time.sleep(2)
            query = (
            ...
            # Step 10: Click history button on enter amount screen
            home_page.click_txn_history()
            logger.debug("Clicked Transaction History from amount screen")
        ...
        if ConfigReader.read_config("Validations", "api_validation") == "True":
            ...
            try:
                ...
                expected_api_values = {
                    "pmt_status":    "EXPIRED",        # TODO: verify on first run (EXPIRED or FAILED)
                    "txn_amt":       float(amount),
                    "pmt_mode":      "UPI",
                    "txn_type":      "CHARGE",
                    "acquirer_code": "HDFC",           # TODO: verify on first run
                    "org_code":      org_code,
                    "date":          date,
                }
