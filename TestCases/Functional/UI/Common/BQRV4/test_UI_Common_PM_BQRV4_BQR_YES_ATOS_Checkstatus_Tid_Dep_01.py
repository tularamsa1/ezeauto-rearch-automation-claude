...
def test_common_100_102_309():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_BQR_Checkstatus_Success_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_BQR Checkstatus Success via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 309: TC309
    """
    try:
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response obtained for BQR Payment Status : {response}")
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_102_310():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_BQR_CheckStatus_After_Expiry_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_BQR Expired Check status transaction when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 310: TC310
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"API details  : {api_details} ")
        response = APIProcessor.send_request(api_details)
        logger.debug(f"Response received for UPI Disabled in preconditions is : {response}")
        ...
        try:
            ...
            rrn = "RE" + txn_id.split('E')[1]
            logger.debug(f"rrn value is : {rrn}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Response after filtering data of second txn_id is : {response_2}")
                status_api_new = response_2["status"]
                logger.debug(f"Value of status obtained from second txn_id is : {status_api_new}")
                amount_api_new = float(response_2["amount"])
                logger.debug(f"Value of amount obtained from second txn_id is : {amount_api_new}")
                payment_mode_api_new = response_2["paymentMode"]
                logger.debug(f"Value of paymentMode obtained from second txn_id is : {payment_mode_api_new}")
                state_api_new = response_2["states"][0]
                logger.debug(f"Value of states obtained from second txn_id is : {state_api_new}")
                rrn_api_new = response_2["rrNumber"]
                logger.debug(f"Value of rrNumber obtained from second txn_id is : {rrn_api_new}")
                settlement_status_api_new = response_2["settlementStatus"]
                logger.debug(f"Value of settlementStatus obtained from second txn_id is : {settlement_status_api_new}")
                issuer_code_api_new = response_2["issuerCode"]
                logger.debug(f"Value of issuerCode obtained from second txn_id is : {issuer_code_api_new}")
                acquirer_code_api_new = response_2["acquirerCode"]
                logger.debug(f"Value of acquirerCode obtained from second txn_id is : {acquirer_code_api_new}")
                org_code_api_new = response_2["orgCode"]
                logger.debug(f"Value of orgCode obtained from second txn_id is : {org_code_api_new}")
                mid_api_new = response_2["mid"]
                logger.debug(f"Value of mid obtained from second txn_id is : {mid_api_new}")
                tid_api_new = response_2["tid"]
                logger.debug(f"Value of tid obtained from second txn_id is : {tid_api_new}")
                txn_type_api_new = response_2["txnType"]
                logger.debug(f"Value of txnType obtained from second txn_id is : {txn_type_api_new}")
                auth_code_api_new = response_2["authCode"]
                logger.debug(f"Value of authCode obtained from second txn_id is : {auth_code_api_new}")
                date_api_new = response_2["createdTime"]
                logger.debug(f"Value of createdTime obtained from second txn_id is : {date_api_new}")
                device_serial_api_new = response_2["deviceSerial"]
                logger.debug(f"Value of deviceSerial obtained from second txn_id is : {device_serial_api_new}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Query to fetch data from bharatqr_txn table for another txn_id : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result of bharatqr_txn table for another txn_id : {result}")
                bqr_status_db_new = result["status_code"].iloc[0]
                logger.debug(f"Value of status_code obtained from bharatqr_txn table  for another txn_id : {bqr_status_db_new}")
                bqr_state_db_new = result["state"].iloc[0]
                logger.debug(f"Value of state obtained from bharatqr_txn table  for another txn_id : {bqr_state_db_new}")
                bqr_amount_db_new = float(result["txn_amount"].iloc[0])
                logger.debug(f"Value of txn_amount obtained from bharatqr_txn table  for another txn_id : {bqr_amount_db_new}")
                bqr_txn_type_db_new = result["txn_type"].iloc[0]
                logger.debug(f"Value of txn_type obtained from bharatqr_txn table  for another txn_id : {bqr_txn_type_db_new}")
                brq_terminal_info_id_db_new = result["terminal_info_id"].iloc[0]
                logger.debug(f"Value of terminal_info_id obtained from bharatqr_txn table  for another txn_id : {brq_terminal_info_id_db_new}")
                bqr_bank_code_db_new = result["bank_code"].iloc[0]
                logger.debug(f"Value of bank_code obtained from bharatqr_txn table  for another txn_id : {bqr_bank_code_db_new}")
                bqr_merchant_config_id_db_new = result["merchant_config_id"].iloc[0]
                logger.debug(f"Value of merchant_config_id obtained from bharatqr_txn table  for another txn_id : {bqr_merchant_config_id_db_new}")
                bqr_txn_primary_id_db_new = result["transaction_primary_id"].iloc[0]
                logger.debug(f"Value of transaction_primary_id obtained from bharatqr_txn table  for another txn_id : {bqr_txn_primary_id_db_new}")
                bqr_rrn_db_new = result['rrn'].values[0]
                logger.debug(f"Value of rrn obtained from bharatqr_txn table  for another txn_id : {bqr_rrn_db_new}")
                bqr_org_code_db_new = result['org_code'].values[0]
                logger.debug(f"Value of org_code obtained from bharatqr_txn table  for another txn_id : {bqr_org_code_db_new}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_values = {'PAID BY:': 'BHARATQR', 'merchant_ref_no': 'Ref # ' + str(order_id),
                                   'RRN': str(rrn),'BASE AMOUNT:': "Rs." + str(amount) + ".00",
                                   'date': txn_date,'time': txn_time,'AUTH CODE': auth_code_new
                                   }
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_102_311():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_BQR_Checkstatus_Amount_Mismatch_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_BQR Amount mismatch Checkstatus Success via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 311: TC311
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Query result of txn table : {result}")
            amount_2 = '418'
            logger.debug(f"Value of amount_2 obtained from txn table : {amount_2}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Value of auth_code obtained from txn table : {auth_code}")
            ...
            logger.debug(f"Value of amount_original obtained from txn table : {amount_original_db}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "mid": mid,
                    "tid": tid,
                    "org_code": org_code, "auth_code": auth_code,
                    "date": date,
                    'txn_amt': float(amount),
                    "device_serial": str(device_serial)
                }
                ...
                logger.debug(f"Value of states obtained from txnlist api is : {state_api}")
                rrn_api = response["rrNumber"]
                logger.debug(f"Value of rrnNumber obtained from txnlist api is : {rrn_api}")
                settlement_status_api = response["settlementStatus"]
                logger.debug(f"Value of settlementStatus obtained from txnlist api is : {settlement_status_api}")
                issuer_code_api = response["issuerCode"]
                logger.debug(f"Value of issuerCode obtained from txnlist api is : {issuer_code_api}")
                ...
                logger.debug(f"Value of amountOriginal obtained from txnlist api is : {amt_original_api}")
                device_serial_api = response["deviceSerial"]
                logger.debug(f"Value of device_serial obtained from txnlist api is : {device_serial_api}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                txn_date, txn_time = date_time_converter.to_chargeslip_format(created_time)
                expected_values = {'PAID BY:': 'BHARATQR', 'merchant_ref_no': 'Ref # ' + str(order_id),
                                   'RRN': str(rrn),'BASE AMOUNT:': "Rs." + str(amount_2) + ".00",
                                   'date': txn_date, 'time': txn_time,'AUTH CODE': auth_code
                                   }
