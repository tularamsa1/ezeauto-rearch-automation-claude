...
def test_common_100_116_029():
    """
    Sub Feature Code: UI_Common_Card_Sale_AXIS_ATOS_TLE_Pre_Auth_Opt_Out_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt out MC Chip KWD Pre auth transaction via AXIS_ATOS_TLEC PG using Credit card
    with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 029: TC029
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_030():
    """
    Sub Feature Code: UI_Common_Card_Sale_AXIS_ATOS_TLE_Pre_Auth_Release_Opt_Out_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt out MC Chip KWD Pre auth release transaction via AXIS_ATOS_TLE PG using
    Credit card with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 030: TC030
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_031():
    """
    Sub Feature Code: UI_Common_Card_Sale_AXIS_ATOS_TLEC_Pre_Auth_Confirm_Opt_Out_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt out MC Chip KWD Pre auth confirm transaction via AXIS_ATOS_TLE PG using
    Credit card with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 031: TC031
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "dcc_conversion_rate_2": "0.52480573",
