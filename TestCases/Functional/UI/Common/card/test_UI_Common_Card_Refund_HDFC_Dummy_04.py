...
def test_common_100_115_131():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the EMV Refund transaction via HDFC Dummy PG using VISA Credit card having
    16 digit PAN length with pin (bin:417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 131: TC131
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Query result, posting_date for refund txn : {refund_posting_date}")
            # ------------------------------------------------------------------------------------------------
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_132():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMV_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description: Performing the EMV Refund transaction via HDFC Dummy PG using MASTER Credit card having
    16 digit PAN length with pin (bin:541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 132: TC132
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_133():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMV_VISA_CreditCard_Without_Pin_417666
    Sub Feature Description: Performing the EMV Refund transaction via HDFC Dummy PG using VISA Credit card having
    16 digit PAN length without pin (bin:417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 133: TC133
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_134():
    """
    Sub Feature Code: UI_Common_Card_Refund_HDFC_Dummy_EMV_MASTER_CreditCard_Without_Pin_541333
    Sub Feature Description: Performing the EMV Refund transaction via HDFC Dummy PG using MASTER Credit card having
    16 digit PAN length without pin (bin:541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 134: TC134
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "CARD TYPE": "MasterCard",
                    "BATCH NO": str(refund_batch_number).strip(),
                    "TID": tid,
                    "payment_option": "REFUND"
                }
                logger.debug(f"expected_charge_slip_values: {expected_charge_slip_values}")
