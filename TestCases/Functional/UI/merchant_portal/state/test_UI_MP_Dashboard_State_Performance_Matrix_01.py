...
def test_mp_700_703_062():
    """
    Sub Feature Code: UI_MP_Performance_Matrix_top10_performer_for_below_20_nodes_as_State_head
    Sub Feature Description: Verifying Performance Matrix for TOP 10 performers when State head has logged in for all lower level hierarchies having  less than 20 nodes
    ...
    try:
        ...
        try:
            ...
            categories = ["'India'", "'West'", "'Maharastra'"]
            for category in categories:
                del names_and_volumes[category]
            top_performer_db = sum(names_and_volumes.values())
            top_performer_db_1 = "name :" + 'Mumbai_City_District' + " , volume :" + str(top_performer_db)
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_703_063():
    """
    Sub Feature Code: UI_MP_Performance_Matrix_top10_and_bottom10_performer_for_more_than_20_nodes_as_State_head
    Sub Feature Description: Verifying Performance Matrix for TOP 10  and BOTTOM 10 performers when State head has logged in for all lower level hierarchies having only more than 20 nodes
    TC naming code description:
    700: Merchant Portal
    703: State
    063: TC063
