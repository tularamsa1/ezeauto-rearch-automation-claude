...
def test_common_100_115_086():
    """
    Sub Feature Code: UI_Common_Card_Void_ATOS_TLE_EMVCTLS_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description:  Performing the EMVCTLS Void transaction via ATOS TLE PG using RUPAY Debit card having 16 digit
     PAN length with pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 086: TC086
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_087():
    """
    Sub Feature Code: UI_Common_Card_Void_ATOS_TLE_EMVCTLS_VISA_CreditCard_Without_Pin_417666
    Sub Feature Description:  Performing the EMVCTLS Void transaction via ATOS TLE PG using VISA Credit card having 16 digit
     PAN length without pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 087: TC087
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_088():
    """
    Sub Feature Code: UI_Common_Card_Void_ATOS_TLE_EMVCTLS_MASTER_CreditCard_Without_Pin_541333
    Sub Feature Description:  Performing the EMVCTLS Void transaction via ATOS TLE PG using MASTER Credit card having 16 digit
     PAN length without pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 088: TC088
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_089():
    """
    Sub Feature Code: UI_Common_Card_Void_ATOS_TLE_EMVCTLS_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description:  Performing the EMVCTLS Void transaction via ATOS TLE PG using MASTER Credit card having 16 digit
     PAN length with pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 089: TC089
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_174():
    """
    Sub Feature Code: UI_Common_Card_Void_ATOS_TLE_EMV_VISA_DebitCard_Without_Pin_476173
    Sub Feature Description:  Performing the EMV Void transaction via ATOS_TLE PG using Visa Debit card having
     16 digit PAN length without pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 174: TC174
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = f"select * from txn where org_code='{org_code}' and external_ref='{order_id}' order by created_time " \
                    f"desc limit 1 "
            logger.debug(f"Query to fetch data from txn table before voiding the txn : {query}")
            result = DBProcessor.getValueFromDB(query=query)
            logger.debug(f"Query to fetch result data from txn table before voiding the txn : {result}")
            txn_id = result["id"].iloc[0]
            logger.debug(f"Fetching Transaction id from txn table before voiding the txn : {txn_id} ")
            ...
            logger.debug(f"Query to fetch data from txn table : {query}")
            result = DBProcessor.getValueFromDB(query=query)
            logger.debug(f"Query to fetch result data from txn table : {result}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching auth code from txn table : {auth_code}")
            created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created_time from txn table : {created_time}")
