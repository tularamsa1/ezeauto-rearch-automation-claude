...
def test_common_100_115_01_016():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Online_Refund_HDFC_Dummy_EMVCTLS_MASTER_DebitCard_Without_pin_222360
    Sub Feature Description: Performing the EMVCTLS sale tip Refund Online transaction via HDFC Dummy PG using MASTER Debit
    card having 16 digit PAN length without pin (bin:222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 016: TC016
    """
    try:
        ...
        try:
            ...
            payment_page.is_payment_page_displayed_card_with_tip(amount=amount, order_id=order_id, tip_amt=tip_amount, device_serial=device_serial)
            ...
            logger.debug(f"to refund selecting the card type as : CTLS_MASTER_DEBIT_222360")
            card_page.select_cardtype(text="CTLS_MASTER_DEBIT_222360")
            logger.debug(f"to refund selected the card type as : CTLS_MASTER_DEBIT_222360")
            payment_page.click_on_proceed_homepage()
            ...
            amount_db = int(result["amount_original"].iloc[0])
            logger.debug(f"Fetching amount from txn table : {amount_db}")
            total_amount_db = int(result["amount"].iloc[0])
            logger.debug(f"Fetching total amount from txn table : {total_amount_db}")
            tip_amount_db = int(result["amount_additional"].iloc[0])
            logger.debug(f"Fetching tip amount from txn table : {tip_amount_db}")
            ...
            logger.debug(f"Fetching Transaction id from txn table for refunded txn : {txn_id_2} ")
            ...
            total_amount_db_2 = int(result["amount"].iloc[0])
            logger.debug(f"Fetching total amount from txn table for refunded txn : {total_amount_db_2}")
            tip_amount_db_2 = int(result["amount_additional"].iloc[0])
            logger.debug(f"Fetching tip amount from txn table for refunded txn : {tip_amount_db_2}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "or_amount": "{:,.2f}".format(total_amount),
                    "or_card_type_desc": "*0011 CTLS",
                    ...
                    "or_tid": tid_db,
                    "or_rrn": rrn,
                    "or_batch_number": batch_number_db,
                    "or_tip_amt": "{:,.2f}".format(tip_amount),
                    "or_ref1": order_id_db,
                    "or_device_serial_2": device_serial,
                    "or_amount_2": "{:,.2f}".format(total_amount),
                    "or_card_type_desc_2": "*0011 CTLS",
                    ...
                    "or_tid_2": tid_db_2,
                    "or_rrn_2": rrn_2,
                    "or_batch_number_2": batch_number_db_2,
                    "or_ref1_2": order_id_db_2
                }
                ...
                or_tip_amt = re.search(r'[0-9,\.]+', online_refund_txn_data["or_tip_amt"].split(" ")[0])
                or_tip_amt = or_tip_amt.group()
                ...
                logger.debug(
                    f"Fetching txn settlement_status from txn history for refunded txn : {txn_id_2}, {app_settlement_status_2}")
                ...
                logger.debug(
                    f"Fetching device serial number from txn history for refunded txn : {txn_id_2}, {app_device_serial_2}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "tip_amt_2": float(0),
                    "original_amt_2": float(total_amount),
                    "pmt_mode_2": "CARD",
                ...
                total_amount_api = float(response_1["amount"])
                logger.debug(f"Fetching total amount from response : {total_amount_api}")
                tip_amount_api = float(response_1["amountAdditional"])
                logger.debug(f"Fetching tip amount from response : {tip_amount_api}")
                amount_api = float(response_1["amountOriginal"])
                logger.debug(f"Fetching original amount from response : {amount_api}")
                ...
                total_amount_api_2 = float(response_2["amount"])
                logger.debug(f"Fetching total amount from response for refunded txn : {total_amount_api_2}")
                tip_amount_api_2 = float(response_2["amountAdditional"])
                logger.debug(f"Fetching tip amount from response for refunded txn : {tip_amount_api_2}")
                amount_api_2 = float(response_2["amountOriginal"])
                logger.debug(f"Fetching original amount from response for refunded txn : {amount_api_2}")
                ...
                actual_api_values = {
                    ...
                    "txn_amt": total_amount_api,
                    "tip_amt": tip_amount_api,
                    "original_amt": amount_api,
                    ...
                    "txn_amt_2": total_amount_api_2,
                    "tip_amt_2": tip_amount_api_2,
                    "original_amt_2": amount_api_2,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    "txn_amt": total_amount_db,
                    "tip_amt": tip_amount_db,
                    "original_amt": amount_db,
                    ...
                    "card_last_four_digit": card_last_four_digit_db,
                    "txn_amt_2": total_amount_db_2,
                    "tip_amt_2": tip_amount_db_2,
                    "original_amt_2": amount_db_2,
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                total_amount_portal_2 = transaction_details[0]['Total Amount'].split()
                ...
                actual_portal_values = {
                    ...
                    "txn_amt_2": total_amount_portal_2[1],
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "RRN": str(rrn_2),
                    "BASE AMOUNT:": "Rs." + str(total_amount) + ".00",
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_01_017():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Online_Refund_HDFC_Dummy_EMVCTLS_VISA_DebitCard_Without_pin_476173
    Sub Feature Description: Performing the EMVCTLS sale tip Refund Online transaction via HDFC Dummy PG using VISA Debit
    card having 16 digit PAN length without pin (bin:476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 017: TC017
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.perform_online_refund(password=app_password, card_last_four_digit=card_last_four_digit_db, device_serial=device_serial)
            logger.debug(f"to refund selecting the card type as : CTLS_VISA_DEBIT_476173")
            card_page.select_cardtype(text="CTLS_VISA_DEBIT_476173")
            logger.debug(f"to refund selected the card type as : CTLS_VISA_DEBIT_476173")
            payment_page.click_on_proceed_homepage()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "or_batch_number": batch_number_db,
                    "or_tip_amt": "{:,.2f}".format(tip_amount),
                    "or_customer_name": customer_name_db,
                    "or_ref1": order_id_db,
                    "or_device_serial_2": device_serial,
                    "or_amount_2": "{:,.2f}".format(total_amount),
                    "or_card_type_desc_2": "*0453 CTLS",
                    "or_date_time_2": date_and_time_app_4,
                    "or_status_2": "REFUNDED",
                    "or_ref3_2": device_serial,
                    "or_auth_code_name_2": auth_code_2,
                    ...
                    "or_rrn_2": rrn_2,
                    "or_batch_number_2": batch_number_db_2,
                    "or_customer_name_2": customer_name_db_2,
                    "or_ref1_2": order_id_db_2
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_01_018():
    """
    Sub Feature Code: UI_Common_Card_Sale_Tip_Online_Refund_HDFC_Dummy_EMVCTLS_RUPAY_DebitCard_Without_pin_608326
    Sub Feature Description: Performing the EMVCTLS sale tip Refund Online transaction via HDFC Dummy PG using RUPAY Debit
    card having 16 digit PAN length without pin (bin:608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 01: Sale+Tip, 018: TC018
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.perform_online_refund(password=app_password, card_last_four_digit=card_last_four_digit_db, device_serial=device_serial)
            logger.debug(f"to refund selecting the card type as : CTLS_RUPAY_DEBIT_608326")
            card_page.select_cardtype(text="CTLS_RUPAY_DEBIT_608326")
            logger.debug(f"to refund selected the card type as : CTLS_RUPAY_DEBIT_608326")
            payment_page.click_on_proceed_homepage()
