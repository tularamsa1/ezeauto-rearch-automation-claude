...
def test_common_100_115_07_132():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Kotak_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand fixed success transaction for an org (not ezetap) via HDFC
    Dummy PG for Kotak issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 132: TC132
    """
    try:
        ...
        logger.debug(
            f"Response received when emi, emi for client, offering_emi cashback, brand_emi and bocashback is enabled in preconditions settings : {response}")
        ...
        try:
            ...
            cal_monthly_emi_amt = (amount - subvention_value) * monthly_interest_rate * (
            ...
            total_interest = round((total_emi - (amount - subvention_value)), 2)
            ...
            emi_payback_fixed = result['emi_payback_fixed'].values[0]
            logger.debug(f"Fetching emi payback fixed amount from txn_emi table : {emi_payback_fixed}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "additional_payback": "{:,.2f}".format(emi_payback_fixed),
                    "loan_amt": "{:,.2f}".format(amount - subvention_value),
                ...
                app_additional_payback = txn_history_page.fetch_additional_payback_text()
                logger.debug(
                    f"Fetching additional cashback from txn history for the txn : {txn_id}, {app_additional_payback}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "loan_amt": float(amount - subvention_value),
                    ...
                    "subvention_amount": emi_payback_fixed,
                    "subvention_fixed": emi_payback_fixed,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    ...
                    "emi_cashback_fixed": emi_payback_fixed,
                    "emi_cashback_amount": emi_payback_amount,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_133():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_Kotak_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand percentage success transaction for an org (not ezetap) via HDFC
    Dummy PG for Kotak issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 133: TC133
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            cal_additional_payback = round((amount * (subvention_value / 100)), 2)
            logger.debug(f"cal_additional_payback is : {cal_additional_payback}")
            monthly_interest_rate = interest_rate / (12 * 100)
            logger.debug(f"monthly_interest_rate is : {monthly_interest_rate}")
            cal_monthly_emi_amt = (amount - cal_additional_payback) * monthly_interest_rate * (
                    (1 + monthly_interest_rate) ** emi_plan_in_months) / (
                                          (1 + monthly_interest_rate) ** emi_plan_in_months - 1)
            logger.debug(f"cal_monthly_emi_amt is : {cal_monthly_emi_amt}")
            monthly_emi = round(cal_monthly_emi_amt, 2)
            ...
            emi_original_amount = result['original_amount'].values[0]
            logger.debug(f"Fetching original_amount from txn_emi table : {emi_original_amount}")
            emi_payback_percentage = result['emi_payback_percent'].values[0]
            logger.debug(f"Fetching emi payback fixed amount from txn_emi table : {emi_payback_percentage}")
            emi_payback_amount = result['emi_payback_amount'].values[0]
            logger.debug(f"Fetching emi payback amount from txn_emi table : {emi_payback_amount}")
            emi_cashback_type = result['emi_cashback_type'].values[0]
            logger.debug(f"Fetching emi cashback type from txn_emi table : {emi_cashback_type}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "entity": "BRAND",
                    "subvention_type": "PAYBACK",
                    "subvention_amount": cal_additional_payback,
                    "subvention_fixed": cal_additional_payback,
                    "emi_interest_rate": interest_rate,
                    "total_amount_with_int": total_emi,
                    "product_serial": str(imei_no),
                    "scheme_name": subvention_scheme_name,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "emi_term": f"{emi_plan_in_months} month",
                    "emi_type": "BRAND_EMI",
                    "emi_cashback_fixed": api_subvention_amt,
                    ...
                    "subvention_value": api_subvention_amt,
                    "subvention_tenure": f"{emi_plan_in_months} month",
                    "subvention_discount_type": None if subvention_discount_type is None else "Additional"
                }
                logger.debug(f"expected_db_values: {expected_db_values}")
                ...
                actual_db_values = {
                    ...
                    "emi_txn_amt": emi_txn_amount,
                    "emi_original_amt": emi_original_amount,
                    "emi_cashback_fixed": cal_additional_payback,
                    "emi_cashback_amount": emi_payback_amount,
                    "emi_total_amount": emi_total_amount,
                    "emi_cashback_type": emi_cashback_type,
                    "emi_interest_rate": emi_interest_rate,
                    ...
                    "subvention_type": subvention_type,
                    "subvention_value_type": subvention_value_type,
                    "subvention_value": cal_additional_payback,
                    "subvention_tenure": subvention_tenure,
                    "subvention_discount_type": subvention_discount_type
                }
                logger.debug(f"actual_db_values: {actual_db_values}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Tenure": f"{emi_plan_in_months} month",
                    "Card Issuer": "Kotak Mahindra",
                    "Payback": str(float(emi_payback_percentage)) + "%",
                    "Total Payback Amount": "Rs." + "{:.2f}".format(emi_payback_amount),
                    "Rate of Interest(P.A.)": f"{interest_rate}%",
                    "Interest Amt": "Rs." + "{:.2f}".format(total_interest),
                    "EMI Amt": "Rs." + "{:.2f}".format(monthly_emi),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_134():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_Kotak_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Cashback Discount payback by brand percentage success transaction for an org (not ezetap)
    via HDFC Dummy PG for Kotak issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 134: TC134
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='CASHBACK' and name LIKE '{brand_name}_KOTAK_DEBIT%'"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_138():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_Kotak_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Cashback Discount payback by brand percentage success transaction for an org (not ezetap)
    via HDFC Dummy for Kotak issuer PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 138: TC138
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='INSTANT' and name LIKE '{brand_name}_KOTAK_DEBIT%'"
        logger.debug(f"Query to fetch data from the rule table : {query}")
        result = DBProcessor.getValueFromDB(query=query, db_name='rule_engine')
        logger.debug(f"Fetching result from rule table :{result}")
        rule_id = result['id'].values[0]
        ...
        try:
            ...
            monthly_interest_rate = interest_rate / (12 * 100)
            logger.debug(f"monthly_interest_rate is : {monthly_interest_rate}")
            cal_monthly_emi_amt = (final_amount - cal_additional_payback) * monthly_interest_rate * (
                        (1 + monthly_interest_rate) ** emi_plan_in_months) / (
                                          (1 + monthly_interest_rate) ** emi_plan_in_months - 1)
            logger.debug(f"cal_monthly_emi_amt is : {cal_monthly_emi_amt}")
            monthly_emi = round(cal_monthly_emi_amt, 2)
            ...
            total_emi = round((monthly_emi * emi_plan_in_months), 2)
            logger.debug(f"total_emi amount is : {total_emi}")
            total_interest = round((total_emi - (final_amount - cal_additional_payback)), 2)
            logger.debug(f"total_interest amount is : {total_interest}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "org_code_bo": org_code,
                    "txn_amount_bo": float(final_amount),
                    "payment_mode_bo": "CARD",
                    "bo_txn_type": "BRAND_EMI",
                    "orig_amt": float(final_amount)
                }
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_142():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_Root_Org_HDFC_Dummy_KOTAK_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand fixed success transaction for root org via HDFC
    Dummy PG for kotak issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 142: TC142
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='ALL' and card_type= 'DEBIT' and bank='KOTAK' and eze_emi_enabled=b'0' order by created_time desc limit 1;"
        logger.debug(f"Query to fetch data from the subvention_plan table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for subvention_plan table : {result}")
        subvention_plan_id = result['id'].values[0]
        ...
        query = f"update subvention_plan set status = 0 where brand_id ='{brand_id}' and bank='KOTAK' and org_code='{org_code}'"
        logger.debug(f"Query to update subvention_plan with status as INACTIVE for an org code : {query}")
        result = DBProcessor.setValueToDB(query)
        ...
        testsuite_teardown.update_emi_status_for_org(root_org_code, 'DEBIT', 'ACTIVE')
        ...
        try:
            ...
            monthly_interest_rate = interest_rate / (12 * 100)
            logger.debug(f"monthly_interest_rate is : {monthly_interest_rate}")
            cal_monthly_emi_amt = (amount - subvention_value) * monthly_interest_rate * ((1 + monthly_interest_rate) ** emi_plan_in_months) / (
                                              (1 + monthly_interest_rate) ** emi_plan_in_months - 1)
            logger.debug(f"cal_monthly_emi_amt is : {cal_monthly_emi_amt}")
            monthly_emi = round(cal_monthly_emi_amt, 2)
            logger.debug(f"Calculated monthly_emi amount : {monthly_emi}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching total emi amount from txn history for the txn : {txn_id}, {app_total_emi_amount}")
                app_additional_payback = txn_history_page.fetch_additional_payback_text()
                logger.debug(f"Fetching additional cashback from txn history for the txn : {txn_id}, {app_additional_payback}")
                app_loan_amount = txn_history_page.fetch_loan_amount_text()
                logger.debug(f"Fetching loan amount from txn history for the txn : {txn_id}, {app_loan_amount}")
                app_interest_amount = txn_history_page.fetch_interest_amount_text()
                logger.debug(f"Fetching interest amount from txn history for the txn : {txn_id}, {app_interest_amount}")
        ...
        query = f"update subvention_plan set status = 1 where brand_id ='{brand_id}' and bank='KOTAK' and org_code='{org_code}'"
