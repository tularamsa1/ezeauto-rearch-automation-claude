...
def test_common_100_101_223():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Callback_Before_QR_Expiry_AutoRefund_Enabled_Razorpay
    Sub Feature Description: Performing a  upi success callback via Razorpay before qr expiry when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 223: TC223
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_224():
    """
    Sub Feature Code: UI_Common_PM_2_Pure_UPI_Success_Callback_Before_QR_Expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Performing two  upi success callback via Razorpay before expiry the qr when autorefund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 224: TC224
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(
                f"Response received for razorpay_callback_generator_HMAC api for 2nd callback is : {response}")
            razorpay_signature_2 = response['razorpay_signature']
            logger.debug(
                f"razorpay_signature received for razorpay_callback_generator_HMAC api"
                f" for 2nd callback is : {razorpay_signature_2}")
            ...
            logger.debug(f"fetched rr_number from txn table is : {rrn_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_225():
    """
    Sub Feature Code: UI_Common_PM_2_Pure_UPI_Success_Callback_Before_QR_Expiry_Razorpay_AutoRefund_Enabled
    Sub Feature Description: Performing two  upi success callback via Razorpay before expiry the qr when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 225: TC225
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(
                f"razorpay_signature received for razorpay_callback_generator_HMAC api"
                f" for 2nd callback is : {razorpay_signature_2}")
            logger.debug(f"performing upi 2nd callback for razorpay")
            api_details = DBProcessor.get_api_details('upi_confirm_razorpay',
                                                      request_body=api_details_hmac_2['RequestBody'])
            api_details['Header'] = {'x-razorpay-signature': razorpay_signature_2,
                                     'Content-Type': 'application/json'}
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_231():
    """
    Sub Feature Code: UI_Common_PM_UPI_2_Failed_Callback_Before_QR_Expiry_Razorpay_AutoRefund_Enabled
    Sub Feature Description: Performing two  upi failed callback via Razorpay before expiry the qr when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 231: TC231
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_232():
    """
    Sub Feature Code: UI_Common_PM_UPI_2_Failed_Callback_After_QR_Expiry_Razorpay_AutoRefund_Enabled
    Sub Feature Description: Performing two  upi failed callback via Razorpay after expiry the qr when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 232: TC232
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
