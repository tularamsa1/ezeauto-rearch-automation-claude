...
from Utilities import Validator, ConfigReader, APIProcessor, DBProcessor, ResourceAssigner, date_time_converter, receipt_validator
...
def test_common_100_102_312():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_BQR_Two_Callback_After_Expired_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4_BQR generation using API and performing two Callback after QR Expiry transaction when auto refund enabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 312: TC312
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Value of txn_id obtained from txn table for first callback : {txn_id_new}")
            rrn_new = result['rr_number'].values[0]
            logger.debug(f"Value of rr_number obtained from txn table for first callback : {rrn_new}")
            auth_code_new = result['auth_code'].values[0]
            logger.debug(f"Value of auth_code obtained from txn table for first callback : {auth_code_new}")
            created_time_new = result['created_time'].values[0]
            logger.debug(f"Value of created_time obtained from txn table for first callback : {created_time_new}")
            customer_name_new = result['customer_name'].values[0]
            logger.debug(f"Value of customer_name obtained from txn table for first callback : {customer_name_new}")
            payer_name_new = result['payer_name'].values[0]
            logger.debug(f"Value of payer_name obtained from txn table for first callback : {payer_name_new}")
            amount_db_new = int(result["amount"].iloc[0])
            logger.debug(f"Value of amount obtained from txn table for first callback : {amount_db_new}")
            payment_mode_db_new = result["payment_mode"].iloc[0]
            logger.debug(f"Value of payment_mode obtained from txn table first callback : {payment_mode_db_new}")
            payment_status_db_new = result["status"].iloc[0]
            logger.debug(f"Value of status obtained from txn table for first callback : {payment_status_db_new}")
            payment_state_db_new = result["state"].iloc[0]
            logger.debug(f"Value of state obtained from txn table for first callback : {payment_state_db_new}")
            acquirer_code_db_new = result["acquirer_code"].iloc[0]
            logger.debug(f"Value of acquirer_code obtained from txn table for first callback : {acquirer_code_db_new}")
            bank_name_db_new = result["bank_name"].iloc[0]
            logger.debug(f"Value of bank_name obtained from txn table for first callback : {bank_name_db_new}")
            payer_name_db_new = result["payer_name"].iloc[0]
            logger.debug(f"Value of payer_name obtained from txn table for first callback : {payer_name_db_new}")
            mid_db_new = result["mid"].iloc[0]
            logger.debug(f"Value of mid obtained from txn table for first callback : {mid_db_new}")
            tid_db_new = result["tid"].iloc[0]
            logger.debug(f"Value of tid obtained from txn table for first callback : {tid_db_new}")
            payment_gateway_db_new = result["payment_gateway"].iloc[0]
            logger.debug(f"Value of payment_gateway obtained from txn table for first callback : {payment_gateway_db_new}")
            rr_number_db_new = result["rr_number"].iloc[0]
            logger.debug(f"Value of rr_number obtained from txn table for first callback : {rr_number_db_new}")
            settlement_status_db_new = result["settlement_status"].iloc[0]
            logger.debug(f"Value of settlement_status obtained from txn table for first callback : {settlement_status_db_new}")
            device_serial_db_new = result['device_serial'].values[0]
            logger.debug(f"Value of device_serial obtained from txn table for first callback : {device_serial_db_new}")

            rrn_new_3 = "RE" + str(order_id)
            logger.debug(f"Value of rrn for second Callback : {rrn_new_3} ")
            ...
            logger.debug(f"Value of rr_number obtained from txn table for second callback : {rrn_new_3}")
