...
def test_common_100_101_189():
    """
    Sub Feature Code: TID_Dep_UI_Common_PM_Pure_UPI_Refund_Posted_via_API_AXIS_DIRECT
    Sub Feature Description: TID Dep- Verification of  a upi refund posted via Axis acq and Axis PG where upi config is Axis_Direct
    TC naming code description: 100: Payment Method, 101: UPI, 189: TC189
    """
    try:
        ...
        try:
            ...
            logger.debug(f"fetched txn_type from txn table is : {txn_type_db}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                customer_name_api_original = response["customerName"]
                payer_name_api_original = response["payerName"]
                ...
                amount_api_refunded = (response_1["amount"])
                ...
                customer_name_api_refunded = response_1["customerName"]
                payer_name_api_refunded = response_1["payerName"]
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_chargeslip_values = {'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id),
                                              'RRN': str(rrn), 'BASE AMOUNT:': "Rs." + str(amount),
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_190():
    """
    Sub Feature Code: TID_Dep_UI_Common_PM_Pure_UPI_Refund_Failed_via_API_AXIS_DIRECT
    Sub Feature Description: TID Dep - Verification of  a upi refund failed via Axis acq and Axis PG where upi config is Axis_Direct
    TC naming code description: 100: Payment Method, 101: UPI, 190: TC190
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                response_1 = [x for x in resp["txns"] if x["txnId"] == refund_txn_id][0]
                logger.debug(f"Response after filtering data of current txn is : {resp}")
                status_api_refunded = response_1["status"]
                amount_api_refunded = (response_1["amount"])
                payment_mode_api_refunded = response_1["paymentMode"]
                state_api_refunded = response_1["states"][0]
                ...
                txn_type_api_refunded = response_1["txnType"]
                date_api_refunded = response_1["createdTime"]
                order_id_refunded = response_1["orderNumber"]
