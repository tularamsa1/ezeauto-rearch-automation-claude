...
def test_sa_100_109_012():
    """
    Sub Feature Code: UI_SA_PM_UPI_Success_Via_SA_CheckStatus_MultiAcc_APB
    Sub Feature Description: Multi Account - Verification of a successful upi txn via APB using SA check status
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 012: TC012
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info("Fetching txn_id from txn history for the txn : {txn_id}, {app_rrn}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_sa_100_109_013():
    """
    Sub Feature Code: UI_SA_PM_UPI_Failed_Via_SA_CheckStatus_MultiAcc_APB
    Sub Feature Description: Multi Account - Verification of a failed UPI txn via APB using SA check status
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 013: TC013
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_sa_100_109_016():
    """
    Sub Feature Code: UI_SA_PM_UPI_Success_Via_SA_CheckStatus_with_2nd_label_MultiAcc_APB
    Sub Feature Description: Multi Account - Performing a successful upi txn via APB using SA check status with second account(ex:acc2 label)
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 016: TC016
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
