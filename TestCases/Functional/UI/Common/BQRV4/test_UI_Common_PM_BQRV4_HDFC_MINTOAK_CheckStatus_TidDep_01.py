...
def test_common_100_102_415():
    """
    Sub Feature Code: Tid_dep_UI_Common_BQRV4_UPI_Success_Checkstatus_HDFC_MINTOAK
    Sub Feature Description: Tid dep - Verification of a BQRV4 successful CHeck status HDFC_MINTOAK
    TC naming code description: 100: Payment Method, 112: BQR, 415: TC415
    """
    try:
        ...
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"result for query {query} : {result}")
        ...
        try:
            ...
            logger.debug(f"Fetching txn_id, rrn, posting date, auth code from the txn table :"
                         f"{txn_id}, {rrn}, {created_time}", {auth_code})
            ...
            logger.debug(
                f"Fetching status_db,payment_mode_db, amount_db, state_db, payment_gateway_db, acquirer_code_db, bank_code_db, settlement_status_db, mid_db, tid_db, device_serial_db,order_id_db from database for "
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching device serial from api: {device_serial_api}")
                order_id_api = response["orderNumber"]
                logger.debug(f"Fetching order id from api: {order_id_api}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching upi txn type db: {upi_txn_type_db}")
                ...
                logger.debug(f"Fetching bqr status from db: {bqr_status_db}")
                bqr_state_db = result["state"].values[0]
                logger.debug(f"Fetching bqr state from db: {bqr_state_db}")
                bqr_amount_db = float(result["txn_amount"].values[0])
                logger.debug(f"Fetching bqr amount from db: {bqr_amount_db}")
                bqr_txn_type_db = result["txn_type"].values[0]
                logger.debug(f"Fetching bqr txn type from db: {bqr_txn_type_db}")
                brq_terminal_info_id_db = result["terminal_info_id"].values[0]
                logger.debug(f"Fetching bqr terminal info id from db: {brq_terminal_info_id_db}")
                bqr_bank_code_db = result["bank_code"].values[0]
                logger.debug(f"Fetching bqr bank code from db: {bqr_bank_code_db}")
                bqr_merchant_config_id_db = result["merchant_config_id"].values[0]
                logger.debug(f"Fetching bqr merchant config id from db: {bqr_merchant_config_id_db}")
                bqr_txn_primary_id_db = result["transaction_primary_id"].values[0]
                logger.debug(f"Fetching bqr txn primary id from db: {bqr_txn_primary_id_db}")
                bqr_org_code_db = result['org_code'].values[0]
                logger.debug(f"Fetching bqr org code from db: {bqr_org_code_db}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_102_420():
    """
    Sub Feature Code: Tid_Dep_UI_Common_BQRV4_UPI_Checkstatus_Amount_Mismatch_HDFC_MINTOAK
    Sub Feature Description: Tid Dep - Verification of a BQRV4 UPI Amount mismatch upon Checkstatus  via HDFC_MINTOAK
    TC naming code description: 100: Payment Method, 102: BQR, 420: TC420
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.debug(f"initiating upi qr for the order id: {order_id}")
            ...
            logger.debug(f"Fetching Txn_id from the API_OUTPUT, Txn_id : {txn_id}")
            logger.debug("Waiting for 75 sec to QR code to get expired")
            time.sleep(75)
            ...
            logger.debug(f"Fetching posting_date from database : {posting_date}")
            created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created time from database : {created_time}")
            amount_db = int(result["amount"].values[0])
            logger.debug(f"Fetching amount from database : {amount_db}")
            payment_mode_db = result["payment_mode"].values[0]
            logger.debug(f"Fetching payment mode from database : {payment_mode_db}")
            payment_status_db = result["status"].values[0]
            logger.debug(f"Fetching payment status from database : {payment_status_db}")
            payment_state_db = result["state"].values[0]
            logger.debug(f"Fetching payment state from database : {payment_state_db}")
            acquirer_code_db = result["acquirer_code"].values[0]
            logger.debug(f"Fetching acquirer code from database : {acquirer_code_db}")
            mid_db = result["mid"].values[0]
            logger.debug(f"Fetching mid from database : {mid_db}")
            tid_db = result["tid"].values[0]
            logger.debug(f"Fetching tid from database : {tid_db}")
            payment_gateway_db = result["payment_gateway"].values[0]
            logger.debug(f"Fetching payment gateway from database : {payment_gateway_db}")
            settlement_status_db = result["settlement_status"].values[0]
            logger.debug(f"Fetching settlement status from database : {settlement_status_db}")
            device_serial_db = result['device_serial'].values[0]
            logger.debug(f"Fetching device serial from database : {device_serial_db}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching auth code from database : {auth_code}")
            rrn = result['rr_number'].values[0]
            logger.debug(f"Fetching rrn from database : {rrn}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching tid from api : {tid_api}")
                txn_type_api = response["txnType"]
                logger.debug(f"Fetching txn type code from api : {txn_type_api}")
                date_api = response["postingDate"]
                logger.debug(f"Fetching date from api : {date_api}")
                device_serial_api = response["deviceSerial"]
                logger.debug(f"Fetching device serial code from api : {device_serial_api}")
