...
from Utilities import Validator, ConfigReader, DBProcessor, APIProcessor, receipt_validator, ResourceAssigner, \
    date_time_converter, merchant_creator, ReportProcessor
...
def test_common_100_103_168():
    ...
    TC naming code description:100: Payment Method,103: RemotePay,168: TC_168
    """
    try:
        ...
        try:
            ...
            logger.debug(f"results from the txn table : rrn : {rrn}")
            txn_id = result['id'].values[0]
            logger.debug(f"results from the txn table : txn_id : {txn_id}")
            status = result['status'].values[0]
            logger.debug(f"results from the txn table : status : {status}")
            ...
            logger.debug(f"results from the txn table : mid : {mid}")
            tid = result['tid'].values[0]
            logger.debug(f"results from the txn table : tid : {tid}")
            org_code_txn = result['org_code'].values[0]
            logger.debug(f"results from the txn table : org_code_txn : {org_code_txn}")
            txn_type = result['txn_type'].values[0]
            logger.debug(f"results from the txn table : txn_type : {txn_type}")
...
def test_common_100_103_169():
    ...
    TC naming code description:100: Payment Method,103: RemotePay,169: TC_169
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.info(f"Completed Precondition setup for the test case : {testcase_id}")
        # -----------------------------PreConditions(Setup to be done for the test case)--------------------------
        Configuration.configureLogCaptureVariables(apiLog=True, portalLog=True, cnpwareLog=True, middlewareLog=False, config_log=False)
        ...
        try:
            ...
            logger.debug(f"Querying auth_code from txn is : {auth_code}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
                ...
                try:
                    ...
                    result = DBProcessor.getValueFromDB(query)
                    logger.debug(f"Query result of upi_merchant_config is : {result}")
                    mid_db = result['mid'].iloc[0]
                    logger.debug(f"mid_db is : {mid_db}")
                    tid_db = result['tid'].iloc[0]
                    logger.debug(f"tid_db is : {tid_db}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Exception e is {e}")
            ...
            try:
                ...
                setting_value = None
                logger.error(f"Exception NameError is {e}")
            except IndexError as e:
                setting_value = None
                logger.error(f"Exception IndexError is {e}")
            except Exception as e:
                logger.error(f"Exception e is {e}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                expected_portal_values = {
                    ...
                    "username": app_username,
                    "txn_id": txn_id,
                    "rrn": "-" if txn_rrn_number is None else str(txn_rrn_number),
