...
def test_common_100_115_07_071():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Cashback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    ...
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 071: TC071
    """
    try:
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='CASHBACK' and name LIKE '{brand_name}_CANARA_CREDIT%'"
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_092():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Txn_With_CardType_UNKNOWN_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenture
    Sub Feature Description: Performing the brand EMI payback by brand fixed transaction with CardType UNKNOWN for an
    org via HDFC Dummy PG for canara issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 092: TC092
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_093():
    """
    ...
    try:
        ...
        try:
            ...
            card_page.select_cardtype("EMV_WITH_PIN_VISA_CREDIT_417666")
            logger.debug(f"Selected the card type as : EMV_WITH_PIN_VISA_CREDIT_417666")
            actual_error_msg = payment_page.fetch_error_msg_brand_emi()
            logger.info(f"No EMI options message is, {actual_error_msg}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                actual_app_values = {
                    "no_emi_options": actual_error_msg
                }
                logger.debug(f"actual_app_values: {actual_app_values}")
                Validator.validateAgainstAPP(expectedApp=expected_app_values, actualApp=actual_app_values)
            except Exception as e:
