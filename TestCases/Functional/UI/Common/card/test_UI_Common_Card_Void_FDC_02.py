...
def test_common_100_115_033():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_CTLS_WITH_PIN_RUPAY_DEBIT_608326
    Sub Feature Description:  Performing the EMV void transaction via FDC PG using CTLS With Pin RUPAY Debit card
    having 16 digit PAN length with pin (bin: 608326)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 033: TC033
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_034():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_CTLS_VISA_CREDIT_417666
    Sub Feature Description:  Performing the EMV void transaction via FDC PG using CTLS VISA Credit card
    having 16 digit PAN length with pin (bin: 417666)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 034: TC034
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_035():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_CTLS_MASTER_CREDIT_541333
    Sub Feature Description:  Performing the EMV void transaction via FDC PG using CTLS MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 017: TC0135
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_036():
    """
    Sub Feature Code: UI_Common_Card_Void_FDC_CTLS_WITH_PIN_MASTER_CREDIT_541333
    Sub Feature Description:  Performing the EMV void transaction via HDFC PG using CTLS with pin MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 036: TC036
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_115_036():
    """
    Sub Feature Code: UI_Common_Card_Void_Dummy_CTLS_VISA_CREDIT_417666
    Sub Feature Description:  Performing the EMV void transaction via HDFC PG using CTLS with pin MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 036: TC036
    """
    try:
        ...
        try:
            ...
            logger.debug(f"response received for the Settlement api is : {settle_response}")

            home_page.check_home_page_logo()
            home_page.wait_for_navigation_to_load()
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            home_page.click_online_refund()
            logger.debug(f"selecting the card type as : CTLS_VISA_CREDIT_417666")
            card_page.select_cardtype("CTLS_VISA_CREDIT_417666")
...
@pytest.mark.chargeSlipVal
@pytest.mark.uiVal
def test_online_refund():  # Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        try:
            ...
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            print(home_page.click_online_refund())
            time.sleep(5)
            # app_driver.find_element(By.ID, '').get_dom_attribute()
            # print(f"++++++++++++++++++++++++++{msg}")
