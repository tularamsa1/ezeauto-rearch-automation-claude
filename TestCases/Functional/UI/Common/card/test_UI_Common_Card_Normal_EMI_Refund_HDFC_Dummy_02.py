...
def test_common_100_115_05_022():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Refund_Via_API_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    Sub Feature Description: Performing the normal EMI refund via API  transaction for an org (not ezetap) via HDFC Dummy PG
    using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 022: TC022
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching posting_date from the txn table for refund txn : {refund_posting_date}")
            # ------------------------------------------------------------------------------------------------
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(
                    f"Fetching monthly emi from txn history for the original txn : {txn_id}, {app_monthly_emi}")
                app_total_interest = txn_history_page.fetch_total_interest_text()
                logger.debug(
                    f"Fetching total interest from txn history for the original txn : {txn_id}, {app_total_interest}")
                app_total_emi_amount = txn_history_page.fetch_total_emi_amount_text()
                logger.debug(
                    f"Fetching total emi amount from txn history for the original txn : {txn_id}, {app_total_emi_amount}")
                app_loan_amount = txn_history_page.fetch_loan_amount_text()
                logger.debug(
                    f"Fetching loan amount from txn history for the original txn : {txn_id}, {app_loan_amount}")
                app_interest_amount = txn_history_page.fetch_interest_amount_text()
                logger.debug(
                    f"Fetching interest amount from txn history for the original txn : {txn_id}, {app_interest_amount}")
                app_net_effective_price = txn_history_page.fetch_net_effective_price_text()
                logger.debug(
                    f"Fetching net effective price from txn history for the original txn : {txn_id}, {app_net_effective_price}")
                ...
                logger.info(
                    f"Fetching settlement_status from txn history for the original txn : {txn_id}, {app_settle_status}")
                ...
                logger.info(
                    f"Fetching device_serial from txn history for the original txn : {txn_id}, {app_device_serial}")
                app_batch_number = txn_history_page.fetch_batch_number_text()
                logger.info(
                    f"Fetching batch_number from txn history for the original txn : {txn_id}, {app_batch_number}")
                app_card_type_desc = txn_history_page.fetch_card_type_desc_text()
                logger.info(
                    f"Fetching card_type_desc from txn history for the original txn : {txn_id}, {app_card_type_desc}")
                ...
                logger.info(
                    f"Fetching customer_name from txn history for the original txn : {txn_id}, {app_customer_name}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(
                    f"From response_original fetch card_txn_type_desc for original txn : {api_card_txn_type_desc}")
                ...
                logger.debug(
                    f"From response_original fetch card_last_four_digit for original txn : {api_card_last_four_digit}")
                api_ext_ref_number = response_original.get('externalRefNumber')
                logger.debug(
                    f"From response_original fetch external_ref_number for original txn : {api_ext_ref_number}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_023():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Refund_Via_API_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_9_Months_Tenure
    Sub Feature Description: Performing the normal EMI refund via API  transaction for an org (not ezetap) via HDFC Dummy PG
    using EMV VISA Debit card with pin for 9 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 023: TC023
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_024():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Refund_Via_API_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_12_Months_Tenure
    Sub Feature Description: Performing the normal EMI refund via API  transaction for an org (not ezetap) via HDFC Dummy PG
    using EMV VISA Debit card with pin for 12 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 024: TC024
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_045():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Refund_Via_API_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the normal EMI refund via API transaction for fallback to root org via HDFC Dummy
    PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 045: TC045
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_046():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Refund_Via_API_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    Sub Feature Description: Performing the normal EMI refund via API transaction for fallback to root org via HDFC Dummy
    PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 046: TC046
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
