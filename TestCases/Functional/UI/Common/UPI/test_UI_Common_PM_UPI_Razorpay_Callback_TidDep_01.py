...
def test_common_100_101_236():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_Success_Via_Pure_UPI_Callback_Razorpay
    Sub Feature Description: Tid Dep - Verification of a successful pure upi txn via Razorpay using callback
    TC naming code description: 100: Payment Method, 101: UPI, 236: TC236
    """
    try:
        ...
        logger.debug(f"query for updating terminal_dependency_config is: {query}")
        ...
        try:
            ...
            logger.debug(f"Fetching status_db,payment_mode_db, amount_db, state_db, payment_gateway_db, acquirer_code_db, bank_code_db, settlement_status_db, mid_db, tid_db, device_serial_db,order_id_db from database for "
                f"current merchant: {status_db},{payment_mode_db}, {amount_db}, {state_db}, {payment_gateway_db}, {acquirer_code_db}, {bank_code_db}, {settlement_status_db}, {mid_db}, {tid_db}, {device_serial_db},{order_id_db}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_239():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_Failed_Via_Pure_UPI_Callback_Razorpay
    Sub Feature Description: Tid Dep - Verification of a failed UPI txn via Razorpay using callback
    TC naming code description: 100: Payment Method, 101: UPI, 239: TC239
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_242():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_Success_Callback_After_QR_Expiry_Razorpay
    Sub Feature Description: Tid Dep - Performing a pure upi success callback via Razorpay after expiry the qr
    TC naming code description: 100: Payment Method, 101: UPI, 242: TC242
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"fetched original_customer_name from txn table is : {original_customer_name}")
            original_payer_name = result['payer_name'].values[0]
            logger.debug(f"fetched payer_name from txn table is : {original_payer_name}")
            original_org_code_txn = result['org_code'].values[0]
            logger.debug(f"fetched org_code from txn table is : {original_org_code_txn}")
            original_txn_type = result['txn_type'].values[0]
            logger.debug(f"fetched txn_type from txn table is : {original_txn_type}")
            original_posting_date = result['posting_date'].values[0]
            logger.debug(f"fetched posting_date from txn table is : {original_posting_date}")
            auth_code = result['auth_code'].values[0]
            ...
            txn_ref = result['txn_ref3'].values[0]
            logger.debug(f"Query result, txn_ref : {txn_ref}")
            payment_id = result['txn_ref'].values[0]
            logger.debug(f"Query result, payment_id : {payment_id}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                app_rrn = txn_history_page.fetch_RRN_text()
                logger.info(
                    f"Fetching txn_id from txn history for the txn : {original_txn_id}, {app_rrn}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                new_txn_status_db = result["status"].values[0]
                new_txn_payment_mode_db = result["payment_mode"].values[0]
                new_txn_amount_db = int(result["amount"].values[0])
                new_txn_state_db = result["state"].values[0]
                new_txn_payment_gateway_db = result["payment_gateway"].values[0]
                new_txn_acquirer_code_db = result["acquirer_code"].values[0]
                new_txn_bank_code_db = result["bank_code"].values[0]
                new_txn_settlement_status_db = result["settlement_status"].values[0]
                ...
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                new_txn_upi_status_db = result["status"].values[0]
                new_txn_upi_txn_type_db = result["txn_type"].values[0]
                new_txn_upi_bank_code_db = result["bank_code"].values[0]
                new_txn_upi_mc_id_db = result["upi_mc_id"].values[0]
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expected_portal_values = {
                    ...
                    "rrn_2": str(rrn),
                    "txn_id_2": new_txn_id,
                    "auth_code_2": "-" if new_auth_code is None else new_auth_code
                }
                logger.debug(f"expected_portal_values : {expected_portal_values}")
                ...
                callback_auth_code = transaction_details[0]['Auth Code']
                ...
                actual_portal_values = {
                    ...
                    "auth_code_2": callback_auth_code
                }
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_243():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_Failed_Callback_After_QR_Expiry_Razorpay
    Sub Feature Description: Tid Dep -  Performing a pure upi failed callback via Razorpay after expiry the qr
    TC naming code description: 100: Payment Method, 101: UPI, 243: TC243
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('TidDepUpiQRGenerate',
                                                      request_body={"username": app_username, "password": app_password,
                                                                    ...
                                                                    "deviceSerial": str(device_serial)})
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received after sending the request for generating qr : {response}")
            original_txn_id = response["txnId"]
            logger.debug(f"Fetching Txn_id from the API_OUTPUT, Txn_id : {original_txn_id}")
            logger.info("waiting for the time till qr get expired...")
            time.sleep(75)
            ...
            logger.debug(f"fetched original_customer_name from txn table is : {original_customer_name}")
            original_payer_name = result['payer_name'].values[0]
            logger.debug(f"fetched original_payer_name from txn table is : {original_payer_name}")
            original_org_code_txn = result['org_code'].values[0]
            logger.debug(f"fetched original_org_code_txn from txn table is : {original_org_code_txn}")
            original_txn_type = result['txn_type'].values[0]
            logger.debug(f"fetched original_txn_type from txn table is : {original_txn_type}")
            original_created_date = result['created_time'].values[0]
            logger.debug(f"fetched original_created_date from txn table is : {original_created_date}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"fetched auth_code from txn table is : {auth_code}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            logger.info(f"Started APP validation for the test case : {testcase_id}")
            try:
                date_and_time = date_time_converter.to_app_format(original_created_date)
                expected_app_values = {
                    "pmt_mode": "UPI",
                    "pmt_status": "EXPIRED",
                    "txn_amt": "{:,.2f}".format(amount),
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            logger.info(f"Started API validation for the test case : {testcase_id}")
            try:
                date = date_time_converter.db_datetime(original_created_date)
                expected_api_values = {
                    "pmt_status": "EXPIRED",
                    "txn_amt": amount, "pmt_mode": "UPI",
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                # --------------------------------------------------------------------------------------------
                date_and_time_portal = date_time_converter.to_portal_format(original_created_date)
                expected_portal_values = {
                    "date_time": date_and_time_portal,
                    "pmt_state": "EXPIRED",
                    "pmt_type": "UPI",
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_244():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_2_Success_Callback_After_QR_Expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Tid Dep - Performing two pure upi success callback via Razorpay after expiry the qr when autorefund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 244: TC244
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"update terminal_dependency_config set terminal_dependent_enabled=1 where org_code ='{org_code}' " \
                "and payment_mode ='UPI' and payment_gateway='RAZORPAY';"
        result = DBProcessor.setValueToDB(query)
        logger.info(f"RESULT of updating terminal_dependency_config table inactive: {result}")
        api_details = DBProcessor.get_api_details('QRExpiryTime', request_body={"username": portal_username,
                                                                                "password": portal_password,
        ...
        query = f"select * from upi_merchant_config where org_code ='{org_code}' AND status = 'ACTIVE' AND bank_code = 'RAZORPAY_PSP' and allowed_mode='HYBRID' and card_terminal_acquirer_code='HDFC';"
        logger.debug(f"Query to fetch upi_mc_id from the upi_merchant_config for the {org_code} : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"query result for upi_merchant_config table is : {result}")
        pg_merchant_id = result['pgMerchantId'].values[0]
            ...
            try:
                ...
                for elements in responseInList:
                    if elements["txnId"] == new_txn_id_1:
                        ...
                        new_txn_amount_api_1 = int(elements["amount"])
                ...
                for elements in responseInList:
                    if elements["txnId"] == new_txn_id_2:
                        ...
                        new_txn_amount_api_2 = int(elements["amount"])
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expected_portal_values = {
                    ...
                    "rrn_2": str(callback_1_rrn),
                    "txn_id_2": new_txn_id_1,
                    "auth_code_2": "-" if new_txn_auth_code_1 is None else new_txn_auth_code_1,
                    ...
                    "rrn_3": str(callback_2_rrn),
                    "txn_id_3": new_txn_id_2,
                    "auth_code_3": "-" if new_txn_auth_code_2 is None else new_txn_auth_code_2
                }
                logger.debug(f"expected_portal_values : {expected_portal_values}")
                ...
                auth_code_portal_1 = transaction_details[1]['Auth Code']
                ...
                actual_portal_values = {
                    ...
                    "auth_code_3": refunded_auth_code_portal,
                }
                logger.debug(f"actual_portal_values : {actual_portal_values} for the testcase_id : {testcase_id}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values_2 = {
                    ...
                    'BASE AMOUNT:': "Rs." + "{:,.2f}".format(amount),
                    "AUTH CODE": "" if new_txn_auth_code_2 is None else new_txn_auth_code_2,
                }
