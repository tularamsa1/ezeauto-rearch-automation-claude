...
def test_common_100_115_226():
    """
    Sub Feature Code: UI_Common_Card_Refund_ATOS_TLE_EMV_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description:  Performing the EVM Refund transaction via ATOS_TLE PG using MASTER Credit card having
     16 digit PAN length with pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 226: TC226
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_227():
    """
    Sub Feature Code: UI_Common_Card_Refund_ATOS_TLE_EMV_RUPAY_DebitCard_Without_Pin_608326
    Sub Feature Description:  Performing the EVM Refund transaction via ATOS_TLE PG using Rupay Debit card having
     16 digit PAN length without pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 227: TC227
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_228():
    """
    Sub Feature Code: UI_Common_Card_Refund_ATOS_TLE_EMV_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description:  Performing the EMV Refund transaction via ATOS_TLE PG using Rupay Debit card having
     16 digit PAN length with pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 228: TC228
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
