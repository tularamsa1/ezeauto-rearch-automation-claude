...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_common_100_104_205():
    """
        Sub Feature Code: NonUI_Common_FDC_Card_Reversal_EMV_WITH_PIN_DEBIT_VISA
        Sub Feature Description:API that performs EMV with pin Reversal txn having DEBIT VISA card via FDC
    ...
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('Card_api',
                                                      request_body={"deviceSerial": merchant_creator.get_device_serial_of_merchant(org_code=org_code,acquisition="ICICI",payment_gateway="FDC"),
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_common_100_104_191():
    """
        Sub Feature Code: NonUI_Common_FDC_Card_Reversal_EMV_WITH_PIN_DEBIT_RUPAY
        Sub Feature Description:API that performs EMV with pin Reversal txn having DEBIT RUPAY card via FDC
        TC naming code description:
        100: Payment Method
        104: CARD
        191: TC191
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_common_100_104_219():
    """
        Sub Feature Code: NonUI_Common_FDC_Card_Reversal_EMV_WITH_PIN_CREDIT_VISA
        Sub Feature Description:API that performs EMV with pin Reversal txn having CREDIT VISA card via FDC
        TC naming code description:
        100: Payment Method
        104: CARD
        219: TC219
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_common_100_104_233():
    """
        Sub Feature Code: NonUI_Common_FDC_Card_Reversal_EMV_WITH_PIN_CREDIT_MASTER
        Sub Feature Description:API that performs EMV with pin Reversal txn having CREDIT MASTER card via FDC
        TC naming code description:
        100: Payment Method
        104: CARD
        233: TC233
