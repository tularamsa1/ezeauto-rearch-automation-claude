...
def test_common_100_115_11_023():
    """
    Sub Feature Code: UI_Common_Card_EZE_EMI_Subvented_By_Ezetap_For_Issuer_Ezetap_Cosubvention_Success_Txn_For_An_Org_HDFC_Dummy_OneCard_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    ...
    HDFC Dummy PG for OneCard issuer using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 023: TC023
    """
    try:
        ...
        query = f"select * from emi where org_code='{org_code}' and status = 'ACTIVE' and " \
                        f"issuer_code='ONECARD' and card_type='CREDIT' and term = '{emi_plan_in_months} month' and emi_type='EZEEMI'" \
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='ONECARD' and eze_emi_enabled= b'1' order by created_time desc limit 1;"
        ...
        try:
            ...
            logger.debug(f"Fetching wallet_balance after the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_after_txn}")
