import sys
import time
import random
import pytest
from datetime import datetime

from Configuration import Configuration, TestSuiteSetup
from DataProvider import GlobalVariables
from PageFactory.ReArch.rearch_login_page import ReArchLoginPage
from PageFactory.ReArch.rearch_home_page import ReArchHomePage
from PageFactory.ReArch.rearch_order_details_page import ReArchOrderDetailsPage
from PageFactory.ReArch.rearch_qr_page import ReArchQRPage
from PageFactory.ReArch.rearch_complete_page import ReArchCompletePage
from PageFactory.ReArch.rearch_txn_history_page import ReArchTxnHistoryPage
from PageFactory.ReArch.rearch_txn_detail_page import ReArchTxnDetailPage
from PageFactory.ReArch.rearch_native_locators import RefundLocators
from Utilities import (
    APIProcessor, ConfigReader, DBProcessor, ResourceAssigner,
    Validator, date_time_converter, receipt_validator,
)
from Utilities.execution_log_processor import EzeAutoLogger

logger = EzeAutoLogger(__name__)


@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_rearch_0056():
    """
    Sub Feature Code: UI_ReArch_PM_UPI_Refund_Success_HDFC
    Sub Feature Description:
        Launch ReArch app, perform a UPI QR payment (HDFC simulator
        auto-authorizes), navigate to txn details, initiate Refund
        (Refund Amount → Enter Password → Continue → Refund Payment → Done),
        then search for the refunded txn and validate via App, API,
        and Charge Slip.

    NL Source Steps:
      Preconditions:
        1. update org_settings: upiEnabled = true

      Test Steps:
        1.  launch rearch app
        2.  login with credentials if required
        3.  wait for initial home screen
        4.  click collect payment
        5.  wait for home page
        6.  enter amount <random 3-digit>
        7.  click upi
        8.  wait for order details screen
        9.  click proceed
        10. wait for qr screen (HDFC simulator auto-authorizes)
        11. cancel payment
        12. verify payment failed screen
        13. click accept more payments
        14. navigate to txn history → search by txn_id → open txn detail
        15. click Refund Amount
        16. click Refund Amount (confirmation)
        17. enter app password
        18. click Continue
        19. click Refund Payment
        20. click Done
        21. search for refunded txn by txn_id → open txn detail
        22. validate app, api, and charge slip

    Generated by: test_generator.md, action_registry.yaml
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        logger.debug(f"Setup Timer resumed in testcase function: {testcase_id}")

        # ══════════════════════════════════════════════════════════════
        # SETUP
        # ══════════════════════════════════════════════════════════════
        app_cred = ResourceAssigner.getAppUserCredentials(testcase_id)
        logger.debug(f"App credentials fetched: {app_cred}")
        app_username = app_cred["Username"]
        app_password = app_cred["Password"]

        portal_cred = ResourceAssigner.getPortalUserCredentials(testcase_id)
        logger.debug(f"Portal credentials fetched: {portal_cred}")
        portal_username = portal_cred["Username"]
        portal_password = portal_cred["Password"]

        query = f"select org_code from org_employee where username='{app_username}';"
        logger.debug(f"Query to fetch org_code: {query}")
        result = DBProcessor.getValueFromDB(query)
        org_code = result["org_code"].values[0]
        logger.debug(f"org_code: {org_code}")

        # ── Preconditions ─────────────────────────────────────────────
        logger.info(f"Starting Precondition setup for the test case: {testcase_id}")

        api_details = DBProcessor.get_api_details('org_settings_update', request_body={
            "username": portal_username,
            "password": portal_password,
            "entityName": "org",
            "settingForOrgCode": org_code,
        })
        api_details["RequestBody"]["settings"]["upiEnabled"] = "true"
        logger.debug(f"Precondition API details: {api_details}")
        response = APIProcessor.send_request(api_details=api_details)
        logger.debug(f"Precondition response: {response}")

        GlobalVariables.setupCompletedSuccessfully = True
        logger.info(f"Completed Precondition setup for the test case: {testcase_id}")

        Configuration.configureLogCaptureVariables(
            apiLog=True, portalLog=False, cnpwareLog=False,
            middlewareLog=True, config_log=False,
        )

        GlobalVariables.time_calc.setup.end()
        logger.debug(f"Setup Timer ended in testcase function: {testcase_id}")

        # ══════════════════════════════════════════════════════════════
        # EXECUTION
        # ══════════════════════════════════════════════════════════════
        try:
            logger.info(f"Starting execution for the test case: {testcase_id}")
            GlobalVariables.time_calc.execution.start()
            logger.debug(f"Execution Timer started in testcase function: {testcase_id}")

            amount = str(random.randint(800, 999))
            display_amount = f"{int(amount):,}.00"
            order_id = f"{datetime.now().strftime('%m%d%H%M%S')}{testcase_id[-4:]}"
            logger.debug(f"amount={amount}, display_amount={display_amount}, order_id={order_id}")

            # ── UPI Success Flow ──────────────────────────────────────

            # Step 1-2: Launch ReArch app and login if required
            app_driver = TestSuiteSetup.initialize_rearch_driver(testcase_id)
            login_page = ReArchLoginPage(app_driver)
            login_page.perform_login_if_required(app_username, app_password)
            logger.debug("Login completed")

            # Step 3-4: Wait for initial home screen and tap Collect Payment
            home_page = ReArchHomePage(app_driver)
            home_page.wait_for_initial_home_screen()
            home_page.click_collect_payment()
            logger.debug("Collect Payment tapped")

            # Step 5-6: Wait for amount screen and enter amount
            home_page.wait_for_home_page_load()
            home_page.enter_amount(amount)
            logger.debug(f"Amount {amount} entered")

            # Step 7: Tap UPI
            home_page.click_pay_by_upi()
            logger.debug("UPI payment method selected")

            # Step 8-9: Order Details overlay → Proceed
            order_details_page = ReArchOrderDetailsPage(app_driver)
            order_details_page.wait_for_order_details_screen()
            order_details_page.click_proceed()
            logger.debug("Order Details: clicked Proceed")

            # Step 10: Wait for QR screen (HDFC simulator auto-authorizes)
            qr_page = ReArchQRPage(app_driver)
            qr_page.wait_for_qr_screen()
            logger.info("QR screen displayed — HDFC simulator auto-authorizing")

            # Step 11: Cancel QR navigation (payment already AUTHORIZED in backend)
            time.sleep(7)
            qr_page.cancel_payment()
            logger.debug("QR screen cancelled")

            # Step 12: Verify failed screen (expected after QR cancel)
            complete_page = ReArchCompletePage(app_driver)
            complete_page.wait_for_failure_screen()
            logger.info("Payment Failed screen confirmed (expected after QR cancel)")

            time.sleep(2)

            # Resolve txn from DB
            query = (
                f"select id, created_time, posting_date, rr_number "
                f"from txn "
                f"where org_code='{org_code}' AND username='{app_username}' "
                f"AND payment_mode='UPI' "
                f"order by created_time desc limit 1;"
            )
            logger.debug(f"Query to resolve txn_id: {query}")
            result = DBProcessor.getValueFromDB(query)
            txn_id       = result["id"].values[0]
            created_time = result["created_time"].values[0]
            posting_date = result["posting_date"].values[0]
            rrn          = str(result["rr_number"].values[0])
            logger.debug(
                f"Resolved txn_id={txn_id}, created_time={created_time}, "
                f"posting_date={posting_date}, rrn={rrn}"
            )

            # Step 13: Click accept more payments → back to home
            complete_page.click_proceed_to_home()
            logger.debug("Clicked Accept More Payments")

            # ── Refund Flow ───────────────────────────────────────────

            # Step 14: Navigate to txn detail (Home → TxnHistory → search by txn_id)
            home_page.wait_for_home_page_load()
            home_page.click_txn_history()
            txn_history_page = ReArchTxnHistoryPage(app_driver)
            txn_history_page.wait_for_txn_list()
            txn_history_page.click_on_transaction_by_txn_id(txn_id=txn_id)
            logger.debug("Navigated to txn detail page for refund")

            txn_detail_page = ReArchTxnDetailPage(app_driver)

            # Step 15: Click Refund Amount
            txn_detail_page.perform_click(RefundLocators.btn_refund_amount)
            logger.debug("Clicked Refund Amount (first)")

            # Step 16: Click Refund Amount (confirmation)
            txn_detail_page.wait_for_element(RefundLocators.btn_refund_amount)
            txn_detail_page.perform_click(RefundLocators.btn_refund_amount)
            logger.debug("Clicked Refund Amount (confirmation)")

            # Step 17-18: Enter app password and click Continue
            txn_detail_page.wait_for_element(RefundLocators.lbl_enter_password)
            txn_detail_page.perform_sendkeys(RefundLocators.txt_password, app_password)
            txn_detail_page.perform_click(RefundLocators.btn_continue)
            logger.debug("Entered password and clicked Continue")

            # Step 19: Click Refund Payment
            txn_detail_page.perform_click(RefundLocators.btn_refund_payment)
            logger.debug("Clicked Refund Payment")

            # Step 20: Click Done
            txn_detail_page.perform_click(RefundLocators.btn_done)
            logger.debug("Clicked Done — refund completed")

            # Allow backend to process refund before querying DB
            time.sleep(3)

            # Re-query DB for updated refund status
            query = (
                f"select id, created_time, posting_date, rr_number "
                f"from txn "
                f"where org_code='{org_code}' AND username='{app_username}' "
                f"AND payment_mode='UPI' "
                f"order by created_time desc limit 1;"
            )
            logger.debug(f"Query to resolve refunded txn: {query}")
            result = DBProcessor.getValueFromDB(query)
            txn_id       = result["id"].values[0]
            created_time = result["created_time"].values[0]
            posting_date = result["posting_date"].values[0]
            rrn          = str(result["rr_number"].values[0])
            logger.debug(
                f"Post-refund txn_id={txn_id}, created_time={created_time}, "
                f"posting_date={posting_date}, rrn={rrn}"
            )

            GlobalVariables.EXCEL_TC_Execution = "Pass"
            GlobalVariables.time_calc.execution.pause()
            logger.debug(f"Execution Timer paused in try block of testcase function: {testcase_id}")
            logger.info(f"Execution is completed for the test case: {testcase_id}")

        except Exception as e:
            Configuration.perform_exe_exception(testcase_id)
            pytest.fail("Test case execution failed due to the exception - " + str(e))

        # ══════════════════════════════════════════════════════════════
        # VALIDATION
        # ══════════════════════════════════════════════════════════════
        logger.info(f"Starting Validation for the test case: {testcase_id}")
        GlobalVariables.time_calc.validation.start()
        logger.debug(f"Validation Timer started in testcase function: {testcase_id}")

        # ── App Validation ────────────────────────────────────────────
        if ConfigReader.read_config("Validations", "app_validation") == "True":
            logger.info(f"Started APP validation for the test case: {testcase_id}")
            try:
                date_and_time = date_time_converter.to_rearch_app_format(created_time)
                expected_app_values = {
                    "txn_status": "Payment Refunded",  # TODO: verify exact text on first run
                    "txn_id":     txn_id,
                    "date":       date_and_time,
                    "amount":     display_amount,
                }
                logger.debug(f"expected_app_values: {expected_app_values}")

                # Step 21: Search for refunded txn by txn_id
                txn_history_page.click_on_transaction_by_txn_id(txn_id=txn_id)

                txn_detail_page = ReArchTxnDetailPage(app_driver)

                # Fetch and assert transaction fields
                app_txn_id     = txn_detail_page.fetch_payment_id()
                app_txn_status = txn_detail_page.fetch_status(display_amount)
                app_date_time  = txn_detail_page.fetch_date_time()
                app_amount     = txn_detail_page.fetch_amount(display_amount)
                logger.info(
                    f"App txn_id={app_txn_id}, date_time={app_date_time}, "
                    f"app_txn_status={app_txn_status}, app_amount={app_amount}"
                )

                actual_app_values = {
                    "txn_status": app_txn_status,
                    "txn_id":     app_txn_id,
                    "date":       app_date_time,
                    "amount":     app_amount,
                }
                logger.debug(f"actual_app_values: {actual_app_values}")
                Validator.validateAgainstAPP(
                    expectedApp=expected_app_values, actualApp=actual_app_values
                )

                assert app_txn_id, "Payment ID should not be empty"
                assert app_date_time, "Date & Time should not be empty"

            except Exception as e:
                Configuration.perform_app_val_exception(testcase_id, e)
            logger.info(f"Completed APP validation for the test case: {testcase_id}")

        # ── API Validation ────────────────────────────────────────────
        if ConfigReader.read_config("Validations", "api_validation") == "True":
            logger.info(f"Started API validation for the test case: {testcase_id}")
            try:
                date = date_time_converter.db_datetime(created_time)
                expected_api_values = {
                    "pmt_status":    "REFUNDED",  # TODO: verify on first run — may be REFUND_PROCESSED
                    "txn_amt":       float(amount),
                    "pmt_mode":      "UPI",
                    "txn_type":      "REFUND",
                    "acquirer_code": "HDFC",
                    # "issuer_code":   "HDFC",
                    "org_code":      org_code,
                    "rrn":           rrn,
                    "date":          date,
                }
                logger.debug(f"expected_api_values: {expected_api_values}")

                api_details = DBProcessor.get_api_details(
                    "txnlist",
                    request_body={"username": app_username, "password": app_password},
                )
                response = APIProcessor.send_request(api_details)
                logger.debug("txnlist API response received")

                txn_data = next(
                    (x for x in response["txns"] if x["txnId"] == txn_id), None
                )
                if txn_data is None:
                    raise ValueError(f"txn_id '{txn_id}' not found in txnlist API response")
                logger.debug(f"Filtered txn entry from txnlist: {txn_data}")

                actual_api_values = {
                    "pmt_status":    txn_data["status"],
                    "txn_amt":       float(txn_data["amount"]),
                    "pmt_mode":      txn_data["paymentMode"],
                    "txn_type":      txn_data["txnType"],
                    "acquirer_code": txn_data["acquirerCode"],
                    # "issuer_code":   txn_data["issuerCode"],
                    "org_code":      txn_data["orgCode"],
                    "rrn":           str(txn_data["rrNumber"]),
                    "date":          date_time_converter.from_api_to_datetime_format(
                                         txn_data["createdTime"]
                                     ),
                }
                logger.debug(f"actual_api_values: {actual_api_values}")
                Validator.validationAgainstAPI(
                    expectedAPI=expected_api_values, actualAPI=actual_api_values
                )

            except Exception as e:
                Configuration.perform_api_val_exception(testcase_id, e)
            logger.info(f"Completed API validation for the test case: {testcase_id}")

        # ── Charge Slip Validation ────────────────────────────────────
        if ConfigReader.read_config("Validations", "charge_slip_validation") == "True":
            logger.info(f"Started ChargeSlip validation for the test case: {testcase_id}")
            try:
                txn_date, txn_time = date_time_converter.to_chargeslip_format(posting_date_db=posting_date)
                expected_charge_slip_values = {
                    "PAID BY:":        "UPI",
                    "RRN":             rrn,
                    "BASE AMOUNT:":    f"Rs.{int(amount):,}.00",
                    "date":            txn_date,
                    "time":            txn_time,
                    # No AUTH CODE for UPI
                }
                logger.debug(f"expected_charge_slip_values: {expected_charge_slip_values}")
                receipt_validator.perform_charge_slip_validations(
                    txn_id=txn_id,
                    credentials={"username": app_username, "password": app_password},
                    expected_details=expected_charge_slip_values,
                )

            except Exception as e:
                Configuration.perform_charge_slip_val_exception(testcase_id, e)
            logger.info(f"Completed ChargeSlip validation for the test case: {testcase_id}")

        GlobalVariables.time_calc.validation.end()
        logger.debug(f"Validation Timer ended in testcase function: {testcase_id}")
        logger.info(f"Completed Validation for the test case: {testcase_id}")

    finally:
        Configuration.executeFinallyBlock(testcase_id)
