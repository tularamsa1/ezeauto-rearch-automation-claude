...
@pytest.mark.appVal
def test_common_100_101_245():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_Success_Callback_After_QR_Expiry_AutoRefund_Enabled_Razorpay
    Sub Feature Description: Tid Dep - Performing a pure upi success callback via Razorpay Tid Dep after qr expiry when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 245: TC245
    """
    try:
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received after sending the request for qr generation : {response}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_246():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_2_Success_Callback_After_QR_Expiry_Razorpay_AutoRefund_Enabled
    Sub Feature Description: Tid Dep - Performing two  upi success callback via Razorpay after expiry the qr when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 246: TC246
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {"pmt_status": "EXPIRED",
                                       ...
                                       "acquirer_code": "RAZORPAY",
                                       "order_id": order_id,
                                       "issuer_code": "RAZORPAY", "rrn": str(original_rrn),
...
def test_common_100_101_248():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_2_Success_Callback_Before_QR_Expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Tid Dep - Performing two pure upi success callback via Razorpay before expiry the qr when autorefund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 248: TC248
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {"pmt_status": "AUTHORIZED",
                                       ...
                                       "order_id": order_id,
                                       "issuer_code": "RAZORPAY",
                                       "txn_type": new_txn_type_1, "mid": mid,
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            logger.info(f"Started PORTAL validation for the test case : {testcase_id}")
            try:
                date_and_time_portal = date_time_converter.to_portal_format(new_txn_posting_date_1)
                date_and_time_portal_2 = date_time_converter.to_portal_format(new_txn_posting_date_2)
                expected_portal_values = {
                    "date_time": date_and_time_portal,
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_250():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_2_Failed_Callback_After_QR_Expiry_Razorpay_AutoRefund_Disabled
    Sub Feature Description: Tid Dep - Performing two pure upi failed callback via Razorpay after expiry the qr when autorefund is disabled
    TC naming code description: 100: Payment Method, 101: UPI, 250: TC250
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_283():
    """
    Sub Feature Code: Tid Dep -  UI_Common_PM_UPI_2_Failed_Callback_After_QR_Expiry_Razorpay_AutoRefund_Enabled
    Sub Feature Description: Tid Dep - Performing two  upi failed callback via TID Dep Razorpay after expiry the qr when autorefund is enabled
    TC naming code description: 100: Payment Method, 101: UPI, 283: TC283
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = f"select * from upi_txn where org_code = '{str(org_code)}' and txn_id = '{str(original_txn_id)}';"
            logger.debug(f"Query to fetch txn_ref from the DB : {query}")
            result = DBProcessor.getValueFromDB(query)
            txn_ref = result['txn_ref'].values[0]
            logger.debug(f"Query result, txn_ref : {txn_ref}")
