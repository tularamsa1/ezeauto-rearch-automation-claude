...
def test_common_rearch_0001():
    ...
    Sub Feature Description:
        Launch ReArch app, enter a magic-number amount (150-160) that triggers a
        Bharat QR failure. Navigate via ⋯ → Order Details → Proceed → Payment Method
        overlay (scroll) → Bharat QR → QR screen. Cancel via back button. Validate
        the cancelled transaction via App (TxnHistory → TxnDetail) and API.
    ...
    NL Source Steps:
      ...
      Test Steps:
        ...
        6.  enter amount <random 150-160>
        7.  click more payment options
        8.  wait for order details screen
        9.  click proceed on order details
        10. wait for payment methods
        11. select bharat qr
        12. wait for qr screen
        13. cancel payment
        14. wait for home page

      App Validation:
        15. go to transaction history
        16. wait for transaction list
        17. click transaction by txn_id
        18. wait for transaction detail
        19. validate payment id is not empty
        20. validate txn status is Payment Failed
        21. validate date time is not empty

      API Validation:
        22. txnlist API — status, amount, payment mode, org_code
            (TODO: verify exact status + payment_mode for BQR cancel in API response)
    ...
    try:
        ...
        try:
            ...
            # Step 14: App returns to enter amount screen after cancel
            home_page.wait_for_home_page_load()
            logger.debug("Returned to home/amount screen after cancel")

            # Resolve txn_id from DB — latest txn by org_code
        ...
        if ConfigReader.read_config("Validations", "api_validation") == "True":
            ...
            try:
                ...
                expected_api_values = {
                    "pmt_status": "FAILED",  # TODO: verify actual API status for BQR cancel
                    "txn_amt":    float(amount),
                    "pmt_mode":   "BHARATQR",         # TODO: verify BQR payment_mode in txnlist API
                    "org_code":   org_code,
                    "date":       date,
                }
