...
def test_common_100_102_058():
    ...
    :Sub Feature code: UI_Common_BQRV4_Callback_Success_YES_ATOS
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                actual_portal_values = { "date_time": date_time,
                    "pmt_state": status,
...
def test_common_100_102_069():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_Two_Callback_Success_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 generation using API and performing two Callback Success transaction
    TC naming code description: 100: Payment Method, 102: BQR, 069: TC069
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {"txn_amt": amount,"pmt_mode": "UPI","pmt_status": "AUTHORIZED",
                                      ...
                                      # "bqr_merchant_pan": bqr_m_pan, "bqr_rrn": str(rrn),
                                      "bqr_org_code": org_code,
                                      "txn_amt_2": amount, "pmt_mode_2": "UPI", "pmt_status_2": "AUTHORIZED",
                                      ...
                                      "upi_txn_type_2": "PAY_BQR", "upi_mc_id_2": upi_mc_id, "upi_bank_code_2": "YES"                                      }
                logger.debug(f"expected_db_values: {expected_db_values}")
                ...
                actual_db_values = {"txn_amt": amount_db,"pmt_mode": payment_mode_db,
                                    ...
                                    "settle_status_2": settlement_status_db_new, "upi_pmt_status_2": upi_status_db_new,
...
def test_common_100_102_071():
    """
    :Description: Verification of a BQRV4 Callback After QR Expiry Success transaction when
     auto refund disabled via YES_ATOS
    :Sub Feature code: UI_Common_BQRV4_Callback_After_expired_Success_YES_ATOS
    :TC naming code description: 100->Payment Method, 102->BQR, 071-> TC071
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        vpa = result['vpa'].values[0]
        upi_mc_id = result['id'].values[0]
        logger.debug(f"Fetching vpa and upi_mc_id from database for current merchant:{vpa}, {upi_mc_id}")
...
def test_common_100_102_072():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_Two_Callback_After_Expired_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 generation using API and performing two Callback transaction when auto refund disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 072: TC072
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                expected_db_values = {"txn_amt": float(amount),"pmt_mode": "BHARATQR","pmt_status": "EXPIRED",
                                      ...
                                      "upi_txn_type_3": "PAY_BQR", "upi_mc_id_3": upi_mc_id,
                                      "upi_bank_code_3": "YES"
                                      }
                logger.debug(f"expected_db_values: {expected_db_values}")
                ...
                actual_db_values = {"txn_amt": amount_db,"pmt_mode": payment_mode_db,
                                    ...
                                    "upi_txn_type_3": upi_txn_type_db_new_3, "upi_mc_id_3": upi_mc_id_db_new_3,
...
def test_common_100_102_064():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_Amount_Mismatch_Callback_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 UPI amount mismatch txn when Auto refund is disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 064: TC064
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
