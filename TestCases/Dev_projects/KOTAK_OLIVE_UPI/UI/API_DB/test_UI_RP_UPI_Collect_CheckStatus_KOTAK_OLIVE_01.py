...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_001():
    """
    Sub Feature Code: UI_RP_upi_collect_Success_Via_upi_Collect_Checkstatus_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay successful upi collect txn via KOTAK OLIVE using check status
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RP_UPI_COLLECT, 001->TC001
    """
    try:
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_002():
    """
    Sub Feature Code: UI_RP_Upi_Collect_Checkstatus_After_timeout_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay upi txn after timeout for KOTAK_OLIVE using check status
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RP_UPI_COLLECT, 002->TC002
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.info(f"You order id is: {order_id}")
            logger.info(f"You amount is: {amount}")
            api_details = DBProcessor.get_api_details('Remotepay_Initiate', request_body={
                "amount": amount, "externalRefNumber": order_id, "username": app_username, "password": app_password})
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for Remotepay_Initiate : {response}")
            ...
            if not response['success']:
                ...
                logger.info("VPA validation completed.")
                remotePayUpiCollectTxn.clickOnRemotePayUpiCollectProceed()
                logger.info("UPI collect txn is initiated.")
            ...
            query = "select * from remotepay_setting where setting_name='cnpTxnTimeoutDuration' and org_code = 'EZETAP'"
            logger.debug(f"Query to fetch txn timeout from the DB : {query}")
            try:
                result = DBProcessor.getValueFromDB(query)
                setting_value = int(result['setting_value'].values[0])
            ...
            if org_setting_value:
                ...
                time.sleep(5 + (org_setting_value * 60))
            else:
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_003():
    """
    Sub Feature Code: UI_RP_upi_collect_Pending_Via_upi_Collect_Checkstatus_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay Pending upi collect txn via KOTAK OLIVE using check status
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RP_UPI_COLLECT, 003->TC003
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_103_026():
    """
    Sub Feature Code: UI_RP_Upi_Collect_Amount_Mismatch_Via_Checkstatus_KOTAK_OLIVE
    Sub Feature Description: Verification of a Remote Pay upi for amount mismatch via KOTAK OLIVE using check status
    TC naming code description: d103: Dev Project[KOTAK_OLIVE_UPI], 103-> RP_UPI_COLLECT, 026->TC026
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
