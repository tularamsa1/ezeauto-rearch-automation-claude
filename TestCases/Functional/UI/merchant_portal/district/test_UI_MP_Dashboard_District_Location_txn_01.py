...
def test_mp_700_704_006():
    """
    Sub Feature Code: UI_MP_Calender_txn_for_specific_location_using_dropdown_values_as_District_head
    Sub Feature Description: Verifying txn details on dashboard  for specific location using dropdown values when logging in as District Head
    ...
    try:
        ...
        login_username = cred_dict['DISTRICT']['username']
        logger.debug(f"Fetched login_username credentials from the ezeauto db : {login_username}")
        login_password = cred_dict['DISTRICT']['password']
        ...
        try:
            ...
            location_hierarchy = ["Mumbai_City_District", "Akole", "Mumbai1", "Hub1", str(node_name)]
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_704_007():
    """
    Sub Feature Code: UI_MP_Calender_txn_for_specific_location_using_search_option_as_District_head
    Sub Feature Description: Verifying txn details on dashboard  for specific location using search option when logging in as District Head
    TC naming code description:
    700: Merchant Portal
    704: District
    007: TC007
    ...
    try:
        ...
        try:
            ...
            login_page_portal.perform_login_to_portal(login_username, login_password)
            portal_dashboard_page = PortalDashboardPage(GlobalVariables.portal_page)
            portal_dashboard_page.select_location_by_search_box(location)
            ...
            query = "select sum(amount) from txn where" \
                    " id between '" + current_start_date_1 + "' and '" + current_end_date_1 + "' and username  = '" + txn_username + "' and status = 'AUTHORIZED';"
            ...
            query = "select count(*) from txn" \
                    " where id between '" + current_start_date_1 + "' and '" + current_end_date_1 + "' and username  = '" + txn_username + "' and status = 'AUTHORIZED';"
            ...
            query = "select parent_node_id from org_structure where " \
                    "org_code = '" + org_code + "' and org_employee_username = '" + txn_username + "' and node_name = '" + location + "';"
            logger.debug(f"query to fetch parent node id from db : {query}")
            result = DBProcessor.getValueFromDB(query)
            parent_node_id = str(result).split()[-1]
            logger.debug(f"parent node from db is : {parent_node_id}")
            ...
            for card_type in txn_by_type_cards:
                total_number_of_txns += sum(
                    [ele['cards'][card_type]['count'] for ele in response['aggregatedTxnResults']])
            logger.info(f"total number of txns of {location} : {total_number_of_txns}")
            org_code_api_1 = response['orgCode']
            logger.info(f"org code from api :{org_code_api_1}")
            username_api_1 = response['username']
            logger.info(f"username from api :{username_api_1}")
