...
def test_common_100_115_07_055():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by Brand fixed success transaction for root org via HDFC
    Dummy PG using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 055: TC055
    """
    try:
        ...
        query = f"select * from emi where org_code='{org_code}' and issuer_code='HDFC' and card_type='CREDIT' and " \
                f"term = '{emi_plan_in_months} month' and emi_type='BRAND' and tid_type='SUBVENTION'; "
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='ALL' and card_type= 'CREDIT' and eze_emi_enabled=b'0';"
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(
                    f"Fetching additional payback from txn history for the txn : {txn_id}, {app_additional_payback}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                date_and_time_portal = date_time_converter.to_portal_format(created_date_db=txn_created_time)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_056():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by Brand fixed success transaction for root org via HDFC
    Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 056: TC056
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        testsuite_teardown.update_emi_status_for_root_org(root_org_code=root_org_code, card_type='CREDIT', status='ACTIVE',
                                                          issuer_code='HDFC', emi_type='BRAND')
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_057():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Fixed_Success_Txn_For_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_9_Months_Tenure
    Sub Feature Description: Performing the brand EMI payback by Brand fixed success transaction for root org via HDFC
    Dummy PG using EMV VISA Credit card with pin for 9 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 057: TC057
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_111():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Instant Discount payback by brand percentage success transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 111: TC111
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "total_interest": "{:,.2f}".format(total_interest),
                    "total_emi_amt": "{:,.2f}".format(total_emi),
                    "loan_amt": "{:,.2f}".format(final_amount - cal_additional_payback),
                    "interest_amt": "{:,.2f}".format(total_interest),
                    "net_eff_price": "{:,.2f}".format(total_emi),
                    "brand": brand_name,
                    "product": brand_sku_name,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Loan Amount": "Rs." + "{:.2f}".format(final_amount - cal_additional_payback),
                    "Applicable Intt Rate(P.A.)": f"{interest_rate}%",
                    "Interest Amt": "Rs." + "{:.2f}".format(total_interest),
                    "EMI Amt": "Rs." + "{:.2f}".format(monthly_emi),
                    "Total Amt with Interest": "Rs." + "{:.2f}".format(total_emi),
                    ...
                    "Discount": "Rs." + "{:.2f}".format(total_discount_value)
                }
                logger.debug(f"expected_charge_slip_values: {expected_charge_slip_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_112():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Payback_by_Brand_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_9_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Instant Discount payback by brand percentage success transaction for an org (not ezetap)
    via HDFC Dummy PG using EMV VISA Debit card with pin for 9 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 112: TC112
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
