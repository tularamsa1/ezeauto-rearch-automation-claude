...
@pytest.mark.apiVal
def test_common_100_103_476():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Card_Success_Credit_Card_RemotePay_CC_Option_Disabled
    Sub Feature Description: Verify Credit Card - Remote Pay txn when Remote Pay is enabled with DC, NB and not enabled with CC
    TC naming code description: 100: Payment Method, 103: RemotePay, 476: TC_476
    """
    try:
        ...
        try:
            ...
            if not response['success']:
                ...
                try:
                    ...
                    actual_credit_pm_text = False
                    logger.debug(f"Credit card payment mode is not present")
                actual_debit_pm_text = remote_pay_txn.fetch_debit_card_payment_mode_text()
                logger.info(f"Fetching debit card payment mode text: {actual_debit_pm_text}")
                ...
                assert actual_credit_pm_text == False, "Credit card payment mode is available"
                assert "Debit Card" == actual_debit_pm_text, "Debit text is not matching."
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_100_103_477():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Card_Success_Debit_Card_RemotePay_DC_Option_Disabled
    Sub Feature Description: Verify Debit Card - Remote Pay txn when Remote Pay is enabled with CC, NB and not enabled with DC
    TC naming code description: 100: Payment Method, 103: RemotePay, 477: TC_477
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            if not response['success']:
                ...
                try:
                    ...
                    actual_debit_pm_text = False
                    logger.debug(f"Debit card payment mode is not present")
                actual_credit_pm_text = remote_pay_txn.fetch_credit_card_payment_mode_text()
                logger.info(f"Fetching credit card payment mode text: {actual_credit_pm_text}")
                actual_nb_pm_text = remote_pay_txn.fetch_net_banking_payment_mode_text()
                logger.info(f"Fetching net banking payment mode text: {actual_nb_pm_text}")

                assert actual_debit_pm_text == False, "Debit card payment mode is available"
...
def test_common_100_103_479():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_Card_Check_PaymentID_Captured_Under_AdditionalField2
    Sub Feature Description:Verify under additional field 2 in txn table payment id will be captuted on completing debit card txn
    TC naming code description: 100: Payment Method, 103: RemotePay, 479: TC_479
    """
    ...
    try:
        ...
        try:
            ...
            if not response['success']:
                ...
                remote_pay_txn.enterDebitCardExpiryMonth("3")
                remote_pay_txn.enterDebitCardExpiryYear("2048")
            ...
            cnp_payment_id = result['additional_field2'].values[0]
            logger.debug(f"payment ID from cnp_txn table : {cnp_payment_id}")
            ...
            cnpware_payment_id = result['additional_field2'].values[0]
            logger.debug(f"Query result, cnpware_payment_id : {cnpware_payment_id}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_480():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_Card_Check_OrderID_Captured_Under_AdditionalField5
    Sub Feature Description:Verify under additional field 5 orderid id will be captuted  on completing debit card txn
    TC naming code description: 100: Payment Method, 103: RemotePay, 480: TC_480
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        try:
            ...
            txn_card_type = result['payment_option'].values[0]
            logger.debug(f"txn card type from db : {txn_card_type}")
            cnp_order_id = result['additional_field5'].values[0]
            logger.debug(f"payment ID from cnp_txn table : {cnp_order_id}")
            ...
            cnpware_payment_gateway = result['payment_gateway'].values[0]
            logger.debug(f"Query result, cnpware_payment_gateway : {cnpware_payment_gateway}")
            cnpware_order_id = result['additional_field5'].values[0]
            logger.debug(f"Query result, cnpware_order_id : {cnpware_order_id}")
