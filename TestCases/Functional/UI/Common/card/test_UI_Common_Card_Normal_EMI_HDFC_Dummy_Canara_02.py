...
def test_common_100_115_05_063():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_BO_Instant_Discount_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the normal EMI BO Instant Discount transaction for an org (not ezetap) via HDFC Dummy PG
    for Canara issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 063: TC063
    """
    try:
        ...
        query = f"select * from subvention_split where rule_id='{rule_id}'"
        ...
        discount_value = result['subvention_value'].values[0]
        logger.debug(f"Fetching subvention_value from the subvention_split table : {discount_value}")
        ...
        try:
            ...
            cal_total_interest = total_emi - final_amount
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "instant_discount": discount_value
                }
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_064():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_BO_Cashback_Discount_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the normal EMI BO Cashback Discount transaction for an org (not ezetap) via HDFC Dummy PG
    for Canara issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 064: TC064
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            total_emi = round(cal_total_emi_amt, 2)
            logger.debug(f"calculated total_emi amount : {total_emi}")
            net_amt = total_emi - discount_value
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "additional_cashback": "{:,.2f}".format(discount_value)
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "emi_type": "NORMAL_EMI",
                    "orig_amt": amount,
                    "bo_additional_cashback": discount_value
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                expected_db_values = {
                    ...
                    "bo_txn_type": "NORMAL_EMI",
                    "orig_amt": float(amount),
                    "bo_additional_cashback_amt": discount_value
                }
                logger.debug(f"expected_db_values: {expected_db_values}")
