...
def test_common_100_115_099():
    """
    Sub Feature Code: UI_Common_Card_Sale_ATOS_TLE_EMVCTLS_VISA_DebitCard_With_Pin_476173
    Sub Feature Description: Performing the EMVCTLS sale transaction via ATOS_TLE PG using VISA Debit card having
    16 digit PAN length with pin (bin:476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 099: TC099
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_100():
    """
    Sub Feature Code: UI_Common_Card_Sale_ATOS_TLE_EMVCTLS_MASTER_DebitCard_With_Pin_222360
    Sub Feature Description: Performing the EMVCTLS sale transaction via ATOS_TLE PG using MASTER Debit card having
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"From response fetch device_serial : {api_device_serial}")
                api_card_txn_type_desc = response.get('cardTxnTypeDesc')
                logger.debug(f"From response fetch ard_txn_type_desc : {api_card_txn_type_desc}")
                api_merchant_name = response.get('merchantName')
                logger.debug(f"From response fetch merchant_name : {api_merchant_name}")
                api_card_last_four_digit = response.get('cardLastFourDigit')
                logger.debug(f"From response fetch card_last_four_digit : {api_card_last_four_digit}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_101():
    """
    Sub Feature Code: UI_Common_Card_Sale_ATOS_TLE_EMVCTLS_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description: Performing the EMVCTLS sale transaction via ATOS_TLE PG using RUPAY Debit card having
    16 digit PAN length with pin (bin:608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 101: TC101
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_102():
    """
    Sub Feature Code: UI_Common_Card_Sale_ATOS_TLE_EMVCTLS_VISA_DebitCard_Without_Pin_476173
    Sub Feature Description: Performing the EMVCTLS sale transaction via ATOS_TLE PG using VISA Debit card having
    16 digit PAN length without pin (bin:476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 102: TC102
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                app_payment_status = txn_history_page.fetch_txn_status_text()
                logger.info(
                    f"Fetching payment_status from txn history for the txn : {txn_id}, {app_payment_status}")
                app_txn_id = txn_history_page.fetch_txn_id_text()
                logger.info(f"Fetching txn_id from txn history for the txn : {txn_id}, {app_txn_id}")
                app_date_time = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching date_time from txn history for the txn : {txn_id}, {app_date_time}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_103():
    """
    Sub Feature Code: UI_Common_Card_Sale_ATOS_TLE_EMVCTLS_MASTER_DebitCard_Without_Pin_222360
    Sub Feature Description: Performing the EMVCTLS sale transaction via ATOS_TLE PG using MASTER Debit card having
    16 digit PAN length without pin (bin:222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 103: TC103
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
