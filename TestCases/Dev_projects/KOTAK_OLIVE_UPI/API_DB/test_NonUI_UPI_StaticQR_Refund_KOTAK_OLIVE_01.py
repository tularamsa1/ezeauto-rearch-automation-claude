...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_107_007():
    """
    Sub Feature Code: NonUI_Common_UPI_StaticQR_Refund_Success_KOTAK_OLIVE
    Sub Feature Description: Initiate upi static QR via api and perform upi success callback and perform refund for the
    same via KOTAK_OLIVE
    TC naming code description: d103->Dev Project[KOTAK_OLIVE_UPI], 107-> UPI Static QR, 007->TC007
    """
    try:
        ...
        try:
            ...
            logger.debug(f"performing refund through api")
            ...
            logger.debug(f"fetched refund txnId is : {txn_id_2}")
            ...
            logger.debug(f"Query to fetch refund txn data from the txn table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result : {result}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                date_time_2 = date_time_converter.db_datetime(created_time_2)
                expected_api_values = {
                ...
                response = [x for x in response_list["txns"] if x["txnId"] == txn_id_2][0]
                logger.debug(f"Response after filtering data of current txn is : {response}")
                ...
                actual_api_values = {
                    ...
                    "mid_2": mid_api_2, "tid_2": tid_api_2,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "upi_txn_ref_2": f"KMBM{prop_value}00{txn_id_2}",
                    "upi_customer_ref_2": rrn_2,
                    "upi_resp_code_2": None
                }
                ...
                upi_txn_ref_db_2 = result["txn_ref"].iloc[0]
                upi_customer_ref_db_2 = result["customer_ref"].iloc[0]
                upi_resp_code_db_2 = result["resp_code"].iloc[0]
                ...
                actual_db_values = {
                    ...
                    "upi_txn_ref_2": upi_txn_ref_db_2,
                    "upi_customer_ref_2": upi_customer_ref_db_2,
                    "upi_resp_code_2": upi_resp_code_db_2,
                }
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_107_008():
    """
    Sub Feature Code: NonUI_Common_UPI_StaticQR_Refund_Failed_KOTAK_OLIVE
    Sub Feature Description: Initiate upi static QR via api and perform upi success callback and perform refund failed
    for the same via KOTAK_OLIVE.
    TC naming code description: d103->Dev Project[KOTAK_OLIVE_UPI], 107-> UPI Static QR, 008->TC008
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
