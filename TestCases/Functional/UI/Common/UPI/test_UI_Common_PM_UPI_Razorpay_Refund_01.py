...
def test_common_100_101_272():
    """
    Sub Feature Code: UI_Common_PM_Pure_UPI_Refund_Posted_via_API_Razorpay
    Sub Feature Description: Verification of a  upi refund posted using api for Razorpay
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                for elements in responseInList:
                    if elements["txnId"] == txn_id_original:
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                status_db_original = result["status"].values[0]
                payment_mode_db_original = result["payment_mode"].values[0]
                amount_db_original = int(result["amount"].values[0])
                state_db_original = result["state"].values[0]
                payment_gateway_db_original = result["payment_gateway"].values[0]
                acquirer_code_db_original = result["acquirer_code"].values[0]
                bank_code_db_original = result["bank_code"].values[0]
                settlement_status_db_original = result["settlement_status"].values[0]
                ...
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                upi_status_db_original = result["status"].values[0]
                upi_txn_type_db_original = result["txn_type"].values[0]
                upi_bank_code_db_original = result["bank_code"].values[0]
                upi_mc_id_db_original = result["upi_mc_id"].values[0]
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_294():
    """
    Sub Feature Code: UI_Common_PM_UPI_Full_Refund_Via_API_Razorpay
    Sub Feature Description: Verification of a refund using api for Razorpay
    TC naming code description: 100: Payment Method, 101: UPI, 294: TC294
    """
    ...
    try:
        ...
        try:
            ...
            amount = random.randint(1500, 1600)
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.debug(f"Order id : {order_id}")
            home_page.enter_amount_and_order_number(amount, order_id)
            logger.debug(f"Entered amount is : {amount}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_298():
    """
    Sub Feature Code: UI_Common_PM_Pure_UPI_Refund_Failed_via_API_Razorpay
    Sub Feature Description:  Verification of a pure upi refund failed using api for Razorpay
    TC naming code description: 100: Payment Method, 101: UPI, 298: TC298
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
