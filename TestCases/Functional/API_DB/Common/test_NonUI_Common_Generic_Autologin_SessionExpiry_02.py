...
# app_password = "A1234567"
# portal_username = "8976986700"
# portal_password = "S1234567"

@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_400_401_020():  # Make sure to add the test case name as same as the sub feature code.
    """
    Sub Feature Code: NonUI_Common_Generic_Autologin_SessionExpiry_EmptyAppID
    Sub Feature Description: Do txn with empty appID in txn after session expiry -> Expiry time = 60sec and autologin disabled. Steps: -> Do login -> Wait to expire -> cash txn should fail
        ...
        try:
            ...
            if login_success == True:
                ...
                if actual_cashTxn_success == True:
                    logger.error(f"Cash txn is Success But expected to be failed due to empty appID")

                else:
                    logger.info(f"Cash txn failed as expected due to empty appID")
            else:
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_400_401_021():  # Make sure to add the test case name as same as the sub feature code.
    """
    Sub Feature Code: NonUI_Common_Generic_Autologin_SessionExpiry_EmptyDeviceID
    Sub Feature Description: Do txn with empty deviceID in txn after session expiry -> Expiry time =60sec and autologin disabled-> Steps: Do login -> Wait to expire -> cash txn should fail
    TC naming code description:
    400: Generic Function
    401: Autologin
    027: TC021
    ...
    try:
        ...
        try:
            ...
            if login_success == True:
                ...
                if actual_cashTxn_success == True:
                    logger.error(f"Cash txn is Success But expected to be failed due to empty deviceIdentifier")

                else:
                    logger.info(f"Cash txn failed as expected due to empty deviceIdentifier")
            else:
                logger.error(f"Login Failed, expected to be success")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_400_401_022():  # Make sure to add the test case name as same as the sub feature code.
    """
    Sub Feature Code: NonUI_Common_Generic_Autologin_SessionExpiry_EmptyDeviceIDType
    Sub Feature Description: Do txn with empty deviceIDType in txn after session expiry -> Expiry time= 60 sec and autologin disabled -> Steps: Do login -> Wait to expire -> cash txn should fail
    TC naming code description:
    400: Generic function
    ...
    try:
        ...
        # Set autologin enabled as True
        orgsettings_apidetails_autologinDisable = DBProcessor.get_api_details('org_settings_update',
                                                                              request_body={"username": portal_username,
                                                                                            "password": portal_password,
                                                                                            "settingForOrgCode": org_code})
        orgsettings_apidetails_autologinDisable["RequestBody"]["settings"]["autoLoginByTokenEnabled"] = "true"
        ...
        try:
            ...
            if login_success == True:
                ...
                if actual_cashTxn_success == True:
                    logger.error(f"Cash txn is Success But expected to be failed due to empty deviceIdentifierType")

                else:
                    logger.info(f"Cash txn failed as expected due to invalid deviceIdentifierType")
            else:
                logger.error(f"Login Failed, expected to be success")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_400_401_023():  # Make sure to add the test case name as same as the sub feature code.
    """
    Sub Feature Code: NonUI_Common_Generic_Autologin_SessionExpiry_AutoLoginDisabled
    Sub Feature Description: Do txn with autologin disabled after session expiry -> Steps: Expiry time= 60 sec and autologin disabled-> Do login -> Wait to expire -> cash txn should fail
    TC naming code description:
    400: Generic function
    401: Autologin feature
    023: TC023
    ...
    try:
        ...
        try:
            ...
            if login_success == True:
                ...
                if actual_cashTxn_success == True:
                    logger.error(f"Cash txn is Success But expected to be failed with autologin disabled")

                else:
                    logger.info(f"Cash txn failed as expected with autologin disabled")
            else:
                logger.error(f"Login Failed, expected to be success")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_400_401_024():  # Make sure to add the test case name as same as the sub feature code.
    """
    Sub Feature Code: NonUI_Common_Generic_Autologin_SessionExpiry_WithoutAppID
    Sub Feature Description: Do txn after session expiry, without appID -> Expiry time= 60 sec and autologin enabled -> Steps: Do login -> Wait to expire -> cash txn should fail
    TC naming code description:
    400: Generic function
    401: Autologin feature
    024: TC024
    ...
    try:
        ...
        try:
            ...
            if login_success == True:
                ...
                if actual_cashTxn_success == True:
                    logger.error(f"Cash txn is Success But expected to be failed without appID")

                else:
                    logger.info(f"Cash txn failed as expected without appID")
            else:
                logger.error(f"Login Failed, expected to be success")
