...
def test_common_100_102_050():
    """
    Sub Feature Code: UI_Common_BQRV4_UPI_Success_Checkstatus_HDFC
    Sub Feature Description: Verification of a BQRV4 successful Check status HDFC
    TC naming code description: 100: Payment Method, 112: BQRV4, 050: TC050
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_131():
    """
    Sub Feature Code: UI_Common_BQRV4_UPI_Checkstatus_Amount_Missmatch_HDFC
    Sub Feature Description: Verification of a BQRV4 UPI Amount mismatch upon Checkstatus via HDFC
    TC naming code description: 100: Payment Method, 102: BQRV4, 131: TC131
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Fetching Txn_id from the API_OUTPUT, Txn_id : {txn_id}")
            logger.debug("Waiting for 1 min to QR code to get expired")
            sleep(60)
