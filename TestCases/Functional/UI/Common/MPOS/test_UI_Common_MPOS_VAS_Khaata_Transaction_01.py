    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"khaat_date : {khaat_date}")
                khaata_entry_amount = khaata.fetch_entry_amount()
                logger.debug(f"khaata_entry_amount : {khaata_entry_amount}")
                khaata_amount = khaata_entry_amount.replace("\xa0", "")
                logger.debug(f"khaata_amount : {khaata_amount}")
                khaata_message = khaata.fetch_entry_message()
                logger.debug(f"khaata_message : {khaata_message}")
                khaata_description = khaata.fetch_entry_description()
                logger.debug(f"khaata_description : {khaata_description}")
...
def test_mpos_600_601_016():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Khaata txn is being done by clicking on you got button")
            khaata.click_on_back()
            khaata.click_khaata_entries()
            khaata.click_on_recent_entry()
            # ------------------------------------------------------------------------------------------------
