...
def test_common_100_115_05_080():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Success_Txn_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the normal EMI success transaction for an org (not ezetap) via HDFC Dummy PG for
    ICICI issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 080: TC080
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_081():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Success_Txn_For_Fallback_Root_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the normal EMI success transaction for fallback to root org via HDFC Dummy PG
    for ICICI issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 081: TC081
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_083():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Txn_EMI_Options_Not_Available_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090
    Sub Feature Description: Performing the normal EMI transaction when the EMI options are not available for an org
    (not ezetap) via HDFC Dummy PG for ICICI issuer using EMV VISA Debit card with pin (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 083: TC083
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_084():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Txn_ICICI_Debit_Issuer_Not_Enabled_In_Org_Settings_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090
    Sub Feature Description: Performing the normal EMI transaction when ICICI debit issuer is not enabled in the org settings
    for an org (not ezetap) via HDFC Dummy PG for ICICI issuer using EMV VISA Debit card with pin (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 084: TC084
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_085():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Block_Success_Txn_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
    Sub Feature Description: Performing the normal EMI Block success transaction for an org (not ezetap) via HDFC Dummy PG
    for ICICI issuer using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 085: TC085
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
