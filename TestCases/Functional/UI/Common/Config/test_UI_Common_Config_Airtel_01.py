...
@pytest.mark.appVal
def test_common_100_300_302_014():
    """
    Sub Feature Code: UI_Common_Config_Airtel_Verify_Config_Screen_With_All_LOB
    Sub Feature Description: Verify config screen with all the LOB's displayed as per configured in UI JSON
    TC naming code description: 100: Payment Method, 300: Config, 302: Airtel, 014: TC014
    """
    try:
        ...
        try:
            ...
            logger.info(f"Fetching all the lobs text in UI screen")
            list_of_lobs = home_page.fetch_all_lobs()
            logger.info(f"Fetched all the lobs text in UI screen, {list_of_lobs}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                expected_app_values = {
                    ...
                    "pre_paid_mobile": "Pre Paid-Mobile",
                ...
                actual_app_values = {
                    "postpaid_mobile": list_of_lobs[0],
                    "fixed_line": list_of_lobs[1],
                    "pre_paid_mobile": list_of_lobs[2],
                    "new_activation": list_of_lobs[3],
                    "pos_sales": list_of_lobs[4],
                    "dth": list_of_lobs[5]
                }
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_300_302_015():
    """
    Sub Feature Code: UI_Common_Config_Airtel_Verify_Fetch_For_Fixed_Line_LOB (Sub_lob : Bill_Collection_Active)
    Sub Feature Description: Verify fetch is successful for fixed line lob (Sub_lob : Bill_Collection_Active)
    TC naming code description: 100: Payment Method, 300: Config, 302: Airtel, 015: TC015
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.click_on_start_btn()
            logger.info(f"Clicked on start button")
            home_page.click_on_fixed_line_LOB()
            logger.info(f"Clicked on fixed line lob")
            logger.info(f"Clicking on bill collection active sub lob")
            home_page.click_on_fixed_line_bill_collection_active()
            logger.info(f"Clicked on fixed line bill collection active sub lob")
            fixed_line_number = "2231231891"
            home_page.enter_bill_collection_active_details(fixed_line_number)
            logger.info(f"Entered bill collection active deatils")

            try:
                logger.info(f"Validating the fixed line tittle and checkout btn after clicking on fetch details")
                home_page.validate_fixed_line_no_title()
                logger.info(f"Fixed line tittle is visible")
                home_page.click_on_check_out_btn()
                logger.info(f"Checkout button is visible")
...
def test_common_100_300_302_016():
    """
    Sub Feature Code: UI_Common_Config_Airtel_Verify_Success_Cash_Txn_Fixed_Line_LOB (Sub_lob : Bill_Collection_Active)
    Sub Feature Description: Verify success cash txn for fixed line lob (Sub_lob : Bill_Collection_Active)
    TC naming code description: 100: Payment Method, 300: Config, 302: Airtel, 016: TC016
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            fixed_line_number = "2231231891"
            home_page.enter_bill_collection_active_details(fixed_line_number)
            logger.info(f"Entered bill collection active details")
            home_page.click_on_check_out_btn()
            logger.info(f"Clicked on check out btn")

            # enter mobile no and email address
            mobile_no = '2028289028'
            email_address = "ezeauto@gmail.com"
            payment_page = PaymentPage(app_driver)
            payment_page.enter_mobile_number_and_email(mobile_no, email_address)
            logger.info(f"Entered mobile_no and email address")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_300_302_017():
    """
    Sub Feature Code: UI_Common_Config_Airtel_Verify_Fetch_For_DTH_LOB (Sub_lob : Search_By_DTH_ID)
    Sub Feature Description: Verify fetch is successful for DTH lob (Sub_lob : Search_By_DTH_ID)
    TC naming code description: 100: Payment Method, 300: Config, 302: Airtel, 017: TC017
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.click_on_start_btn()
            logger.info(f"Clicked on start button")
            home_page.click_on_dth_LOB()
            logger.info(f"Clicked on DTH LOB")

            try:
                dth_id_val = "2782820909"
                customer_mobile_number = "2345678920"
                alternative_number = "3293393456"
                email = "ezeauto@gamil.com"

                home_page.enter_customer_details_for_search_by_id(dth_id_val, customer_mobile_number,
                                                                  alternative_number, email)
                logger.info(f"Clicked on search by DTH ID and clicked on search details")
                logger.info("Able to fetch details and not navigate to amount screen")
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal  ##  Chargeslip validation is failing ticket is raised and validation is not covered
def test_common_100_300_302_018():
    """
    Sub Feature Code: UI_Common_Config_Airtel_Verify_Success_Cash_Txn_DTHLOB (Sub_lob : Search_By_DTH_ID)
    Sub Feature Description: Verify success cash txn for DTH LOB (Sub_lob : Search_By_DTH_ID)
    TC naming code description: 100: Payment Method, 300: Config, 302: Airtel, 018: TC018
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "merchant_code": org_code,
                    "external_ref": order_no_api,
                    "external_ref4": dth_id_val,
                    "external_ref5": float(amount),
                    "txn_amt": amount,
                    "amt_original": amount
                }
                logger.debug(f"expected_db_values: {expected_db_values}")
                ...
                actual_db_values = {
                    ...
                    "external_ref5": float(external_ref5_db),
                    "txn_amt": amount,
                    "amt_original": amount_original_db
                }
                logger.debug(f"actual_db_values : {actual_db_values}")
