...
def test_common_100_103_141():
    """
    Sub Feature Code: UI_Common_PM_RP_2_UPI_Collect_success_callback_after_expiry_AXIS_DIRECT_AutoRefund_Disabled
    Sub Feature Description: Performing two UPI Collect success callback via Axis Direct after expiry when autorefund is disabled
    100: Payment Method
    103: RemotePay
    141: TC141
    """
    try:
        ...
        try:
            ...
            if org_setting_value:
                ...
                logger.info(f"Txn timeout for Ezetap org is: {org_setting_value}")
            ...
            query = "select * from payment_intent where org_code = '" + str(
                org_code) + "' AND external_ref = '" + str(
            ...
            logger.debug(f"replacing the Txn_id with {payment_intent_id}, amount with {amount}.00, vpa with {vpa} and rrn with {callback_1_rrn} in the curl_data")
            ...
            data_buffer = ''
            ssh_stdin, ssh_stdout, ssh_stderr = TestSuiteSetup.GlobalVariables.ssh.exec_command(curl_data,
                                                                                                get_pty=True)
            ...
            logger.debug(
                f"preparing the request payload data to trigger the confirm api")
            api_details = DBProcessor.get_api_details('confirm_axisdirect',
                                                      request_body={"data": data_buffer})
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response : {response}")
            ...
            new_txn_created_date_api = result['created_time'].values[0]
            logger.info(f"New modified time for api is : {new_txn_created_date_api}")
            new_txn_posting_date_app = result['created_time'].values[0]
            logger.info(f"New created time for app is : {new_txn_posting_date_app}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "txn_amount_1": str(amount),
                    "settle_status_1": "SETTLED",
                    "txn_id_1": new_txn_id_1,
                    # "rrn_1": str(callback_1_rrn),
                    "customer_name_1": new_txn_customer_name_1,
                    "payer_name_1": new_txn_payer_name_1,
                    "order_id_1": order_id,
                    "payment_msg_1": "PAYMENT SUCCESSFUL",
                    "date_1": new_txn_date_and_time_1,

                    "pmt_mode_2": "UPI",
                    "pmt_status_2": "AUTHORIZED",
                    "txn_amount_2": str(amount),
                ...
                actual_app_values = {"pmt_mode_original": app_payment_mode,
                                    ...
                                    "pmt_status_1": app_payment_status_1,
                                    "txn_amount_1": app_amount_1.split(' ')[1],
                                    "settle_status_1": app_settlement_status_1,
                                    "txn_id_1": app_txn_id_1,
                                    # "rrn_1": str(app_rrn_1),
                                    "customer_name_1": app_customer_name_1,
                                    "payer_name_1": app_payer_name_1,
                                    "order_id_1": app_order_id_1,
                                    "payment_msg_1": app_payment_msg_1,
                                    "date_1": app_date_and_time_1,

                                    "pmt_mode_2": app_payment_mode_2,
                                    "pmt_status_2": app_payment_status_2,
                                    "txn_amount_2": app_amount_2.split(' ')[1],
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                new_txn_date_2 = date_time_converter.db_datetime(new_txn_created_date_api)
                expected_api_values = {"pmt_status": "FAILED",
                                       ...
                                       "new_txn_type_1": new_txn_type_1,
                                       "new_mid_1": mid,
                                       "new_tid_1": tid,
                                       "new_org_code_1": org_code,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                txn_date, txn_time = date_time_converter.to_chargeslip_format(new_txn_created_date_api)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_142():
    """
    Sub Feature Code: UI_Common_PM_RP_2_UPI_Collect_success_callback_after_expiry_AXIS_DIRECT_AutoRefund_Enabled
    Sub Feature Description: Performing two UPI Collect success callback via Axis Direct after expiry when autorefund is enabled
    100: Payment Method
    103: RemotePay
    142: TC142
    """
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_143():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_Success_One_Callback_AutoRefundDisabled_AXIS_DIRECT
    Sub Feature Description: Verification of a successful upi Collect callback via Axis Direct when auto refund is disabled.
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_144():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_failed_callback_after_expiry_AutoRefund_Enabled_AXIS_DIRECT
    Sub Feature Description: Performing a UPI Collect failed callback via Axis Direct after expiry when autorefund is enabled
    100: Payment Method
    103: RemotePay
    144: TC144
    """
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_145():
    """
    Sub Feature Code: UI_Common_PM_RP_2_UPI_Collect_failed_callback_after_expiry_AXIS_DIRECT_AutoRefund_Disabled
    Sub Feature Description: Performing two UPI Collect failed callback via Axis Direct after expiry when autorefund is disabled
    100: Payment Method
    103: RemotePay
    145: TC145
    """
