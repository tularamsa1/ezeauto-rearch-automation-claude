...
def test_mp_700_708_061():
    """
    Sub Feature Code: UI_MP_Report_based_on_Advance_filter_ALL_Payment_mode_as_Store_head
    Sub Feature Description: Verifying txn details on Report Page using Advance Filter for ALL payment mode when logging in as Store Head
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "'  AND (payment_mode='CARD' OR payment_mode='CASH' OR payment_mode='CHEQUE' OR payment_mode='CNP' OR payment_mode='WALLET' OR payment_mode='UPI' OR payment_mode='BHARATQR' OR payment_mode='NBFC EMI') and username IN ('"+txn_username+ "', '" + login_username + "') order by created_time asc limit 1;"
