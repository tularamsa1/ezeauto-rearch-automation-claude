...
def rerunTestImmediately(testCaseID, testCaseFileName, rerunCount, request):
    ...
    if setRerunCount(testCaseID, rerunCount):
        ...
        if rerunCount == -1 and configReader.read_config("Validations", "rerun_at_the_end").lower() == "false" and Base_Actions.enter_data_logs("forLastRun_Capture_Logs") == "True":
...
def prepareImmediateRerunExcel():
    ...
    # Added on Apr 11
    df_overallTClist = pd.read_excel("/home/oem/PycharmProjects/EzeAuto/TestCases/AllTestcaseSuite.xlsx")
    ...
    for i in range(0, len(df_overallTClist.index)):
        ...
        try:
            configuredImmediateRerunCount = int(configReader.read_config("Validations", "rerun_count"))
