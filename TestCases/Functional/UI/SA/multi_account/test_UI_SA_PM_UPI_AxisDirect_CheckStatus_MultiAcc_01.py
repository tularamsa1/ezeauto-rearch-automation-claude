...
def test_sa_100_109_024():
    """
    Sub Feature Code: UI_SA_PM_UPI_Success_Via_SA_CheckStatus_MultiAcc_AxisDirect
    Sub Feature Description: Multi Account - Verification of a successful upi txn via AxisDirect using SA check status
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 024: TC024
    """
    try:
        ...
        try:
            ...
            query = f"select * from upi_merchant_config where org_code ='{org_code}' AND status = 'ACTIVE' AND " \
                    f"bank_code = 'AXIS_DIRECT' AND acc_label_id=(select id from label " \
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_sa_100_109_025():
    """
    Sub Feature Code: UI_SA_PM_UPI_Failed_Via_SA_CheckStatus_MultiAcc_AxisDirect
    Sub Feature Description: Multi Account - Verification of a failed UPI txn via AxisDirect using SA check status
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 025: TC025
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_sa_100_109_026():
    """
    Sub Feature Code: UI_SA_PM_UPI_Success_Via_SA_CheckStatus_with_2nd_label_MultiAcc_AXIS_DIRECT
    Sub Feature Description: Multi Account - Performing a successful upi txn via AXIS_DIRECT using SA check status with second account(ex:acc2 label)
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 026: TC026
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
