...
@pytest.mark.appVal
def test_common_100_102_272():
    """
    Sub Feature Code: Tid Dep – UI_Common_PM_BQRV4_BQR_UPG_AUTHORIZED_KOTAK_ATOS
    Sub Feature Description: Tid Dep - Verification of a upg authorized txn using BQRV4 BQR Success Callback via Kotak_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 272: TC272
    """
    try:
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_102_273():
    """
    Sub Feature Code: Tid Dep – UI_Common_PM_BQRV4_BQR_amount_mismatch_Via_BQRV4_BQR_Success_Callback_KOTAK_ATOS
    Sub Feature Description: Tid Dep - Performing a amount mismatch using BQRV4 bqr success callback via KOTAK_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 273: TC273
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Fetching customer_name from the txn table : customer_name : {customer_name_db}")
            payer_name_db = result['payer_name'].values[0]
            logger.debug(f"Fetching payer_name from the txn table : payer_name : {payer_name_db}")
            org_code_db = result['org_code'].values[0]
            logger.debug(f"Fetching org_code from the txn table : org_code : {org_code_db}")
            txn_type_db = result['txn_type'].values[0]
            logger.debug(f"Fetching txn_type from the txn table : txn_type : {txn_type_db}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "pmt_state_1": "PENDING",
                    "settle_status_1": "PENDING",
                    "acquirer_code_1": "KOTAK",
                    "issuer_code_1": "KOTAK",
                    "txn_type_1": "CHARGE", "mid_1": mid, "tid_1": tid,
                    "org_code_1": org_code,
                ...
                for elements in response_in_list:
                    ...
                    if elements["txnId"] == txn_id:
                        device_serial_api_1 = elements["deviceSerial"]
