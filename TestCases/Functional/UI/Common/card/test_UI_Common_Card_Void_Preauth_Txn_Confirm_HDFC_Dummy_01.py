...
def test_common_100_115_03_008():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Query to fetch data from the txn table based first txn : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Fetching result for txn table based first txn : {result}")
            txn_created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created_time value from the txn table based first txn : {txn_created_time}")
            rrn = result['rr_number'].values[0]
            logger.debug(f"Fetching rrn value from the txn table based first txn : {rrn}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching auth_code value from the txn based first txn : {auth_code}")
            batch_number = result['batch_number'].values[0]
            logger.debug(f"Fetching batch_number value from the txn table based first txn : {batch_number}")
            amount_db = result['amount'].values[0]
            logger.debug(f"Fetching amount value from the txn table based first txn : {amount_db}")
            payment_mode_db = result['payment_mode'].values[0]
            logger.debug(f"Fetching payment_mode value from the txn table based first txn : {payment_mode_db}")
            payment_status_db = result['status'].values[0]
            logger.debug(f"Fetching status value from the txn table based first txn : {payment_status_db}")
            payment_state_db = result['state'].values[0]
            logger.debug(f"Fetching state value from the txn table based first txn : {payment_state_db}")
            acquirer_code_db = result['acquirer_code'].values[0]
            logger.debug(f"Fetching acquirer_code value from the txn table based first txn : {acquirer_code_db}")
            mid_db = result['mid'].values[0]
            logger.debug(f"Fetching mid value from the txn table based first txn : {mid_db}")
            tid_db = result['tid'].values[0]
            logger.debug(f"Fetching tid value from the txn table based first txn : {tid_db}")
            payment_gateway_db = result['payment_gateway'].values[0]
            logger.debug(f"Fetching payment_gateway value from the txn table based first txn : {payment_gateway_db}")
            settlement_status_db = result['settlement_status'].values[0]
            logger.debug(f"Fetching settlement_status value from the txn table based first txn : {settlement_status_db}")
            device_serial_db = result['device_serial'].values[0]
            logger.debug(f"Fetching device_serial value from the txn table based first txn : {device_serial_db}")
            merchant_name = result['merchant_name'].values[0]
            logger.debug(f"Fetching merchant_name value from the txn table based first txn : {merchant_name}")
            merchant_code_db = result["merchant_code"].iloc[0]
            logger.debug(f"Fetching merchant_code value from the txn table based first txn : {merchant_code_db}")
            payment_card_brand_db = result["payment_card_brand"].iloc[0]
            logger.debug(f"Fetching payment_card_brand value from the txn table based first txn : {payment_card_brand_db}")
            payment_card_type_db = result["payment_card_type"].iloc[0]
            logger.debug(f"Fetching payment_card_type value from the txn table based first txn : {payment_card_type_db}")
            payment_card_bin_db = result["payment_card_bin"].iloc[0]
            logger.debug(f"Fetching payment_card_bin value from the txn table based first txn : {payment_card_bin_db}")
            terminal_info_id_db = result["terminal_info_id"].iloc[0]
            logger.debug(f"Fetching terminal_info_id value from the txn table based first txn : {terminal_info_id_db}")
            txn_type_db = result["txn_type"].iloc[0]
            logger.debug(f"Fetching txn_type value from the txn table based first txn : {txn_type_db}")
            card_txn_type_db = result["card_txn_type"].iloc[0]
            logger.debug(f"Fetching card_txn_type value from the txn table based first txn : {card_txn_type_db}")
            card_last_four_digit_db = result["card_last_four_digit"].iloc[0]
            logger.debug(f"Fetching card_last_four_digit value from the txn table based first txn : {card_last_four_digit_db}")
            org_code_db = result["org_code"].iloc[0]
            logger.debug(f"Fetching org_code value from the txn table based first txn : {org_code_db}")
            order_id_db = result["external_ref"].iloc[0]
            logger.debug(f"Fetching external_ref value from the txn table based first txn : {order_id_db}")
            posting_date = result['posting_date'].values[0]
            logger.debug(f"Fetching posting_date value from the txn table based first txn : {posting_date}")
            ...
            logger.debug(f"Fetching batch_number value from the txn table for void txn : {batch_number_2}")
            amount_db_2 = result['amount'].values[0]
            logger.debug(f"Fetching amount value from the txn table for void txn : {amount_db_2}")
            amt_cash_back_db_2 = result['amount_cash_back'].values[0]
            logger.debug(f"Fetching amount_cash_back value from the txn table for void txn : {amt_cash_back_db_2}")
            payment_mode_db_2 = result['payment_mode'].values[0]
            logger.debug(f"Fetching payment_mode value from the txn table for void txn : {payment_mode_db_2}")
            payment_status_db_2 = result['status'].values[0]
            logger.debug(f"Fetching status value from the txn table for void txn : {payment_status_db_2}")
            payment_state_db_2 = result['state'].values[0]
            logger.debug(f"Fetching state value from the txn table for void txn : {payment_state_db_2}")
            acquirer_code_db_2 = result['acquirer_code'].values[0]
            logger.debug(f"Fetching acquirer_code value from the txn table for void txn : {acquirer_code_db_2}")
            mid_db_2 = result['mid'].values[0]
            logger.debug(f"Fetching mid value from the txn table for void txn : {mid_db_2}")
            tid_db_2 = result['tid'].values[0]
            logger.debug(f"Fetching tid value from the txn table for void txn : {tid_db_2}")
            payment_gateway_db_2 = result['payment_gateway'].values[0]
            logger.debug(f"Fetching payment_gateway value from the txn table for void txn : {payment_gateway_db_2}")
            settlement_status_db_2 = result['settlement_status'].values[0]
            logger.debug(f"Fetching settlement_status value from the txn table for void txn : {settlement_status_db_2}")
            device_serial_db_2 = result['device_serial'].values[0]
            logger.debug(f"Fetching device_serial value from the txn table for void txn : {device_serial_db_2}")
            merchant_name_db_2 = result['merchant_name'].values[0]
            logger.debug(f"Fetching merchant_name value from the txn table for void txn : {merchant_name_db_2}")
            payment_card_brand_db_2 = result["payment_card_brand"].iloc[0]
            logger.debug(f"Fetching payment_card_brand value from the txn table for void txn : {payment_card_brand_db_2}")
            payment_card_type_db_2 = result["payment_card_type"].iloc[0]
            logger.debug(f"Fetching payment_card_type value from the txn table for void txn : {payment_card_type_db_2}")
            payment_card_bin_db_2 = result["payment_card_bin"].iloc[0]
            logger.debug(f"Fetching payment_card_bin value from the txn table for void txn : {payment_card_bin_db_2}")
            terminal_info_id_db_2 = result["terminal_info_id"].iloc[0]
            logger.debug(f"Fetching terminal_info_id value from the txn table for void txn : {terminal_info_id_db_2}")
            txn_type_db_2 = result["txn_type"].iloc[0]
            logger.debug(f"Fetching txn_type value from the txn table for void txn : {txn_type_db_2}")
            card_txn_type_db_2 = result["card_txn_type"].iloc[0]
            logger.debug(f"Fetching card_txn_type value from the txn table for void txn : {card_txn_type_db_2}")
            card_last_four_digit_db_2 = result["card_last_four_digit"].iloc[0]
            logger.debug(f"Fetching card_last_four_digit value from the txn table for void txn : {card_last_four_digit_db_2}")
            org_code_db_2 = result["org_code"].iloc[0]
            logger.debug(f"Fetching org_code value from the txn table for void txn : {org_code_db_2}")
            order_id_db_2 = result["external_ref"].iloc[0]
            logger.debug(f"Fetching external_ref value from the txn table for void txn : {order_id_db_2}")
            merchant_code_db_2 = result["merchant_code"].iloc[0]
            logger.debug(f"Fetching merchant_code value from the txn table for void txn : {merchant_code_db_2}")
            posting_date_2 = result['posting_date'].values[0]
            logger.debug(f"Fetching posting_date value from the txn table for void txn : {posting_date_2}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching payment status value from txn history for void txn : {txn_id}, {payment_status_2}")
                payment_mode_2 = txn_history_page.fetch_txn_type_text()
                logger.info(f"Fetching payment mode value from txn history for void txn : {txn_id}, {payment_mode_2}")
                app_txn_id_2 = txn_history_page.fetch_txn_id_text()
                logger.info(f"Fetching txn_id value from txn history for void txn : {txn_id}, {app_txn_id_2}")
                app_amount_2 = txn_history_page.fetch_txn_amount_text()
                logger.info(f"Fetching amount value from txn history for void txn : {txn_id}, {app_amount_2}")
                app_settlement_status_2 = txn_history_page.fetch_settlement_status_text()
                logger.info(f"Fetching settlement status value from txn history for void txn : {txn_id}, {app_settlement_status_2}")
                app_payment_msg_2 = txn_history_page.fetch_txn_payment_message_text()
                logger.info(f"Fetching payment msg value from txn history for void txn : {txn_id}, {app_payment_msg_2}")
                app_date_and_time_2 = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching date and time value from txn history for void txn : {txn_id}, {app_date_and_time_2}")
                app_rrn_2 = txn_history_page.fetch_RRN_text()
                logger.info(f"Fetching rrn value from txn history for void txn : {txn_id}, {app_rrn_2}")
                app_auth_code_2 = txn_history_page.fetch_auth_code_text()
                logger.info(f"Fetching auth_cde value from txn history for void txn : {txn_id}, {app_auth_code_2}")
                app_batch_no_2 = txn_history_page.fetch_batch_number_text()
                logger.info(f"Fetching batch number value from txn history for void txn : {txn_id}, {app_batch_no_2}")
                app_device_serial_2 = txn_history_page.fetch_device_serial_text()
                logger.info(f"Fetching device serial from txn history for void txn : {txn_id}, {app_device_serial_2}")
                app_order_id_2 = txn_history_page.fetch_order_id_text()
                logger.info(f"Fetching txn order id value from txn history for void txn : {txn_id}, {app_order_id_2}")
                app_mid_2 = txn_history_page.fetch_mid_text()
                logger.info(f"Fetching mid value from txn history for void txn : {txn_id}, {app_mid_2}")
                app_tid_2 = txn_history_page.fetch_tid_text()
                logger.info(f"Fetching tid value from txn history for void txn : {txn_id}, {app_tid_2}")
                app_card_type_desc_2 = txn_history_page.fetch_card_type_desc_text()
                logger.info(f"Fetching card type value from txn history for void txn : {txn_id}, {app_card_type_desc_2}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "merchant_name_2": merchant_name_db_2,
                ...
                logger.debug(f"API DETAILS for txn list api : {api_details}")
                response = APIProcessor.send_request(api_details)
                logger.debug(f"Response received for transaction list api is : {response}")
                response_in_list = response["txns"]
                logger.debug(f"Response after filtering data of txn list api is : {response_in_list}")
                ...
                for elements in response_in_list:
                    if elements["txnId"] == txn_id:
                        ...
                        logger.debug(f"Value of status obtained from txnlist api for void txn : {status_api_2}")
                        amount_api_2 = float(elements["amount"])
                        logger.debug(f"Value of amount obtained from txnlist api for void txn : {amount_api_2}")
                        payment_mode_api_2 = elements["paymentMode"]
                        logger.debug(f"Value of paymentMode obtained from txnlist api void txn : {payment_mode_api_2}")
                        state_api_2 = elements["states"][0]
                        logger.debug(f"Value of states obtained from txnlist api for void txn : {state_api_2}")
                        rrn_api_2 = elements["rrNumber"]
                        logger.debug(f"Value of rrNumber obtained from txnlist api for void txn : {rrn_api_2}")
                        settlement_status_api_2 = elements["settlementStatus"]
                        logger.debug(f"Value of settlementStatus obtained from txnlist api for void txn : {settlement_status_api_2}")
                        acquirer_code_api_2 = elements["acquirerCode"]
                        logger.debug(f"Value of acquirerCode obtained from txnlist api for void txn : {acquirer_code_api_2}")
                        org_code_api_2 = elements["orgCode"]
                        logger.debug(f"Value of orgCode obtained from txnlist api for void txn : {org_code_api_2}")
                        mid_api_2 = elements["mid"]
                        logger.debug(f"Value of mid obtained from txnlist api for void txn : {mid_api_2}")
                        tid_api_2 = elements["tid"]
                        logger.debug(f"Value of tid obtained from txnlist api for void txn : {tid_api_2}")
                        txn_type_api_2 = elements["txnType"]
                        logger.debug(f"Value of txnType obtained from txnlist api for void txn : {txn_type_api_2}")
                        auth_code_api_2 = elements["authCode"]
                        logger.debug(f"Value of authCode obtained from txnlist api for void txn : {auth_code_api_2}")
                        date_and_time_api_2 = elements["createdTime"]
                        logger.debug(f"Value of createdTime obtained from txnlist api for void txn : {date_and_time_api_2}")
                        device_serial_api_2 = elements["deviceSerial"]
                        logger.debug(f"Value of deviceSerial obtained from txnlist api for void txn : {device_serial_api_2}")
                        username_api_2 = elements["username"]
                        logger.debug(f"Value of username obtained from txnlist api for void txn : {username_api_2}")
                        txn_id_api_2 = elements["txnId"]
                        logger.debug(f"Value of txnId obtained from txnlist api for void txn : {txn_id_api_2}")
                        payment_card_brand_api_2 = elements["paymentCardBrand"]
                        logger.debug(f"Value of paymentCardBrand obtained from txnlist api for void txn : {payment_card_brand_api_2}")
                        payment_card_type_api_2 = elements["paymentCardType"]
                        logger.debug(f"Value of paymentCardType obtained from txnlist api for void txn : {payment_card_type_api_2}")
                        card_txn_type_api_2 = elements["cardTxnTypeDesc"]
                        logger.debug(f"Value of cardTxnTypeDesc obtained from txnlist api for void txn : {card_txn_type_api_2}")
                        batch_number_api_2 = elements["batchNumber"]
                        logger.debug(f"Value of batchNumber obtained from txnlist api for void txn : {batch_number_api_2}")
                        card_last_four_digit_api_2 = elements["cardLastFourDigit"]
                        logger.debug(f"Value of cardLastFourDigit obtained from txnlist api for void txn : {card_last_four_digit_api_2}")
                        external_ref_number_api_2 = elements["externalRefNumber"]
                        logger.debug(f"Value of externalRefNumber obtained from txnlist api for void txn : {external_ref_number_api_2}")
                        merchant_name_api_2 = elements["merchantName"]
                        logger.debug(f"Value of merchantName obtained from txnlist api for void txn : {merchant_name_api_2}")
                        payment_card_bin_api_2 = elements["paymentCardBin"]
                        logger.debug(f"Value of paymentCardBin obtained from txnlist api for void txn : {payment_card_bin_api_2}")
                        display_pan_api_2 = elements["displayPAN"]
                        logger.debug(f"Value of displayPAN obtained from txnlist api for void txn : {display_pan_api_2}")
