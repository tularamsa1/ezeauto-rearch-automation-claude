...
def test_common_100_103_180():
    """
    Sub Feature Code: UI_Common_PM_CNP_Credit_Card_Success_Razorpay
    Sub Feature Description: Verification of a Remote Pay successful credit card txn
    TC naming code description: 100: Payment Method, 103: RemotePay, 180: TC_180
    """
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"cnpware payment flow from db : {cnpware_payment_flow}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_181():
    """
    Sub Feature Code: UI_Common_PM_CNP_Credit_Card_Failed_Razorpay
    Sub Feature Description: Verification of failed remote pay credit card txn for Razorpay source pg
    TC naming code description: 100: Payment Method, 103: RemotePay, 181: TC_181
    """
    try:
        # -------------------------------Reset Settings to default(started)--------------------------------------------
        testcase_id = sys._getframe().f_code.co_name
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_103_182():
    """
    Sub Feature Code: UI_Common_PM_CNP_Credit_Card_failed_Screen_Message_Razorpay
    Sub Feature Description: Verification of a message when txn done after timeout via CNP link
    TC naming code description:100: Payment Method, 103: RemotePay, 182: TC_182
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = f"select setting_value from ezetap_demo.remotepay_setting where setting_name='remotePayExpTime' " \
                    f"and org_code = '{str(org_code)}';"
            logger.debug(f"Query to fetch Txn_id from the DB : {query}")
            result = DBProcessor.getValueFromDB(query)
            ...
            logger.info(f"expiry time from db : {expiry_time_from_db}")
            ...
            try:
                ...
                logger.info("Remote payment link has expired, cannot proceed further")
                failed_msg = "visible"
            except:
                logger.info("failed message is not visible")
            assert failed_msg == "visible"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_183():
    """
    Sub Feature Code: UI_Common_PM_CNP_Debit_Card_After_Timeout_Razorpay
    Sub Feature Description: Verification of credit card txn after timeout via CNP link
    TC naming code description: 100: Payment Method, 103: RemotePay, 183: TC_183
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query1 = f"select * from cnp_txn where txn_id='{txn_id}';"
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                ...
                logger.debug(f"Response after filtering data of current txn is : {response}")
                status_api = response["status"]
                logger.debug(f"Fetching txn status from txn api : {status_api} ")
                amount_api = response["amount"]
                logger.debug(f"Fetching txn amount from txn api : {amount_api} ")
                payment_mode_api = response["paymentMode"]
                logger.debug(f"Fetching txn payment mode from txn api : {payment_mode_api} ")
                payment_card_brand = response["paymentCardBrand"]
                logger.debug(f"Fetching txn payment Card Brand from txn api : {payment_card_brand} ")
                payment_card_type = response["paymentCardType"]
                logger.debug(f"Fetching txn payment Card Type from txn api : {payment_card_type} ")
                acquirer_code_api = response["acquirerCode"]
                logger.debug(f"fetching acquirer_code from api: {acquirer_code_api}")
                settlement_status_api = response["settlementStatus"]
                ...
                txn_type_api = response["txnType"]
                logger.debug(f"fetching txn tpe from api : {txn_type_api}")
                date_api = response["postingDate"]
                logger.debug(f"fetching date and time from api : {date_api}")
                org_cde_api = response["orgCode"]
                logger.debug(f"fetching org_code from api : {org_code}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                expected_db_values = {
                    ...
                    "cnp_pmt_status": "PENDING_AFTER_TIMEOUT",
                    "cnp_pmt_state": "TIME_OUT_PENDING",
                    "cnp_pmt_card_brand": "MASTER_CARD",
                    "cnp_pmt_card_type": "CREDIT",
                    "auth_code": txn_auth_code,
                    "cnp_pmt_gateway": "RAZORPAY",
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_184():
    """
    Sub Feature Code: UI_Common_PM_CNP_Credit_Card_Refund_Razorpay
    Sub Feature Description: Verification of credit card txn refund using refund via api
    TC naming code description: 100: Payment Method, 103: RemotePay, 184: TC_184
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching Transaction status from MPOS app: Txn status = {app_payment_status_refunded}")
                app_payment_mode_refunded = transactions_history_page.fetch_txn_type_text()
                logger.debug(f"Fetching Transaction payment mode from MPOS app: Txn Mode = {app_payment_mode_refunded}")
                ...
                logger.debug(f"Fetching Transaction amount from  of MPOS app: Txn Amt = {app_payment_amt_refunded}")
                app_settlement_status_refunded = transactions_history_page.fetch_settlement_status_text()
                logger.debug(f"Fetching settlement status of original txn from app = {app_settlement_status_refunded}")
                payment_msg_refunded = transactions_history_page.fetch_txn_payment_msg_text()
                logger.debug(f"Fetching Transaction id of original txn from of MPOS app: msg Id {payment_msg_refunded}")
                ...
                logger.debug(f"Fetching Transaction status of original txn from MPOS app:{app_payment_status_original}")
                app_payment_mode_original = transactions_history_page.fetch_txn_type_text()
                logger.debug(f"Fetching Transaction payment mode of original txn from {app_payment_mode_original}")
                ...
                logger.debug(f"Fetching Transaction amount of orginal txn from of MPOS app:{app_payment_amt_original}")
                app_settlement_status_original = transactions_history_page.fetch_settlement_status_text()
                logger.debug(f"settlement status of original txn from of MPOS app: {app_settlement_status_original}")
                ...
                logger.debug(f"Fetching order id from app transaction history: {app_order_id}")
                actual_app_values = {
