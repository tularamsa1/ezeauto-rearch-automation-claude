...
def test_common_100_113_013():
    ...
    try:
        ...
        logger.debug(f"fetched mid from upi_merchant_config table : {mid}")
        tid = result['tid'].values[0]
        logger.debug(f"fetched tid from upi_merchant_config table : {tid}")
        vpa = result['vpa'].values[0]
        logger.debug(f"fetched vpa from upi_merchant_config table : {vpa}")
        config_id = result['id'].values[0]
        logger.debug(f"fetched id from upi_merchant_config table : {config_id}")
        ...
        try:
            ...
            logger.debug(f"Response received for generating static qr for pure upi is : {response}")
            generateqr_publish_id = response["publishId"]
            logger.debug(f"Value of publishId fetched from static qr generation response : {generateqr_publish_id}")
            ...
            # Performing UPI callback for Kotak Atos Static QR    #transaction_type: 2 is for upi
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Response received for static qr pure upi callback is : {response}")
