...
def test_common_100_115_07_076():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Brand_Fixed_Success_Txn_For_Root_Org_HDFC_Dummy_Axis_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand fixed success transaction for root org via HDFC
    Dummy PG for Axis issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 076: TC076
    """
    try:
        ...
        query = f"select * from terminal_info where org_code= '{org_code}' and status = 'ACTIVE' and " \
                f"acquirer_code='HDFC' and payment_gateway='DUMMY'"
        ...
        logger.debug(f"Response received when emi, emi for client, offering_emi cashback, brand_emi and bocashback is "
                     f"enabled in preconditions settings : {response}")

        # Below function to update bank_code, bank for the bin: 417666
        testsuite_teardown.update_bin_info(bin_number='417666', bank_code='AXIS', bank='AXIS')
        ...
        query = f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and " \
                f"subventing_entity ='BRAND' and subvention_value_type= 'FIXED' and subvention_type='CASHBACK' and " \
        ...
        query = f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' and " \
                f"subventing_entity='BRAND' and subvention_type='CASHBACK' and subvention_value_type='FIXED' and " \
                f"tenure='{emi_plan_in_months} month'"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_077():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Brand_Fixed_Success_Txn_For_Root_Org_HDFC_Dummy_Axis_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand fixed success transaction for root org via HDFC
    Dummy PG for Axis issuer using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 077: TC077
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_078():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Brand_Fixed_Success_Txn_For_Root_Org_HDFC_Dummy_Axis_EMV_VISA_CreditCard_With_Pin_417666_For_9_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand fixed success transaction for root org via HDFC
    Dummy PG for Axis issuer using EMV VISA Credit card with pin for 9 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 078: TC078
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "product_serial": str(imei_no),
                    "scheme_name": subvention_scheme_name,
                    "user_agreement": f"I agree to pay as per the card issuer agreement. I understand that purchase("
                                      f"s) would be converted into EMI at the sole discretion of the Bank by chargi"
                                      f"ng an interest of {interest_rate}% on the monthly reducing balance."
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
