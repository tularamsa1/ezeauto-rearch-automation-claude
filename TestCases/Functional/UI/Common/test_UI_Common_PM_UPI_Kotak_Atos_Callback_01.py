...
def test_common_100_101_163():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Callback_KOTAK_ATOS
    Sub Feature Description: Verification of a successful upi txn using UPI Success Callback via Kotak_ATOS
    TC naming code description: 100: Payment method, 101: UPI, 163: Testcase ID
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching txn_id from the txn table for the {org_code} : txn_id : {txn_id}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching txn auth_code from txn history for the txn : {txn_id}, {app_auth_code}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_164():
    """
    Sub Feature Code: UI_Common_PM_UPI_2_Success_Callback_After_QR_Expiry_KOTAK_ATOS
    Sub Feature Description: Verification of a successful upi txn using 2 UPI Success Callback after QR expiry via Kotak_ATOS
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Query result for upi_txn table is : {result}")
            txn_ref = result['txn_ref'].iloc[0]
            logger.debug(f"Fetching txn_ref from the upi_txn table : txn_ref : {txn_ref}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info("Restarting MPOS-X app to check status of QR")
                login_page = LoginPage(app_driver)
                login_page.perform_login(app_username, app_password)
                home_page.wait_for_navigation_to_load()
                home_page.wait_for_home_page_load()
...
@pytest.mark.dbVal
@pytest.mark.chargeSlipVal
def test_common_100_101_165():
    """
    Sub Feature Code: UI_Common_PM_UPI_2_Success_Callback_Before_QR_Expiry_KOTAK_ATOS
    Sub Feature Description: Verification of a successful upi txn using 2 UPI Success Callback before QR expiry via Kotak_ATOS
    TC naming code description: 100: Payment method, 101: UPI, 164: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        try:
            ...
            auth_code = str(random.randint(110000000, 110099999))
            logger.debug(f"Generated random auth_code for first callback is : {auth_code}")
            ref_id = "R" + str(random.randint(110000000, 110099999))
            logger.debug(f"Generated random ref_id for first callback is : {ref_id}")
            ...
            logger.debug(f"Fetching id from the txn table from first callback is : {txn_id_2}")
            customer_name = result['customer_name'].values[0]
            logger.debug(f"Fetching customer_name from the txn table from first callback is : {customer_name}")
            payer_name = result['payer_name'].values[0]
            logger.debug(f"Fetching payer_name from the txn table from first callback is : {payer_name}")
            org_code_txn = result['org_code'].values[0]
            logger.debug(f"Fetching org_code from the txn table from first callback is : {org_code_txn}")
            txn_type = result['txn_type'].values[0]
            logger.debug(f"Fetching txn_type from the txn table from first callback is : {txn_type}")
            created_time = result['created_time'].values[0]
            logger.debug(f"Fetching created_time from the txn table from first callback is : {created_time}")

            auth_code_2 = str(random.randint(110000000, 110099999))
            logger.debug(f"Generated random auth_code for second callback is : {auth_code_2}")
            ref_id_2 = "R" + str(random.randint(110000000, 110099999))
            logger.debug(f"Generated random ref_id for second callback is : {ref_id_2}")
            ...
            logger.debug(f"Fetching id from the txn table from second callback is : {txn_id_3}")
            customer_name_2 = result['customer_name'].values[0]
            logger.debug(f"Fetching customer_name from the txn table from second callback is : {customer_name_2}")
            payer_name_2 = result['payer_name'].values[0]
            logger.debug(f"Fetching payer_name from the txn table from second callback is : {payer_name_2}")
            org_code_txn_2 = result['org_code'].values[0]
            logger.debug(f"Fetching org_code from the txn table from second callback is : {org_code_txn_2}")
            txn_type_2 = result['txn_type'].values[0]
            logger.debug(f"Fetching txn_type from the txn table from second callback is : {txn_type_2}")
            created_time_2 = result['created_time'].values[0]
            logger.debug(f"Fetching created_time from the txn table from second callback is : {created_time_2}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                txn_history_page.click_on_transaction_by_txn_id(txn_id_2)
                payment_status = txn_history_page.fetch_txn_status_text()
                logger.info(f"Fetching status from txn history for the txn : {txn_id_2}, {payment_status}")
                app_date_and_time = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching date from txn history for the txn : {txn_id_2}, {app_date_and_time}")
                app_auth_code = txn_history_page.fetch_auth_code_text()
                logger.info(f"Fetching AUTH CODE from txn history for the txn : {txn_id_2}, {app_auth_code}")
                payment_mode = txn_history_page.fetch_txn_type_text()
                logger.info(f"Fetching payment mode from txn history for the txn : {txn_id_2}, {payment_mode}")
                app_txn_id = txn_history_page.fetch_txn_id_text()
                logger.info(f"Fetching txn_id from txn history for the txn : {txn_id_2}, {app_txn_id}")
                app_amount = txn_history_page.fetch_txn_amount_text()
                logger.info(f"Fetching txn amount from txn history for the txn : {txn_id_2}, {app_amount}")
                app_customer_name = txn_history_page.fetch_customer_name_text()
                logger.info(f"Fetching txn customer name from txn history for the txn : {txn_id_2}, {app_customer_name}")
                app_settlement_status = txn_history_page.fetch_settlement_status_text()
                logger.info(f"Fetching txn settlement_status from txn history for the txn : {txn_id_2}, {app_settlement_status}")
                app_payer_name = txn_history_page.fetch_payer_name_text()
                logger.info(f"Fetching txn payer name from txn history for the txn : {txn_id_2}, {app_payer_name}")
                app_payment_msg = txn_history_page.fetch_txn_payment_msg_text()
                logger.info(f"Fetching txn status msg from txn history for the txn : {txn_id_2}, {app_payment_msg}")
                app_order_id = txn_history_page.fetch_order_id_text()
                logger.info(f"Fetching txn order_id from txn history for the txn : {txn_id_2}, {app_order_id}")
                app_rrn = txn_history_page.fetch_RRN_text()
                logger.info(f"Fetching txn_id from txn history for the txn : {txn_id_2}, {app_rrn}")

                txn_history_page.click_back_Btn_transaction_details()
                txn_history_page.click_on_transaction_by_txn_id(txn_id_3)
                payment_status_2 = txn_history_page.fetch_txn_status_text()
                logger.info(f"Fetching status from txn history for the txn : {txn_id_3}, {payment_status_2}")
                app_date_and_time_2 = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching date from txn history for the txn : {txn_id_3}, {app_date_and_time_2}")
                app_auth_code_2 = txn_history_page.fetch_auth_code_text()
                logger.info(f"Fetching AUTH CODE from txn history for the txn : {txn_id_3}, {app_auth_code_2}")
                payment_mode_2 = txn_history_page.fetch_txn_type_text()
                logger.info(f"Fetching payment mode from txn history for the txn : {txn_id_3}, {payment_mode_2}")
                app_txn_id_2 = txn_history_page.fetch_txn_id_text()
                logger.info(f"Fetching txn_id from txn history for the txn : {txn_id_3}, {app_txn_id_2}")
                app_amount_2 = txn_history_page.fetch_txn_amount_text()
                logger.info(f"Fetching txn amount from txn history for the txn : {txn_id_3}, {app_amount_2}")
                app_customer_name_2 = txn_history_page.fetch_customer_name_text()
                logger.info(f"Fetching txn customer name from txn history for the txn : {txn_id_3}, {app_customer_name_2}")
                app_settlement_status_2 = txn_history_page.fetch_settlement_status_text()
                logger.info(f"Fetching txn settlement_status from txn history for the txn : {txn_id_3}, {app_settlement_status_2}")
                app_payer_name_2 = txn_history_page.fetch_payer_name_text()
                logger.info(f"Fetching txn payer name from txn history for the txn : {txn_id_3}, {app_payer_name_2}")
                app_payment_msg_2 = txn_history_page.fetch_txn_payment_msg_text()
                logger.info(f"Fetching txn status msg from txn history for the txn : {txn_id_3}, {app_payment_msg_2}")
                app_order_id_2 = txn_history_page.fetch_order_id_text()
                logger.info(f"Fetching txn order_id from txn history for the txn : {txn_id_3}, {app_order_id_2}")
                app_rrn_2 = txn_history_page.fetch_RRN_text()
                logger.info(f"Fetching txn_id from txn history for the txn : {txn_id_3}, {app_rrn_2}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                for elements in response_in_list:
                    if elements["txnId"] == txn_id_2:
                        status_api = elements["status"]
                        logger.debug(f"Value of status obtained from txnlist api for first callback is : {status_api}")
                        amount_api = int(elements["amount"])
                        logger.debug(f"Value of amount obtained from txnlist api for first callback is : {amount_api}")
                        payment_mode_api = elements["paymentMode"]
                        logger.debug(f"Value of paymentMode obtained from txnlist api for first callback is : {payment_mode_api}")
                        state_api = elements["states"][0]
                        logger.debug(f"Value of states obtained from txnlist api for first callback is : {state_api}")
                        rrn_api = elements["rrNumber"]
                        logger.debug(f"Value of rrNumber obtained from txnlist api for first callback is : {rrn_api}")
                        settlement_status_api = elements["settlementStatus"]
                        logger.debug(f"Value of settlementStatus obtained from txnlist api for first callback is : {settlement_status_api}")
                        issuer_code_api = elements["issuerCode"]
                        logger.debug(f"Value of issuerCode obtained from txnlist api for first callback is : {issuer_code_api}")
                        acquirer_code_api = elements["acquirerCode"]
                        logger.debug(f"Value of acquirerCode obtained from txnlist api for first callback is : {acquirer_code_api}")
                        org_code_api = elements["orgCode"]
                        logger.debug(f"Value of orgCode obtained from txnlist api for first callback is : {org_code_api}")
                        mid_api = elements["mid"]
                        logger.debug(f"Value of mid obtained from txnlist api for first callback is : {mid_api}")
                        tid_api = elements["tid"]
                        logger.debug(f"Value of tid obtained from txnlist api for first callback is : {tid_api}")
                        txn_type_api = elements["txnType"]
                        logger.debug(f"Value of txnType obtained from txnlist api for first callback is : {txn_type_api}")
                        auth_code_api = elements["authCode"]
                        logger.debug(f"Value of authCode obtained from txnlist api for first callback is : {auth_code_api}")
                        date_api = elements["createdTime"]
                        logger.debug(f"Value of createdTime obtained from txnlist api for first callback is : {date_api}")
                        customer_name_api = elements["customerName"]
                        logger.debug(f"Value of customerName obtained from txnlist api for first callback is : {customer_name_api}")
                        payer_name_api = elements["payerName"]
                        logger.debug(f"Value of payerName obtained from txnlist api for first callback is : {payer_name_api}")

                # txn list for 2nd callback
                for elements in response_in_list:
                    if elements["txnId"] == txn_id_3:
                        status_api_2 = elements["status"]
                        logger.debug(f"Value of status obtained from txnlist api for second callback is : {status_api_2}")
                        amount_api_2 = int(elements["amount"])
                        logger.debug(f"Value of amount obtained from txnlist api for second callback is : {amount_api_2}")
                        payment_mode_api_2 = elements["paymentMode"]
                        logger.debug(f"Value of paymentMode obtained from txnlist api for second callback is : {payment_mode_api_2}")
                        state_api_2 = elements["states"][0]
                        logger.debug(f"Value of states obtained from txnlist api for second callback is : {state_api_2}")
                        rrn_api_2 = elements["rrNumber"]
                        logger.debug(f"Value of rrNumber obtained from txnlist api for second callback is : {rrn_api_2}")
                        settlement_status_api_2 = elements["settlementStatus"]
                        logger.debug(f"Value of settlementStatus obtained from txnlist api for second callback is : {settlement_status_api_2}")
                        issuer_code_api_2 = elements["issuerCode"]
                        logger.debug(f"Value of issuerCode obtained from txnlist api for second callback is : {issuer_code_api_2}")
                        acquirer_code_api_2 = elements["acquirerCode"]
                        logger.debug(f"Value of acquirerCode obtained from txnlist api for second callback is : {acquirer_code_api_2}")
                        org_code_api_2 = elements["orgCode"]
                        logger.debug(f"Value of orgCode obtained from txnlist api for second callback is : {org_code_api_2}")
                        mid_api_2 = elements["mid"]
                        logger.debug(f"Value of mid obtained from txnlist api for second callback is : {mid_api_2}")
                        tid_api_2 = elements["tid"]
                        logger.debug(f"Value of tid obtained from txnlist api for second callback is : {tid_api_2}")
                        txn_type_api_2 = elements["txnType"]
                        logger.debug(f"Value of txnType obtained from txnlist api for second callback is : {txn_type_api_2}")
                        auth_code_api_2 = elements["authCode"]
                        logger.debug(f"Value of authCode obtained from txnlist api for second callback is : {auth_code_api_2}")
                        date_api_2 = elements["createdTime"]
                        logger.debug(f"Value of createdTime obtained from txnlist api for second callback is : {date_api_2}")
                        customer_name_api_2 = elements["customerName"]
                        logger.debug(f"Value of customerName obtained from txnlist api for second callback is : {customer_name_api_2}")
                        payer_name_api_2 = elements["payerName"]
                        logger.debug(f"Value of payerName obtained from txnlist api for second callback is : {payer_name_api_2}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Query result for actual db values from txn table for first callback : {result}")
                status_db = result["status"].iloc[0]
                logger.debug(f"Fetching actual db status value from the txn table for first callback : {status_db}")
                payment_mode_db = result["payment_mode"].iloc[0]
                logger.debug(f"Fetching actual db payment_mode value from the txn table for first callback : {payment_mode_db}")
                amount_db = int(result["amount"].iloc[0])
                logger.debug(f"Fetching actual db amount value from the txn table for first callback : {amount_db}")
                state_db = result["state"].iloc[0]
                logger.debug(f"Fetching actual db state value from the txn table for first callback : {state_db}")
                payment_gateway_db = result["payment_gateway"].iloc[0]
                logger.debug(f"Fetching actual db payment_gateway value from the txn table for first callback : {payment_gateway_db}")
                acquirer_code_db = result["acquirer_code"].iloc[0]
                logger.debug(f"Fetching actual db acquirer_code value from the txn table for first callback : {acquirer_code_db}")
                bank_code_db = result["bank_code"].iloc[0]
                logger.debug(f"Fetching actual db bank_code value from the txn table for first callback : {bank_code_db}")
                tid_db = result['tid'].values[0]
                logger.debug(f"Fetching actual db tid value from the txn table for first callback : {tid_db}")
                mid_db = result['mid'].values[0]
                logger.debug(f"Fetching actual db mid value from the txn table for first callback : {mid_db}")
                settlement_status_db = result["settlement_status"].iloc[0]
                logger.debug(f"Fetching actual db settlement_status value from the txn table for first callback : {settlement_status_db}")
                bank_name_db = result["bank_name"].iloc[0]
                logger.debug(f"Fetching actual db bank_name value from the txn table for first callback : {bank_name_db}")
                payer_name_db = result['payer_name'].values[0]
                logger.debug(f"Fetching actual db payer_name value from the txn table for first callback : {payer_name_db}")
                rrn_db = result['rr_number'].values[0]
                logger.debug(f"Fetching actual db rr_number value from the txn table for first callback : {rrn_db}")
                ...
                logger.debug(f"Query result for actual db values from upi_txn table for first callback : {result}")
                upi_status_db = result["status"].iloc[0]
                logger.debug(f"Fetching actual db status value from the upi_txn table for first callback : {upi_status_db}")
                upi_txn_type_db = result["txn_type"].iloc[0]
                logger.debug(f"Fetching actual db txn_type value from the upi_txn table for first callback : {upi_txn_type_db}")
                upi_bank_code_db = result["bank_code"].iloc[0]
                logger.debug(f"Fetching actual db bank_code value from the upi_txn table for first callback : {upi_bank_code_db}")
                upi_mc_id_db = result["upi_mc_id"].iloc[0]
                logger.debug(f"Fetching actual db upi_mc_id value from the upi_txn table for first callback : {upi_mc_id_db}")
                ...
                logger.debug(f"Query result for actual db values from txn table for second callback : {result}")
                status_db_2 = result["status"].iloc[0]
                logger.debug(f"Fetching actual db status value from the txn table for second callback : {status_db_2}")
                payment_mode_db_2 = result["payment_mode"].iloc[0]
                logger.debug(f"Fetching actual db payment_mode value from the txn table for second callback : {payment_mode_db_2}")
                amount_db_2 = int(result["amount"].iloc[0])
                logger.debug(f"Fetching actual db amount value from the txn table for second callback : {amount_db_2}")
                state_db_2 = result["state"].iloc[0]
                logger.debug(f"Fetching actual db state value from the txn table for second callback : {state_db_2}")
                payment_gateway_db_2 = result["payment_gateway"].iloc[0]
                logger.debug(f"Fetching actual db payment_gateway value from the txn table for second callback : {payment_gateway_db_2}")
                acquirer_code_db_2 = result["acquirer_code"].iloc[0]
                logger.debug(f"Fetching actual db acquirer_code value from the txn table for second callback : {acquirer_code_db_2}")
                settlement_status_db_2 = result["settlement_status"].iloc[0]
                logger.debug(f"Fetching actual db settlement_status value from the txn table for second callback : {settlement_status_db_2}")
                bank_code_db_2 = result["bank_code"].iloc[0]
                logger.debug(f"Fetching actual db bank_code value from the txn table for second callback : {bank_code_db_2}")
                mid_db_2 = result['mid'].values[0]
                logger.debug(f"Fetching actual db mid value from the txn table for second callback : {mid_db_2}")
                tid_db_2 = result['tid'].values[0]
                logger.debug(f"Fetching actual db tid value from the txn table for second callback : {tid_db_2}")
                bank_name_db_2 = result["bank_name"].iloc[0]
                logger.debug(f"Fetching actual db bank_name value from the txn table for second callback : {bank_name_db_2}")
                payer_name_db_2 = result['payer_name'].values[0]
                logger.debug(f"Fetching actual db payer_name value from the txn table for second callback : {payer_name_db_2}")
                rrn_db_2 = result['rr_number'].values[0]
                logger.debug(f"Fetching actual db rr_number value from the txn table for second callback : {rrn_db_2}")
                ...
                logger.debug(f"Query result for actual db values from upi_txn table for second callback : {result}")
                upi_status_db_2 = result["status"].iloc[0]
                logger.debug(f"Fetching actual db status value from the upi_txn table for second callback : {upi_status_db_2}")
                upi_txn_type_db_2 = result["txn_type"].iloc[0]
                logger.debug(f"Fetching actual db txn_type value from the upi_txn table for second callback : {upi_txn_type_db_2}")
                upi_bank_code_db_2 = result["bank_code"].iloc[0]
                logger.debug(f"Fetching actual db bank_code value from the upi_txn table for second callback : {upi_bank_code_db_2}")
                upi_mc_id_db_2 = result["upi_mc_id"].iloc[0]
                logger.debug(f"Fetching actual db upi_mc_id value from the upi_txn table for second callback : {upi_mc_id_db_2}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expectedPortalValues = {
                    ...
                    "username": app_username,
                    "txn_id": txn_id_2,
                    "rrn": "-" if ref_id is None else str(ref_id),
                    "date_time": date_and_time_portal,
                    "auth_code": "-" if auth_code is None else auth_code,
