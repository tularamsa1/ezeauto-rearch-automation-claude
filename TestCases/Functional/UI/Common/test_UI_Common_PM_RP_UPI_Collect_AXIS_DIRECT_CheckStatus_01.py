...
@pytest.mark.appVal
def test_common_100_103_128():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_Success_Via_CheckStatus_AXIS_DIRECT
    Sub Feature Description: Verification of a Remote Pay successfull UPI collect txn via AXIS DIRECT using check status
    ...
    try:
        ...
        try:
            ...
            externalRef = response.get('externalRefNumber') # remove since it is not required
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_129():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_Failed_Via_CheckStatus_AXIS_DIRECT
    Sub Feature Description: Verification of a Remote Pay failed UPI collect txn via AXIS DIRECT using check status
    TC naming code description:
    100: Payment Method
    103: RemotePay
    129: TC_129
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
                ...
                try:
                    ...
                    actualAPIValues = {"Payment Status": status_api, "Amount": amount_api,
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_130():
    """
    Sub Feature Code: UI_Common_PM_RP_upi_Collect_Pending_AXIS_DIRECT
    Sub Feature Description: Verification of a check status for upi collect pending txn via AXIS_DIRECT
    TC naming code description:
    100: Payment Method
    101: RemotePay
    130: TC130
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_131():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_Amount_Mismatch_AXIS_DIRECT
    ...
    try:
        ...
        testsuite_teardown.revert_cnp_payment_settings_default(org_code, bank_code='AXIS_DIRECT', portal_un=portal_username,
...
def test_common_100_103_149():
    """
    UI_Common_PM_RP_RP_UPI_Collect_CheckStatus_After_timeout_AXIS_DIRECT
    Verification of a Remote Pay upi Collect txn after timeout for Axis Direct using check status
    TC naming code description:
    100: Payment Method
    103: RemotePay
    149: TC_149
    ...
    try:
        ...
        try:
            ...
            payment_intent_id = response.get('paymentIntentId')
            portal_driver.get(paymentLinkUrl)
            remote_pay_upi_collect_txn = remotePayTxnPage(portal_driver)
            remote_pay_upi_collect_txn.clickOnRemotePayUPI()
            remote_pay_upi_collect_txn.clickOnRemotePayUpiCollect()
            logger.info("Opening UPI Collect to start the txn.")
            remote_pay_upi_collect_txn.clickOnRemotePayUpiCollectAppSelection()
            ...
            try:
                defaultValue = DBProcessor.getValueFromDB(query1)
                setting_value = int(defaultValue['setting_value'].values[0])
                logger.info(f"timeout for UPI txn is: {setting_value}")
            except NameError as e:
                setting_value = None
                print(e)
            except IndexError as e:
            ...
            if org_setting_value:
                logger.info(f"Value for max upi timeout is: {org_setting_value} min.")
                time.sleep(3 + (org_setting_value * 60))
            else:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            try:
                ...
                Validator.validateAgainstAPP(expectedApp=expected_app_values, actualApp=actualAppValues)
            except Exception as e:
                Configuration.perform_app_val_exception(testcase_id, e)
            logger.info(f"Completed APP validation for the test case : {testcase_id}")
        # -----------------------------------------End of App Validation---------------------------------------
