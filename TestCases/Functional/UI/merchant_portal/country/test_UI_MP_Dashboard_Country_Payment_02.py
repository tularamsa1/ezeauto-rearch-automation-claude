...
def test_mp_700_701_013():
    """
    Sub Feature Code: UI_MP_Payment_using_Cheque_as_country_head
    Sub Feature Description: Verifying txn details on dashboard for payment done using Cheque when logging in as Country Head
    ...
    try:
        ...
        try:
            ...
            query = "select sum(amount) from txn where " \
                    "org_code = '" + org_code + "' and status = 'AUTHORIZED' and payment_mode = 'CHEQUE' and id between '" + current_start_date_1 + "' and '" + current_end_date_1 + "';"
            ...
            cheque_total_amount_api = sum([float(element['amount']) for element in response['paymentModeSegregation']
                                          if element['paymentMode'] == 'CHEQUE'])
            logger.info(f"cheque total sale from api  :{cheque_total_amount_api}")
            cheque_total_number_of_txns_api = sum([element['count'] for element in response['paymentModeSegregation']
                                                   if element['paymentMode'] == 'CHEQUE'])
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_701_014():
    """
    Sub Feature Code: UI_MP_Payment_using_Normal_EMI_as_country_head
    Sub Feature Description: Verifying txn details on dashboard for payment done using Normal_EMI when logging in as Country Head
    TC naming code description:
    700: Merchant Portal
    701: Country
    014: TC014
    ...
    try:
        ...
        try:
            ...
            query = "SELECT sum(amount) from txn where " \
                    "id BETWEEN '" + current_start_date_1 + "' AND '" + current_end_date_1 + "' and org_code='" + org_code + "' and status = 'AUTHORIZED' and payment_mode = 'CARD' and payment_card_type ='CREDIT';"
            ...
            query = "SELECT count(*) from txn where id " \
                    "BETWEEN '" + current_start_date_1 + "' AND '" + current_end_date_1 + "' and org_code='" + org_code + "' and status = 'AUTHORIZED' and payment_mode = 'CARD' and payment_card_type ='CREDIT';"
            ...
            query = "select sum(amount) from txn where o" \
                    "rg_code = '" + org_code + "' and status = 'AUTHORIZED' and id between '" + current_start_date_1 + "' and '" + current_end_date_1 + "' and payment_mode = 'CARD';"
            logger.info(f"query to fetch total overall sale : {query}")
            result = DBProcessor.getValueFromDB(query)
            ...
            response = APIProcessor.send_request(api_details)
            logger.info(f"Response obtained for Txn Dashboard is: {response}")
            total_card_amount_api = sum([float(element['amount']) for element in response['paymentModeSegregation']
                                         if element['paymentMode'] == 'CARD'])
            logger.info(f"total card sale from api  :{total_card_amount_api}")
            credit_card_total_amount_api = sum([float(element['amount']) for element in response['paymentModeSegregation']
                                               if element['subPaymentMode'] == 'credit'])
            logger.info(f"credit card total sale from api  :{credit_card_total_amount_api}")
            credit_card_total_number_of_txns_api = sum([element['count'] for element in response['paymentModeSegregation']
                                                        if element['subPaymentMode'] == 'credit'])
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_701_015():
    """
    Sub Feature Code: UI_MP_Payment_using_Brand_EMI_as_country_head
    Sub Feature Description: Verifying txn details on dashboard for payment done using BrandEMI when logging in as Country Head
    TC naming code description:
    700: Merchant Portal
    701: Country
    015: TC015
    ...
    try:
        ...
        try:
            ...
            query = "select sum(amount) from txn where" \
                    " org_code = '" + org_code + "' and status = 'AUTHORIZED' and id between '" + current_start_date_1 + "' and '" + current_end_date_1 + "' and payment_mode = 'CARD';"
            logger.info(f"query to fetch total overall sale : {query}")
            result = DBProcessor.getValueFromDB(query)
            overall_card_sale = result.values[0]
            logger.info(f"overall card sale from db : {overall_card_sale}")
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_701_016():
    """
    Sub Feature Code: UI_MP_Payment_as_Refund_as_country_head
    Sub Feature Description: Verifying txn details on dashboard for payment done as Refund when logging in as Country Head
    TC naming code description:
    700: Merchant Portal
    701: Country
    016: TC016
    ...
    try:
        ...
        try:
            ...
            total_sale_amount_api = sum([float(element['amount']) for element in response['dailySegrgationForAmount']])
            logger.info(f"total sale from api  :{total_sale_amount_api}")
            total_number_of_txns_api = sum([txns['count'] for txns in response['paymentModeSegregation']])
...
@pytest.mark.apiVal
@pytest.mark.portalVal
def test_mp_700_701_017():
    """
    Sub Feature Code: UI_MP_Payment_as_Void_as_country_head
    Sub Feature Description: Verifying txn details on dashboard for payment which is Voided when logging in as Country Head
    TC naming code description:
    700: Merchant Portal
    701: Country
    017: TC017
    ...
    try:
        ...
        try:
            ...
            query = "select sum(amount) from txn where " \
                    "org_code = '" + org_code + "' and status = 'AUTHORIZED' and payment_mode = 'CARD' and id between '" + current_start_date_1[2:] + "' and '" + current_end_date_1[2:] + "';"
            ...
            query = "select sum(amount) from txn where " \
                    "org_code = '" + org_code + "' and status = 'AUTHORIZED' and id between '" + current_start_date_1[2:] + "' and '" + current_end_date_1[2:] + "';"
            logger.info(f"query to fetch total overall sale : {query}")
            result = DBProcessor.getValueFromDB(query)
            overall_sale = result.values[0]
            logger.info(f"overall sale from db : {overall_sale}")
