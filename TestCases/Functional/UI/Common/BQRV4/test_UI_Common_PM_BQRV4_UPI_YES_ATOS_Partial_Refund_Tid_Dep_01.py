...
def test_common_100_102_321():
    """
    Sub Feature Code: Tid Dep - UI_Common_PM_BQRV4_UPI_Partial_Refund_YES_ATOS
    Sub Feature Description: Tid Dep - Verification of a BQRV4 Partial Refund via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQRV4, 321: TC321
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Value of txn_id obtained from txn table after refund is : {txn_id}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Value of auth_code obtained from txn table after refund is : {auth_code}")
            rrn = result['rr_number'].iloc[0]
            logger.debug(f"Value of rr_number obtained from txn table after refund is : {rrn}")
            posting_date = result['created_time'].values[0]
            logger.debug(f"Value of created_time obtained from txn table after refund is : {posting_date}")
            customer_name = result['customer_name'].values[0]
            logger.debug(f"Value of customer_name obtained from txn table after refund is : {customer_name}")
            payer_name = result['payer_name'].values[0]
            logger.debug(f"Value of payer_name obtained from txn table after refund is : {payer_name}")
            amount_db = float(result["amount"].iloc[0])
            logger.debug(f"Value of amount obtained from txn table after refund is : {amount_db}")
            payment_mode_db = result["payment_mode"].iloc[0]
            logger.debug(f"Value of payment_mode obtained from txn table after refund is : {payment_mode_db}")
            payment_status_db = result["status"].iloc[0]
            logger.debug(f"Value of status obtained from txn table after refund is : {payment_status_db}")
            payment_state_db = result["state"].iloc[0]
            logger.debug(f"Value of state obtained from txn table after refund is : {payment_state_db}")
            acquirer_code_db = result["acquirer_code"].iloc[0]
            logger.debug(f"Value of acquirer_code obtained from txn table after refund is : {acquirer_code_db}")
            bank_name_db = result["bank_name"].iloc[0]
            logger.debug(f"Value of bank_name obtained from txn table after refund is : {bank_name_db}")
            payer_name_db = result["payer_name"].iloc[0]
            logger.debug(f"Value of payer_name obtained from txn table after refund is : {payer_name_db}")
            mid_db = result["mid"].iloc[0]
            logger.debug(f"Value of mid obtained from txn table after refund is : {mid_db}")
            tid_db = result["tid"].iloc[0]
            logger.debug(f"Value of tid obtained from txn table after refund is : {tid_db}")
            payment_gateway_db = result["payment_gateway"].iloc[0]
            logger.debug(f"Value of payment_gateway obtained from txn table after refund is : {payment_gateway_db}")
            rr_number_db = result["rr_number"].iloc[0]
            logger.debug(f"Value of rr_number obtained from txn table after refund is : {rr_number_db}")
            settlement_status_db = result["settlement_status"].iloc[0]
            logger.debug(f"Value of settlement_status obtained from txn table after refund is : {settlement_status_db}")
            device_serial_db = result['device_serial'].values[0]
            logger.debug(f"Value of device_serial obtained from txn table after refund is : {device_serial_db}")
