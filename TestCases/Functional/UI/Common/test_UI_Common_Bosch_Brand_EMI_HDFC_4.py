...
def test_common_100_115_07_172():
    """
    Sub Feature Code: UI_Common_Verify_Bosch_Brand_EMI_Flow_EMV_VISA_DebitCard_With_Pin_428090_with_3_month_tenure_for_Merchant_level
    Sub Feature Description: Performing the Bosch brand EMI subvention success transaction for an org (not ezetap) via HDFC-HDFC using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
    ...
    try:
        ...
        query = (f"select * from subvention_plan where org_code='{org_code}' and card_type= 'DEBIT' and bank='HDFC' "
        ...
        query = (f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' "
                 f"and subventing_entity='BRAND' and subvention_type='PAYBACK' and"
        ...
        try:
            ...
            query = f"select * from brand_sku_details where sku_name = '{product_name}'"
            ...
            sku_code = result['sku_code'].values[0]
            logger.debug(f"Fetching sku_code from brand_sku_details table : {sku_code}")
            ...
            query = (f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and "
                     f"subventing_entity='BRAND' and subvention_value_type= 'PERCENTAGE' and "
                     f"subvention_type='PAYBACK' and tenure='{emi_plan_in_months} month' ;")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "subvention_amount": round(cal_instant_amt, 2),
                ...
                api_scheme_id = response['emiDetails']['schemeId']
                logger.debug(f"Value of scheme id obtained from txnlist api : {api_scheme_id}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "subvention_brand_id": int(brand_id),
                ...
                actual_db_values = {
                    ...
                    "subvention_brand_id": int(subvention_plan_brand_id),
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Total Payback Amount": "Rs." + "{:.2f}".format(cal_instant_amt),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_171():
    """
    Sub Feature Code: UI_Common_Verify_Bosch_Brand_EMI_Flow_EMV_VISA_CreditCard_With_Pin_417666_with_3_Month_Tenure_for_Merchant_Level
    Sub Feature Description: Performing the Bosch brand EMI subvention success transaction for an org (not ezetap) via HDFC-HDFC using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 172: TC171
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.info(f"Enter the Product")
            payment_page.click_and_enter_imei_no(imei_code)
            logger.info(f"Entered the IMEI number")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_173():
    """
    ...
    try:
        ...
        query = f"select b.id, bs.sku_name from brand b join brand_sku_details bs on b.id = bs.brand_id where b.brand_name = 'Bosch' and b.status = 1 and bs.sku_code = '{sku_code}' order by b.created_time desc, bs.created_time desc limit 1;"
        logger.debug(f"Query to fetch data from the brand_sku_details table for the {org_code} : {query}")
        result = DBProcessor.getValueFromDB(query=query)
        logger.debug(f"Fetching result for brand_sku_details table : {result}")
        product_name = result["sku_name"].iloc[0]
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_175():
    """
    Sub Feature Code: UI_Common_Verify_Bosch_Brand_EMI_Flow_EMV_VISA_DebitCard_With_Pin_428090_with_6_month_tenure_for_Merchant_level
    Sub Feature Description: Performing Successful Bosch Brand EMI transaction at Merchant level via HDFC-HDFC using EMV VISA Debit card with pin for 6 months tenure (bin: 428090) with cash back type none
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 175: TC175
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
def test_common_100_115_07_164():
    ...
    try:
        ...
        # updating the status as inactive in subvention_plan_details table
        testsuite_teardown.update_subvention_plan_details(subvention_plan_id)
        #updating the status as inactive in emi table for merchant
        testsuite_teardown.update_emi_status_for_org(org_code, 'CREDIT', 'INACTIVE')
