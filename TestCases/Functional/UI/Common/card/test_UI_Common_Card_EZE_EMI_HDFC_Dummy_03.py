...
@pytest.mark.appVal
def test_common_100_115_11_011():
    """
        Sub Feature Code: UI_Common_Card_Eze_EMI_For_Payback_by_Brand_Percentage_Verify_Wallet_When_Txn_Amount_Greater_Than_MCEW_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
        Sub Feature Description: Performing the EZE EMI payback by brand percentage and verify the wallet when transaction amount is greater than the MCEW
        for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 011: TC011
    """
    try:
        ...
        logger.debug(f"collecting the wallet balance from ezeemi_club_wallet_account table : {mcew_wallet_balance}")
        ...
        try:
            ...
            amount = mcew_wallet_balance_2 + 100
        ...
        logger.debug(f"Query to update ezeemi_club_wallet_account table with wallet_balance  : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"Query to fetch result from ezeemi_club_wallet_account table with wallet_balance : {result}")
...
def test_common_100_115_11_012():
    """
        Sub Feature Code: UI_Common_Card_Eze_EMI_For_Payback_by_Brand_Percentage_Verify_Wallet_When_Txn_Amount_Less_Than_MCEW_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
        Sub Feature Description: Performing the EZE EMI payback by brand percentage and verify the wallet when transaction amount is less than the MCEW.
        Specific transaction amount should be deducted from MCEW and GCEW Wallet of that particular brand when use wallet option is selected in the Emi Tenure Page and txn should be successful
        for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 012: TC012
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from ezeemi_club_wallet_account where org_code='{org_code}' and wallet_type='MCEW' and brand_id='{brand_id}'"
        logger.debug(f"Query to fetch data from the ezeemi_club_wallet_account table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
        mcew_wallet_balance_before_txn = result['wallet_balance'].values[0]
        logger.debug(f"Fetching wallet_balance before voiding the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_before_txn}")

        query = f"select * from ezeemi_club_wallet_account where wallet_type='GCEW' and brand_id='{brand_id}'"
        logger.debug(f"Query to fetch data from the ezeemi_club_wallet_account table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
        gcew_wallet_balance_before_txn = result['wallet_balance'].values[0]
        logger.debug(f"Fetching wallet_balance before voiding the txn from ezeemi_club_wallet_account table : {gcew_wallet_balance_before_txn}")
        ...
        try:
            ...
            payment_page.select_emi_plan_for_eze_emi_brand_flow(emi_plan_in_months=emi_plan_in_months)
            ...
            brand_subvention_entity = result['subventing_entity'].values[0]
            logger.debug(f"Fetching subventing_entity from subvention_plan_details table : {brand_subvention_entity}")
            brand_subvention_type = result['subvention_type'].values[0]
            logger.debug(f"Fetching subvention_type from subvention_plan_details table : {brand_subvention_type}")
            brand_subvention_value_type = result['subvention_value_type'].values[0]
            logger.debug(f"Fetching subvention_value_type from subvention_plan_details table : {brand_subvention_value_type}")
            ...
            brand_subvention_tenure = result['tenure'].values[0]
            logger.debug(f"Fetching tenure from subvention_plan_details table : {brand_subvention_tenure}")
            brand_subvention_discount_type = result['discount_type'].values[0]
            logger.debug(f"Fetching discount_type from subvention_plan_details table : {brand_subvention_discount_type}")
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for subvention_plan_details table : {result}")
            ezetap_subvention_entity = result['subventing_entity'].values[0]
            logger.debug(f"Fetching subventing_entity from subvention_plan_details table : {ezetap_subvention_entity}")
            ezetap_subvention_type = result['subvention_type'].values[0]
            logger.debug(f"Fetching subvention_type from subvention_plan_details table : {ezetap_subvention_type}")
            ezetap_subvention_value_type = result['subvention_value_type'].values[0]
            logger.debug(f"Fetching subvention_value_type from subvention_plan_details table : {ezetap_subvention_value_type}")
            ...
            ezetap_subvention_tenure = result['tenure'].values[0]
            logger.debug(f"Fetching tenure from subvention_plan_details table : {ezetap_subvention_tenure}")
            ezetap_subvention_discount_type = result['discount_type'].values[0]
            logger.debug(f"Fetching discount_type from subvention_plan_details table : {ezetap_subvention_discount_type}")

            # Calculating the emi's using formula
            cal_additional_payback = (amount * ((brand_subvention_value + ezetap_subvention_value) / 100))
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
            mcew_wallet_balance_after_txn = result['wallet_balance'].values[0]
            logger.debug(f"Fetching wallet_balance after voiding the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_after_txn}")

            query = f"select * from ezeemi_club_wallet_account where wallet_type='GCEW' and brand_id='{brand_id}'"
            logger.debug(f"Query to fetch data from the ezeemi_club_wallet_account table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
            gcew_wallet_balance_after_txn = result['wallet_balance'].values[0]
            logger.debug(f"Fetching wallet_balance after voiding the txn from ezeemi_club_wallet_account table : {gcew_wallet_balance_after_txn}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "subvention_rate_1": float(brand_subvention_value),
                    "subvention_entity_2": "EZETAP",
                    "subvention_type_2": "PAYBACK",
                    "subvention_rate_2": float(ezetap_subvention_value),
                ...
                api_entity_1 = response['emiDetails']['subventionDetails'][0]['entity']
                logger.debug(f"Value of entity obtained from txnlist api : {api_entity_1}")
                api_subvention_type_1 = response['emiDetails']['subventionDetails'][0]['subventionType']
                logger.debug(f"Value of subventionType obtained from txnlist api : {api_subvention_type_1}")
                api_subvention_rate_1 = response['emiDetails']['subventionDetails'][0]['subventionRate']
                logger.debug(f"Value of subventionAmount obtained from txnlist api : {api_subvention_rate_1}")
                api_entity_2 = response['emiDetails']['subventionDetails'][1]['entity']
                logger.debug(f"Value of entity obtained from txnlist api : {api_entity_2}")
                api_subvention_type_2 = response['emiDetails']['subventionDetails'][1]['subventionType']
                logger.debug(f"Value of subventionType obtained from txnlist api : {api_subvention_type_2}")
                api_subvention_rate_2 = response['emiDetails']['subventionDetails'][1]['subventionRate']
                logger.debug(f"Value of subventionAmount obtained from txnlist api : {api_subvention_rate_2}")
                api_subvention_amt = response['emiDetails']['totalPayBack']
                ...
                actual_api_values = {
                    ...
                    "subvention_entity_1": api_entity_1,
                    "subvention_type_1": api_subvention_type_1,
                    "subvention_rate_1": api_subvention_rate_1,
                    "subvention_entity_2": api_entity_2,
                    "subvention_type_2": api_subvention_type_2,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "subvention_tenure_1": f"{emi_plan_in_months} month",
                    "subvention_tenure_2": f"{emi_plan_in_months} month",
                    "subvention_entity_1": "BRAND",
                    "subvention_type_1": "PAYBACK",
                    "subvention_rate_1": api_subvention_rate_1,
                    "subvention_entity_2": "EZETAP",
                ...
                actual_db_values = {
                    ...
                    "subvention_tenure_1": brand_subvention_tenure,
                    "subvention_tenure_2": ezetap_subvention_tenure,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Payback": f"{brand_subvention_value+ezetap_subvention_value}%",
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_11_013():
    """
        Sub Feature Code: UI_Common_Card_Eze_EMI_For_Payback_by_Brand_Percentage_Verify_EzeEMI_Txn_When_EzeEMI_Program_Ended_For_Current_Month_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
        Sub Feature Description: Performing the EZE EMI payback by brand percentage and verify the Ezeemi transaction when EZEEMI program has ended for current month
        for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 013: TC013
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"update ezeemi_club_wallet_account set wallet_balance=0 where wallet_type='MCEW' and brand_id='{brand_id}'"
        logger.debug(f"Query to update ezeemi_club_wallet_account by subtracting the max amount of product : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"Query to fetch result from ezeemi_club_wallet_account after subtracting with the max amount of product : {result}")
