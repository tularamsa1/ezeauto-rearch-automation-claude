...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_03_008():
    """
        Sub Feature Code: UI_Common_Card_Preauth_Txn_Confirm_Void_Txn_HDFC_Dummy_VISA_CreditCard_417666
        Sub Feature Description: Performing the preauth txn confirm void transaction via HDFC Dummy PG using VISA Credit card (bin : 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 03:PreAuth , 008: TC008
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            time.sleep(10)
            logger.debug(f"Waiting for 10 secs after selecting card type as CTLS_VISA_CREDIT_417666")
            app_driver.back()
            txn_history_page.click_on_transaction_by_txn_id(txn_id)
