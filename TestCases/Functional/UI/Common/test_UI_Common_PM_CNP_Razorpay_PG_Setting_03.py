...
def test_common_100_103_474():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_PG_Setting_Incorrect_API_Key_3_Set
    Sub Feature Description: Verify able to do txn when incorrect API key3 set
    TC naming code description: 100: Payment Method, 103: RemotePay, 474: TC474
    """
    ...
    try:
        ...
        query = f"update merchant_pg_config set api_key3='FIRST1_%s_0.122' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
        ...
        try:
            query = f"update merchant_pg_config set api_key3='' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
            logger.debug(f"Query to update merchant_pg_config table: {query}")
            result = DBProcessor.setValueToDB(query=query)
            logger.debug(f"query result : {result}")
