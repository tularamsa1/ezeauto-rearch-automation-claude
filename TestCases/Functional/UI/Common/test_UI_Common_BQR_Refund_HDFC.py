...
def test_common_100_102_014():
    """
    :Description: Verification of a BQRV4 BQR Refund transaction via HDFC pg
    :Sub feature code: UI_Common_BQRV4_BQR_Refund_HDFC
    :TC naming code description:100->Payment Method, 102->BQR, 014-> TC014
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_015():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Refund_API_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Refund transaction through API via HDFC pg
    TC naming code description: 100: Payment Method, 102: BQR, 015: TC015
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Fetching refund created time for {txn_id} : {refund_created_time} ")
            posting_date_refunded = result['created_time'].values[0]
            logger.debug(f"Fetching auth_code, rrn, txn_id, and posting date from database for "
                 f"current merchant:{auth_code_refunded}, {rrn_refunded}, {txn_id_refunded}, {posting_date_refunded}")
...
def test_common_100_102_083():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Refund_Failed_via_API_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Refund failed transaction through API via HDFC HDFC
...
def test_common_100_102_084():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Refund_Posted_via_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Refund posted transaction through API via HDFC ATOS
