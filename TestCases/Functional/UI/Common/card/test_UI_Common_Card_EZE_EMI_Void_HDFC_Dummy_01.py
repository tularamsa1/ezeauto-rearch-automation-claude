...
def test_common_100_115_11_018():
    """
    Sub Feature Code: UI_Common_Card_EZE_EMI_Subvented_By_Brand_Ezetap_for_Brand_Ezetap_Cosubvention_Void_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the EZE EMI subvented by brand ezetap for brand-ezetap co-subvention void transaction for an org via
    HDFC Dummy PG using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 018: TC018
    """
    try:
        ...
        query = f"select * from emi where org_code='{org_code}' and status = 'ACTIVE' and issuer_code='HDFC' and " \
                f"card_type='CREDIT' and term = '{emi_plan_in_months} month' and emi_type='BRAND' and " \
                f"tid_type='SUBVENTION';"
        ...
        query = f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and subventing_entity='EZETAP' and subvention_value_type= 'PERCENTAGE' and subvention_type='CASHBACK' and tenure='{emi_plan_in_months} month';"
        logger.debug(f"Query to fetch data from the subvention_plan_details table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for subvention_plan_details table : {result}")
        ...
        mcew_wallet_balance_before_void = result['wallet_balance'].values[0]
        logger.debug(f"Fetching wallet_balance before voiding the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_before_void}")

        query = f"select * from ezeemi_club_wallet_account where wallet_type='GCEW' and brand_id='{brand_id}'"
        logger.debug(f"Query to fetch data from the ezeemi_club_wallet_account table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
        gcew_wallet_balance_before_void = result['wallet_balance'].values[0]
        logger.debug(f"Fetching wallet_balance before voiding the txn from ezeemi_club_wallet_account table : {gcew_wallet_balance_before_void}")
        ...
        query = f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' and subventing_entity in ('BRAND','EZETAP') and subvention_type='CASHBACK' and subvention_value_type='PERCENTAGE' and tenure='{emi_plan_in_months} month'"
        ...
        try:
            ...
            subvention_value = (brand_subvention_value + ezetap_subvention_value)
            cal_additional_cashback = round((amount * (subvention_value / 100)), 2)
            logger.debug(f"cal_additional_payback is : {cal_additional_cashback}")
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
            mcew_wallet_balance_after_void = result['wallet_balance'].values[0]
            logger.debug(f"Fetching wallet_balance after voiding the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_after_void}")

            query = f"select * from ezeemi_club_wallet_account where wallet_type='GCEW' and brand_id='{brand_id}'"
            logger.debug(f"Query to fetch data from the ezeemi_club_wallet_account table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for ezeemi_club_wallet_account table : {result}")
            gcew_wallet_balance_after_void = result['wallet_balance'].values[0]
            logger.debug(f"Fetching wallet_balance after voiding the txn from ezeemi_club_wallet_account table : {gcew_wallet_balance_after_void}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "cashback": "{:,.2f}".format(subvention_value),
                    "brand": brand_name,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "subvention_amount": api_subvention_amt,
                    "mcew_wallet_balance": mcew_wallet_balance_before_void,
                    "gcew_wallet_balance": gcew_wallet_balance_before_void,
                    "emi_cashback_fixed": api_subvention_fixed,
                    "emi_cashback_amount": cal_additional_cashback,
                ...
                actual_db_values = {
                    ...
                    "subvention_amount": api_subvention_amt,
                    "mcew_wallet_balance": mcew_wallet_balance_after_void,
                    "gcew_wallet_balance": gcew_wallet_balance_after_void,
