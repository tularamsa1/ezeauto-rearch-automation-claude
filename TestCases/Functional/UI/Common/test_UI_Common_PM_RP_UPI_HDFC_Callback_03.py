...
def test_common_100_103_067():
    ...
    Sub Feature Description: Performing two UPI  success callback via HDFC after expiry the when autorefund is disabled
    100: Payment Method
    103: RemotePay
    067: TC067
    """
...
def test_common_100_103_069():
    ...
    Sub Feature Description: Performing two UPI  success callback via HDFC after expiry the when autorefund is enabled
    100: Payment Method
    103: RemotePay
    069: TC069
    """
    try:
        ...
        try:
            ...
            if response['success'] == False:
                ...
                response = APIProcessor.send_request(api_details)
                ui_broswer = TestSuiteSetup.initialize_ui_browser()
                paymentLinkUrl = response['paymentLink']
                ui_broswer.goto(paymentLinkUrl)
                logger.info("Opening the link in the browser")
                rp_upi_txn = RemotePayTxnPage(ui_broswer)
                logger.info("Clicking on UPI to start the txn.")
                rp_upi_txn.clickOnRemotePayUPI()
                logger.info("Launching UPI")
                rp_upi_txn.clickOnRemotePayLaunchUPI()
