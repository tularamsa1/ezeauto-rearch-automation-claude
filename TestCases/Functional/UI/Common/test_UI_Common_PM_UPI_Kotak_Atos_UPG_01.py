...
def test_common_100_101_171():
    """
    Sub Feature Code: UI_Common_PM_UPI_amount_mismatch_Via_UPI_Success_Callback_KOTAK_ATOS
    Sub Feature Description: Performing a amount mismatch using pure upi success callback via KOTAK_ATOS
    TC naming code description: 100: Payment method, 101: UPI, 171: Testcase ID
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info("Restarting MPOS-X app")
                login_page.perform_login(app_username, app_password)
                home_page.wait_for_navigation_to_load()
                home_page.wait_for_home_page_load()
                home_page.check_home_page_logo()
...
def test_common_100_101_172():
    """
    Sub Feature Code: UI_Common_PM_UPI_UPG_AUTHORIZED_KOTAK_ATOS
    Sub Feature Description: Verification of a upg authorized txn using  UPI Success Callback via Kotak_ATOS
    TC naming code description: 100: Payment method, 101: UPI, 172: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
