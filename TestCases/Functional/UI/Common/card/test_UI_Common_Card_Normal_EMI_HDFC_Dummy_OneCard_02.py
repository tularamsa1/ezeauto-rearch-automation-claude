...
@pytest.mark.appVal
def test_common_100_115_05_055():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Invalid_Amount_Greater_Than_EMI_Range_Txn_For_An_Org_HDFC_Dummy_OneCard_EMV_VISA_CreditCard_With_Pin_417666
    ...
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 055: TC055
    """
    try:
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_056():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Txn_Inactive_EMI_Setting_For_An_Org_HDFC_Dummy_OneCard_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the normal EMI transaction when the EMI setting is made as inactive for an org
    (not ezetap) via HDFC Dummy PG for OneCard issuer using EMV VISA Credit card with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 056: TC056
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
