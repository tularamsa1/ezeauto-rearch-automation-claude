...
def test_common_100_115_04_007():
    """
    Sub Feature Code: UI_Common_Card_Online_Refund_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090
    Sub Feature Description: Performing the EMV Refund Online transaction via HDFC Dummy PG using VISA Debit card
    having 16 digit PAN length with pin (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 04: Online_Refund, 007: TC007
    """
    try:
        ...
        try:
            ...
            logger.debug(f"To refund selecting the card type as : EMV_WITH_PIN_VISA_DEBIT_428090")
            card_page.select_cardtype(text="EMV_WITH_PIN_VISA_DEBIT_428090")
            logger.debug(f"To refund selected the card type as : EMV_WITH_PIN_VISA_DEBIT_428090")
            payment_page.click_on_proceed_homepage()
            ...
            query = f"select * from txn where orig_txn_id = '{txn_id}' and org_code='{org_code}'"
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "or_card_type_desc": "*0321 EMV with PIN",
                    ...
                    "or_card_type_desc_2": "*0321 EMV with PIN",
                ...
                online_refund_txn_data = refund_page.capture_online_refund_txn_data(password=app_password, card_last_four_digit=card_last_four_digit_db, customer_name=customer_name_db, txn_type=None)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_04_008():
    """
    Sub Feature Code: UI_Common_Card_Online_Refund_HDFC_Dummy_EMVCTLS_MASTER_DebitCard_With_Pin_222360
    Sub Feature Description: Performing the EMVCTLS Refund Online transaction via HDFC Dummy PG using MASTER Debit card
    having 16 digit PAN length with pin (bin: 222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 04: Online_Refund, 008: TC008
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.perform_online_refund(password=app_password, card_last_four_digit=card_last_four_digit_db, device_serial=device_serial)
            logger.debug(f"to refund selecting the card type as : CTLS_WITH_PIN_MASTER_DEBIT_222360")
            card_page.select_cardtype(text="CTLS_WITH_PIN_MASTER_DEBIT_222360")
            logger.debug(f"to refund selected the card type as : CTLS_WITH_PIN_MASTER_DEBIT_222360")
            payment_page.click_on_proceed_homepage()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_04_009():
    """
    Sub Feature Code: UI_Common_Card_Online_Refund_HDFC_Dummy_EMV_MASTER_DebitCard_Without_Pin_222360
    Sub Feature Description: Performing the EMV Refund Online transaction via HDFC Dummy PG using MASTER Debit card
    having 16 digit PAN length without pin (bin: 222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 04: Online_Refund, 009: TC009
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.perform_online_refund(password=app_password, card_last_four_digit=card_last_four_digit_db, device_serial=device_serial)
            logger.debug(f"to refund selecting the card type as : EMV_MASTER_DEBIT_222360")
            card_page.select_cardtype(text="EMV_MASTER_DEBIT_222360")
            logger.debug(f"to refund selected the card type as : EMV_MASTER_DEBIT_222360")
            payment_page.click_on_proceed_homepage()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching tid from txn history for refunded txn : {txn_id_2}, {app_tid_2}")
                app_batch_number_2 = txn_history_page.fetch_batch_number_text()
                logger.debug(f"Fetching batch number from txn history for refunded txn : {txn_id_2}, {app_batch_number_2}")
                app_customer_name_2 = txn_history_page.fetch_customer_name_text()
                logger.debug(f"Fetching customer name from txn history for refunded txn : {txn_id_2}, {app_customer_name_2}")
                app_card_type_desc_2 = txn_history_page.fetch_card_type_desc_text()
                logger.debug(f"Fetching card type desc from txn history for refunded txn : {txn_id_2}, {app_card_type_desc_2}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_04_010():
    """
    Sub Feature Code: UI_Common_Card_Online_Refund_HDFC_Dummy_EMVCTLS_VISA_DebitCard_With_Pin_476173
    Sub Feature Description: Performing the EMVCTLS Refund Online transaction via HDFC Dummy PG using VISA Debit card
    having 16 digit PAN length with pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 04: Online_Refund, 010: TC010
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.perform_online_refund(password=app_password, card_last_four_digit=card_last_four_digit_db, device_serial=device_serial)
            logger.debug(f"to refund selecting the card type as : CTLS_WITH_PIN_VISA_DEBIT_476173")
            card_page.select_cardtype(text="CTLS_WITH_PIN_VISA_DEBIT_476173")
            logger.debug(f"to refund selected the card type as : CTLS_WITH_PIN_VISA_DEBIT_476173")
            payment_page.click_on_proceed_homepage()
