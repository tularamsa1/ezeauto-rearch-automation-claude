...
def test_common_100_101_209():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Via_CheckStatus_API_Razorpay
    Sub Feature Description: Initiate qr by app and perform checkStatus by api for success using magic number
    TC naming code description: 100: Payment Method, 101: UPI, 209: TC209
    """
    try:
        ...
        try:
            ...
            amount = random.randint(2100, 2200)
            ...
            logger.debug(f"response received for the stopPayment api : {response}")
            api_details = DBProcessor.get_api_details('paymentStatus', request_body={
                "username": app_username, "password": app_password, "txnId": str(txn_id)
            })
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for the paymentStatus api : {response}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_210():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Via_SA_CheckStatus_Razorpay
    Sub Feature Description: Verification of a successful upi txn via Razorpay using SA check status
    TC naming code description: 100: Payment Method, 101: UPI, 210: TC210
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_212():
    """
    Sub Feature Code: UI_Common_PM_UPI_Failed_Via_CheckStatus_API_Razorpay
    Sub Feature Description: Initiate qr by app and perform checkStatus by api for failed using magic number
    TC naming code description: 100: Payment Method, 101: UPI, 212: TC212
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            amount = random.randint(2010, 2020)
            order_id = datetime.now().strftime('%m%d%H%M%S')
            home_page = HomePage(app_driver)
            home_page.wait_for_navigation_to_load()
            home_page.wait_for_home_page_load()
            ...
            logger.info("Selected payment mode is UPI")
            payment_page.validate_upi_bqr_payment_screen()
            logger.debug(f"Payment QR generated and displayed successfully")
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for the stopPayment api is : {response}")
            api_details = DBProcessor.get_api_details('paymentStatus', request_body={
                "username": app_username, "password": app_password, "txnId": str(txn_id)
            })
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for the paymentStatus api is : {response}")
            logger.info(f"API RESP settlementStatus : {response['settlementStatus']}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_101_213():
    """
    Sub Feature Code: UI_Common_PM_Initiate_UPI_QR_Via_API_Success_Via_CheckStatus_API_Razorpay
    Sub Feature Description: Initiate qr by api and perform checkStatus by api for success using magic number
    TC naming code description: 100: Payment Method, 101: UPI, 213: TC213
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for the upiqrGenerate : {response}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_214():
    """
    Sub Feature Code: UI_Common_PM_Initiate_UPI_QR_Via_API_Failed_Via_CheckStatus_API_Razorpay
    Sub Feature Description: Initiate qr by api and perform checkStatus by api for failed using magic number
    TC naming code description: 100: Payment Method, 101: UPI, 214: TC214
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
