...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_107_001():
    """
    Sub Feature Code: NonUI_Common_UPI_Static_QR_Generation_Success_KOTAK_OLIVE
    Sub Feature Description: Generate UPI Static QR through api successfully for KOTAK_OLIVE pg
    TC naming code description:d103->Dev Project[KOTAK_OLIVE_UPI], 107-> UPI Static QR, 001->TC001
    """
    try:
        ...
        logger.info(f"fetched pg_merchant_id : {pg_merchant_id}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                qrcode_uri_db = result["qrcode_uri"].iloc[0]
                ...
                actual_db_values = {
                    ...
                    "tr_in_qrcode_uri": publish_id in str(qrcode_uri_db)
                }
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_107_002():
    """
    Sub Feature Code: NonUI_Common_UPI_Static_QR_Re_Generation_Success_KOTAK_OLIVE
    Sub Feature Description: Re-Generate UPI Static QR through api successfully for KOTAK_OLIVE pg
    TC naming code description:d103->Dev Project[KOTAK_OLIVE_UPI], 107-> UPI Static QR, 002->TC002
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"fetching success status,publish_id, username, org code, mid, tid from api response is : "
                         f"{success_api_2},{publish_id_2},{username_api_2},{org_code_api_2},{mid_api_2},{tid_api_2}")
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d103_107_003():
    """
    Sub Feature Code: NonUI_Common_UPI_Static_QR_Regeneration_Different_User_KOTAK_OLIVE
    Sub Feature Description: Re-generate the UPI static QR with different user and verify the tables getting updated.
    TC naming code description:d103->Dev Project[KOTAK_OLIVE_UPI], 107-> UPI Static QR, 003->TC003
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            if is_user_required:  # Create a new user via API
                ...
                if response["success"]:
                    ...
                    logger.debug(
                        f"fetching success status,publish_id,username,org code,mid,tid from second api response is : "
                        f"{success_api_second},{publish_id_second},{username_api_second},{org_code_api_second}, {mid_api_second},{tid_api_second}")
                else:
