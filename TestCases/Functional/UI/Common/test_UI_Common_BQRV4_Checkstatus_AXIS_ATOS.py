...
def test_common_100_102_076():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_Checkstatus_Success_AXIS_ATOS
    Sub Feature Description: Verification of a BQRV4 Checkstatus Success via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 076: TC076
    """
    try:
...
def test_common_100_102_077():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_CheckStatus_Expired_AXIS_ATOS
    Sub Feature Description: Verification of a BQRV4 Expired Check status transaction when auto refund disabled via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 077: TC077
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_146():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_Checkstatus_Amount_Mismatch_AXIS_ATOS
    Sub Feature Description: Verification of a BQRV4 UPI Amount mismatch upon Checkstatus Success via AXIS_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 146: TC146
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Fetching amount_db, payment_mode_db, payment_status_db, payment_state_db, acquirer_code_db, bank_name_db, mid_db, tid_db, payment_gateway_db, rr_number_db, settlement_status_db, amount_original_db from database for "
                f"current merchant:{amount_db}, {payment_mode_db}, {payment_status_db}, {payment_state_db}, {acquirer_code_db}, {bank_name_db}, {mid_db}, {tid_db}, {payment_gateway_db}, {rr_number_db}, {settlement_status_db}, {amount_original_db}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                date_and_time = date_time_converter.to_app_format(created_time)
                expected_app_values = {"pmt_mode": "UPI", "pmt_status": "AUTHORIZED","txn_amt": str(amount_2)+".00",
                                       "settle_status": "SETTLED","txn_id": txn_id, "rrn": str(rrn),
                                       "customer_name": customer_name,"payer_name": payer_name,
                                       "order_id": order_id,"msg": "PAYMENT SUCCESSFUL",
                                       "auth_code": auth_code, "date": date_and_time}
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                date = date_time_converter.db_datetime(created_time)
                expected_api_values = {"pmt_status": "AUTHORIZED","txn_amt_2": float(amount_2),"pmt_mode": "UPI",
                                       "pmt_state": "SETTLED", "rrn": str(rrn),"settle_status": "SETTLED",
                                       "acquirer_code": "AXIS", "issuer_code": "AXIS","txn_type": "CHARGE",
                                       "mid": mid, "tid": tid, "org_code": org_code, "auth_code": auth_code,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            logger.info(f"Started DB validation for the test case : {testcase_id}")
            try:
                expected_db_values = {"txn_amt_2": float(amount_2),"pmt_mode": "UPI","pmt_status": "AUTHORIZED",
                                      "pmt_state": "SETTLED","acquirer_code" : "AXIS", "bank_name" : "Axis Bank",
                                      "payer_name": payer_name, "mid" :mid, "tid" : tid, "pmt_gateway": "ATOS",
                                      "rrn" : str(rrn), "settle_status": "SETTLED",
                                      "bqr_pmt_status": "INITIATED BY UPI",
