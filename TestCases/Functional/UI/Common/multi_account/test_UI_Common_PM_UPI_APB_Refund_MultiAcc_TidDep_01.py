...
def test_common_100_109_021():
    """
    Sub Feature Code: UI_Common_PM_Pure_UPI_full_Refund_via_API_MultiAcc_TidDep_APB
    Sub Feature Description: Multi Account Tid Dep - Verification of a UPI full refund using api for APB
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 021: TC021
    """
    ...
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('TidDepUpiQRGenerate_multiAcc', request_body={
                "username": app_username, "password": app_password, "amount": str(amount), "orderNumber": str(order_id),
                "deviceSerial": str(device_serial), "accountLabel": str(account_label_name)})
            response = APIProcessor.send_request(api_details)
            ...
            logger.debug(f"fetched rrn from txn table is : {rrn_original}")
            ...
            logger.debug(f"Query to fetch transaction data of refunded txn from database : {query}")
            result = DBProcessor.getValueFromDB(query)
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_chargeslip_values = {
                    'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id), 'RRN': str(rrn_refunded),
                    'BASE AMOUNT:': "Rs." + str(amount) + ".00", 'date': txn_date, 'time': txn_time, 'AUTH CODE': ""
                }
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_109_022():
    """
    Sub Feature Code: UI_Common_PM_Pure_UPI_partial_Refund_via_API_MultiAcc_TidDep_APB
    Sub Feature Description: Multi Account Tid Dep - Verification of a UPI partial refund using api for APB
    TC naming code description: 100: Payment Method, 109: MultiAcc_UPI, 022: TC022
    """
    ...
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('paymentRefund', request_body={
                "username": app_username, "password": app_password, "amount": refund_amount, "originalTransactionId": str(txn_id)})
            response = APIProcessor.send_request(api_details)
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_chargeslip_values_refunded = {
                    'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id), 'date': txn_date_2,
                    'RRN': str(rrn_2), 'BASE AMOUNT:': "Rs." + str(refund_amount) + ".00", 'time': txn_time_2
                }
