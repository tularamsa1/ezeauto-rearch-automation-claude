...
def test_common_100_115_161():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the EMV sale transaction via PRIZM PG using VISA Credit card having
    16 digit PAN length with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 161: TC161
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_162():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMV_MASTER_CreditCard_With_Pin_541333
    ...
    TC naming code description: 100: Payment Method, 115: CARD_UI, 162: TC162
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_163():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMV_VISA_CreditCard_Without_Pin_417666
    Sub Feature Description: Performing the EMV sale transaction via PRIZM PG using VISA Credit card having
    16 digit PAN length without pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 163: TC163
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
