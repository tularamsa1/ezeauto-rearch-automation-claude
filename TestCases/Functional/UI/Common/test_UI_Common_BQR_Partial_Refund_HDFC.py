...
def test_common_100_102_016():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Partial_Refund_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Partial Refund transaction via HDFC pg
    TC naming code description: 100: Payment Method, 102: BQR, 016: TC016
    """
    try:
...
def test_common_100_102_017():
    """
    :Description: Verification of a BQRV4 BQR Partial Refund transaction through portal via HDFC pg
    :Sub feature code: UI_Common_BQRV4_BQR_Partial_Refund_Portal_HDFC
    :TC naming code description:100->Payment Method, 102->BQR, 017-> TC017
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_138():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Partial_Refund_Decimal_Amount_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Partial Refund transaction where refund amount will be in decimals
    TC naming code description: 100: Payment Method, 102: BQR, 138: TC138
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_139():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Two_Times_Successful_Partial_Refund_Via_API_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR two times Successful Partial Refund via HDFC
    TC naming code description: 100: Payment Method, 102: BQR, 139: TC139
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                app_payment_mode_refunded_2 = transactions_history_page.fetch_txn_type_text()
                logger.debug(
                    f"Fetching Transaction payment mode from transaction history of MPOS app: Txn Mode = {app_payment_mode_refunded_2}")
                app_txn_id_refunded_2 = transactions_history_page.fetch_txn_id_text()
                logger.debug(
                    f"Fetching Transaction id from transaction history of MPOS app: Txn Id = {app_txn_id_refunded_2}")
                app_payment_amt_refunded_2 = transactions_history_page.fetch_txn_amount_text().split()[1]
                logger.debug(
                    f"Fetching Transaction amount from transaction history of MPOS app: Txn Amt = {app_payment_amt_refunded_2}")
                app_settlement_status_refunded_2 = transactions_history_page.fetch_settlement_status_text()
                logger.debug(
                    f"Fetching settlement status of original txn from transaction history of MPOS app: Txn Id = {app_settlement_status_refunded_2}")
                payment_msg_refunded_2 = transactions_history_page.fetch_txn_payment_msg_text()
                logger.debug(
                    f"Fetching Transaction id of original txn from transaction history of MPOS app: Txn Id = {payment_msg_refunded_2}")
                app_order_id_refunded_2 = transactions_history_page.fetch_order_id_text()
                logger.info(f"Fetching txn order_id from txn history for the txn : {txn_id}, {app_order_id_refunded_2}")
                app_date_and_time_refunded_2 = transactions_history_page.fetch_date_time_text()
                logger.info(f"Fetching date from txn history for the txn : {txn_id}, {app_date_and_time_refunded_2}")
                ...
                actual_app_values = {
                    "pmt_status": app_payment_status_original,
                    "pmt_status_2": app_payment_status_refunded,
                    "pmt_status_3": app_payment_status_refunded_2,
                    "pmt_mode": app_payment_mode_original,
                    "pmt_mode_2": app_payment_mode_refunded,
                    "pmt_mode_3": app_payment_mode_refunded_2,
                    "settle_status": app_settlement_status_original,
                    "settle_status_2": app_settlement_status_refunded,
                    "settle_status_3": app_settlement_status_refunded_2,
                    "txn_id": app_txn_id_original,
                    "txn_id_2": app_txn_id_refunded,
                    "txn_id_3": app_txn_id_refunded_2,
                    "txn_amt": str(app_payment_amt_original),
                    "txn_amt_2": str(app_payment_amt_refunded),
                    "txn_amt_3": str(app_payment_amt_refunded_2),
                    "order_id": app_order_id_original,
                    "order_id_2": app_order_id_refunded,
                    "order_id_3": app_order_id_refunded_2,
                    "pmt_msg": payment_msg_original,
                    "pmt_msg_2": payment_msg_refunded,
                    "pmt_msg_3": payment_msg_refunded_2,
                    "rrn": str(app_rrn_original),
                    "auth_code": app_auth_code_original,
                    "date": app_date_and_time,
                    "date_2": app_date_and_time_refunded,
                    "date_3": app_date_and_time_refunded_2,
                }
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                response = APIProcessor.send_request(api_details)
                logger.debug(f"Response received for transaction details api is : {response}")
                response = [x for x in response["txns"] if x["txnId"] == txn_id_refunded_2][0]
                status_api_refunded_2 = response["status"]
                amount_api_refunded_2 = float(response["amount"])
                payment_mode_api_refunded_2 = response["paymentMode"]
                state_api_refunded_2 = response["states"][0]
                settlement_status_api_refunded_2 = response["settlementStatus"]
                acquirer_code_api_refunded_2 = response["acquirerCode"]
                org_code_api_refunded_2 = response["orgCode"]
                ...
                actual_api_values = {
                    "pmt_status": status_api_original,
                    "pmt_status_2": status_api_refunded,
                    "pmt_status_3": status_api_refunded_2,
                    "pmt_state": state_api_original,
                    "pmt_state_2": state_api_refunded,
                    "pmt_state_3": state_api_refunded_2,
                    "pmt_mode": payment_mode_api_original,
                    "pmt_mode_2": payment_mode_api_refunded,
                    "pmt_mode_3": payment_mode_api_refunded_2,
                    "settle_status": settlement_status_api_original,
                    "settle_status_2": settlement_status_api_refunded,
                    "settle_status_3": settlement_status_api_refunded_2,
                    "txn_amt": amount_api_original,
                    "txn_amt_2": amount_api_refunded,
                    "txn_amt_3": amount_api_refunded_2,
                    "order_id": order_id_original,
                    "order_id_2": order_id_refunded,
                    "order_id_3": order_id_refunded_2,
                    "rrn": str(rrn_api_original),
                    "acquirer_code": acquirer_code_api_original,
                    "issuer_code": issuer_code_api_original,
                    "txn_type": txn_type_api_original,
                    ...
                    "mid_2": mid_api_refunded, "tid_2": tid_api_refunded,
                    "org_code_2": org_code_api_refunded,
                    "acquirer_code_3": acquirer_code_api_refunded_2,
                    "txn_type_3": txn_type_api_refunded_2,
                    "mid_3": mid_api_refunded_2, "tid_3": tid_api_refunded_2,
                    "org_code_3": org_code_api_refunded_2,
                    "auth_code": auth_code_api_original,
                    "date": date_time_converter.from_api_to_datetime_format(date_api_original),
                    "date_2": date_time_converter.from_api_to_datetime_format(date_api_refunded),
                    "date_3": date_time_converter.from_api_to_datetime_format(date_api_refunded_2),
                }
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    # "bqr_merchant_pan_2": bqr_m_pan,
                    "bqr_org_code_2": org_code,
                    "bqr_pmt_state_3": "REFUNDED",
                    "bqr_txn_amt_3": float(full_refund_amount),
                    "bqr_txn_type_3": "REFUND",  # "brq_terminal_info_id_3": terminal_info_id,
                    "bqr_bank_code_3": "HDFC",
                    "bqr_merchant_config_id_3": bqr_mc_id, "bqr_txn_primary_id_3": txn_id_refunded_2,
                ...
                query = "select * from txn where id='" + txn_id_refunded_2 + "'"
                logger.debug(f"Query to fetch data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                status_db_refunded_2 = result["status"].iloc[0]
                payment_mode_db_refunded_2 = result["payment_mode"].iloc[0]
                amount_db_refunded_2 = float(result["amount"].iloc[0])
                state_db_refunded_2 = result["state"].iloc[0]
                payment_gateway_db_refunded_2 = result["payment_gateway"].iloc[0]
                acquirer_code_db_refunded_2 = result["acquirer_code"].iloc[0]
                settlement_status_db_refunded_2 = result["settlement_status"].iloc[0]
                tid_db_refunded_2 = result['tid'].values[0]
                mid_db_refunded_2 = result['mid'].values[0]
                ...
                query = "select * from bharatqr_txn where id='" + txn_id_refunded_2 + "'"
                logger.debug(f"Query to fetch data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                #bqr_status_db_refunded = result["status_desc"].iloc[0]
                bqr_state_db_refunded_2 = result["state"].iloc[0]
                bqr_amount_db_refunded_2 = float(result["txn_amount"].iloc[0])
                bqr_txn_type_db_refunded_2 = result["txn_type"].iloc[0]
                #brq_terminal_info_id_db_refunded = result["terminal_info_id"].iloc[0]
                bqr_bank_code_db_refunded_2 = result["bank_code"].iloc[0]
                bqr_merchant_config_id_db_refunded_2 = result["merchant_config_id"].iloc[0]
                bqr_txn_primary_id_db_refunded_2 = result["transaction_primary_id"].iloc[0]
                #bqr_merchant_pan_db_refunded = result["merchant_pan"].iloc[0]
                bqr_org_code_db_refunded_2 = result['org_code'].values[0]

                actual_db_values = {
                    "pmt_status": status_db_original,
                    "pmt_status_2": status_db_refunded,
                    "pmt_status_3": status_db_refunded_2,
                    "pmt_state": state_db_original,
                    "pmt_state_2": state_db_refunded,
                    "pmt_state_3": state_db_refunded_2,
                    "pmt_mode": payment_mode_db_original,
                    "pmt_mode_2": payment_mode_db_refunded,
                    "pmt_mode_3": payment_mode_db_refunded_2,
                    "txn_amt": amount_db_original,
                    "txn_amt_2": amount_db_refunded,
                    "txn_amt_3": amount_db_refunded_2,
                    "settle_status": settlement_status_db_original,
                    "settle_status_2": settlement_status_db_refunded,
                    "settle_status_3": settlement_status_db_refunded_2,
                    "acquirer_code": acquirer_code_db_original,
                    "acquirer_code_2": acquirer_code_db_refunded,
                    "acquirer_code_3": acquirer_code_db_refunded_2,
                    "bank_code": bank_code_db_original,
                    "pmt_gateway": payment_gateway_db_original,
                    "pmt_gateway_2": payment_gateway_db_refunded,
                    "pmt_gateway_3": payment_gateway_db_refunded_2,
                    "mid": mid_db_original,
                    "tid": tid_db_original,
                    "mid_2": mid_db_refunded,
                    "tid_2": tid_db_refunded,
                    "mid_3": mid_db_refunded_2,
                    "tid_3": tid_db_refunded_2,
                    "bqr_pmt_status": bqr_status_db, "bqr_pmt_state": bqr_state_db,
                    "bqr_txn_amt": bqr_amount_db,
                    "bqr_txn_type": bqr_txn_type_db, "brq_terminal_info_id": brq_terminal_info_id_db,
                    "bqr_bank_code": bqr_bank_code_db,
                    ...
                    # "bqr_merchant_pan_2": bqr_merchant_pan_db_refunded,
                    "bqr_org_code_2": bqr_org_code_db_refunded,
                    "bqr_pmt_state_3": bqr_state_db_refunded_2,
                    "bqr_txn_amt_3": bqr_amount_db_refunded_2,
                    "bqr_txn_type_3": bqr_txn_type_db_refunded_2,
                    # "brq_terminal_info_id_3": brq_terminal_info_id_db_refunded_2,
                    "bqr_bank_code_3": bqr_bank_code_db_refunded_2,
                    "bqr_merchant_config_id_3": bqr_merchant_config_id_db_refunded_2,
                    "bqr_txn_primary_id_3": bqr_txn_primary_id_db_refunded_2,
                    # "bqr_merchant_pan_3": bqr_merchant_pan_db_refunded_2,
                    "bqr_org_code_3": bqr_org_code_db_refunded_2,                }
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_140():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Two_Partial_Refund_Amount_Greater_Than_Original_Amount_HDFC
    Sub Feature Description: Verification of a BQRV4 BQR Partial Refund transaction where partial refund amount
    TC naming code description: 100: Payment Method, 102: BQR, 140: TC140
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Error msg of payment refund for refund amount greater than"
                         f" original amt is : {api_error_message}")
