...
def test_common_100_115_07_107():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Cashback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    ...
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 107: TC107
    """
    try:
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='FEDERAL' and eze_emi_enabled=b'0' order by created_time desc limit 1;"
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='CASHBACK' and name LIKE '{brand_name}_FEDERAL_CREDIT%'"
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_108():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Cashback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    ...
    via HDFC Dummy PG for Federal issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 108: TC108
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='INSTANT' and name LIKE '{brand_name}_FEDERAL_CREDIT%' "
        logger.debug(f"Query to fetch data from the rule table : {query}")
        result = DBProcessor.getValueFromDB(query=query, db_name='rule_engine')
        logger.debug(f"Fetching result from rule table :{result}")
        rule_id = result['id'].values[0]
