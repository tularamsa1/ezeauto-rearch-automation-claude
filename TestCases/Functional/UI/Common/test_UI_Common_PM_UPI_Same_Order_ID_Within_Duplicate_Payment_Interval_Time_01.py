...
def test_common_100_101_317():
    """
    Sub Feature Code: UI_Common_PM_UPI_HDFC_Success_Transaction_Same_Amount_Same_Order_ID
    Sub Feature Description: Perform UPI HDFC Successful Transactions Via CheckStatus Api
                            with same amount and order ID within duplicate payment interval time.
    TC naming code description: 100: Payment Method, 101: UPI, 317: TC317
    """
    try:
        ...
        try:
            ...
            amount = random.randint(201, 1000)
            ...
            logger.debug(f"Fetching payer name from the payment status api, payer name : {payer_name}")
            customer_name = response['customerName']
            logger.debug(f"Fetching customer name from the payment status api, customer name : {customer_name}")
            auth_code = response['authCode']
            logger.debug(f"Fetching auth code from the payment status api, auth code : {auth_code}")
            ...
            logger.info(f"Fetching customer name from txn table: {customer_name_db}")
            ...
            logger.debug(f"Fetching auth code from txn table : {auth_code_db}")
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_319():
    """
    Sub Feature Code: UI_Common_PM_UPI_KOTAK_WL_Success_Transaction_Same_Amount_Same_Order_ID
    Sub Feature Description: Perform UPI KOTAK_WL Successful Transactions Via CheckStatus Api
                            with same amount and order ID within duplicate payment interval time.
    TC naming code description: 100: Payment Method, 101: UPI, 319: TC319
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_321():
    """
    Sub Feature Code: UI_Common_PM_UPI_AXIS_DIRECT_Success_Transaction_Same_Amount_Same_Order_ID
    Sub Feature Description: Perform UPI AXIS_DIRECT Successful Transactions Via CheckStatus Api
                            with same amount and order ID within duplicate payment interval time.
    TC naming code description: 100: Payment Method, 101: UPI, 321: TC321
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
