...
def test_common_100_103_191():
    """
    Sub Feature Code: UI_Common_PM_CNP_NetBanking_Successful_Razorpay
    Sub Feature Description: Verification successful netbanking txn for Razorpay pg
    TC naming code description:100: Payment Method, 103: NetBanking, 191: TC191
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {"pmt_mode": "PAY LINK",
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"settlementStatus_api: {settlementStatus_api}")
                rrNumber_api = response["rrNumber"]
                logger.debug(f"rrNumber_api: {rrNumber_api}")
                txnType_api = response["txnType"]
                logger.debug(f"txnType_api: {txnType_api}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {"Net Banking": cnpware_payment_option_value_1,
                                               'RRN': str(cnp_txn_rrn),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_192():
    """
    Sub Feature Code: UI_Common_PM_CNP_NetBanking_Failed_TPSL
    Sub Feature Description: Verification failed netbanking txn for RAZORPAY pg
    TC naming code description:100: Payment Method, 103: NetBanking, 192: TC0192
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Query result : {result}")
                payment_intent_status = result["status"].iloc[0]
                logger.debug(f"Query result, payment_intent_status : {payment_intent_status}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_193():
    """
    Sub Feature Code: UI_Common_PM_CNP_NetBanking_Timeout_RAZORPAY
    Sub Feature Description: Verification netbanking txn after timeout for RAZORPAY pg
    TC naming code description: 100: Payment Method, 103: NetBanking, 193: TC193
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {"pmt_status": "FAILED",
                                      ...
                                      "cnpware_txn_amt": amount,
                                      "cnpware_pmt_mode": "CNP",
                                      "cnpware_pmt_state": "CONFIRM_PENDING",
                                      "cnp_pmt_gateway": "RAZORPAY",
                                      "cnpware_pmt_gateway": "RAZORPAY",
                                      "cnpware_pmt_flow": "REMOTEPAY",
                                      "auth_code": txn_auth_code,
