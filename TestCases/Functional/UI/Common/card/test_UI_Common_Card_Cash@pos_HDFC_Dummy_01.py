...
def test_common_100_115_02_007():
    """
        Sub Feature Code: UI_Common_Card_Cash@pos_Success_Txn_Without_Sale_Via_HDFC_Dummy_EMVCTLS_VISA_DebitCard_Without_Pin_476173
        Sub Feature Description: Performing the EMVCTLS cash@pos success transaction without sale via HDFC Dummy PG using VISA Debit card without pin (bin : 476173)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 02:Cash@Posh , 007: TC007
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching card type desc value from txn history for the txn : {txn_id}, {app_card_type_desc}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Value of customerName obtained from txnlist api for cash@pos txn : {customer_name_api}")
                ...
                logger.debug(f"Value of payerName obtained from txnlist api for cash@pos txn : {payer_name_api}")
                ...
                logger.debug(f"Value of nameOnCard obtained from txnlist api for cash@pos txn : {name_on_card_api}")
...
def test_common_100_115_02_008():
    """
        Sub Feature Code: UI_Common_Card_Cash@pos_Void_Txn_Without_Sale_Via_HDFC_Dummy_EMVCTLS_VISA_DebitCard_Without_Pin_476173
        Sub Feature Description: Performing the EMVCTLS cash@pos void transaction without sale via HDFC Dummy PG using VISA Debit card without pin (bin : 476173)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 02:Cash@Posh , 008: TC008
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
