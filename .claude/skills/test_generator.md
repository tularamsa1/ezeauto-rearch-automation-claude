...
Step 6: Register in TestCasesDetail.xlsx
  Immediately after Step 5 passes, register the new test:
    python Tools/register_testcase.py <generated_file_path>
  Expected outputs:
    [OK]   Registered test_common_rearch_NNNN in sheet '<sheet>'  ← success
    [SKIP] test_common_rearch_NNNN is already registered ...       ← already exists, no action needed
...
CARD API VALIDATION RULE
For every Card test, the API validation block MUST include these 3 fields
in addition to the standard fields (pmt_status, txn_amt, pmt_mode, etc.):

  # Expected:
  "pmt_card_brand": "<VISA|MASTERCARD|RUPAY|...>",  # card brand under test
  "pmt_card_type":  "<DEBIT|CREDIT>",
  "card_txn_type":  "<EMV|SWIPE|CONTACTLESS>",
...
Set expected values based on which card variant was selected in the test
(e.g., click_visa_debit_emv() → VISA / DEBIT / EMV).
See test_validations.md for the full Card-specific API fields table.
...
DB TXN LOOKUP RULE
Always include BOTH org_code AND username when querying the txn table:
...
NEVER filter by org_code alone — multiple users share an org, causing cross-contamination
in parallel test runs. NEVER use external_ref/order_id as the filter (not stored in txn).
...
- Function: test_common_rearch_<NNNN> where NNNN is a zero-padded 4-digit sequence number
            (e.g. test_common_rearch_0001, test_common_rearch_0002)
            Increment NNNN for each new test — check existing files in TestCases/Functional/UI/ReArch/
            to find the next available number.
- File: test_UI_ReArch_PM_<METHOD>_<FLOW>_<NUMBER>.py
- No class wrapper — tests are standalone functions.
