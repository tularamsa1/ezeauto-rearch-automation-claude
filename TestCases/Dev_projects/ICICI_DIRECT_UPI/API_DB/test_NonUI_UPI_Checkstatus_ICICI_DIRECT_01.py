...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d102_101_001():
    """
    Sub Feature Code: NonUI_Common_UPI_ICICI_Direct_Checkstatus_Success
    Sub Feature Description: Generate QR through api and perform checkstatus success for UPI txn of ICICI_Direct pg
    TC naming code description: d102: ICICI DIRECT UPI Dev, 101-> UPI, 001->TC001
    """
    try:
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for checkstatus api is : {response}")
...
def test_d102_101_002():
    ...
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('stopPayment', request_body={
                "username": app_username, "password": app_password, "txnId": str(txn_id)})
            logger.debug(f"API DET: {api_details}")
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d102_101_013():
    """
    Sub Feature Code: NonUI_Common_UPI_ICICI_Direct_Checkstatus_via_Api_After_QR_Code_Expiry
    Sub Feature Description: Generate QR through api and perform UPI txn via checkstatus api after qr expiry ICICI_Direct pg
    TC naming code description: d102: ICICI DIRECT UPI Dev, 101:- UPI, 013:-TC013
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received for paymentStatus api is : {response}")
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d102_101_014():
    """
    Sub Feature Code: NonUI_Common_UPI_ICICI_Direct_Checkstatus_Pending
    Sub Feature Description: Generate QR through api and perform UPI Pending txn via checkstatus api via ICICI_Direct pg
    TC naming code description: d102: ICICI DIRECT UPI Dev, 101:- UPI, 014:-TC014
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.apiVal
@pytest.mark.dbVal
def test_d102_101_038():
    """
    Sub Feature Code: NonUI_Common_PM_UPI_Amount_Mismatch_Upon_Check_Status_ICICI_DIRECT
    Sub Feature Description: Verification of amount mismatch upon check status for upi txn via ICICI DIRECT
    TC naming code description: d102: ICICI DIRECT UPI Dev, 101: UPI, 038: TC038
    """
