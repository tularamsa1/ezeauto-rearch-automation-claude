...
def test_common_100_116_026():
    """
    Sub Feature Code: UI_Common_Card_Sale_AXIS_ATOS_TLE_Pre_Auth_Opt_In_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt in MC Chip KWD Pre auth transaction via AXIS_ATOS_TLE PG using Credit
    card with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 026: TC026
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_027():
    """
    Sub Feature Code: UI_Common_Card_Sale_AXIS_ATOS_TLE_Pre_Auth_Release_Opt_In_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt in MC Chip KWD Pre auth release transaction via AXIS_ATOS_TLET PG using
    Credit card with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 027: TC027
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_028():
    """
    Sub Feature Code: UI_Common_Card_Sale_AXIS_ATOS_TLE_Pre_Auth_Confirm_Opt_In_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt in MC Chip KWD Pre auth confirm transaction via AXIS_ATOS_TLE PG using Credit card with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 028: TC028
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "opted_type_2": "Yes",
                    "local_card_2": "No",
                    "txn_currency_2": "JPY {:,.2f}".format(converted_amount),
                    "markup_fee_2": "4.50 %",
                    "currency_conversion_fee_rate_2": "0.52480573 JPY/INR",
                    "currency_converted_rate": "JPY {:,.2f}".format(converted_amount),
                    "offered_currency_selection": message
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
