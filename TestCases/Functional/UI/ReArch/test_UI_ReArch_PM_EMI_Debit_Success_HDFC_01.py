...
def test_common_rearch_0037():
    """
    Sub Feature Code: UI_ReArch_PM_EMI_Debit_Success_HDFC
    Sub Feature Description:
        ...
        select HDFC Bank Debit Card, choose 3-month EMI plan, verify
        breakup amounts (Item Price, Interest, Total Amount), proceed
        through card type selection (Visa Debit EMV), verify success,
        then validate via App, API, and Charge Slip.

        NOTE: For debit card EMI, the backend records ₹1 as the txn amount.
        Txn history and detail page will show ₹1, not ₹5,959.
    ...
    try:
        ...
        try:
            ...
            amount = "5959"
            # Debit card EMI: backend records ₹1 as txn amount
            backend_amount = "1"
            backend_amount_formatted = "\u20b91"
            logger.debug(f"amount={amount}, backend_amount={backend_amount}")
            ...
            # Resolve txn from DB — debit EMI records ₹1 as amount
            query = (
                f"select id, created_time, posting_date, rr_number, auth_code "
                f"from txn "
                f"where org_code='{org_code}' AND username='{app_username}' "
                f"AND payment_mode='CARD' "  # TODO: verify if debit EMI uses CARD or different mode
        ...
        if ConfigReader.read_config("Validations", "app_validation") == "True":
            ...
            try:
                ...
                # NOTE: Debit EMI shows ₹1 in txn history, not the original ₹5,959
                date_and_time = date_time_converter.to_rearch_app_format(created_time)
                expected_app_values = {
                    "txn_status": "Payment Authorized",  # TODO: verify exact label for debit EMI on first run
                    "txn_id":     txn_id,
                    "date":       date_and_time,
                    "amount":     backend_amount_formatted,
                }
        ...
        if ConfigReader.read_config("Validations", "api_validation") == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "txn_amt":       float(backend_amount),  # Debit EMI: ₹1 in backend
                    "pmt_mode":      "CARD",             # TODO: verify if debit EMI uses CARD or EMI
                    "txn_type":      "CHARGE",
                    "acquirer_code": "HDFC",
                    "org_code":      org_code,
                    "rrn":           rrn,
