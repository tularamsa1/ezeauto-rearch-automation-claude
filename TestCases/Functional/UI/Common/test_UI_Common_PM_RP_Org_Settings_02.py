...
def test_common_100_103_327():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Enable_Time_Based_Restriction_Before_Start_Time_Via_HDFC
    Sub Feature Description: Verify remote Pay success transactions via HDFC, before the start time of "Time Based Restriction"
    TC naming code description: 100: Payment Method, 103: RemotePay, 327: TC327
    """
    try:
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_100_103_328():
    """
     Sub Feature Code: UI_Common_PM_RP_Org_Setting_Enable_Time_Based_Restriction_After_Start_Time
     Sub Feature Description: Verification of RP link generation after the start time of "Time Based Restriction" and current time< End date
     TC naming code description: 100: Payment Method, 103: RemotePay, 328: TC328
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        time_based_restriction_end_time = (datetime.now(indian_timezone) + timedelta(minutes=2)).strftime("%H%M")
        logger.debug(f"time_based_restriction_start_time (IST) : {time_based_restriction_start_time}")
        logger.debug(f"time_based_restriction_end_time (IST) : {time_based_restriction_end_time}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "err_msg": "Transaction not allowed now, please try after some time."
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_329():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Enable_Time_Based_Restriction_After_End_Time_Success_Txn_Via_HDFC
    Sub Feature Description: Perform a successful Remote Pay transaction via HDFC after the end time of "Time Based Restriction"
    TC naming code description: 100: Payment Method, 103: RemotePay, 329: TC329
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_330():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Enable_Time_Based_Restriction_with_RemotePay_Option_Restricted_Success_Transaction_via_HDFC
    Sub Feature Description: Successful Remote Pay transaction via HDFC, when "Time Based Restriction" is enabled and Remote Payment Mode is not selected
    TC naming code description: 100: Payment Method, 103: RemotePay, 330: TC330
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_331():
    """
    Sub Feature Code: UI_Common_PM_RP_Org_Setting_Disable_Time_Based_Restriction_Successful_Transaction_via_HDFC
    Sub Feature Description: Successful Remote Pay transaction when "Time Based Restriction" is disabled via HDFC
    TC naming code description: 100: Payment Method, 103: RemotePay, 331: TC331
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
