...
# def test_mp_700_706_033():
#     """
#     Sub Feature Code: UI_MP_Report_based_on_Txn_status_Voided_as_City_head
#     Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Voided when logged in as City Head
...
#                     "BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and status='" + txn_status + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "', '" + taluk_username + "', '" + district_username + "') order by created_time asc limit 1;"
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_709_034():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Reversed_as_Cashier
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Reversed when logged in as Cashier
    TC naming code description:
    700: Merchant Portal
    709: Cashier
    034: TC034
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where id " \
                    "BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "' and org_code='" + org_code + "' and status='" + txn_status + "' and username = '" + txn_username + "' order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_709_035():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Refund_pending_as_Cashier
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Refund Pending when logged in as Cashier
    TC naming code description:
    700: Merchant Portal
    709: Cashier
    035: TC035
    ...
    try:
        ...
        try:
            ...
            txn_id = response['txnId']
            logger.debug(f'txn_id for UPI txn: {txn_id}')
            ...
            logger.debug(f"fetched txn_id_refund from txn table is : {txn_id_refund}")
            ...
            query = "select * from txn where id = '" + txn_id_refund + "';"
            logger.debug(f"Query to fetch transaction id from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_709_036():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_Pre_Auth_as_Cashier
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status Pre Auth when logged in as Cashier
    TC naming code description:
    700: Merchant Portal
    709: Cashier
    036: TC036
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_709_037():
    """
    Sub Feature Code: UI_MP_Report_based_on_Txn_status_REL_Pre_Auth_as_Cashier
    Sub Feature Description: Verifying txn details on Report Page using Filter Txn Status REL_Pre_Auth when logged in as Cashier
    TC naming code description:
    700: Merchant Portal
    709: Cashier
    037: TC037
        ...
        try:
            ...
            if preauth_payment_success == True:
                ...
                if confirm_success == True:
                    ...
                    api_details = DBProcessor.get_api_details('preauth_release_txn',
                                                              request_body={"username": txn_username,
                                                                            "password": txn_password,
                                                                            "txnId": txn_id,
                                                                            })
                    rel_preauth_response = APIProcessor.send_request(api_details)
                    logger.info(f"Response received from Pre Auth release API: {rel_preauth_response}")
                else:
