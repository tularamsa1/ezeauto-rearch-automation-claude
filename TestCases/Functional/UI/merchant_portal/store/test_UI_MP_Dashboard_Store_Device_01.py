...
@pytest.mark.dbVal
@pytest.mark.apiVal
def test_mp_700_708_018():
    """
    Sub Feature Code: UI_MP_Device_Usage_as_Store_head
    Sub Feature Description: Verifying device usage details on dashboard for payment done thru device when logging in as Store Head
    ...
    try:
        ...
        try:
            ...
            query = "select sum(amount) as SUM, org_code from txn where id between '" + current_date_2[2:] + "' AND '" + \
                    ...
                    " '" + org_code + "' and username IN ('"+txn_username+ "', '" + login_username + "')  and payment_mode = 'CARD' and status = 'AUTHORIZED';"
            ...
            query = "select device_serial, count(*) from txn where id between '" + current_date_2[2:] + "' AND '" + \
                    current_date_actual[2:] + "' and org_code =" \
                    " '" + org_code + "' and username IN ('"+txn_username+ "', '" + login_username + "') and payment_mode = 'CARD' and status = 'AUTHORIZED' group by device_serial;"
...
@pytest.mark.dbVal
@pytest.mark.apiVal
def test_mp_700_708_019():
    """
    Sub Feature Code: UI_MP_Devices_as_Store_head
    Sub Feature Description: Verifying devices details on dashboard for payment done thru device when logging in as Store Head
    TC naming code description:
    700: Merchant Portal
    708: Store
    019: TC019
