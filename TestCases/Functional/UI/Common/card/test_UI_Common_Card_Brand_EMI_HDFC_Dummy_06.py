...
def test_common_100_115_07_049():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Success_Txn_For_Root_Org_HDFC_Dummy_DebitCard_Without_Pin_476173_For_3_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand percentage success transaction for root org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 476173)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 049: TC049
    """
    try:
        ...
        query = f"select * from subvention_plan where brand_id='{brand}' and org_code='ALL' and card_type= 'DEBIT' " \
                f"and bank='HDFC' and eze_emi_enabled=b'0'"
        ...
        try:
            ...
            emi_emi_payback_amount = result['emi_payback_amount'].values[0]
            logger.debug(f"Fetching emi cashback amount from txn_emi table : {emi_emi_payback_amount}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    ...
                    "loan_amt": float(emi_loan_amount),
                    ...
                    "emi_payback_amount": emi_emi_payback_amount,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_051():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Success_Txn_For_Root_Org_HDFC_Dummy_DebitCard_Without_Pin_476173_For_6_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand percentage success transaction for root org via HDFC Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 476173)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 051: TC051
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_053():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Payback_by_Brand_Percentage_Success_Txn_For_Root_Org_HDFC_Dummy_DebitCard_Without_Pin_476173_For_9_Months_Tenure
        Sub Feature Description: Performing the brand EMI payback by brand percentage success transaction for root org via HDFC Dummy PG using EMV VISA Debit card with pin for 9 months tenure (bin: 476173)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 053: TC053
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_079():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Txn_Amount_Less_Than_Product_Amount_Range_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
        Sub Feature Description: Performing the brand EMI transaction amount less than the product amount range for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 079: TC079
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        brand_sku_name = result['sku_name'].values[0]
        logger.debug(f"Fetching sku_name value from the brand_sku_details table : {brand_sku_name}")
        brand_min_amount = float(result['min_amount'].values[0])
        logger.debug(f"Fetching min_amount value from the brand_sku_details table : {brand_min_amount}")
        brand_max_amount = float(result['max_amount'].values[0])
        logger.debug(f"Fetching max_amount value from the brand_sku_details table : {brand_max_amount}")
        ...
        try:
            ...
            logger.debug(f"After subracting the minimum amount of product with 100 is : {amount}")
            order_id = datetime.now().strftime('%m%d%H%M%S')
            home_page.enter_amount_and_order_number_and_device_serial_for_card(amount, order_id, device_serial)
            logger.debug(f"Entered order_id and device_serial is :{order_id}, {device_serial}")
            payment_page = PaymentPage(app_driver)
            payment_page.is_payment_page_displayed_card(amount=amount, order_id=order_id, device_serial=device_serial)
            payment_page.click_on_brand_emi_pmt_mode()
            logger.debug(f"Selected payment mode is Brand EMI")
            payment_page.click_and_enter_search_products_or_brands(brand_sku_name)
            logger.debug(f"Entered the products")
            app_invalid_product_msg = payment_page.fetch_error_msg_brand_emi()
            logger.info(f"Invalid Product message is, {app_invalid_product_msg}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                actual_app_values = {
                    "invalid_product_msg": app_invalid_product_msg
                }
                logger.debug(f"actual_app_values: {actual_app_values}")
                Validator.validateAgainstAPP(expectedApp=expected_app_values, actualApp=actual_app_values)
            except Exception as e:
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_080():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Txn_Amount_Greater_Than_Product_Amount_Range_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
        Sub Feature Description: Performing the brand EMI transaction amount greater than the product amount range for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 080: TC080
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"After adding the maximum amount of product with 100 is : {amount}")
            order_id = datetime.now().strftime('%m%d%H%M%S')
            home_page.enter_amount_and_order_number_and_device_serial_for_card(amount, order_id, device_serial)
            logger.debug(f"Entered order_id and device_serial is :{order_id}, {device_serial}")
            payment_page = PaymentPage(app_driver)
