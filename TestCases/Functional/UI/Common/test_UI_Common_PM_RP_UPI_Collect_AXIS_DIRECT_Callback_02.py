...
def test_common_100_103_136():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_Success_Callback_After_Expiry_AutoRefundEnabled_AXIS_DIRECT
    Sub Feature Description: Verification of a one upi collect success callback after expiry via Axis Direct when auto refund is enalbed.
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_103_137():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_Two_Success_Callback_After_Expiry_AutoRefundEnabled_AXIS_DIRECT
    Sub Feature Description: Verification of a two upi collect success callback after expiry via Axis Direct when auto refund is enabled.
    100: Payment Method
    103: RemotePay
    137: TC137
    """
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_138():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Collect_success_callback_after_expiry_AutoRefund_Disabled_AXIS_DIRECT
    Sub Feature Description: Performing a UPI Collect success callback via Axis Direct after expiry when autorefund is disabled
    TC naming code description:
    100: Payment Method
    103: RemotePay
    138: TC_138
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_139():
    """
    Sub Feature Code: UI_Common_PM_RP_2_UPI_Collect_success_callback_before_expiry_AXIS_DIRECT_AutoRefund_Disabled
    Sub Feature Description: Verification of a two UPI Collect success callback via Axis Direct before expiry when autorefund is disabled
    TC naming code description:
    100: Payment Method
    103: RemotePay
    139: TC139
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "date": date_and_time,
                    "new_txn_date": new_date_and_time,
                }
                logger.debug(f"expected_app_values: {expected_app_values}")
                app_driver = TestSuiteSetup.initialize_app_driver(testcase_id)  # logger
                loginPage = LoginPage(app_driver)
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_140():
    """
    Sub Feature Code: UI_Common_PM_RP_2_UPI_Collect_success_callback_before_expiry_AXIS_DIRECT_AutoRefund_Enabled
    Sub Feature Description: Verification of two UPI Collect success callback via Axis Direct before expiry when autorefund is enabled
    TC naming code description:
    100: Payment Method
    103: RemotePay
    140: TC_140
