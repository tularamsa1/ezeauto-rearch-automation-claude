...
def test_common_100_102_030():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Checkstatus_Success_Via_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR Checkstatus Success via YES_ATOS
...
def test_common_100_102_031():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Checkstatus_Expired_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR Expired Check status transaction when auto refund disabled via YES_ATOS
...
def test_common_100_102_135():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_Checkstatus_Amount_Mismatch_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 BQR Amount mismatch upon Checkstatus Success via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 135: TC135
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                date_and_time = date_time_converter.to_app_format(created_time)
                expected_app_values = {"pmt_mode": "BHARAT QR", "pmt_status": "STATUS:AUTHORIZED","txn_amt": str(amount_2)+".00",
                                       "settle_status": "SETTLED","txn_id": txn_id, "rrn": str(rrn),
