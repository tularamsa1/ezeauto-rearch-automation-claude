...
def test_common_100_115_05_111():
    """
        Sub Feature Code: UI_Common_Card_Normal_EMI_Instant_Subvention_Payback_By_Merchant_Percentage_Success
        _Txn_For_An_Org_HDFC_HDFC_EMV_VISA_DebitCard_With_Pin_428090_6_Months_Tenure
        Sub Feature Description: Performing the Normal EMI instant subvention payback by merchant percentage success
        transaction for an org via HDFC HDFC PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 111: TC111
    """
    try:
        ...
        logger.debug(f"Response received when emi, instant_emi enabled, emi_for_client and offering_cashback is enabled in preconditions settings : {response}")
        ...
        logger.debug(f"Active emi setting for debit card")
        ...
        query = f"select * from subvention_plan where org_code='{org_code}' and card_type= 'DEBIT' and bank='HDFC' order by created_time desc limit 1;"
        ...
        query = (f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' "
                 f"and subventing_entity='MERCHANT' and subvention_type='INSTANT' and subvention_value_type='PERCENTAGE' and tenure='{emi_plan_in_months} month'")
        ...
        try:
            ...
            query = (f"select * from subvention_plan where org_code='{org_code}' and card_type= 'DEBIT'"
                     f" and bank='HDFC' order by created_time desc limit 1;")
            logger.debug(f"Query to fetch data from the subvention_plan table : {query}")
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for subvention_plan table : {result}")
            subvention_plan_id = result['id'].values[0]
            ...
            query = (f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and "
                     f"subventing_entity='MERCHANT' and subvention_value_type= 'PERCENTAGE' and "
                     f"subvention_type='INSTANT' and tenure='{emi_plan_in_months} month' ;")
            ...
            cal_instant_amt = amount * (subvention_value / 100)
            logger.debug(f"cal_instant_amt is : {cal_instant_amt}")
            monthly_interest_rate = interest_rate / (12 * 100)
            logger.debug(f"monthly_interest_rate is : {monthly_interest_rate}")
            cal_monthly_emi_amt = (amount - cal_instant_amt) * monthly_interest_rate * (
                    (1 + monthly_interest_rate) ** emi_plan_in_months) / ((1 + monthly_interest_rate) ** emi_plan_in_months - 1)
            ...
            total_interest = round((total_emi - (amount - cal_instant_amt)), 2)
            logger.debug(f"total_interest amount is : {total_interest}")
            # Subtract the original amount with instant subvention amount
            normal_inst_amt = round((amount - cal_instant_amt), 2)
            logger.debug(f"Subtract interest amount from original amount : {normal_inst_amt}")
            ...
            txn_aid = result['aid'].values[0]
            logger.debug(f"Fetching aid value from the txn table : {txn_aid}")
            txn_tc = result['tc'].values[0]
            logger.debug(f"Fetching tc value from the txn table : {txn_tc}")
            ...
            emi_loan_amount = round(result['emi_loan_amount'].values[0],2)
            ...
            emi_txn_amount = round(result['txn_amount'].values[0],2)
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    "txn_amt": "{:,.2f}".format(normal_inst_amt),
                    "pmt_mode": "CARD",
                    "txn_id": txn_id,
                    ...
                    "loan_amt": "{:,.2f}".format(normal_inst_amt),
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    "txn_amt": float(normal_inst_amt),
                    "original_txn_amt": float(normal_inst_amt),
                    "currency_code": "INR",
                    "pmt_mode": "CARD",
                    "pmt_status": "AUTHORIZED",
                    ...
                    "ext_ref_number_2": device_serial,
                    "ext_ref_number_7": "NORMAL_EMI",
                    ...
                    "loan_amt": float(normal_inst_amt),
                    ...
                    "subvention_amount": float(total_interest),
                    "subvention_rate": float(subvention_value),
                    "order_number": order_id,
                    "total_amt": float(normal_inst_amt),
                    "user_agreement": f"I agree to pay as per the card issuer agreement. I understand that purchase(s) would be converted into EMI at the sole discretion of the Bank by charging an interest of {float(emi_interest_rate)}% on the monthly reducing balance."
                }
                ...
                api_original_txn_amt = round(response['amountOriginal'],2)
                logger.debug(f"Value of amountOriginal obtained from txnlist api : {api_original_txn_amt}")
                api_currency_code = response['currencyCode']
                logger.debug(f"Value of currencyCode obtained from txnlist api : {api_currency_code}")
                ...
                api_merchant_code = response['merchantCode']
                logger.debug(f"Value of merchantCode obtained from txnlist api : {api_merchant_code}")
                ...
                api_pmt_gateway = response['paymentGateway']
                logger.debug(f"Value of paymentGateway obtained from txnlist api : {api_pmt_gateway}")
                ...
                api_ext_ref_number_2 = response['externalRefNumber2']
                logger.debug(f"Value of externalRefNumber2 obtained from txnlist api : {api_ext_ref_number_2}")
                api_ext_ref_number_7 = response['externalRefNumber7']
                logger.debug(f"Value of externalRefNumber7 obtained from txnlist api : {api_ext_ref_number_7}")
                ...
                api_loan_amt = round(response['emiDetails']['loanAmount'],2)
                ...
                api_subvention_amt = round(response['emiDetails']['subventionDetails'][0]['subventionAmount'],2)
                ...
                api_order_id = response['orderNumber']
                logger.debug(f"Value of orderNumber obtained from txnlist api : {api_order_id}")
                api_total_amt = response['totalAmount']
                logger.debug(f"Value of totalAmount obtained from txnlist api : {api_total_amt}")
                api_user_agreement = response['userAgreement']
                ...
                actual_api_values = {
                    "txn_amt": api_amount,
                    "original_txn_amt": api_original_txn_amt,
                    "currency_code": api_currency_code,
                    ...
                    "ext_ref_number_7": api_ext_ref_number_7,
                    ...
                    "total_amt": api_total_amt,
                    "user_agreement": api_user_agreement
                }
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    "txn_amt": "1.00",
                    "orig_amt": float(normal_inst_amt),
                    "pmt_mode": "CARD",
                    "pmt_status": "AUTHORIZED",
                    "pmt_state": "AUTHORIZED",
                    "acquirer_code": "HDFC",
                    ...
                    "loan_amt": normal_inst_amt,
                    "subvention_org_code": org_code,
                    "subvention_brand_id": "ALL",
                    "subvention_card_type": 'DEBIT',
                    "subvention_entity": "MERCHANT",
                    "subvention_type": "INSTANT",
                ...
                actual_db_values = {
                    ...
                    "orig_amt": float(original_amount_db),
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "AID": "" if txn_aid is None else txn_aid,
                    "APP": "VISA",
                    "TC": txn_tc,
                    ...
                    "Txn Amount": "Rs." + "{:.2f}".format(amount),
                    "Instant EMI Discount": f"{subvention_value}%",
                    "Total Instant EMI Discount Amount": "Rs." + "{:.2f}".format(total_interest),
                    "Loan Amount": "Rs." + "{:.2f}".format(emi_loan_amount),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_112():
    """
        Sub Feature Code: UI_Common_Card_Normal_EMI_Instant_Subvention_Payback_By_Merchant_Percentage_Success
        _Txn_For_An_Org_HDFC_HDFC_EMV_VISA_DebitCard_With_Pin_428090_9_Months_Tenure
        Sub Feature Description: Performing the Normal EMI instant subvention payback by merchant percentage success
        transaction for an org via HDFC HDFC PG using EMV VISA Debit card with pin for 9 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 112: TC112
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_113():
    """
        Sub Feature Code: UI_Common_Card_Normal_EMI_Instant_Subvention_Payback_By_Merchant_Percentage_Success
        _Txn_For_An_Org_HDFC_HDFC_EMV_VISA_DebitCard_With_Pin_428090_3_Months_Tenure
        Sub Feature Description: Performing the Normal EMI instant subvention payback by merchant percentage success
        transaction for an org via HDFC HDFC PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 113: TC113
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_114():
    """
        Sub Feature Code: UI_Common_Card_Normal_EMI_Instant_Subvention_Txn_For_Amount_Greater_Than_Subvention_For_An_Org_
        HDFC_HDFC_EMV_VISA_CreditCard_With_Pin_417666_3_Months_Tenure
        Sub Feature Description: Verify the normal instant EMI amount greater than the subvention for an org via HDFC HDFC PG
        using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 114: TC114
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = (f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' "
                 f"and subventing_entity='MERCHANT' and subvention_type='INSTANT' and"
                 f" subvention_value_type='PERCENTAGE' and tenure='{emi_plan_in_months} month'")
        logger.debug(f"Query to update subvention_plan_details with status as ACTIVE : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"Query to fetch result from subvention_plan_details for status as ACTIVE : {result}")
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for subvention_plan table : {result}")
            subvention_max_amt = result['max_amount'].values[0]
            logger.debug(f"Fetching max_amount from subvention_plan table : {subvention_max_amt}")
            ...
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            amount = int(subvention_max_amt) + 100
            logger.info(f"Normal instant subvention EMI maximum amount after adding 100, {amount}")
            order_id = datetime.now().strftime('%m%d%H%M%S')
            ...
            actual_no_emi_options = payment_page.fetch_error_msg_normal_emi()
            logger.info(f"Error message for maximum subvention amount is : {actual_no_emi_options}")
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_115():
    """
        Sub Feature Code: UI_Common_Card_Normal_EMI_Instant_Subvention_Txn_For_Amount_Less_Than_Subvention_For_An_Org_
        HDFC_HDFC_EMV_VISA_CreditCard_With_Pin_417666_3_Months_Tenure
        Sub Feature Description: Verify the normal instant EMI amount lesser than the subvention for an org via HDFC HDFC PG
        using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 115: TC115
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            result = DBProcessor.getValueFromDB(query)
            logger.debug(f"Query result for subvention_plan table : {result}")
            subvention_min_amt = result['min_amount'].values[0]
            logger.debug(f"Fetching min_amount from subvention_plan table : {subvention_min_amt}")
            ...
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            amount = int(subvention_min_amt) - 100
            logger.info(f"Normal instant subvention EMI minimum amount after subtracting 100, {amount}")
            order_id = datetime.now().strftime('%m%d%H%M%S')
            home_page.enter_amount_and_order_number_and_device_serial_for_card(amount, order_id, device_serial)
            logger.debug(f"Entered amount, order_id and device_serial is  : {amount}, {order_id}, {device_serial}")
            payment_page = PaymentPage(app_driver)
            ...
            logger.debug(f"Selected the card type as : EMV_WITH_PIN_VISA_CREDIT_417666")
            actual_no_emi_options = payment_page.fetch_error_msg_normal_emi()
            logger.info(f"Error message for minimum subvention amount is : {actual_no_emi_options}")
