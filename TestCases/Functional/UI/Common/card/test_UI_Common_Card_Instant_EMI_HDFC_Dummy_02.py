...
def test_common_100_115_06_014():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    ...
    Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 014: TC014
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Fetching emi type from the txn table : {emi_type_db}")
            invoice_number_db = result['pg_invoice_number'].values[0]
            logger.debug(f"Fetching pg invoice number from the txn table : {invoice_number_db}")
            customer_name_db = result['customer_name'].values[0]
            logger.debug(f"Fetching customer name from the txn table : {customer_name_db}")
            ...
            logger.debug(f"Fetching amount original from txn table : {amount_original_db}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching payment by from txn history for the txn : {txn_id}, {payment_by}")
                card_type = txn_history_page.fetch_card_type_text()
                logger.debug(f"Fetching card type from txn history for the txn : {txn_id}, {card_type}")
                emi_status = txn_history_page.fetch_emi_status_text()
                logger.debug(f"Fetching emi status from txn history for the txn : {txn_id}, {emi_status}")
                lender = txn_history_page.fetch_lender_text()
                logger.debug(f"Fetching lender from txn history for the txn : {txn_id}, {lender}")
                monthly_emi = txn_history_page.fetch_monthly_emi_text()
                logger.debug(f"Fetching monthly emi from txn history for the txn : {txn_id}, {monthly_emi}")
                total_emi_amount = txn_history_page.fetch_total_emi_amount_text()
                logger.debug(f"Fetching total emi amount from txn history for the txn : {txn_id}, {total_emi_amount}")
                total_interest = txn_history_page.fetch_total_interest_text()
                logger.debug(f"Fetching total interest from txn history for the txn : {txn_id}, {total_interest}")
                loan_amount = txn_history_page.fetch_loan_amount_text()
                logger.debug(f"Fetching loan amount from txn history for the txn : {txn_id}, {loan_amount}")
                interest_amount = txn_history_page.fetch_interest_amount_text()
                logger.debug(f"Fetching interest amount from txn history for the txn : {txn_id}, {interest_amount}")
                net_effective_price = txn_history_page.fetch_net_effective_price_text()
                logger.debug(
                    f"Fetching net effective price from txn history for the txn : {txn_id}, {net_effective_price}")
                tenure = txn_history_page.fetch_tenure_text()
                logger.debug(f"Fetching tenure from txn history for the txn : {txn_id}, {tenure}")
                customer_app = txn_history_page.fetch_customer_text()
                logger.debug(f"Fetching customer from txn history for the txn : {txn_id}, {customer_app}")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching emi_term from response : {api_emi_term}")
                api_emi_status = response.get('emiStatus')
                logger.debug(f"Fetching emi_status from response : {api_emi_status}")
                api_interest_rate = response.get('emiInterestRate')
                logger.debug(f"Fetching emi_interest_rate from response : {api_interest_rate}")
                api_emi_type = response.get('externalRefNumber7')
                logger.debug(f"Fetching emi_type from response : {api_emi_type}")
                api_loan_amt = response.get('emiDetails')['loanAmount']
                logger.debug(f"Fetching loan_amount from response : {api_loan_amt}")
                api_monthly_emi = response.get('emiDetails')['emi']
                logger.debug(f"Fetching monthly emi from response : {api_monthly_emi}")
                api_interest_amt = response.get('emiDetails')['interestAmount']
                logger.debug(f"Fetching interest_amount from response : {api_interest_amt}")
                api_total_emi_amt = response.get('emiDetails')['totalAmountWithInt']
                logger.debug(f"Fetching total emi amount from response : {api_total_emi_amt}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "BASE AMOUNT:": "Rs." + "{:.2f}".format(1.00),
                    "unnamed_section_text": f"RAJA / I agree to pay as per the card issuer agreement."
                }
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_015():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_9_Months_Tenure
    Sub Feature Description: Performing the instant EMI success transaction for an org (not ezetap) via HDFC Dummy PG using EMV VISA
    Debit card with pin for 9 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 015: TC015
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_016():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_12_Months_Tenure
    Sub Feature Description: Performing the instant EMI success transaction for an org (not ezetap) via HDFC Dummy PG using EMV VISA
    Debit card with pin for 9 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 016: TC016
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_025():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Success_txn_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_Pay_In_Full
    Sub Feature Description: Performing the instant EMI success transaction via HDFC Dummy PG using EMV VISA Credit
    card with pin via pay in full (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 025: TC025
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            payment_page.select_payment_option_pay_in_full()
            payment_page.click_on_proceed_homepage()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_06_026():
    """
    Sub Feature Code: UI_Common_Card_Instant_EMI_Success_Txn_For_Fallback_Root_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenture
    Sub Feature Description: Performing the instant EMI success transaction for fallback to root org via HDFC Dummy PG
    using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 06: Instant_EMI, 026: TC026
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
