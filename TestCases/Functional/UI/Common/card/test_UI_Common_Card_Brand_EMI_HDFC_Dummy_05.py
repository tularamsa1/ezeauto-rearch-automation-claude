...
def test_common_100_115_07_041():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Merchant_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666_For_9_Months_Tenure
        Sub Feature Description: Performing the brand EMI cashback by merchant fixed success transaction for an org via HDFC Dummy PG using EMV VISA Credit card with pin for 9 months tenure (bin: 417666)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 041: TC041
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_043():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Merchant_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_3_Months_Tenure
        Sub Feature Description: Performing the brand EMI cashback by merchant percentage success transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 3 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 043: TC043
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"update subvention_plan_details set status = 1 where subvention_plan_id ='{subvention_plan_id}' and subventing_entity='MERCHANT' and subvention_type='CASHBACK' and subvention_value_type='PERCENTAGE' and tenure='{emi_plan_in_months} month'"
        logger.debug(f"Query to update subvention_plan_details with status as ACTIVE : {query}")
        result = DBProcessor.setValueToDB(query)
        logger.debug(f"Query to fetch result from subvention_plan_details for status as ACTIVE : {result}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                query = f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and subventing_entity='MERCHANT' and subvention_value_type= 'PERCENTAGE' and subvention_type='CASHBACK' and tenure='{emi_plan_in_months} month' ;"
                logger.debug(f"Query to fetch data from the subvention_plan_details table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result for subvention_plan_details table : {result}")
                subvention_entity = result['subventing_entity'].values[0]
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_045():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Merchant_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
        Sub Feature Description: Performing the brand EMI cashback by merchant percentage success transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 045: TC045
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_047():
    """
        Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Merchant_Percentage_Success_Txn_For_An_Org_HDFC_Dummy_EMV_VISA_DebitCard_With_Pin_428090_For_9_Months_Tenure
        Sub Feature Description: Performing the brand EMI cashback by merchant percentage success transaction for an org via HDFC Dummy PG using EMV VISA Debit card with pin for 9 months tenure (bin: 428090)
        TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand EMI, 047: TC047
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
