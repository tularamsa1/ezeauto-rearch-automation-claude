...
def test_common_100_115_063():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMVCTLS_VISA_DebitCard_With_Pin_476173
    Sub Feature Description: Performing the EMVCTLS sale transaction via PRIZM PG using VISA Debit card having
    16 digit PAN length with pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 063: TC063
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_064():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMVCTLS_MASTER_DebitCard_With_Pin_222360
    Sub Feature Description: Performing the EMVCTLS sale transaction via PRIZM PG using MASTER Debit card having
    16 digit PAN length with pin (bin: 222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 064: TC064
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_065():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMVCTLS_RUPAY_DebitCard_With_Pin_608326
    Sub Feature Description: Performing the EMVCTLS sale transaction via PRIZM PG using RUPAY Debit card having
    16 digit PAN length with pin (bin: 608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 065: TC065
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Query to fetch org_code from the DB org_employee table : {query}")
        result = DBProcessor.getValueFromDB(query=query)
        logger.debug(f"Fetching result for query {query} : {result}")
        org_code = result['org_code'].values[0]
        logger.debug(f"Query result, org_code : {org_code}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_066():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMVCTLS_VISA_DebitCard_Without_Pin_476173
    Sub Feature Description: Performing the EMVCTLS sale transaction via PRIZM PG using VISA Debit card having
    16 digit PAN length without pin (bin:476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 066: TC066
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_067():
    """
    Sub Feature Code: UI_Common_Card_Sale_PRIZM_EMVCTLS_MASTER_DebitCard_Without_Pin_222360
    Sub Feature Description: Performing the EMVCTLS sale transaction via PRIZM PG using MASTER Debit card having
    16 digit PAN length without pin (bin: 222360)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 067: TC067
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
