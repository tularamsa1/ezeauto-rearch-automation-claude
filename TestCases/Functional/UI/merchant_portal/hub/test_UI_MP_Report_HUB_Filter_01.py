...
def test_mp_700_707_023():
    """
    Sub Feature Code: UI_MP_Report_based_on_username_as_HUB_head
    Sub Feature Description: Verifying txn details on Report Page using Filter username when logging in as HUB Head
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_707_024():
    """
    Sub Feature Code: UI_MP_Report_based_on_order_number_as_HUB_head
    Sub Feature Description: Verifying txn details on Report Page using Filter order number when logging in as HUB Head
    TC naming code description:
    700: Merchant Portal
    707: HUB
    024: TC024
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_707_025():
    """
    Sub Feature Code: UI_MP_Report_based_on_calender_dates_as_HUB_head
    Sub Feature Description: Verifying txn details on Report Page using Filter calender dates when logging in as HUB Head
    TC naming code description:
    700: Merchant Portal
    707: HUB
    025: TC025
    ...
    try:
        ...
        try:
            ...
            query = "SELECT * from txn where (id BETWEEN '" + start_date_time_utc + "' AND '" + end_date_time_utc + "') and org_code='" + org_code + "' and username NOT IN ('" + country_username + "', '" + region_username + "', '" + state_username + "', '" + district_username + "', '" + taluk_username + "', '" + city_username + "') order by created_time asc limit 1;"
            logger.debug(f"Query to fetch last transaction from database is: {query}")
            result = DBProcessor.getValueFromDB(query)
            db_txn_id_last = result['id'].values[0]
            logger.debug(f"txn_id from db : {db_txn_id_last}")
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_707_026():
    """
    Sub Feature Code: UI_MP_Report_based_on_Location_as_HUB_head
    Sub Feature Description: Verifying txn details on Report Page using Filter All Location when logging in as HUB Head
    TC naming code description:
    700: Merchant Portal
    707: HUB
    026: TC026
...
@pytest.mark.dbVal
@pytest.mark.portalVal
def test_mp_700_707_027():
    """
    Sub Feature Code: UI_MP_Report_based_on_User_as_HUB_head
    Sub Feature Description: Verifying txn details on Report Page using Filter User when logging in as HUB Head
    TC naming code description:
    700: Merchant Portal
    707: HUB
    027: TC027
