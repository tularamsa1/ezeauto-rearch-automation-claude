...
def test_common_100_102_066():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_UPG_AUTOREFUND_ENABLED_YES_ATOS
    Sub Feature Description:  Verification of a BQRV4 UPI UPG txn when Auto refund is enabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 066: TC066
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {"pmt_mode": "UPI", "pmt_status": "UPG_REFUND_PENDING","txn_amt": "{:.2f}".format(amount),
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {"txn_amt": float(amount),"pmt_mode": "UPI",
                                      ...
                                      "pmt_state": "UPG_REFUND_PENDING",
                                      "acquirer_code" : "YES", "bank_name" : "Yes Bank",
                                      "mid" :mid, "tid" : tid,
                                      "pmt_gateway": "ATOS",
                                      "rrn" : str(rrn), "settle_status": "SETTLED",
                                      "upi_pmt_status": "UPG_REFUND_PENDING",
                                      "upi_txn_type": "UNKNOWN",
                                      "upi_mc_id": upi_mc_id, "upi_bank_code": "YES",
                                      "ipr_pmt_mode": "UPI",
                                      "ipr_bank_code": "YES",
...
def test_common_100_102_067():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_UPG_AUTOREFUND_DISABLED_YES_ATOS
    Sub Feature Description: Verification of a BQRV4 UPI UPG txn when Auto refund is disabled via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 067: TC067
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            # pgMerchantId = result["pgMerchantId"].iloc[0]
            customer_vpa = ''.join(random.choices(string.ascii_lowercase + string.digits, k=7))+"@upi"
            logger.debug(f"Merchant vpa and Customer vpa for this merchant is:{vpa}, {customer_vpa}")
            logger.debug(f"Fetching Txn_id,Auth code,RRN, merchant pan : Txn_id: {upg_txn_id}, Auth code : {auth_code}, RRN : {rrn}, {vpa}, merchant pan")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                date_and_time = date_time_converter.to_app_format(created_time)
                expected_app_values = {"pmt_mode": "UPI", "pmt_status": "UPG_AUTHORIZED","txn_amt": "{:.2f}".format(amount),
                                       "settle_status": "SETTLED","txn_id": txn_id, "rrn": str(rrn),
                                       #"customer_name": customer_name,"payer_name": payer_name,
                                       # "order_id": external_ref,
...
def test_common_100_102_068():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_UPI_UPG_Refund_Via_API_YES_ATOS
    Sub Feature Description: Verification of  a BQRV4 UPG Refund txn when Auto refund is disabled through API via YES_ATOS
    TC naming code description: 100: Payment Method, 102: BQR, 068: TC068
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.info(f"fetched refund_username {refund_username}")
