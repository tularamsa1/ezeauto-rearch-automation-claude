...
def test_common_100_111_001():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Success_Via_Pure_RP_UPI_Success_Callback_MultiAcc_HDFC
    Sub Feature Description: Multi Account - Performing a successful pure RP_UPI callback for HDFC
    Sub Feature Code: UI_Common_PM_UPI_success_callback_before_expiry_AutoRefund_Enabled_HDFC_MultiAcc //Discuss with team
    Sub Feature Description: Multi Account - Performing a  UPI success callback via HDFC before expiry when auto refund is enabled
    ...
    try:
        ...
        try:
            ...
            query = "select * from upi_merchant_config where bank_code = 'HDFC' AND status = 'ACTIVE' AND org_code = " \
                    "'" + str(org_code) + "' AND acc_label_id=(select id from label " \
            ...
            api_details = DBProcessor.get_api_details('upi_success_curl', curl_data={
                'ref_id': txn_ref, 'Txn_id': payment_intent_id, 'amount': str(amount) + ".00", 'vpa': vpa, 'rrn': rrn})
            ...
            logger.info(f"Query result,mid : {mid}")
            logger.info(f"Query result,tid : {tid}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_111_002():
    """
    Sub Feature Code: UI_Common_PM_RP_UPI_Failed_Via_UPI_Callback_MultiAcc_HDFC
    Sub Feature Description: Multi Account - Verification of a Remote Pay failed  upi txn via HDFC using callback
    TC naming code description:
    100: Payment Method
    111: MultiAcc_RemotePay
    002: TC_002
