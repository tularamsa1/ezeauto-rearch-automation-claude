...
def test_common_100_103_170():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"txn_created_time is :, {txn_created_time}")
            txn_auth_code = result['auth_code'].values[0]
            logger.debug(f"txn_auth_code is :, {txn_auth_code}")
            txn_status = result['status'].values[0]
            logger.debug(f"txn_status is :, {txn_status}")
...
def test_common_100_103_171():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Query result of txn_rrn is : {txn_rrn}")
            txn_created_time = result['created_time'].values[0]
            logger.debug(f"Query result of created_time is : {txn_created_time}")
            txn_auth_code = result['auth_code'].values[0]
            logger.debug(f"Query result of txn_auth_code is : {txn_auth_code}")
