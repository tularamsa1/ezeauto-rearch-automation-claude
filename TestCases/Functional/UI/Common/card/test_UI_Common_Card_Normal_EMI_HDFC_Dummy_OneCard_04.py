...
@pytest.mark.appVal
def test_common_100_115_05_057():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Txn_Available_Debit_Card_EMI_Setting_For_An_Org_HDFC_Dummy_OneCard_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the normal EMI transaction when the EMI setting is available for Debit card
    for an org (not ezetap) via HDFC Dummy PG for OneCard issure using EMV VISA Credit card with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 057: TC057
    """
    try:
        ...
        logger.debug(f"Response received when emi, emi for client is enabled in preconditions settings : {response}")
...
def test_common_100_115_05_067():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_BO_Instant_Discount_Txn_For_An_Org_HDFC_Dummy_Onecard_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    ...
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 067: TC067
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
