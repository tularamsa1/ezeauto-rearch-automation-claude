...
def test_common_100_115_02_011():
    ...
    TC naming code description: 100: Payment Method, 115: CARD, 02: Sale+Cash@Posh, 007: TC007
    """
    try:
        ...
        try:
            ...
            logger.debug(f"Selected card type CTLS_VISA_DEBIT_476173")
            card_page.click_on_ok_error_msg()
            logger.debug(f"Clicked on ok button in error message")
            home_page.click_on_back_btn_cash_at_pos_page()
            logger.debug(f"Clicked on back button")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                expected_portal_values = {
                    ...
                    "rrn": "-" if auth_code is None else rrn,
...
def test_common_100_115_02_012():
    ...
    TC naming code description: 100: Payment Method, 115: CARD, 02: Sale+Cash@Pos, 008: TC008
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            logger.debug(f"Selected card type CTLS_VISA_CREDIT_417666")
            card_page.click_on_ok_error_msg()
            logger.debug(f"Clicked on ok button in error message")
            home_page.click_on_back_btn_cash_at_pos_page()
            logger.debug(f"Clicked on back button")
