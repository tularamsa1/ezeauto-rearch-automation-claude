...
def test_common_100_102_370():
    """
    Sub Feature Code:   UI_Common_BQRV4_UPI_SA_Success_Checkstatus_HDFC_MINTOAK
    Sub Feature Description: Verification of a BQRV4 successful SA Check status HDFC_MINTOAK
    TC naming code description: 100: Payment Method, 102: BQRV4, 370: TC370
    """
    try:
        ...
        logger.debug(f"Value of bqr_merchant_pan from bharatqr_merchant_config table: {bqr_m_pan}")

        query = f"select * from upi_merchant_config where org_code ='{str(org_code)}' AND status = 'ACTIVE' " \
                f"AND bank_code = 'HDFC_MINTOAK'"
        logger.debug(f"Query to fetch upi_mc_id from the upi_merchant_config for the {org_code} : {query}")
        result = DBProcessor.getValueFromDB(query)
        upi_mc_id = result['id'].values[0]
        logger.debug(f"Fetching upi_mc_id from database for current merchant: {upi_mc_id}")
        pg_merchant_id = result['pgMerchantId'].values[0]
        logger.debug(f"Fetching pg_merchant_id from database for current merchant: {pg_merchant_id}")
        vpa = result['vpa'].values[0]
        logger.debug(f"Fetching vpa from database: {vpa}")
        ...
        try:
            ...
            logger.info(f"Fetching order_id from txn table: {order_id_db}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                logger.debug(f"Fetching bqr_merchant_config_id from bharatqr_txn table : {bqr_merchant_config_id_db}")
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                ...
                logger.info(f"Fetching date time from portal {date_time}")
                transaction_id = transaction_details[0]['Transaction ID']
                logger.info(f"Fetching txn_id from portal {transaction_id}")
                total_amount = transaction_details[0]['Total Amount'].split()
                logger.debug(f"Fetching total amount from portal {total_amount}")
                rr_number = transaction_details[0]['RR Number']
                logger.debug(f"Fetching rr_number from portal {rr_number}")
                transaction_type = transaction_details[0]['Type']
                logger.info(f"Fetching txn_type from portal {transaction_type}")
                status = transaction_details[0]['Status']
                logger.info(f"Fetching status {status}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_102_375():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_HDFC_MINTOAK_CheckStatus_01
    Sub Feature Description: Verification of a BQRV4 successful Check status HDFC_MINTOAK
    TC naming code description: 100: Payment Method, 102: BQRV4, 375: TC375
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from upi_merchant_config where org_code ='{str(org_code)}' AND status = 'ACTIVE' AND " \
                f"bank_code = 'HDFC_MINTOAK'"
        logger.debug(f"Query to fetch upi_mc_id from the upi_merchant_config table for the {org_code} : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"result : {result}, for the query : {query}")
        upi_mc_id = result['id'].values[0]
        logger.debug(f"Fetching upi_mc_id from upi_merchant_config table for current merchant: {upi_mc_id}")
        pg_merchant_id = result['pgMerchantId'].values[0]
        logger.debug(f"Fetching pg_merchant_id from upi_merchant_config table for current merchant: {pg_merchant_id}")
        vpa = result['vpa'].values[0]
        logger.debug(f"Fetching vpa from upi_merchant_config table for current merchant: {vpa}")
        ...
        try:
            ...
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received from bqr generate api: {response}")
            ...
            logger.info(f"Fetching mid from txn table: {mid_db}")
            order_id_db = result['external_ref'].values[0]
            logger.debug(f"Fetching order_id from txn table: {order_id_db}")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_102_411():
    """
    Sub Feature Code: UI_Common_BQRV4_UPI_Checkstatus_Amount_Missmatch_HDFC_MINTOAK
    Sub Feature Description: Verification of a BQRV4 UPI Amount mismatch upon Checkstatus via HDFC_MINTOAK
    TC naming code description: 100: Payment Method, 102: BQRV4, 411: TC411
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        mid = result["mid"].values[0]
        logger.debug(f"Mid from bharatqr_merchant_config: {mid}")
        tid = result["tid"].values[0]
        logger.debug(f"Tid from bharatqr_merchant_config: {tid}")
        terminal_info_id = result["terminal_info_id"].values[0]
        logger.debug(f"Terminal info ID from bharatqr_merchant_config: {terminal_info_id}")
        bqr_mc_id = result["id"].values[0]
        logger.debug(f"BQR MC ID from bharatqr_merchant_config: {bqr_mc_id}")
        bqr_m_pan = result["merchant_pan"].values[0]
        logger.debug(f"BQR Merchant PAN from bharatqr_merchant_config: {bqr_m_pan}")
        ...
        try:
            ...
            logger.debug(f"Fetching Payment Status: {payment_status_db}")
            payment_state_db = result["state"].values[0]
            logger.debug(f"Fetching Payment State: {payment_state_db}")
            acquirer_code_db = result["acquirer_code"].values[0]
            ...
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching Auth Code: {auth_code}")
            created_time = result['created_time'].values[0]
            logger.debug(f"Fetching Created Time: {created_time}")
            rrn = result['rr_number'].values[0]
            logger.debug(f"Fetching rrn : {rrn}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                logger.debug("Fetching details from bharatqr_txn table for txn ID:")
                bqr_state_db = result["state"].values[0]
