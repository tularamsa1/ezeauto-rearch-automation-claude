...
def test_common_100_101_184():
    """
    Sub Feature Code: TID_Dep_UI_Common_PM_Pure_UPI_full_Refund_via_API_AXIS_DIRECT
    Sub Feature Description: TID Dep-Verification of  a upi refund via Axis acq and Axis PG where upi config is Axis_Direct
    TC naming code description: 100: Payment Method, 101: UPI, 184: TC184
    """
    try:
        ...
        try:
            ...
            logger.debug(
                f"replacing the Txn_id with {txn_id_original}, amount with {amount}.00, vpa with {vpa} and rrn with {rrn} in the curl_data")
            api_details = DBProcessor.get_api_details('axis_direct_upi_success_curl', curl_data={
                'merchantTransactionId': txn_id_original,
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                response = [x for x in resp["txns"] if x["txnId"] == txn_id_original][0]
                ...
                response_refund = [x for x in resp["txns"] if x["txnId"] == refund_txn_id][0]
                logger.debug(f"Response after filtering data of current txn is : {response_refund}")
                status_api_refunded = response_refund["status"]
                amount_api_refunded = int(response_refund["amount"])
                payment_mode_api_refunded = response_refund["paymentMode"]
                state_api_refunded = response_refund["states"][0]
                settlement_status_api_refunded = response_refund["settlementStatus"]
                acquirer_code_api_refunded = response_refund["acquirerCode"]
                org_code_api_refunded = response_refund["orgCode"]
                mid_api_refunded = response_refund["mid"]
                tid_api_refunded = response_refund["tid"]
                txn_type_api_refunded = response_refund["txnType"]
                date_api_refunded = response_refund["createdTime"]
                order_id_api_refunded = response_refund["orderNumber"]
                customer_name_api_refunded = response_refund["customerName"]
                payer_name_api_refunded = response_refund["payerName"]
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_185():
    """
    Sub Feature Code: TID_Dep_UI_Common_PM_Pure_UPI_partial_Refund_via_API_AXIS_DIRECT
    Sub Feature Description: TID Dep - Verification of  a upi partial refund via Axis acq and Axis PG where upi config is Axis_Direct
    TC naming code description: 100: Payment Method, 101: UPI, 185: TC185
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                response_1 = [x for x in resp["txns"] if x["txnId"] == refund_txn_id][0]
                logger.debug(f"Response after filtering data of current txn is : {response_1}")
                status_api_refunded = response_1["status"]
                amount_api_refunded = int(response_1["amount"])
                payment_mode_api_refunded = response_1["paymentMode"]
                state_api_refunded = response_1["states"][0]
                settlement_status_api_refunded = response_1["settlementStatus"]
                acquirer_code_api_refunded = response_1["acquirerCode"]
                org_code_api_refunded = response_1["orgCode"]
                mid_api_refunded = response_1["mid"]
                tid_api_refunded = response_1["tid"]
                txn_type_api_refunded = response_1["txnType"]
                date_api_refunded = response_1["createdTime"]
                order_id_api_refunded = response_1["orderNumber"]
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values_2 = {
                    'PAID BY:': 'UPI', 'merchant_ref_no': 'Ref # ' + str(order_id),
                    'RRN': str(rrn_original), 'BASE AMOUNT:': "Rs." + str(amount) + ".00",
                    'date': txn_date_2, 'time': txn_time_2,
                }

                chargeslip_val_result_1 = receipt_validator.perform_charge_slip_validations(
                    txn_id_original, {"username": app_username, "password": app_password},
                    expected_charge_slip_values_2)
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_186():
    """
    Sub Feature Code: TID_Dep_UI_Common_PM_UPI_two_times_partial_refund_via_api_amount_greater_than_original_amount_AXIS_DIRECT
    Sub Feature Description: TID Dep - Verification of two UPI partial refund via api when partial refund amount is greater than original amount for AXIS_DIRECT
    TC naming code description: 100: Payment Method, 101: UPI, 186: TC186
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_187():
    """
    Sub Feature Code: TID_Dep_UI_Common_PM_UPI_two_times_successful_partial_refund_via_api_AXIS_DIRECT
    Sub Feature Description: TID Dep - Verification of two times partial refund success for upi txn via api for Tid Dep AXIS_DIRECT
    TC naming code description: 100: Payment Method, 101: UPI, 187: TC187
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            ...
            try:
                date_and_time_portal = date_time_converter.to_portal_format(created_date_time)
                partial_refund_date_and_time_portal_1 = date_time_converter.to_portal_format(partial_refund_created_date_time_1)
                partial_refund_date_and_time_portal_2 = date_time_converter.to_portal_format(partial_refund_created_date_time_2)
                expected_portal_values = {
                    "date_time": date_and_time_portal,
                    "pmt_state": "AUTHORIZED_REFUNDED",
                    "pmt_type": "UPI",
                    ...
                    "txn_id": txn_id,
                    "rrn": rrn,
                    "date_time_2": partial_refund_date_and_time_portal_1,
                    "pmt_state_2": "REFUNDED",
                    "pmt_type_2": "UPI",
                    "txn_amt_2": f"{str(partial_refunded_amount)}.00",
                    "username_2": app_username,
                    "txn_id_2": partial_refund_txn_id_1,
                    "date_time_3": partial_refund_date_and_time_portal_2,
                    "pmt_state_3": "REFUNDED",
                    "pmt_type_3": "UPI",
                ...
                actual_portal_values = {
                    ...
                    "txn_amt_3": str(total_amount_3[1]),
...
@pytest.mark.portalVal
@pytest.mark.chargeSlipVal
def test_common_100_101_188():
    """
    Sub Feature Code: TID_Dep_UI_Common_PM_Pure_UPI_PartialRefund_In_Decimal_via_api_AXIS_DIRECT
    Sub Feature Description: TID Dep - Verification of  a pure UPI txn when Partial refund with decimal is performed via api for AXIS_DIRECT
    TC naming code description: 100: Payment Method, 101: UPI, 188: TC188
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
