...
def test_common_100_101_166():
    """
    Sub Feature Code: UI_Common_PM_UPI_Success_Via_API_CheckStatus_KOTAK_ATOS
    Sub Feature Description: Verification of a successful upi txn via UPI via KOTAK_ATOS using check status api
    TC naming code description: 100: Payment method, 101: UPI, 166: Testcase ID
    """
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                logger.info("Restarting MPOS-X app to perform check status of the transaction")
                login_page = LoginPage(app_driver)
                login_page.perform_login(app_username, app_password)
                home_page.wait_for_navigation_to_load()
                home_page.wait_for_home_page_load()
...
@pytest.mark.appVal
@pytest.mark.portalVal
def test_common_100_101_167():
    """
    Sub Feature Code: UI_Common_PM_UPI_Failed_Via_API_CheckStatus_KOTAK_ATOS
    Sub Feature Description: Verification of a failed upi txn via UPI via KOTAK_ATOS using check status api
    TC naming code description: 100: Payment method, 101: UPI, 167: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.portalVal
def test_common_100_101_168():
    """
    Sub Feature Code: UI_Common_PM_UPI_Pending_Via_API_CheckStatus_KOTAK_ATOS
    Sub Feature Description: Verification of a pending upi txn via UPI  via KOTAK_ATOS using check status api
    TC naming code description: 100: Payment method, 101: UPI, 168: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
