...
def test_common_100_115_07_109():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_payback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_HDFC_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Instant Discount payback by brand fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG for HDFC issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 109: TC109
    """
    try:
        ...
        try:
            ...
            cal_monthly_emi_amt = (final_amount - subvention_value) * monthly_interest_rate * (
            ...
            total_interest = round((total_emi - (final_amount - subvention_value)), 2)
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "loan_amt": "{:,.2f}".format(final_amount - subvention_value),
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "loan_amt": float(final_amount - subvention_value),
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Loan Amount": "Rs." + "{:.2f}".format(final_amount - subvention_value),
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_110():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Cashback_Discount_Payback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_HDFC_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI BO Cashback Discount payhback by brand fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG for HDFC issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 110: TC110
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='HDFC';"
        logger.debug(f"Query to fetch data from the subvention_plan table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for subvention_plan table : {result}")
        subvention_plan_id = result['id'].values[0]
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    ...
                    "total_emi_amt": "{:,.2f}".format(total_emi),
                    "additional_payback": "{:,.2f}".format(emi_payback_fixed),
                    # "cashback": "{:,.2f}".format(total_discount_value),
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "EMI Amt": "Rs." + "{:.2f}".format(monthly_emi),
                    "Total Amt with Interest": "Rs." + "{:.2f}".format(total_emi),
                    "Additional Cashback": "Rs. " + "{:.2f}".format(total_discount_value),
                    "Total Cashback Amt": "Rs. " + "{:.2f}".format(total_discount_value),
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_121():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Settings_is_Disabled_In_Org_Settings
    Sub Feature Description: Verfiy the brand emi option is disabled in org settings and check for brand emi option in app
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 121: TC121
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"API details  : {api_details} ")
        response = APIProcessor.send_request(api_details=api_details)
        logger.debug(f"Response received when emi, emi for client, offering_emi cashback, businessDiscountEnabled and bocashback is enabled and brand_emi is disabled in preconditions settings : {response}")
        ...
        try:
            ...
            try:
                payment_page.check_for_brand_emi_pmt_mode()
