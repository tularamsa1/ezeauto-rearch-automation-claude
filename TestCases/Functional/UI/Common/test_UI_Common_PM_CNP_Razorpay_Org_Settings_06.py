...
def test_common_100_103_452():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Org_Settings_Remote_Pay_Txn_Before_Start_Of_Time_Based_Restriction
    Sub Feature Description: To verify the Remote Pay transaction before the start time of "Time Based Restriction".
    TC naming code description: 100: Payment Method, 103: RemotePay, 452: TC_452
    """
    ...
    try:
        ...
        time_based_restriction_start_time = (datetime.now(indian_timezone) + timedelta(minutes=3)).strftime("%H%M")
        time_based_restriction_end_time = (datetime.now(indian_timezone) + timedelta(minutes=5)).strftime("%H%M")
        logger.debug(f"time_based_restriction_start_time (IST) : {time_based_restriction_start_time}")
        logger.debug(f"time_based_restriction_end_time (IST) : {time_based_restriction_end_time}")
        ...
        api_details["RequestBody"]["settings"]["timeBasedTxnRestrictionEnabled"] = "true"
        api_details["RequestBody"]["settings"]["timeBasedTxnRestrictionPaymentMode"] = "CNP"
        api_details["RequestBody"]["settings"]["txnRestrictionStartTime"] = time_based_restriction_start_time
        api_details["RequestBody"]["settings"]["txnRestrictionEndTime"] = time_based_restriction_end_time
        ...
        try:
            ...
            api_details["RequestBody"]["settings"]["timeBasedTxnRestrictionEnabled"] = "false"
            api_details["RequestBody"]["settings"]["timeBasedTxnRestrictionPaymentMode"] = "NA"
            api_details["RequestBody"]["settings"]["txnRestrictionStartTime"] = ""
            api_details["RequestBody"]["settings"]["txnRestrictionEndTime"] = ""
            logger.debug(f"API details  : {api_details} ")
            response = APIProcessor.send_request(api_details=api_details)
            logger.debug(f"Response received for time based txn restriction to be disabled:  {response}")
        except Exception as e:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_453():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Org_Settings_Remote_Pay_Txn_After_End_Of_Time_Based_Restriction
    Sub Feature Description: To verify the Remote Pay transaction after the end time of "Time Based Restriction".
    TC naming code description: 100: Payment Method, 103: RemotePay, 453: TC_453
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        time_based_restriction_start_time = current_time_in_india.strftime("%H%M")
        time_based_restriction_end_time = (current_time_in_india + timedelta(minutes=1)).strftime("%H%M")
        logger.debug(f"time_based_restriction_start_time (IST) : {time_based_restriction_start_time}")
        logger.debug(f"time_based_restriction_end_time (IST) : {time_based_restriction_end_time}")
        ...
        try:
            ...
            logger.debug(f"Waiting till time based restriction expired...")
            logger.debug(f"Waiting time is 120 sec")
            time.sleep(2 * 60)
            ...
            logger.debug(f"Fetching created time from the txn table : {created_time}")
            auth_code = result['auth_code'].values[0]
            logger.debug(f"Fetching auth_code from the txn table: {auth_code}")
            status_db = result["status"].iloc[0]
            logger.debug(f"Fetching status from the txn table: {status_db}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_454():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Org_Settings_Time_Based_Restriction_Enabled_RemotePay_PaymentMode_Not_Selected
    Sub Feature Description: To verify the Remote Pay transaction when "Time Based Restriction" is enabled and
    Remote Pay Payment Mode in the list is not selected.
    TC naming code description: 100: Payment Method, 103: RemotePay, 454: TC_454
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        api_details["RequestBody"]["settings"]["remotePaymentEnabled"] = "true"
        api_details["RequestBody"]["settings"]["timeBasedTxnRestrictionEnabled"] = "true"
        api_details["RequestBody"]["settings"]["timeBasedTxnRestrictionPaymentMode"] = "CASH"
        api_details["RequestBody"]["settings"]["txnRestrictionStartTime"] = time_based_restriction_start_time
        api_details["RequestBody"]["settings"]["txnRestrictionEndTime"] = time_based_restriction_end_time
        logger.debug(f"API details : {api_details} ")
        response = APIProcessor.send_request(api_details=api_details)
        logger.debug(f"Response received for setting preconditions for Time Based Txn Restriction to be enabled: {response}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_455():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Org_Settings_Time_Based_Restriction_Disabled
    Sub Feature Description: To verify the Remote Pay transaction when "Time Based Restriction" is disabled.
    TC naming code description: 100: Payment Method, 103: RemotePay, 455: TC_455
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_103_456():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_Org_Settings_Disable_NetBanking_Card_&Enable_RemotePay
    Sub Feature Description:Verify able to see Remote option in Mpos when remote pay enable, disable net banking or card payment option, from Org setting
    TC naming code description: 100: Payment Method, 103: RemotePay, 456: TC_456
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = f"update merchant_pg_config set cnp_cardpay_enabled = 'INACTIVE' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
            logger.debug(f"Query to update cnp_cardpay_enabled to inactive : {query}")
            result = DBProcessor.setValueToDB(query=query)
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                pay_link_btn_found = True
                try:
                    payment_page.validate_payment_link_btn()
                except:
                    pay_link_btn_found = False
                assert pay_link_btn_found == True, "Pay Link Button Not Found in MPOS"
                logger.info("Payment link Button present")
        ...
        try:
            query = f"update merchant_pg_config set cnp_cardpay_enabled = 'ACTIVE' where org_code='{org_code}' and payment_gateway='RAZORPAY';"
            logger.debug(f"Query to update cnp_cardpay_enabled to active : {query}")
            result = DBProcessor.setValueToDB(query=query)
            logger.debug(f"query result after updating : {result}")
