...
def test_common_100_102_069():
    ...
    :Sub Feature code: UI_Common_BQRV4_two_Callback_Success_YES_ATOS_069
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_102_071():
    """
    :Description: Verification of  a BQRV4 Amount missmatch upon callback when Auto refund is disabled via YES_ATOS
    :Sub Feature code: UI_Common_BQRV4_Amount_Missmatch_Callback_YES_ATOS_71
    :TC naming code description: 100->Payment Method, 102->BQR, 071-> TC71
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Result fetched frp, databse is : vpa={vpa}, upi_mc_id={upi_mc_id}, "
                     f"mid={mid}, tid={tid}")
        ...
        try:
            ...
            logger.info("Payment QR generated and displayed successfully")
            app_driver.reset()
            logger.debug("Restarting MPOS app")
            ...
            auth_code = "AE" + txn_id.split('E')[1]
            rrn = "RE" + txn_id.split('E')[1]
            miss_match_amt = amount -10
            logger.debug(
                f"Fetching Txn_id,Auth code,RRN, primary id and secondary id from data base : Txn_id : {txn_id}, Auth code : {auth_code}, RRN : {rrn}, Primary id : {pid}, Secondary id : {sid}, Mismatch amount: {miss_match_amt}")
            api_details = DBProcessor.get_api_details('callbackUpiYES',
                                                      request_body={"primary_id": pid, "secondary_id": sid,
                                                                    "txn_amount": str(miss_match_amt),"merchant_vpa":vpa,
                                                                    "auth_code": auth_code, "ref_no": rrn})
            customer_vpa = api_details["RequestBody"]["customer_vpa"]
            response = APIProcessor.send_request(api_details)
            logger.debug(f"Fetching API Response for call back : {response}")
            ...
            query = "select * from txn where org_code='" + org_code + "' and id LIKE '" + datetime.utcnow().strftime(
                '%y%m%d') + "%' order by created_time desc limit 1;"
            result = DBProcessor.getValueFromDB(query)
            upg_txn_id = result['id'].values[0]
            external_ref = result['external_ref'].values[0]
            upg_posting_date = result['posting_date'].values[0]
            logger.debug(f"Transaction id, posting date of upg txn is : {upg_txn_id}, {upg_posting_date}")



            query = "select posting_date from txn where id = '" + txn_id + "';"
            result = DBProcessor.getValueFromDB(query)
            posting_date = result['posting_date'].values[0]
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                date_and_time = date_time_converter.to_app_format(posting_date)
                date_and_time_upg = date_time_converter.to_app_format(upg_posting_date)
                ...
                expected_app_values = {"pmt_mode": "BHARAT QR", "pmt_status": "PENDING","txn_amt": str(amount),
                                       ...
                                       "order_id": order_id,"msg": "PAYMENT PENDING",
                                        "date": date_and_time,"pmt_mode_upg": "UPI",
                                        "pmt_status_upg": "UPG_AUTHORIZED","settle_status_upg": "SETTLED",
                                       "txn_id_upg": upg_txn_id,"txn_amt_upg": str(miss_match_amt),"rrn_upg": str(rrn),
                                       "order_id_upg": external_ref,"payment_msg_upg": "PAYMENT SUCCESSFUL",
                                       "date_upg": date_and_time_upg}
                app_driver.reset()
                ...
                app_order_id = txn_history_page.fetch_order_id_text()
                logger.info(f"Fetching txn order_id from txn history for the txn : {txn_id}, {app_order_id}")
                # behavior is diff on both emulator and device (Number/NUMBER)
                txn_history_page.click_back_Btn_transaction_details()
                txn_history_page.click_on_transaction_by_txn_id(upg_txn_id)

                app_payment_status_upg = txn_history_page.fetch_txn_status_text()
                logger.info(f"Fetching status from txn history for the txn : {upg_txn_id}, {app_payment_status_upg }")
                app_date_and_time_upg  = txn_history_page.fetch_date_time_text()
                logger.info(f"Fetching date from txn history for the txn : {upg_txn_id}, {app_date_and_time_upg }")
                app_payment_mode_upg  = txn_history_page.fetch_txn_type_text()
                logger.info(f"Fetching payment mode from txn history for the txn : {upg_txn_id}, {app_payment_mode_upg}")
                app_txn_id_upg  = txn_history_page.fetch_txn_id_text()
                logger.info(f"Fetching txn_id from txn history for the txn : {upg_txn_id}, {app_txn_id_upg}")
                app_amount_upg  = txn_history_page.fetch_txn_amount_text()
                logger.info(f"Fetching txn amount from txn history for the txn : {upg_txn_id}, {app_amount_upg}")
                app_settlement_status_upg = txn_history_page.fetch_settlement_status_text()
                logger.info(
                    f"Fetching txn settlement_status from txn history for the txn : {upg_txn_id}, {app_settlement_status_upg}")
                app_payment_msg_upg = txn_history_page.fetch_txn_payment_msg_text()
                logger.info(f"Fetching txn status msg from txn history for the txn : {upg_txn_id}, {app_payment_msg_upg}")
                app_order_id_upg = txn_history_page.fetch_order_id_text()
                logger.info(f"Fetching txn order_id from txn history for the txn : {upg_txn_id}, {app_order_id_upg}")
                app_rrn_upg = txn_history_page.fetch_RRN_text()
                logger.info(
                    f"Fetching txn_id from txn history for the txn : {upg_txn_id}, {app_rrn_upg}")  # behavior is diff on both emulator and device (Number/NUMBER)

                actual_app_values = {
                    "pmt_mode": app_payment_mode, "pmt_status": app_payment_status.split(':')[1],
                    "txn_amt": app_amount.split(' ')[1], "txn_id": app_txn_id,
                    "settle_status": app_settlement_status,
                    "order_id": app_order_id,
                    "msg": app_payment_msg, "date": app_date_and_time,
                    "pmt_status_upg": app_payment_status_upg.split(':')[1],
                    "pmt_mode_upg": app_payment_mode_upg,
                    "txn_id_upg": app_txn_id_upg,
                    "txn_amt_upg": str(app_amount_upg).split(' ')[1],
                    "rrn_upg": str(app_rrn_upg),
                    "settle_status_upg": app_settlement_status_upg,
                    "order_id_upg": app_order_id_upg,
                    "payment_msg_upg": app_payment_msg_upg,
                    "date_upg": app_date_and_time_upg
                }
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                date = date_time_converter.db_datetime(posting_date)
                date_upg = date_time_converter.db_datetime(upg_posting_date)
                expected_api_values = {
                    "pmt_status": "PENDING", "txn_amt": amount, "pmt_mode": "BHARATQR",
                    "pmt_state": "PENDING", "settle_status": "PENDING",
                    "acquirer_code": "YES", "issuer_code": "YES", "txn_type": "CHARGE",
                    "mid": mid, "tid": tid, "org_code": org_code,"date": date,
                    "pmt_status_upg": "UPG_AUTHORIZED",
                    "txn_amt_upg": miss_match_amt, "pmt_mode_upg": "UPI",
                    "pmt_state_upg": "UPG_AUTHORIZED", "rrn_upg": str(rrn),
                    "settle_status_upg": "SETTLED",
                    "acquirer_code_upg": "YES",
                    "issuer_code_upg": "YES",
                    "txn_type_upg": 'CHARGE', "mid_upg": mid, "tid_upg": tid,
                    "org_code_upg": org_code,
                    "date_upg": date_upg
                }
                logger.debug(f"expected_api_values: {expected_api_values}")
                api_details = DBProcessor.get_api_details('txnDetails',
                ...
                api_details = DBProcessor.get_api_details('txnDetails',
                                                          request_body={"username": app_username,
                                                                        ...
                                                                        "txnId": app_txn_id_upg})
                print("API DETAILS for original_txn_id:", api_details)
                response = APIProcessor.send_request(api_details)
                logger.debug(f"Response received for transaction details api is : {response}")
                logger.debug("API DETAILS:", api_details)
                response = APIProcessor.send_request(api_details)
                status_api_upg = response["status"]
                amount_api_upg = int(response["amount"])  # actual=345.00, expected should be in the same format
                payment_mode_api_upg = response["paymentMode"]
                state_api_upg = response["states"][0]
                rrn_api_upg = response["rrNumber"]
                settlement_status_api_upg = response["settlementStatus"]
                issuer_code_api_upg = response["issuerCode"]
                acquirer_code_api_upg = response["acquirerCode"]
                orgCode_api_upg = response["orgCode"]
                mid_api_upg = response["mid"]
                tid_api_upg = response["tid"]
                txn_type_api_upg = response["txnType"]
                date_api_upg = response["postingDate"]
                ...
                actual_api_values = {
                    ...
                    "pmt_status_upg": status_api_upg, "txn_amt_upg": amount_api_upg,
                    "pmt_mode_upg": payment_mode_api_upg,
                    "pmt_state_upg": state_api_upg, "rrn_upg": str(rrn_api_upg),
                    "settle_status_upg": settlement_status_api_upg,
                    "acquirer_code_upg": acquirer_code_api_upg,
                    "issuer_code_upg": issuer_code_api_upg,
                    "txn_type_upg": txn_type_api_upg, "mid_upg": mid_api_upg, "tid_upg": tid_api_upg,
                    "org_code_upg": orgCode_api_upg,
                    "date_upg": date_time_converter.from_api_to_datetime_format(date_api_upg)
                }
                logger.debug(f"actual_api_values: {actual_api_values}")
                Validator.validationAgainstAPI(expectedAPI=expected_api_values, actualAPI=actual_api_values)
            except Exception as e:
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "pmt_status_upg": "UPG_AUTHORIZED",
                    "pmt_state_upg": "UPG_AUTHORIZED",
                    "pmt_mode_upg": "UPI",
                    "txn_amt_upg": miss_match_amt,
                    "upi_txn_status": "UPG_AUTHORIZED",
                    "settle_status_upg": "SETTLED",
                    "acquirer_code_upg": "YES",
                    "bank_code_upg": "YES",
                    "payment_gateway_upg": "ATOS",
                ...
                query = "select * from txn where id='" + upg_txn_id + "'"
                logger.debug(f"Query to fetch data from txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                status_db_upg = result["status"].iloc[0]
                payment_mode_db_upg = result["payment_mode"].iloc[0]
                amount_db_upg = int(result["amount"].iloc[0])  # actual=345.0000, expected should be in the same format
                state_db_upg = result["state"].iloc[0]
                payment_gateway_db_upg = result["payment_gateway"].iloc[0]
                acquirer_code_db_upg = result["acquirer_code"].iloc[0]
                bank_code_db_upg = result["bank_code"].iloc[0]
                settlement_status_db_upg = result["settlement_status"].iloc[0]
                tid_db_upg = result['tid'].values[0]
                mid_db_upg = result['mid'].values[0]

                query = "select * from upi_txn where txn_id='" + upg_txn_id + "'"
                logger.debug(f"Query to fetch data from upi_txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                upi_status_db = result["status"].iloc[0]
                ...
                query = ("select * from invalid_pg_request where txn_id ='" + upg_txn_id + "';")
                ...
                actual_db_values = {
                    ...
                    "pmt_status_upg": status_db_upg,
                    "pmt_state_upg": state_db_upg,
                    "pmt_mode_upg": payment_mode_db_upg,
                    "txn_amt_upg": amount_db_upg,
                    "upi_txn_status": upi_status_db,
                    "settle_status_upg": settlement_status_db_upg,
                    "acquirer_code_upg": acquirer_code_db_upg,
                    "bank_code_upg": bank_code_db_upg,
                    "payment_gateway_upg": payment_gateway_db_upg,
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_102_072():
    """
    :Description: Verification of  a BQRV4 Amount missmatch upon callback when Auto refund is enabled via YES_ATOS
    :Sub Feature code: UI_Common_BQRV4_Amount_Missmatch_Callback_Upg_AutoRefund_Enabled_YES_ATOS_72
    :TC naming code description: 100->Payment Method, 102->BQR, 072-> TC72
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {"pmt_mode": "BHARAT QR", "pmt_status": "PENDING","txn_amt": str(amount),
                                       ...
                                       "order_id": order_id,"msg": "PAYMENT PENDING",
                                        "date": date_and_time,"pmt_mode_upg": "UPI",
                                        "pmt_status_upg": "UPG_REFUND_PENDING","settle_status_upg": "SETTLED",
                                       "txn_id_upg": upg_txn_id,"txn_amt_upg": str(miss_match_amt),"rrn_upg": str(rrn),
                                       "order_id_upg": external_ref,"payment_msg_upg": "PAYMENT SUCCESSFUL",
                                       "date_upg": date_and_time_upg}
                app_driver.reset()
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "acquirer_code": "YES", "issuer_code": "YES", "txn_type": "CHARGE",
                    "mid": mid, "tid": tid, "org_code": org_code,"date": date,
                    "pmt_status_upg": "UPG_REFUND_PENDING",
                    "txn_amt_upg": miss_match_amt, "pmt_mode_upg": "UPI",
                    "pmt_state_upg": "UPG_REFUND_PENDING", "rrn_upg": str(rrn),
                    "settle_status_upg": "SETTLED",
                    "acquirer_code_upg": "YES",
                    "issuer_code_upg": "YES",
                    "txn_type_upg": 'CHARGE', "mid_upg": mid, "tid_upg": tid,
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "settle_status": "PENDING",
                    "pmt_status_upg": "UPG_REFUND_PENDING",
                    "pmt_state_upg": "UPG_REFUND_PENDING",
                    "pmt_mode_upg": "UPI",
                    "txn_amt_upg": miss_match_amt,
