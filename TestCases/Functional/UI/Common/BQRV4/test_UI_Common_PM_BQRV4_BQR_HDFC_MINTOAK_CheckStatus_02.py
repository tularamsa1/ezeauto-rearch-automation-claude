...
def test_common_100_102_364():
    """
    Sub Feature Code: UI_Common_PM_BQRV4_BQR_SA_Checkstatus_AfterExpiry_HDFC_MINTOAK
    Sub Feature Description: Verification of a BQRV4 BQR Check Status by app After QR Expiry transaction via HDFC_MINTOAK
    TC naming code description: 100: Payment Method, 102: BQR, 364: TC364
    """
    try:
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_102_371():
    """
       Sub Feature Code: UI_Common_PM_BQRV4_BQR_Checkstatus_Amount_Mismatch_HDFC_MINTOAK
       Sub Feature Description: Verification of a BQRV4 BQR Amount mismatch upon Checkstatus via HDFC_MINTOAK
    ...
    try:
        ...
        bqr_m_pan = result["merchant_pan"].values[0]
        logger.debug(f"Fetching bqr_m_pan from bharatqr_merchant_config for current merchant:{bqr_m_pan}")
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('paymentStatus',request_body={"username": app_username, "password": app_password,
                                                                    "txnId": txn_id})
            response = APIProcessor.send_request(api_details)
            ...
            logger.debug(f"Fetched rrn number from txn table:{rrn}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                bqr_txn_type_db = result["txn_type"].values[0]
                logger.debug(f"Fetching bqr_txn_type_db from txn table:{bqr_txn_type_db}")
                brq_terminal_info_id_db = result["terminal_info_id"].values[0]
