...
def test_common_100_115_11_019():
    ...
    try:
        ...
        query = f"select * from emi where org_code='{org_code}' and status = 'ACTIVE' and " \
                f"issuer_code='ICICI' and card_type='DEBIT' and term = '{emi_plan_in_months} month' and emi_type='EZEEMI'" \
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and status=1 and org_code='{org_code}' and card_type= 'DEBIT' and eze_emi_enabled=b'1';"
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "Tenure": f"{emi_plan_in_months} month",
                    "Card Issuer": f"{issuer_code} Bank",
                    "Loan Amount": f"Rs." + "{:.2f}".format(round(amount - cal_subvention_value, 2)),
                    "Rate of Interest(P.A.)": f"{interest_rate}%",
                    "Interest Amt": "Rs." + "{:.2f}".format(total_interest),
                    "EMI Amt": "Rs." + "{:.2f}".format(monthly_emi),
                    "Total Amt with Interest": "Rs." + "{:.2f}".format(total_emi),
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                app_driver.reset()
                logger.info(
                    f"Logging in the MPOSX application after reseting driver using username and password : {app_username}, {app_password}")
                login_page.perform_login(username=app_username, password=app_password)
                home_page.wait_for_home_page_load()
                home_page.click_on_history()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.dbVal
def test_common_100_115_11_022():
    """
    Sub Feature Code: UI_Common_Card_Eze_EMI_Failed_Failed_txn_For_An_Org_HDFC_Dummy_ICICI_EMV_VISA_DebitCard_With_Pin_428090_For_6_Months_Tenure
    Sub Feature Description: Performing the brand EMI failed_failed txn for an org (not ezetap) via HDFC Dummy PG
    for ICICI issuer using EMV VISA Debit card with pin for 6 months tenure (bin: 428090)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: Eze EMI, 022: TC022
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
