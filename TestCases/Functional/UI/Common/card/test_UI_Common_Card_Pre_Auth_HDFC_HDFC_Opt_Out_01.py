...
def test_common_100_116_016():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Pre_Auth_Opt_Out_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt out MC Chip KWD Pre auth transaction via HDFC-HDFC PG using Credit card
    with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 016: TC016
    """
    try:
        ...
        try:
            ...
            logger.debug(f"inr_currency_rate : {inr_currency_rate}")
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    'payment_option': 'PREAUTH SALE', 'TID': tid,
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_017():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Pre_Auth_Opt_Out_MC_ctls_USD_CreditCard_With_Online_Pin_556763
    Sub Feature Description: Performing the opt out MC ctls USD Pre auth transaction via HDFC-HDFC PG using Credit card
    with online pin  (bin:556763)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 017: TC017
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        logger.debug(f"Fetching result for terminal_info table : {result}")
        tid_1 = result["tid"].iloc[0]
        logger.debug(f"Fetching tid_1 from the terminal_info table : tid_1 : {tid_1}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_018():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Pre_Auth_Opt_Out_VISA_ctls_GBP_CreditCard_With_Online_Pin_417666
    Sub Feature Description: Performing the opt out VISA ctls GBP Pre auth transaction via HDFC-HDFC PG using Credit
    card with online pin  (bin:417666)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 018: TC018
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_019():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Pre_Auth_Release_Opt_Out_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt out MC Chip KWD Pre auth release transaction via HDFC HDFC PG using
    Credit card with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 019: TC019
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_116_020():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_HDFC_Pre_Auth_Confirm_Opt_Out_MC_Chip_KWD_CreditCard_With_Online_Pin_222297
    Sub Feature Description: Performing the opt out MC Chip KWD Pre auth confirm transaction via HDFC PLANET PG using
    Credit card with online pin  (bin:222297)
    TC naming code description: 100: Payment Method, 116: CARD_UI, 020: TC020
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
