...
def test_common_100_115_006():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_CTLS_WITH_PIN_RUPAY_DEBIT_608326
    Sub Feature Description:  Performing the EMV sale transaction via HDFC PG using CTLS With Pin RUPAY Debit card
    having 16 digit PAN length with pin (bin : 608326)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 006: TC006
    """
    try:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_007():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_CTLS_VISA_CREDIT_417666
    Sub Feature Description:  Performing the EMV sale transaction via HDFC PG using CTLS VISA Credit card
    having 16 digit PAN length with pin (bin: 417666)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 007: TC007
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                app_rrn = txn_history_page.fetch_RRN_text()
                app_auth_code = txn_history_page.fetch_auth_code_text()
                txn_history_page.scroll_to_text("BATCHNO.")
                app_batch_no = txn_history_page.fetch_batch_number_text()
                # app_customer_name = txn_history_page.fetch_customer_name_text()
                app_mid = txn_history_page.fetch_mid_text()
                app_tid = txn_history_page.fetch_tid_text()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_008():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_CTLS_MASTER_CREDIT_541333
    Sub Feature Description:  Performing the EMV sale transaction via HDFC PG using CTLS MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 008: TC008
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_009():
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_CTLS_WITH_PIN_MASTER_CREDIT_541333
    Sub Feature Description:  Performing the EMV sale transaction via HDFC PG using CTLS with pin MASTER Credit card
    having 16 digit PAN length with pin (bin: 541333)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 009: TC009
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
