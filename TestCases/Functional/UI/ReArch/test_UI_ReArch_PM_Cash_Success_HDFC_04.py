...
from PageFactory.ReArch.rearch_customer_auth_page import ReArchCustomerAuthPage
...
def test_common_rearch_0029():
    """
    Sub Feature Code: UI_ReArch_PM_Cash_Success_HDFC
    Sub Feature Description:
        Launch ReArch app with customer auth enabled, enter amount 95,
        tap overlay button, proceed through Order Details, scroll down
        and select Cash, enter PAN (KOOPS5450E) on the customer auth
        screen, confirm payment, click Accept More Payments, then
        perform app, API and charge slip validation.

    NL Source Steps:
      Preconditions:
        1. update org_settings:
           preAuthOption = 1
           autoLoginByTokenEnabled = true
           cardPaymentEnabled = true
           enableWebAppRevamp = true
           p2pEnabled = true
           mqttEnabled = true
           cashPaymentEnabled = true
           addlAuthReqdForCash = false
           orderNumberInputEnabled = false
           customerAuthDataCaptureEnabled = true
           amountCutOffForCustomerAuth = 10000
           eSignatureForNonCardEnabled = false

      Test Steps:
        1.  launch rearch app and login if present
        2.  apply preconditions settings (refresh via Payment History)
        3.  wait for home page
        4.  enter amount as 95
        5.  click on overlay button
        6.  wait for order details screen
        7.  click proceed
        8.  scroll down and click on Cash
        9.  tap on PAN text field and enter KOOPS5450E
        10. click on confirm payment
        11. click on accept more payment
        12. do app, api and chargeslip validation

      App Validation:
        navigate to history from collect payment screen → click transaction
        by txn_id → verify amount matches and status is "Payment Settled"

      API Validation:
        pmt_status=AUTHORIZED, txn_amt, pmt_mode=CASH, txn_type=CHARGE,
        acquirer_code=NONE, org_code, date
    ...
    try:
        ...
        settings["orderNumberInputEnabled"] = "false"
        ...
        try:
            ...
            # Step 2-3: Wait for initial home screen, tap Collect Payment, then
            # refresh settings via Payment History navigation
            ...
            # Step 4: Wait for amount screen and enter amount 95
            home_page.wait_for_home_page_load()
            ...
            # Step 6-7: Order Details overlay → click Proceed
            ...
            # Step 8: Scroll down and click Cash from payment methods
            ...
            # Step 9: Customer Auth screen → enter PAN
            customer_auth_page = ReArchCustomerAuthPage(app_driver)
            customer_auth_page.wait_for_customer_auth_screen()
            customer_auth_page.enter_pan_number("KOOPS5450E")
            logger.debug("PAN number KOOPS5450E entered")

            # Step 10: Confirm Payment on Customer Auth screen
            customer_auth_page.click_confirm_payment()
            logger.debug("Confirm Payment clicked on Customer Auth screen")
            ...
            # Step 11: Click Accept More Payments → back to collect payment screen
