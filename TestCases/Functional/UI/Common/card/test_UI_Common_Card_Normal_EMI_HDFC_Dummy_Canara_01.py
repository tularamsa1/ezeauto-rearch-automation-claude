...
def test_common_100_115_05_058():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Success_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the normal EMI success transaction for an org (not ezetap) via HDFC Dummy PG for
    Canara issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 058: TC058
    """
    try:
        ...
        testsuite_teardown.update_bin_info(bin_number='417666', bank_code='CANARA', bank='CANARA')
        ...
        try:
            ...
            query = f"select * from emi where org_code='{org_code}' and status = 'ACTIVE' and " \
                    f"issuer_code='CANARA' and card_type='CREDIT' and term = '{emi_plan_in_months} month' and emi_type='NORMAL'" \
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "unnamed_section_text": f"{customer_name} I agree to pay as per the card issuer agreement."
                }
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_059():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Invalid_Amount_Lesser_Than_EMI_Range_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the normal EMI invalid amount(lesser than EMI range) transaction for an org
    (not ezetap) via HDFC Dummy PG for Canara issuer using EMV VISA Credit card with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 059: TC059
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_060():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Invalid_Amount_Greater_Than_EMI_Range_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the normal EMI invalid amount(greater than EMI range) transaction for an org
    (not ezetap) via HDFC Dummy PG for Canara issuer using EMV VISA Credit card with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 060: TC060
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_05_061():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Txn_Inactive_EMI_Setting_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the normal EMI transaction when the EMI setting is made as inactive for an org
    (not ezetap) via HDFC Dummy PG for Canara issuer using EMV VISA Credit card with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 061: TC061
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_05_079():
    """
    Sub Feature Code: UI_Common_Card_Normal_EMI_Cashback_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_12_Months_Tenure
    Sub Feature Description: Performing the normal EMI cashback fixed success transaction for an org (not ezetap) via
    HDFC Dummy PG for Canara issuer using EMV VISA Credit card with pin for 12 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 05: NORMAL_EMI, 079: TC079
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            query = f"select * from subvention_plan where brand_id='ALL' and org_code='{org_code}' and card_type= 'CREDIT' " \
                    f"and bank='CANARA';"
            ...
            query = f"select * from subvention_plan_details where subvention_plan_id='{subvention_plan_id}' and " \
                    f"subventing_entity ='MERCHANT' and subvention_value_type= 'FIXED' and subvention_type='CASHBACK' and " \
                    f"tenure='{emi_plan_in_months} month' ;"
