...
def test_common_100_115_280():
    """
    Sub Feature Code: UI_Common_Card_Success_Txn_HDFC_Dummy_EMV_WITH_PIN_RUPAY_DEBIT_608326
    Sub Feature Description: Performing the EMV With Pin success transaction via HDFC Dummy PG using RUPAY Debit card
    having 16 digit PAN length without pin (bin:608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI,  100: TC100
    """
    try:
        ...
        try:
            ...
            # ------------------------------------------------------------------------------------------------
            total_amount = random.randint(10, 300)
            ...
            home_page.enter_amount_and_order_number_and_device_serial_for_card(amt=total_amount,
                                                                               order_number=order_id,
                                                                               device_serial=device_serial)
            logger.debug(f"Entered amount is : {total_amount}")
            logger.debug(f"Entered order_id is : {order_id}")
            logger.debug(f"Entered device_serial is : {device_serial}")
            payment_page = PaymentPage(driver=app_driver)
            payment_page.is_payment_page_displayed_card(amount=total_amount, order_id=order_id,
                                                        device_serial=device_serial)
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                txn_history_page.scroll_to_given_input_text("RRNUMBER")
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    ...
                    "amt_original": float(total_amount),
                    "auth_code": auth_code,
                ...
                api_amount_original = response.get('amountOriginal')
                logger.debug(f"From response fetch amount_original : {api_amount_original}")
...
def test_common_100_115_281(amount):
    """
    Sub Feature Code: UI_Common_Card_Success_Txn_HDFC_Dummy_EMV_WITH_PIN_RUPAY_DEBIT_608326
    Sub Feature Description: Performing the EMV With Pin success transaction via HDFC Dummy PG using RUPAY Debit card
    having 16 digit PAN length without pin (bin:608326)
    TC naming code description: 100: Payment Method, 115: CARD_UI,  001: TC00
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            home_page.wait_for_home_page_load()
            logger.info(f"App homepage loaded successfully")
            home_page.enter_amount_and_order_number_and_device_serial_for_card(amt=amount,
                                                                               order_number=order_id,
                                                                               device_serial=device_serial)
            ...
            logger.debug(f"Entered device_serial is : {device_serial}")
            payment_page = PaymentPage(driver=app_driver)
            payment_page.is_payment_page_displayed_card(amount=amount, order_id=order_id,
                                                        device_serial=device_serial)
            payment_page.click_on_Card_paymentMode()
            logger.info(f"Selected payment mode is Card")
            card_page = CardPage(driver=app_driver)
...
@pytest.mark.chargeSlipVal
@pytest.mark.parametrize("amount", [500, 501, 5000])
def test_common_100_115_282(amount):
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_Dummy_EMVCTLS_VISA_DebitCard_With_Pin_476173
    Sub Feature Description: Performing the EMVCTLS sale transaction via HDFC Dummy PG using VISA Debit card having 16
    digit PAN length with pin (bin: 476173)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 037: TC037
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.chargeSlipVal
@pytest.mark.parametrize("amount", [500, 501, 5000])
def test_common_100_115_283(amount):
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_Dummy_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description:  Performing the EMV sale transaction via HDFC Dummy PG using VISA Credit card having
    16 digit PAN length with pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 122: TC122
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.chargeSlipVal
@pytest.mark.parametrize("amount", [500, 501, 5000])
def test_common_100_115_284(amount):
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_Dummy_EMV_MASTER_CreditCard_With_Pin_541333
    Sub Feature Description: Performing the EMV sale transaction via HDFC Dummy PG using MASTER Credit card having
    16 digit PAN length with pin  (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 123: TC123
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.chargeSlipVal
@pytest.mark.parametrize("amount", [500, 501, 5000])
def test_common_100_115_285(amount):
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_Dummy_EMVCTLS_MASTER_CreditCard_Without_Pin_541333
    Sub Feature Description: Performing the EMVCTLS sale transaction via HDFC Dummy PG using MASTER Credit card having
    16 digit PAN length without pin (bin: 541333)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 043: TC043
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.chargeSlipVal
@pytest.mark.parametrize("amount", [500, 501, 5000])
def test_common_100_115_286(amount):
    """
    Sub Feature Code: UI_Common_Card_Sale_HDFC_Dummy_EMVCTLS_VISA_CreditCard_Without_Pin_417666
    Sub Feature Description:  Performing the EMVCTLS sale transaction via HDFC Dummy PG using VISA Credit card having
    16 digit PAN length without pin (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 042: TC042
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
