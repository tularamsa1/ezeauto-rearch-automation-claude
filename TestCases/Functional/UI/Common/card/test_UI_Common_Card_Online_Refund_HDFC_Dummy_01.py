...
def test_common_100_115_04_001():
    ...
    try:
        ...
        logger.debug(f"Response received for setting preconditions for online refund to be enabled :  {response}")
        ...
        try:
            ...
            logger.debug(f"Fetching txn_id from the txn table for refund txn : {refund_txn_id}")
            refund_total_amount = result['amount'].values[0]
            logger.debug(f"Fetching amount from the txn table for refund txn : {refund_total_amount}")
            ...
            logger.debug(f"Fetching issuer_code from the txn table for refund txn : {refund_issuer_code_txn}")
            ...
            logger.debug(f"Fetching device_serial from the txn table for refund txn : {refund_device_serial_txn}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                date_time_2 = date_time_converter.to_online_refund_app_format(posting_date_db=posting_date)
                refund_date_and_time_2 = date_time_converter.to_online_refund_app_format(posting_date_db=refund_posting_date)
                expected_app_values = {
                    ...
                    "or_date_time_2": refund_date_and_time_2,
