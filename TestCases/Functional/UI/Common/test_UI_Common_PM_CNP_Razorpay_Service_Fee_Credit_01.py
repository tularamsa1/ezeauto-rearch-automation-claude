...
def test_common_100_103_499():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_Service_Fee_Credit_Only_Enabled_Amt_FIXED_JSON_Credit_With_FIXED_Scheme_Credit_Card_Details
    Sub Feature Description:Verify Service Fee calculated when credit only enabled and amount ranges with FIXED is passed and JSON is credit with FIXED scheme with Credit Card details
    TC naming code description: 100: Payment Method, 103: RemotePay, 499: TC_499
    """
    ...
    try:
        ...
        scheme = "MASTER_CARD"
        payment_by = "CREDIT"
        json_id = "service_fee_json_TC_100_103_499"
        payment_mode = "CNP"
        acc_label = ""
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                txn_amt_db = (result["amount"].iloc[0])
        ...
        try:
            ...
            refresh_db()
            logger.debug(f"Refreshing the db after reverting back the changes")
        except Exception as e:
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_500():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_Service_Fee_Credit_Only_Enabled_Amt_PERCENT_JSON_Credit_With_PERCENT_Scheme_Credit_Card_Details
    Sub Feature Description:Verify Service Fee calculated when credit only enabled and amount ranges with PERCENT is passed and JSON is credit with PERCENT scheme with Credit Card details
    TC naming code description: 100: Payment Method, 103: RemotePay, 500: TC_500
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        scheme = "MASTER_CARD"
        payment_by = "CREDIT"
        json_id = "service_fee_json_TC_100_103_500"
        payment_mode = "CNP"
        acc_label = ""
        ...
        try:
            ...
            flat_fee = float(flat_fee) if flat_fee is not 'NULL' else 0.00
            percent = float(percent) if percent is not 'NULL' else 0.00
            logger.debug(f"{flat_fee},{percent}")
            service_fee = round((amount * percent / 100) + flat_fee, 2)
            total_amount = amount + service_fee
            order_id = datetime.now().strftime('%m%d%H%M%S')
            logger.debug(f"Amount used:{amount} and Order ID used :{order_id}")
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_501():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_Service_Fee_Credit_Only_Enabled_Amt_FIXED_PERCENT_JSON_Credit_With_FIXED_PERCENT_Scheme_Credit_Card_Details
    Sub Feature Description:Verify Service Fee calculated when credit only enabled and amount ranges with FIXED PERCENT is passed and JSON is credit with FIXED PERCENT scheme with Credit Card details
    TC naming code description: 100: Payment Method, 103: RemotePay, 501: TC_501
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        testcase_id = sys._getframe().f_code.co_name
        ...
        scheme = "MASTER_CARD"
        payment_by = "CREDIT"
        json_id = "service_fee_json_TC_100_103_501"
        payment_mode = "CNP"
        acc_label = ""
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_100_103_502():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Service_Fee_Credit_Only_Enabled_JSON_Credit_With_FIXED_PERCENT_Scheme_Debit_Card_Details
    Sub Feature Description: Verify Error message when credit only enabled and JSON is credit with FIXED_PERCENT scheme  with Debit Card details
    with Credit Card details.
    TC naming code description: 100: Payment Method, 103: RemotePay, 493: TC_502
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        scheme = "MASTER_CARD"
        payment_by = "CREDIT"
        json_id = "service_fee_json_TC_100_103_502"
        payment_mode = "CNP"
        acc_label = ""
        derivation_type = "FIXED_PERCENT"
        bank = "NULL"
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.apiVal
def test_common_100_103_503():
    """
    Sub Feature Code:UI_Common_PM_CNP_Razorpay_Service_Fee_Credit_Only_Enabled_JSON_Debit_With_FIXED_PERCENT_ANY_Debit_Scheme_Debit_Card_Details
    Sub Feature Description:Verify Service fee calculated when credit only enabled and JSON is debit with FIXED_PERCENT and ANY(Debit) scheme  with Debit Card details
    TC naming code description: 100: Payment Method, 103: RemotePay, 503: TC_503
    """
    ...
    try:
        ...
        scheme = "ANY"
        payment_by = "DEBIT"
        json_id = "service_fee_json_TC_100_103_503"
        payment_mode = "CNP"
        acc_label = ""
        derivation_type = "FIXED"
        bank = 'NULL'
