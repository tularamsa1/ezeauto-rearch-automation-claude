...
@pytest.mark.appVal
def test_common_100_115_07_062():
    """
    ...
    try:
        ...
        max_amount = result['max_amount'].values[0]
        logger.debug(f"fetching min_amount from emi table : {max_amount}")
...
def test_common_100_115_07_063():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG for Canara issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 063: TC063
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='CANARA' and eze_emi_enabled=b'0'"
...
@pytest.mark.usefixtures("log_on_success", "method_setup")
@pytest.mark.appVal
def test_common_100_115_07_064():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Invalid_Amount_Lesser_Than_EMI_Range_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_065():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_Cashback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    Sub Feature Description: Performing the brand EMI cashback by brand fixed success transaction for an org (not ezetap)
    via HDFC Dummy PG for Canara issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 065: TC065
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='CANARA' and eze_emi_enabled=b'0' order by created_time desc limit 1;"
        logger.debug(f"Query to fetch data from the subvention_plan table : {query}")
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Query result for subvention_plan table : {result}")
        subvention_plan_id = result['id'].values[0]
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_07_070():
    """
    Sub Feature Code: UI_Common_Card_Brand_EMI_BO_Instant_Discount_Cashback_by_Brand_Fixed_Success_Txn_For_An_Org_HDFC_Dummy_Canara_EMV_VISA_CreditCard_With_Pin_417666_For_3_Months_Tenure
    ...
    via HDFC Dummy PG for Canara issuer using EMV VISA Credit card with pin for 3 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 07: Brand_EMI, 070: TC070
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        query = f"select * from rule where org_code='{org_code}' and credit_type='INSTANT' and name LIKE '{brand_name}_CANARA_CREDIT%'"
