...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_115_238():
    """
    Sub Feature Code: UI_Common_Card_Sale_MASHREQ_EMV_VISA_CreditCard_With_Pin_417666
    Sub Feature Description: Performing the EMV sale transaction via MASHREQ PG using VISA Credit card having 16 digit PAN length with pin (bin: 417666)
    TC naming code description:  100: Payment Method, 115: CARD_UI, 238: TC238
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
