...
@pytest.mark.appVal
def test_common_300_302_006(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "airtelNo": "1-3775086096255", "customerAccountId": "1-3775086096255", "currentOutstanding":"214"}
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_postpaid_Account') and active = 1);"
        ...
        Configuration.executeFinallyBlock("test_common_300_302_006")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_300_302_007(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "airtelNo": "01141550070", "customerAccountId": "7035432683", "currentOutstanding":"590"}
                logger.debug(f"expectedAPIValues: {expectedAPIValues}")
                actualAPIValues = {"success": success, "airtelNo": airtelNo, "customerAccountId": customerAccountId,"currentOutstanding":currentOutstanding}
                logger.debug(f"actualAPIValues: {actualAPIValues}")
        #          ---------------------------------------------------------------------------------------------
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_fixedLine_Mobile') and active = 1);"
                logger.debug(f"Query to fetch data from external_api_adapter table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result URL: {result}")
                url_db = result["url"].iloc[0]
        ...
        Configuration.executeFinallyBlock("test_common_300_302_007")
        if GlobalVariables.EXCEL_TC_Execution == "Fail" or GlobalVariables.str_api_val_result == "Fail" or GlobalVariables.str_db_val_result == 'Fail':
            query = "select * from ca_usergroup_org_map where org_code='AIRTELULB_540943' and is_active;"
            logger.debug(f"Query to fetch data from ca_usergroup_org_map table : {query}")
            result = DBProcessor.getValueFromDB(query)
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_300_302_008(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "airtelNo": "12299726", "customerAccountId": "12299726", "currentOutstanding":"0"}
                logger.debug(f"expectedAPIValues: {expectedAPIValues}")
                actualAPIValues = {"success": success, "airtelNo": airtelNo, "customerAccountId": customerAccountId,"currentOutstanding":currentOutstanding}
                logger.debug(f"actualAPIValues: {actualAPIValues}")
        #          ---------------------------------------------------------------------------------------------
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_fixedLine_Account') and active = 1);"
                logger.debug(f"Query to fetch data from external_api_adapter table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result URL: {result}")
                url_db = result["url"].iloc[0]
        ...
        Configuration.executeFinallyBlock("test_common_300_302_008")
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_300_302_009(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "cust_cat": "SILVERNEW", "si": "919988735802", "si_sts":"ACTIV"}
                logger.debug(f"expectedAPIValues: {expectedAPIValues}")
                actualAPIValues = {"success": success, "cust_cat": cust_cat, "si": si,"si_sts":si_sts}
                logger.debug(f"actualAPIValues: {actualAPIValues}")
        #          ---------------------------------------------------------------------------------------------
                Validator.validationAgainstAPI(expectedAPI= expectedAPIValues, actualAPI=actualAPIValues)
            except Exception as e:
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_prepaid_details') and active = 1);"
                logger.debug(f"Query to fetch data from external_api_adapter table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result URL: {result}")
                url_db = result["url"].iloc[0]
        ...
        Configuration.executeFinallyBlock("test_common_300_302_009")
        if GlobalVariables.EXCEL_TC_Execution == "Fail" or GlobalVariables.str_api_val_result == "Fail" or GlobalVariables.str_db_val_result == 'Fail':
            query = "select * from ca_usergroup_org_map where org_code='AIRTELULB_540943' and is_active;"
            logger.debug(f"Query to fetch data from ca_usergroup_org_map table : {query}")
            result = DBProcessor.getValueFromDB(query)
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_300_302_010(): #Make sure to add the test case name as same as the sub feature code.
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            try:
                # --------------------------------------------------------------------------------------------
                expectedAPIValues = {"success": True, "orderNo": "21-4186354405372", "Amount": "2500", "airtelNo":"9898863555"}
                logger.debug(f"expectedAPIValues: {expectedAPIValues}")
                actualAPIValues = {"success": success, "orderNo": orderNo, "Amount": Amount,"airtelNo":airtelNo}
                logger.debug(f"actualAPIValues: {actualAPIValues}")
        #          ---------------------------------------------------------------------------------------------
                Validator.validationAgainstAPI(expectedAPI= expectedAPIValues, actualAPI=actualAPIValues)
            except Exception as e:
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            try:
                ...
                query = "select url from external_api_adapter where rule_id = (select id from rule where fetch_option_id = (select id from fetch_option where fetch_key = 'get_active_butterfly') and active = 1);"
                logger.debug(f"Query to fetch data from external_api_adapter table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result URL: {result}")
                url_db = result["url"].iloc[0]
        ...
        Configuration.executeFinallyBlock("test_common_300_302_010")
        if GlobalVariables.EXCEL_TC_Execution == "Fail" or GlobalVariables.str_api_val_result == "Fail" or GlobalVariables.str_db_val_result == 'Fail':
            query = "select * from ca_usergroup_org_map where org_code='AIRTELULB_540943' and is_active;"
            logger.debug(f"Query to fetch data from ca_usergroup_org_map table : {query}")
            result = DBProcessor.getValueFromDB(query)
