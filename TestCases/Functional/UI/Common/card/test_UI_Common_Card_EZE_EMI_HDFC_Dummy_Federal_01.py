...
def test_common_100_115_11_024():
    """
    Sub Feature Code: UI_Common_Card_EZE_EMI_Subvented_By_Ezetap_For_Issuer_Ezetap_Cosubvention_Success_Txn_For_An_Org_HDFC_Dummy_Federal_EMV_VISA_CreditCard_With_Pin_417666_For_6_Months_Tenure
    Sub Feature Description: Performing the EZE EMI subvented by ezetap for issuer-ezetap co-subvention success transaction for an org (not ezetap) via
    HDFC Dummy PG for Federal issuer using EMV VISA Credit card with pin for 6 months tenure (bin: 417666)
    TC naming code description: 100: Payment Method, 115: CARD_UI, 11: EZE EMI, 024: TC024
    """
    try:
        ...
        query = f"select * from emi where org_code='{org_code}' and status = 'ACTIVE' and " \
                        f"issuer_code='FEDERAL' and card_type='CREDIT' and term = '{emi_plan_in_months} month' and emi_type='EZEEMI'" \
        ...
        query = f"select * from subvention_plan where brand_id='{brand_id}' and org_code='{org_code}' and card_type= 'CREDIT' and bank='FEDERAL' and eze_emi_enabled= b'1' order by created_time desc limit 1;"
        ...
        logger.debug(f"Fetching wallet_balance before the txn from ezeemi_club_wallet_account table : {mcew_wallet_balance_before_txn}")
        ...
        logger.debug(f"Fetching wallet_balance before  the txn from ezeemi_club_wallet_account table : {gcew_wallet_balance_before_txn}")
        ...
        try:
            ...
            cal_subvention_value = round((amount * (ezetap_subvention_value / 100)), 2)
            logger.debug(f"cal subvention value is : {cal_subvention_value}")
            ...
            total_interest = round((total_emi - (amount - cal_subvention_value)), 2)
            ...
            logger.debug(f"Fetching emi_instant_discount from txn_emi table : {emi_instant_discount}")
            ...
            logger.debug(f"Fetching wallet_balance after the txn from ezeemi_club_wallet_account table : {gcew_wallet_balance_after_txn}")
        ...
        if (ConfigReader.read_config("Validations", "app_validation")) == "True":
            ...
            try:
                ...
                expected_app_values = {
                    "txn_amt": "{:,.2f}".format(round(amount - cal_subvention_value, 2)),
                    "pmt_mode": "CARD",
                    "txn_id": txn_id,
                    ...
                    "loan_amt": "{:,.2f}".format(round(amount - cal_subvention_value, 2)),
        ...
        if (ConfigReader.read_config("Validations", "api_validation")) == "True":
            ...
            try:
                ...
                expected_api_values = {
                    "txn_amt": float(round(amount - cal_subvention_value, 2)),
                    "pmt_mode": "CARD",
                    "pmt_status": "AUTHORIZED",
                    ...
                    "loan_amt": float(round(amount - cal_subvention_value, 2)),
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                actual_db_values = {
                    ...
                    "emi_txn_amt": float(round(emi_txn_amount, 2)),
                    ...
                    "subvention_entity": ezetap_subvention_entity,
        ...
        if (ConfigReader.read_config("Validations", "charge_slip_validation")) == "True":
            ...
            try:
                ...
                expected_charge_slip_values = {
                    ...
                    "BASE AMOUNT:": "Rs." + "{:,.2f}".format(round(amount - cal_subvention_value, 2)),
                    ...
                    "Total Instant EMI Discount Amount": "Rs." + "{:.2f}".format(emi_instant_discount),
                    "Instant EMI Discount": f"{ezetap_subvention_value}%",
                    "Mobile": customer_no,
