...
@pytest.mark.apiVal
def test_common_100_103_478():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_Card_Success_Netbanking_RemotePay_Netbanking_Option_Disabled
    Sub Feature Description: Verify Netbanking - Remote Pay option not seen on payment page when Remote Pay is enabled
     with CC, DC and not enabled with Netbanking.
    TC naming code description: 100: Payment Method, 103: RemotePay, 478: TC_478
    """
    try:
        ...
        try:
            ...
            if not response['success']:
                ...
                try:
                    ...
                    actual_nb_pm_text = False
                    logger.debug(f"Net banking payment mode is not present")
                ...
                assert actual_nb_pm_text == False, "Net banking payment mode is available"
...
def test_common_100_103_482():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_NetBanking_Successful_Txn_Canara_Bank
    Sub Feature Description: Verification successful netbanking txn for Razorpay pg using Canara bank.
    TC naming code description: 100: Payment Method, 103: RemotePay, 481: TC_481
    """
    ...
    try:
        ...
        if (ConfigReader.read_config("Validations", "portal_validation")) == "True":
            try:
                ...
                total_amt_portal = transaction_details[0]['Total Amount'].split()
...
@pytest.mark.appVal
@pytest.mark.chargeSlipVal
def test_common_100_103_483():
    """
    Sub Feature Code: UI_Common_PM_CNP_Razorpay_NetBanking_Successful_Txn_DCB_Bank
    Sub Feature Description: Verification successful netbanking txn for Razorpay pg using DCB bank.
    TC naming code description: 100: Payment Method, 103: RemotePay, 481: TC_481
    """
    expected_message = "Your payment is successfully completed! You may close the browser now."
    try:
        ...
        try:
            ...
            if not response['success']:
                ...
                remote_pay_txn.remote_pay_click_and_expand_netbanking()
                logger.info(f"Selecting bank")
                remote_pay_txn.select_bank_from_list('DCB Bank')
                remote_pay_txn.clickOnProceedToPay()
                logger.info(f"click on proceed button")
                remote_pay_txn.clickOnSuccessBtn()
                logger.info(f"click on success button")
