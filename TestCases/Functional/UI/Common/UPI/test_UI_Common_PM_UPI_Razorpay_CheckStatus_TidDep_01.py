...
@pytest.mark.appVal
def test_common_100_101_237():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_CheckStatus_Via_API_After_Expiry_Razorpay
    Sub Feature Description: Tid Dep - Verification of a upi txn after expiry for Razorpay using check status api
    TC naming code description: 100: Payment Method, 101: UPI, 237: TC237
    """
    try:
        ...
        try:
            ...
            api_details = DBProcessor.get_api_details('paymentStatus',
                                                      request_body={"username": app_username, "password": app_password,
                                                                    "txnId": str(txn_id)})
            response = APIProcessor.send_request(api_details)
            logger.debug(f"response received after hitting  paymentStatus is: {response}")
...
def test_common_100_101_238():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_Success_Via_CheckStatus_API_Razorpay
    Sub Feature Description: Tid Dep - Verification of a successful upi txn via Razorpay using check status api
    TC naming code description: 100: Payment Method, 101: UPI, 238: TC238
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_241():
    """
    Sub Feature Code: Tid_Dep_UI_Common_PM_UPI_Failed_Via_CheckStatus_API_Razorpay
    Sub Feature Description: Tid Dep - Verification of upi failed using check status via Razorpay
    TC naming code description: 100: Payment Method, 101: UPI, 241: TC241
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.portalVal
@pytest.mark.appVal
def test_common_100_101_251():
    """
    Sub Feature Code:  Tid_Dep_UI_Common_PM_UPI_Pure_UPI_Pending_Via_Razorpay
    Sub Feature Description: Tid Dep - Verification of check status for upi pending txn via Razorpay
    TC naming code description: 100: Payment Method, 101: UPI, 251: TC251
    """
