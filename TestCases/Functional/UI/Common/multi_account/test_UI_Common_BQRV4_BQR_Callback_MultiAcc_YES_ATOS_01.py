...
def test_common_100_110_041():
    ...
    Sub Feature Description: Multi Account - Performing a Authorized Txn using pure BQRV4_BQR Success callback for YES_ATOS
    TC naming code description: 100: Payment Method 110: MultiAcc_BQR, 041: TC041
    """
    try:
...
def test_common_100_110_014():
    ...
    Sub Feature Description: Multi Account - Performing a Failed Txn using pure BQRV4_BQR Failed callback for YES_ATOS
    TC naming code description: 100: Payment Method 110: MultiAcc_BQR, 014: TC014
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        result = DBProcessor.getValueFromDB(query)
        logger.debug(f"Result obtained from bharatqr_merchant_config table is : {result}")
        mid = result["mid"].iloc[0]
        tid = result["tid"].iloc[0]
        terminal_info_id = result["terminal_info_id"].iloc[0]
        bqr_mc_id = result["id"].iloc[0]
