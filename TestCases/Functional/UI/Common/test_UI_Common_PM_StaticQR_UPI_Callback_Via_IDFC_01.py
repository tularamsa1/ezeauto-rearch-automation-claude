...
def test_common_100_107_016():
    ...
    try:
        ...
        try:
            ...
            txn_amt = int(result["amount"].iloc[0])
            ...
            txn_ref_number = result['external_ref'].values[0]
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "txn_type": "CHARGE",
                    "txn_bank_code": "IDFC",
                    "txn_issuer_code": "IDFC",
                    "txn_username": app_username,
                    "txn_tid": db_upi_config_tid,
                    "txn_mid": db_upi_config_mid,
                    "txn_acquirer_code": "IDFC",
                    "txn_pmt_mode": "UPI",
                    "txn_pmt_status": "AUTHORIZED",
                    "txn_pmt_state": "SETTLED",
                    "txn_settle_status": "SETTLED",
...
@pytest.mark.chargeSlipVal
@pytest.mark.portalVal
def test_common_100_107_017():
    """
    Sub Feature Code: UI_Common_PM_StaticQR_UPI_Callback_Success_Autorefund_Disabled_Via_IDFC
    Sub Feature Description: Verifying UPI static QR Success callback when autorefund is disabled via IDFC
    TC naming code description:: 100: payment method, 107: UPI Static QR, 017: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
...
@pytest.mark.appVal
@pytest.mark.portalVal
def test_common_100_107_018():
    """
    Sub Feature Code: UI_Common_PM_StaticQR_UPI_Callback_Success_Autorefund_Disabled_Via_IDFC
    Sub Feature Description: Verifying UPI static QR failed callback when autorefund is disabled via IDFC
    TC naming code description:: 100: payment method, 107: UPI Static QR, 018: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                expected_db_values = {
                    ...
                    "txn_acquirer_code": "IDFC",
                    "txn_pmt_mode": "UPI",
                    "txn_pmt_status": "FAILED",
                    "txn_pmt_state": "FAILED",
                    "txn_settle_status": "FAILED",
                    "upi_customer_ref": str(orig_cust_ref_id),
                    "upi_org_code": org_code,
                    "upi_status": "FAILED",
                    "upi_additional_field1": res_generateqr_publish_id,
...
@pytest.mark.chargeSlipVal
@pytest.mark.portalVal
def test_common_100_107_029():
    """
    Sub Feature Code: UI_Common_PM_StaticQR_2_UPI_Callback_Same_RefID_Via_IDFC
    Sub Feature Description: Verifying 2 UPI static QR callback with same ref id not to create new txn via IDFC
    TC naming code description:: 100: payment method, 107: UPI Static QR, 029: Testcase ID
    """
    try:
        testcase_id = sys._getframe().f_code.co_name
        GlobalVariables.time_calc.setup.resume()
        ...
        try:
            ...
            txn_settle_status = result["settlement_status"].iloc[0]
            txn_created_time = result['created_time'].values[0]
            txn_auth_code = result['auth_code'].iloc[0]
            txn_rrn_number = result['rr_number'].values[0]
            ...
            logger.debug(f"Response received for second time staticQR_UPI_IDFC_callback api is : {response}")
        ...
        if (ConfigReader.read_config("Validations", "db_validation")) == "True":
            ...
            try:
                ...
                query = "select * from upi_txn where txn_id = '" + str(txn_id_2) + "';"
                logger.debug(f"Query to fetch data from upi_txn table : {query}")
                result = DBProcessor.getValueFromDB(query)
                logger.debug(f"Query result : {result}")
                upi_txn_id_2 = result["txn_id"].iloc[0]
                upi_additional_field1 = result["additional_field1"].iloc[0]
                upi_additional_field2 = result["additional_field2"].iloc[0]
                upi_resp_code = result["resp_code"].iloc[0]
                upi_customer_ref = result["customer_ref"].iloc[0]
